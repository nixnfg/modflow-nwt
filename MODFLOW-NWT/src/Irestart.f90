...
SUBROUTINE RESTART1WRITE()
   ...
   !
   !
   !2------WRITE DATA FOR RESTART OPTION
   IF ( IUNIT(68)>0 ) THEN
...
SUBROUTINE WRITE_MNW2(IOUT,IURESTARTMNW2)
   ...
   &NODTOT,INTTOT,NTOTNOD,LIMQ,&
   &SMALL,WELLID,MNWAUX,MNW2,&
   &MNWNOD,MNWINT,CapTable
   IMPLICIT NONE
...
SUBROUTINE U2DREL_RST(A,ANAME,II,JJ,K,IN,IOUT)
   ...
   !     ------------------------------------------------------------------
   !
   !1------SET LOCAT TO NEGATIVE UNIT NUMBER FOR UNFORMATTED FILE
   LOCAT = -IN
   !2------SET FMTIN to read binary
   FMTIN = 'BINARY'
   CNSTNT = 1.0
   !
