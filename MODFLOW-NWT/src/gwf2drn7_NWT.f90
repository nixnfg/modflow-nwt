...
101 FORMAT(1X,/1X,I6,' DRAINS')
   !
   !4------CHECK THAT DRAIN ELEVATIONS ARE GREATER THAN CELL BOTTOM.
   DO 102 L=1,NDRAIN
   !5------GET COLUMN, ROW AND LAYER OF CELL CONTAINING DRAIN.
      IL=DRAI(1,L)
      IR=DRAI(2,L)
      IC=DRAI(3,L)
   !
...
SUBROUTINE GWF2DRN7FM(IGRID)
   ...
   CALL SGWF2DRN7PNT(IGRID)
   !
   !1------IF NDRAIN<=0 THERE ARE NO DRAINS. RETURN.
   IF(NDRAIN.LE.0) RETURN
   !
   !2------PROCESS EACH CELL IN THE DRAIN LIST.
   DO 100 L=1,NDRAIN
   !
...
SUBROUTINE GWF2DRN7BD(KSTP,KPER,IGRID)
   ...
   IF(IBD.EQ.2) THEN
      ...
      &NDRAIN,IOUT,DELT,PERTIM,TOTIM,IBOUND)
   END IF
   !
   !3------CLEAR THE BUFFER.
   DO 50 IL=1,NLAY
...
50 CONTINUE
   !
   !4------IF THERE ARE NO DRAINS THEN DO NOT ACCUMULATE DRAIN FLOW.
   ...
   !
   !5C-----GET DRAIN PARAMETERS FROM DRAIN LIST.
      EL=DRAI(4,L)
      EEL=EL
      C=DRAI(5,L)
      HHNEW=HNEW(IC,IR,IL)
   ...
   !5D-----SUBTRACT Q FROM RATOUT.
      ...
      END IF
   !
   !5E-----PRINT THE INDIVIDUAL RATES IF REQUESTED(IDRNCB<0).
      IF(IBD.LT.0) THEN
...
62       FORMAT(1X,'DRAIN ',I6,'   LAYER ',I3,'   ROW ',I5,'   COL ',I5,&
   ...
   !
   !5G-----IF SAVING CELL-BY-CELL FLOWS IN A LIST, WRITE FLOW.  ALSO
   !5G-----COPY FLOW TO DRAI.
99    IF(IBD.EQ.2) CALL UBDSVB(IDRNCB,NCOL,NROW,IC,IR,IL,Q,&
      &DRAI(:,L),NDRNVL,NAUX,6,IBOUND,NLAY)
      DRAI(NDRNVL,L)=Q
100 CONTINUE
   !
...
SUBROUTINE SGWF2DRN7PNT(IGRID)
   !  Change DRN data to a different grid.
   USE GWFDRNMODULE
   !
