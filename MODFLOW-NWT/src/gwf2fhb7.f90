...
SUBROUTINE GWF2FHB7AR(IN,IGRID)
   !     ******************************************************************
   !     ALLOCATE ARRAY STORAGE AND READ DATA FOR FLOW AND HEAD BOUNDARY
   !     PACKAGE
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
...
1  FORMAT(1X,/1X,'FHB1 -- SPECIFIED FLOW PACKAGE, VERSION 1,12/3/96',&
   &' INPUT READ FROM',I3)
   !
   !2------READ NUMBER OF TIMES, NUMBER OF SPECIFIED-FLOW CELLS AND
   !2------UNIT OR FLAG FOR CELL-BY-CELL FLOW TERMS, NUMBER OF
   !2------AUXILIARY VARIABLES.
   READ(IN,*) NBDTIM,NFLW,NHED,IFHBSS,IFHBCB,NFHBX1,NFHBX2
   !
   !3------PRINT NBDTIM, STOP IF NO TIMES ARE TO BE SPECIFIED
...
10    FORMAT(1H ,'TOTAL OF',I5,' TIMES WILL BE USED TO DEFINE ',&
   ...
   ENDIF
   !
   !4------PRINT NFLW AND NHED AND STEADY-STATE OPTION
   !ERB  This section was changed to use ITRSS instead of ISS
   !ERB  I also eliminated use of Hollerith constants -- ERB 10/8/2002
   WRITE(IOUT,12) NFLW
...
18       FORMAT(1X,'FLOW, HEAD, AND AUX VARIABLES WILL BE ',&
      ...
      ENDIF
   ENDIF
   !ERB  End of changed section
   !
   !5------IF CELL-BY-CELL FLOW TERMS ARE TO BE SAVED THEN PRINT UNIT #
...
129   FORMAT(1X,A16,F11.2)
      ...
      ENDIF
130 CONTINUE
   !7------ALLOCATE SPACE FOR ARRAYS BDTIM, IFLLOC, FLWRAT, BDFV,
   !7------IHDLOC, SBHED, AND BDHV
200 IFHBD3=NBDTIM*(1+NFHBX1)
   ...
   !
   !1------READ TIMES AT WHICH SPECIFIED FLOW AND HEAD VALUES WILL BE READ
...
326   FORMAT(17X,5G12.4)
   ENDIF
   !
   !3------MAKE SURE THAT FIRST TIME IS ZERO AND THAT TIMES INCREASE
   ICHK1=0
   ICHK2=0
...
42 IF(ICHK1.EQ.1.OR.ICHK2.EQ.1) CALL USTOP(' ')
   !
   !4A-----READ CELL INDICIES AND SPECIFIED-FLOW RATES
...
56    CONTINUE
   !
   !4B-----IF DESIRED, PRINT TABLE OF SPECIFIED-FLOW CELL LOCATIONS
   !4B-----AND RATES
      IF(IFHBPT.GT.0) THEN
...
59 CONTINUE
   !
   !5A------READ VALUES OF AUXILIARY VARIABLES FOR SPECIFIED-FLOW CELLS
...
66       CONTINUE
   !
   !5B------IF DESIRED, PRINT TABLE OF AUXILIARY VARIABLE VALUES AT
   !5B------SPECIFIED-FLOW CELL LOCATIONS
         IF(IFHBPT.GT.0) THEN
...
69 CONTINUE
   !
   !6------READ CELL INDICIES AND SPECIFIED-HEAD VALUES
...
75    CONTINUE
   !
   !7------AT SPECIFIED-HEAD LOCATIONS, SET IBOUND TO NEGATIVE NUMBER.
   !7------IGNORE SPECIFIED-HEAD CONDITIONS AT CELLS WHERE IBOUND IS ZERO
      K=IHDLOC(1,N)
      I=IHDLOC(2,N)
      J=IHDLOC(3,N)
...
76       FORMAT(1X,'SPECIFIED-HEAD VALUE IGNORED AT ROW',I5,', COLUMN',&
      ...
      ENDIF
   !
   !8------IF DESIRED, PRINT TABLE OF SPECIFIED-FLOW CELL LOCATIONS
   !8------AND RATES
      IF(IFHBPT.GT.0) THEN
...
80 CONTINUE
   !
   !9A------READ VALUES OF AUXILIARY VARIABLES FOR SPECIFIED-HEAD CELLS
...
166      CONTINUE
   !
   !9B------IF DESIRED, PRINT TABLE OF AUXILIARY VARIABLE VALUES AT
   !9B------SPECIFIED-HEAD CELL LOCATIONS
         IF(IFHBPT.GT.0) THEN
...
SUBROUTINE GWF2FHB7AD(IGRID)
   !     ******************************************************************
   !     COMPUTE SPECIFIED FLOWS AND HEADS AT CURRENT TIME STEP
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !1------WITH CONSTANT SPECIFIED FLOWS AND HEADS, SET VALUES AND RETURN
...
10    CONTINUE
   ...
   ENDIF
   !
   !2------FIND ARRAY INDICES OF TIMES AROUND TIME AT START OF CURRENT
   !2------TIME STEP
...
20 CONTINUE
   ...
   IB2=NBDTIM
   !
   !3------COMPUTE FACTOR FOR INTERPOLATION OR EXTRAPOLATION OF FLOW AT
   !3------START OF CURRENT TIME STEP
40 QFACT1=(T1-BDTIM(IB1))/(BDTIM(IB2)-BDTIM(IB1))
   !
   !4------FIND ARRAY INDICES OF TIMES AROUND TIME AT END OF CURRENT
   !4------TIME STEP
   DO 60 L=IB2,NBDTIM
...
60 CONTINUE
   ...
   IB3=NBDTIM-1
   !
   !5------COMPUTE FACTOR FOR INTERPOLATION OR EXTRAPOLATION OF FLOW AT
   !5------END OF CURRENT TIME STEP
70 QFACT2=(T2-BDTIM(IB3))/(BDTIM(IB4)-BDTIM(IB3))
   !
   !6------COMPUTE SPECIFIED FLOW RATES FOR THIS TIME STEP
   NPI=IB4-IB2
   DO 90 NF=1,NFLW
      QA=FLWRAT(IB1,NF)
      QB=FLWRAT(IB2,NF)
...
90 CONTINUE
   !
   !7-----COMPUTE VALUES OF AUXILIARY VARIABLES FOR SPECIFIED-FLOW
   !7-----CELLS FOR CURRENT TIME STEP
...
150   CONTINUE
190 CONTINUE
   !8------FIND ARRAY INDICES OF TIMES AROUND TIME AT END OF CURRENT
   !8------TIME STEP, COMPUTE FACTOR OF INTERPOLATION OR EXTRAPOLATION
   !8------OF HEAD
...
240 HFACT=(TT-BDTIM(IB1))/(BDTIM(IB2)-BDTIM(IB1))
   !
   !9------AT EACH SPECIFIED-HEAD LOCATION, INTERPOLATE OR EXTRAPOLATE
   !9------HEAD. SET HNEW AND HOLD EQUAL TO COMPUTED HEAD
   DO 250 NH=1,NHED
      K=IHDLOC(1,NH)
      I=IHDLOC(2,NH)
      J=IHDLOC(3,NH)
...
250 CONTINUE
   !
   !10----COMPUTE VALUES OF AUXILIARY VARIABLES FOR FOR SPECIFIED-HEAD
   !10----CELLS FOR CURRENT TIME STEP
...
SUBROUTINE GWF2FHB7FM(IGRID)
   ...
   DO 100 L=1,NFLW
      ...
      Q=BDFV(1,L)
   !
   !1A-----IF THE CELL IS INACTIVE THEN BYPASS PROCESSING.
      IF(IBOUND(IC,IR,IL).LE.0) GO TO 100
   !
   !1B-----IF THE CELL IS VARIABLE HEAD THEN SUBTRACT Q FROM
   !       THE RHS ACCUMULATOR.
...
SUBROUTINE GWF2FHB7BD(KSTP,KPER,IGRID)
   !     ******************************************************************
   !     CALCULATE VOLUMETRIC BUDGET FOR SPECIFIED FLOWS
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   CALL SGWF2FHB7PNT(IGRID)
   !
   !1------CLEAR RATIN AND RATOUT ACCUMULATORS.
   ZERO=0.
   RATIN=ZERO
   RATOUT=ZERO
   IBD=0
   ...
   !
   !2A----IF CELL-BY-CELL FLOWS WILL BE SAVED AS A LIST, WRITE HEADER.
   IF(IBD.EQ.2) THEN
      NAUX=1
...
50 CONTINUE
   !
   !3A----IF THERE ARE NO SPECIFIED-FLOW CELLS, DO NOT ACCUMULATE FLOW
   IF(NFLW.EQ.0) GO TO 200
   !
   !3B-----PROCESS SPECIFIED-FLOW CELLS ONE AT A TIME.
60 DO 100 L=1,NFLW
   !
   !3C-----GET LAYER, ROW, AND COLUMN NUMBERS
      IR=IFLLOC(2,L)
      IC=IFLLOC(3,L)
      IL=IFLLOC(1,L)
      Q=ZERO
   !
   !3D-----IF THE CELL IS NO-FLOW OR CONSTANT-HEAD, IGNORE IT.
      IF(IBOUND(IC,IR,IL).LE.0)GO TO 97
   !
   !4A-----GET FLOW RATE FROM SPECIFIED-FLOW LIST
      Q=BDFV(1,L)
      QQ=Q
   !
   !4B-----PRINT THE INDIVIDUAL RATES IF REQUESTED(IFHBCB<0).
      IF(IBD.LT.0) THEN
...
900      FORMAT(1H0,4A4,'   PERIOD',I3,'   STEP',I6,' SEQ NO',I4,& !gsf
      ...
      ENDIF
   !
   !4C-----ADD FLOW RATE TO BUFFER.
      BUFF(IC,IR,IL)=BUFF(IC,IR,IL)+Q
   !
   !5A-----SEE IF FLOW RATE IS NEGATIVE, ZERO, OR POSITIVE.
      IF(Q.GE.ZERO) THEN
   !
   !5B-----FLOW RATE IS POSITIVE (RECHARGE). ADD IT TO RATIN.
         RATIN=RATIN+QQ
      ELSE
   !
   !5C-----FLOW RATE IS NEGATIVE(DISCHARGE). ADD IT TO RATOUT.
         RATOUT=RATOUT-QQ
      END IF
   !
   !6------IF CELL-BY-CELL FLOWS ARE BEING SAVED AS A LIST, WRITE FLOW.
   !6------RETURN THE ACTUAL FLOW IN THE BDFV ARRAY.
97    IF(IBD.EQ.2) THEN
         XFACE(1)=IFLLOC(4,L)
...
100 CONTINUE
   !
   !7------IF CELL-BY-CELL FLOWS WILL BE SAVED AS A 3-D ARRAY,
   !7------CALL UBUDSV TO SAVE THEM
   IF(IBD.EQ.1) CALL UBUDSV(KSTP,KPER,TEXT,IFHBCB,BUFF,NCOL,NROW,&
   &NLAY,IOUT)
   !
   !8------MOVE RATES, VOLUMES & LABELS INTO ARRAYS FOR PRINTING.
200 RIN=RATIN
   ROUT=RATOUT
   VBVL(3,MSUM)=RIN
   VBVL(4,MSUM)=ROUT
   ...
   VBNM(MSUM)=TEXT
   !
   !9------INCREMENT BUDGET TERM COUNTER (MSUM).
   MSUM=MSUM+1
   !
   !10-----RETURN
   RETURN
