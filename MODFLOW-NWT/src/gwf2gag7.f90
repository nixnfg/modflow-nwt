...
END MODULE GWFGAGMODULE
   !
   !-------SUBROUTINE GWF2GAG7AR
SUBROUTINE GWF2GAG7AR(INGAGE,IUNITSFR,IUNITLAK,IGRID)
   !     ******************************************************************
   !     READ FIRST GAGING STATION PACKAGE RECORD
   !     ALLOCATE ARRAY STORAGE FOR GAGING STATION PACKAGE
   !     READ GAGING STATION LOCATIONS
   !     ******************************************************************
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   NUMGAGE = 0
   !
   !1------TURN OFF GAGE PACKAGE IF ACTIVE AND
   !         STREAMS AND LAKES ARE INACTIVE.
...
1     FORMAT(' GAGE PACKAGE ACTIVE EVEN THOUGH SFR AND LAK ',&
   ...
   END IF
   !
   !2------READ NUMBER OF GAGES, TURN OFF GAGE PACKAGE IF NUMBER OF GAGES
...
2     FORMAT(' NUMGAGE=0, SO GAGE IS BEING TURNED OFF')
   ...
   !
   !3------IGGLST ARRAY IS:
   !         (1) SEGMENT (or LAKE) NUMBER;
   !         (2) REACH NUMBER (NULL FOR LAKE);
   !         (3) UNIT #; and
   !         (4) OUTTYPE
   NGAGESAR = 1
   ...
   !
   !4------INITIALIZE GAGE COUNTERS.
   NSG=0
   NLG=0
   !
   !5------READ THE FIRST RECORD OF LIST.
   ...
   !7------LAKE GAGE.
            ...
            IGGLST(2,IOB)=0
   !
   !8------CHECK FOR NEGATIVE UNIT NUMBER, WHICH DESIGNATES THAT
   !         THAT OUTTYPE FOR A LAKE IS READ.
...
140 FORMAT(///I4,' GAGING STATIONS WERE SPECIFIED.',/5X,'(Lakes are ',&
   ...
   &,' WILL BE WRITTEN TO SEPARATE OUTPUT FILES REPRESENTED BY ',&
   &'FOLLOWING UNIT NUMBERS:',/)
141 FORMAT(///I4,' GAGING STATION WAS SPECIFIED.',/5X,'(Lakes are ',&
   &'identified by a negative value of the Lake Number)',/5X,'RECORDS'&
   &,' WILL BE WRITTEN TO SEPARATE OUTPUT FILE REPRESENTED BY ',&
   &'FOLLOWING UNIT NUMBER:')
...
END SUBROUTINE GWF2GAG7AR
   !
   !-------SUBROUTINE GWF2GAG7RP
SUBROUTINE GWF2GAG7RP(IUNITGWT,IUNITLAK,IUNITUZF,NSOL,IGRID)
   !     ******************************************************************
   !     GWF2GAG5RP GAGING STATIONS--WRITE HEADER LINES TO OUTPUT FILES
   !                       --DETERMINE & SAVE CROSS-REFERENCE INDEX
   !                       --RECORD INITIAL CONDITIONS FOR LAKE GAGES
   !     ******************************************************************
   ...
   END IF
   !
   !2------LOOP OVER GAGING STATIONS.
   DO 10 IOG=1,NUMGAGE
      IG=IGGLST(1,IOG)
      IG3=ABS(IGGLST(3,IOG))
      IF (IG.GT.0) THEN
   !
   !3------STREAM GAGE; SAVE STREAM REACH INDEX; WRITE HEADER LINES.
   ...
   !
   !4------CONVERT REACH NUMBER FROM SEGMENT LIST TO MASTER LIST.
               IGGLST(2,IOG)=IRCH
               GO TO 30
            END IF
20       CONTINUE
...
30       CONTINUE
   ...
   !
   !5------CHECK IF GAGE STATION IS FOR A DIVERSION (OUTTYPE IS 5).
   ...
   !
   !7------GET VARIABLE OUTTYPE.
               SELECT CASE (IGGLST(4,IOG))
                CASE (0)
                  WRITE (IG3,250)
                CASE (1)
   !dep  Revised output to include precipitation, et, and runoff
   ...
   ! LFK:  warning messages added below
                CASE(6)
                  WRITE (IG3,294) IOG
                CASE(7)
                  WRITE (IG3,294) IOG
      ...
      ELSE
   !
   !10-----LAKE GAGE; SAVE LAKE INDEX; WRITE HEADER LINES.
         LK=-IG
   ...
   !12-----GET VARIABLE OUTTYPE.
               SELECT CASE (IGGLST(4,IOG))
                ...
                CASE (1)
                  IF (IUNITUZF.LE.0) THEN
                     WRITE (IG3,306)
   !dep 6/9/2009  added 1 dummy term for time step lake budget error
                     WRITE (IG3,401) DUM,STAGES(LK),VOL(LK),DUM,DUM,&
                     &DUM,DUM,DUM,DUM,DUM,DUM,DUM,DUM,&
                     &DUM
                  ELSE
   !dep 6/9/2009 added 2 dummy terms for lake seepage to unsaturated zone and
   !dep          time step lake budget error
                     WRITE (IG3,310)
   ...
   !13-----TRANSPORT IS ON.
            ELSE
   !
   !14-----PREPARE ARRAY OF HEADER NAMES FOR MULTIPLE CONSTITUENTS.
...
1000           CONTINUE
   ...
   !-LFK     *              DUM,DUM,(DUMMY(LK,ISOL),ISOL=1,NSOL)
...
240 FORMAT (/2X,'*** ERROR ***   NSOL NEEDED BUT NOT DEFINED IN ',&
   ...
   !     minor format adjustments below by LFK, July 2006
...
308 FORMAT (4X,'"DATA: Time',7X,'Stage(H)',9X,'Volume',8X,&
   &'Precip.',10x,'Evap.',9x,'Runoff',7x,'GW-Inflw',6x,'GW-Outflw',&
   &7x,'SW-Inflw',6x,'SW-Outflw',5x,'Withdrawal',5x,'Lake-Inflx',4x,&
   &'Total-Cond.',7x,'Del-H-TS',7x,'Del-V-TS',6x,'Del-H-Cum',6x,&
   &'Del-V-Cum',2x,'Cum-Prcnt-Err "')
   !dep 4/20/2009 revised format to include lake seepage to UZF
...
310 FORMAT (4X,'"DATA: Time',7X,'Stage(H)',9X,'Volume',8X,'Precip.'&
   &,10x,'Evap.',5x,'LAK-Runoff',5x,'UZF-Runoff',7x,&
   &'GW-Inflw',6x,'GW-Outflw',5x,'LAK-to-UZF',7x,'SW-Inflw',6x,&
...
318 FORMAT('( 1X,''"DATA: Time'',9X,''Stage(H)'',7X,''Volume'',3X,'&
   ...
   !dep 4/20/2009 revised format to denote option 4
...
403 FORMAT (4X,1PE14.7,1X,0PF14.7,1X,16(1PE14.7,1X))
   !dep 4/20/2009 added one term to FORMATS 404 and 405
...
END SUBROUTINE GWF2GAG7RP
   !
   !
   ! SGWF2GAG5LO Lake GAGING STATIONS--RECORD DATA
   !dep 4/20/2009 added Lake seepage to unsaturated flow
SUBROUTINE SGWF2GAG7LO(IUNITGWT,IUNITUZF,CLAKE,GAGETM,GWIN,GWOUT,&
&SEEP,FLXINL,VOLOLD,CLKOLD,CLAKINIT,NSOL)
   !     ******************************************************************
   !     WRITE TIME SERIES OUTPUT FOR EACH LAKE GAGE
   !     EACH TIME SERIES IS WRITTEN TO A SEPERATE FILE
   !dep  FIXED MISS MATCH OF ARRAYS PASSED FROM GWF2LAK3BD   12/06/2005
   !     ******************************************************************
   USE GWFGAGMODULE
   USE GWFBASMODULE, ONLY: DELT
   USE GWFLAKMODULE, ONLY:NLAKES,RNF,VOL,STGNEW,PRECIP,EVAP,&
   &SURFIN,SURFOT,WITHDRW,SUMCNN,DELH,TDELH,&
   &VOLINIT,OVRLNDRNF,TSLAKERR,CMLAKERR,DELVOL,&
   &SEEPUZ,PRCPLK,BGAREA
   ...
   !     ------------------------------------------------------------------
   !
   !1------LOOP OVER LAKE GAGING STATIONS.
   DO 10 IOG=1,NUMGAGE
      IG1=IGGLST(1,IOG)
      IG3=ABS(IGGLST(3,IOG))
      IF (IG1.GT.0) THEN
      ...
      ELSE
   !
   !2------LAKE GAGE: WRITE TIME, STAGE, VOLUME, AND
   ...
   !lfk add 3/8/19
            ...
            IF(RNF(LK).LT.0.0) THEN
               ...
               RUNFRT = RUNF/DELT
   !      RUNFRT IS THE RATE OF OVERLAND RUNOFF TO THE LAKE
            END IF
   !
            SRIN = SURFIN(LK)*DELT
            SROT = SURFOT(LK)*DELT
   ...
   !3------TRANSPORT IS OFF.
            IF (IUNITGWT.LE.0) THEN
   !
   !4------GET VARIABLE OUTTYPE.
   !dep   4/17/2009 fixed output from uzfrunoff and ground inflow and outflow
               SELECT CASE (IGGLST(4,IOG))
                CASE (0)
                ...
                CASE (1)
                  ...
                  ELSE
   !dep   4/20/2009 added lake seepage to unsaturated zone
   ...
   !5------TRANSPORT IS ON.
            ELSE
   !
   !6------GET VARIABLE OUTTYPE.
   !dep 4/20/2009 changed variable names according to if volumes or
   !       volumetric rates
               SELECT CASE (IGGLST(4,IOG))
                CASE (0)
                  WRITE (LFRMAT,425) NSOL
...
745               CONTINUE
   ...
   !dep New option for printing times series of volumetric rates (7/6/2009)
                CASE (4)
                  WRITE (LFRMAT,426) NSOL
   !-LFK                   WRITE (IG3,LFRMAT) GAGETM,STGNEW(LK),VOLRATE,
                  WRITE (IG3,LFRMAT) GAGETM,STGNEW(LK),VOL(LK),&
...
10 CONTINUE
   ...
   !7------FORMATS
   !
   !lfk  change formats in following for consistency with p/o for initial conds.
   !dep  added one value to each format for printing percent error
...
END SUBROUTINE SGWF2GAG7LO
   !
   ! SUBROUTINE SGWF2GAG7SO
SUBROUTINE SGWF2GAG7SO(IUNITGWT,IUNITUZF,GAGETM,COUT,SFRQ,IBD,&
&NSOL)
   !     ******************************************************************
   !     WRITE TIME SERIES OUTPUT FOR EACH STREAM GAGE
   !     EACH TIME SERIES IS WRITTEN TO A SEPERATE FILE
   !     ******************************************************************
   USE GWFGAGMODULE
   ...
   !     ------------------------------------------------------------------
   !
   !1------LOOP OVER STREAM GAGING STATIONS.
   DO 10 IOG=1,NUMGAGE
      IG1=IGGLST(1,IOG)
      IG3=IGGLST(3,IOG)
      IF (IG1.GT.0) THEN
         II=IGGLST(2,IOG)
   !
   !2------DETERMINE STREAM DEPTH.
         DEPTH=STRM(7,II)
         IF (ISEG(1,IG1).EQ.0) THEN
            DEPTH=STRM(15,II)-STRM(3,II)
         END IF
   !
   !3------COMPUTE DIVERSION RATES IF OUTTYPE IS 5.
         IF(IGGLST(4,IOG).EQ.5) THEN
            IUPSEG=IDIVAR(1,IG1)
   ...
   !5------GET VARIABLE OUTTYPE.
            SELECT CASE (IGGLST(4,IOG))
             ...
             CASE (4)
               ...
               END IF
   !
   !6------OUTTYPE 5 IS USED TO PRINT TIME SERIES FOR A DIVERSION.
             CASE (5)
               WRITE (IG3,270) GAGETM,STRM(15,II),PMXDVRT,&
               &STRM(10,II),UPSTRFLW
   !
   !7------OUTTYPE 6 IS USED TO PRINT TIME SERIES FOR UNSATURATED FLOW.
             CASE (6)
               WRITE (IG3,275) GAGETM,STRM(15,II),STRM(7,II),&
               &STRM(19,II),SFRQ(1,II),STRM(11,II),&
               &STRM(21,II),STRM(22,II),STRM(23,II)
   !
   !8------OUTTYPE 7 IS USED TO PRINT WATER CONTENT PROFILES BENEATH
   !         STREAMBED.
             CASE (7)
               IF(IBD.NE.0) THEN
   ...
   !dep   Revised format to include precip, ET, and specified runoff
...
6              CONTINUE
   ...
   !dep   Revised format to include precip, ET, and specified runoff
               ...
               &(COUT(II,ISOL),CLOAD(ISOL),ISOL=1,NSOL)
   !
   !11-----OUTTYPE 5 IS USED TO PRINT TIME SERIES FOR A DIVERSION.
             CASE (5)
               DO 7 ISOL=1,NSOL
                  CLOAD(ISOL)=STRM(9,II)*COUT(II,ISOL)
7              CONTINUE
