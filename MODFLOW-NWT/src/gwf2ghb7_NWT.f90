...
SUBROUTINE GWF2GHB7AR(IN,IGRID)
   !     ******************************************************************
   !     ALLOCATE ARRAY STORAGE AND READ PARAMETER DEFINITIONS FOR GHB
   !     PACKAGE
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   ...
   !
   !1------IDENTIFY PACKAGE AND INITIALIZE NBOUND.
   WRITE(IOUT,1)IN
1  FORMAT(1X,/1X,'GHB -- GENERAL-HEAD BOUNDARY PACKAGE, VERSION 7',&
   ...
   NNPGHB=0
   !
   !2------READ MAXIMUM NUMBER OF GHB'S AND UNIT OR FLAG FOR
...
13    FORMAT(1X,'LISTS OF GENERAL-HEAD BOUNDARY CELLS WILL NOT BE',&
   ...
   NGHBVL=6+NAUX
   !
   !4------ALLOCATE SPACE FOR THE BNDS ARRAY.
   IGHBPB=MXACTB+1
   MXBND=MXACTB+MXPB
...
1000 FORMAT(1X,//1X,I5,' GHB parameters')
   ...
   !5A-----READ LIST OF CELLS WITHOUT INSTANCES.
...
SUBROUTINE GWF2GHB7RP(IN,IGRID)
   !     ******************************************************************
   !     READ GHB HEAD, CONDUCTANCE AND BOTTOM ELEVATION
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !
   !1------READ ITMP (NUMBER OF GHB'S OR FLAG TO REUSE DATA) AND
   ...
   !
   !2------DETERMINE THE NUMBER OF NON-PARAMETER GHB'S.
   IF(ITMP.LT.0) THEN
      WRITE(IOUT,7)
7     FORMAT(1X,/1X,&
...
99       FORMAT(1X,/1X,'THE NUMBER OF ACTIVE GHB CELLS (',I6,&
   ...
   NBOUND=NNPGHB
   !
   !1C-----IF THERE ARE ACTIVE GHB PARAMETERS, READ THEM AND SUBSTITUTE
   CALL PRESET('GHB')
   IF(NP.GT.0) THEN
      NREAD=NGHBVL-1
      DO 30 N=1,NP
...
30    CONTINUE
   ...
   DO 100 L=1,NBOUND
   !
   !5------GET COLUMN, ROW AND LAYER OF CELL CONTAINING BOUNDARY.
      IL=BNDS(1,L)
      IR=BNDS(2,L)
      IC=BNDS(3,L)
   !
   !6------IF THE CELL IS EXTERNAL THEN SKIP IT.
      IF(IBOUND(IC,IR,IL).LE.0) GO TO 100
   !
   !7------SINCE THE CELL IS INTERNAL GET THE BOUNDARY DATA.
      HB=BNDS(4,L)
...
100 CONTINUE
   ...
   !
   !3------PRINT NUMBER OF GHB'S IN CURRENT STRESS PERIOD.
...
SUBROUTINE GWF2GHB7FM(IGRID)
   ...
   CALL SGWF2GHB7PNT(IGRID)
   !
   !1------IF NBOUND<=0 THEN THERE ARE NO GENERAL HEAD BOUNDS. RETURN.
   IF(NBOUND.LE.0) RETURN
   !
   !2------PROCESS EACH ENTRY IN THE GENERAL HEAD BOUND LIST (BNDS).
   DO 100 L=1,NBOUND
   !
   !3------GET COLUMN, ROW AND LAYER OF CELL CONTAINING BOUNDARY.
      ...
      IC=BNDS(3,L)
   !
   !4------IF THE CELL IS EXTERNAL THEN SKIP IT.
      IF(IBOUND(IC,IR,IL).LE.0) GO TO 100
   !
   !5------SINCE THE CELL IS INTERNAL GET THE BOUNDARY DATA.
      HB=BNDS(4,L)
      C=BNDS(5,L)
   !
   !6------ADD TERMS TO RHS AND HCOF.
...
SUBROUTINE GWF2GHB7BD(KSTP,KPER,IGRID)
   ...
   IF(IBD.EQ.2) THEN
      ...
      &NBOUND,IOUT,DELT,PERTIM,TOTIM,IBOUND)
   END IF
   !
   !3------CLEAR THE BUFFER.
   DO 50 IL=1,NLAY
...
50 CONTINUE
   !
   !4------IF NO BOUNDARIES, SKIP FLOW CALCULATIONS.
   IF(NBOUND.EQ.0) GO TO 200
   !
   !5------LOOP THROUGH EACH BOUNDARY CALCULATING FLOW.
   DO 100 L=1,NBOUND
   !
   !5A-----GET LAYER, ROW AND COLUMN OF EACH GENERAL HEAD BOUNDARY.
      IL=BNDS(1,L)
      IR=BNDS(2,L)
      IC=BNDS(3,L)
      RATE=ZERO
   !
   !5B-----IF CELL IS NO-FLOW OR CONSTANT-HEAD, THEN IGNORE IT.
      IF(IBOUND(IC,IR,IL).LE.0) GO TO 99
   !
   !5C-----GET PARAMETERS FROM BOUNDARY LIST.
      HB=BNDS(4,L)
      C=BNDS(5,L)
      CCGHB=C
   !
   !5D-----CALCULATE THE FOW RATE INTO THE CELL.
      CHB=C*HB
      RRATE=CHB - CCGHB*HNEW(IC,IR,IL)
      RATE=RRATE
   !
   !5E-----PRINT THE INDIVIDUAL RATES IF REQUESTED(IGHBCB<0).
      IF(IBD.LT.0) THEN
...
62       FORMAT(1X,'BOUNDARY ',I6,'   LAYER ',I3,'   ROW ',I5,'   COL ',&
      ...
      END IF
   !
   !5F-----ADD RATE TO BUFFER.
      BUFF(IC,IR,IL)=BUFF(IC,IR,IL)+RATE
   !
   !5G-----SEE IF FLOW IS INTO AQUIFER OR OUT OF AQUIFER.
      IF(RATE.LT.ZERO) THEN
   !
   !5H------FLOW IS OUT OF AQUIFER SUBTRACT RATE FROM RATOUT.
         RATOUT=RATOUT-RRATE
      ELSE
   !
   !5I-----FLOW IS INTO AQIFER; ADD RATE TO RATIN.
         RATIN=RATIN+RRATE
      END IF
   !
   !5J-----IF SAVING CELL-BY-CELL FLOWS IN LIST, WRITE FLOW.  ALSO
   !5J-----FLOW TO BNDS.
99    IF(IBD.EQ.2) CALL UBDSVB(IGHBCB,NCOL,NROW,IC,IR,IL,RATE,&
      &BNDS(:,L),NGHBVL,NAUX,6,IBOUND,NLAY)
      BNDS(NGHBVL,L)=RATE
100 CONTINUE
   !
   !6------IF CELL-BY-CELL TERMS WILL BE SAVED AS A 3-D ARRAY, THEN CALL
   !6------UTILITY MODULE UBUDSV TO SAVE THEM.
   IF(IBD.EQ.1) CALL UBUDSV(KSTP,KPER,TEXT,IGHBCB,BUFF,NCOL,NROW,&
   &NLAY,IOUT)
   !
   !7------MOVE RATES, VOLUMES AND LABELS INTO ARRAYS FOR PRINTING.
200 RIN=RATIN
   ROUT=RATOUT
   VBVL(3,MSUM)=RIN
...
SUBROUTINE SGWF2GHB7PNT(IGRID)
   !  Change GHB data to a different grid.
   USE GWFGHBMODULE
   !
