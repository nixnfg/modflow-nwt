...
SUBROUTINE GWF2HUF7AR(IN,ILVDA,IKDEP,IGRID)
   !
   !     ******************************************************************
   !     ALLOCATE ARRAY STORAGE FOR HYDROGEOLOGIC UNIT PACKAGE
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   USE GLOBAL,      ONLY:NCOL,NROW,NLAY,BOTM,ITRSS,LAYHDT,LAYHDS,&
   &IOUT,ISSFLG,NPER,IBOUND,LBOTM
   USE GWFBASMODULE,ONLY:HDRY
   USE GWFHUFMODULE,ONLY:IHUFCB,NHUF,NPHUF,IWETIT,IHDWET,IOHUFHDS,&
   &IOHUFFLWS,WETFCT,HGUNAM,LTHUF,LAYWT,&
   &IHGUFLG,HGUHANI,HGUVANI,HUFHK,HUFVK,HUFSS,&
...
18 FORMAT(1X,'FLOWS IN HYDROGEOLOGIC UNITS WILL BE SAVED',&
   &' ON UNIT',I3)
   !
   !  Allocate space in parameter-information arrays
...
31    FORMAT(1X,/1X,'KDEP1 -- HYDRAULIC CONDUCTIVITY DEPTH-DECAY&
   ...
   !
   !4A-----PRINT TABLES OF VALUES FOR LTHUF, HGUHANI, HGUVANI
   !4B-----BASED ON LTHUF, HUFLAYAVG, HGUHANI, LAYWT, COUNT THE NUMBER OF EACH
   !4B-----TYPE OF 2-D ARRAY; CHECK VALUES FOR CONSISTENCY; AND SETUP
   !4B-----POINTERS IN LTHUF, HGUHANI, AND LAYWT FOR CONVENIENT ACCESS
...
78    FORMAT(1X,I4,2(I10,A14))
   !     SET GLOBAL HEAD-DEPENDENT THICKNESS INDICATOR
      IF (LTHUF(K).NE.0) THEN
         LAYHDT(K)=1
         LAYHDS(K)=1
      ELSE
...
50 CONTINUE
   !
   !
   !5------COMPUTE THE NUMBER OF CELLS IN THE ENTIRE GRID AND IN ONE LAYER.
   !     NRC=NROW*NCOL
   !
   ...
   !7------READ DATA FOR HYDROGEOLOGIC-UNIT FLOW PACKAGE

   !---Read rewetting information
   IWDFLG=0
   KLAYFLG=0
   DO 10 K=1,NLAY
...
112   FORMAT(1X,/,1X,'WETTING CAPABILITY IS ACTIVE IN',I4,' LAYERS')
   ...
   END IF
   !
   !2H-----READ WETDRY CODES IF WETTING CAPABILITY HAS BEEN INVOKED
   !2H-----(LAYWT NOT 0).
   DO 300 K=1,NLAY
      IF(LAYWT(K).NE.0) THEN
...
147 FORMAT(&
   ...
   &/1X,75('-'))
   !
   !-------READ HYDROGEOLOGIC-UNIT GEOMETRY
   Call SGWF2HUF7GEOMRP(IN)
   !
   !---Read HANI and VANI values for each named unit
   DO 100 NU=1,NHUF
      READ(IN,'(A)') LINE
   !  Get the name of the new unit
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,IOUT,IN)
   !  Find the unit name in the list
...
20    CONTINUE
   END IF
   !
   !  Make some basic parameter checks
...
480 CONTINUE
   ...
   END DO
   IF(ISS.EQ.0) THEN
   !   IF CONVERTIBLE LAYER GET PRIMARY STORAGE
      DO  I=1,NROW
         DO  J=1,NCOL
            K = 0
            KK = 1
...
SUBROUTINE SGWF2HUF7GEOMRP(IN)
   !
   !     ******************************************************************
   !     Read and prepare HYDROGEOLOGIC-UNIT GEOMETRY.
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !     ------------------------------------------------------------------
   !
   !-----Read the hydrogeologic-unit names and arrays
   DO 100 M=1,NHUF
   !  Read a line describing a hydrogeologic unit
      READ (IN,'(A)') LINE
   !  Get the name of the new unit
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,0,N,R,IOUT,IN)
   !  Add new unit name into list
      HGUNAM(M)=LINE(ISTART:ISTOP)
   !  Read top elevation of unit
      ANAME=' TOP ELEVATN: '//HGUNAM(M)
      CALL U2DREL(HUFTHK(:,:,M,1),ANAME,NROW,NCOL,0,IN,IOUT)
   !  Read thickness of unit
...
SUBROUTINE SGWF2HUF7SP(KITER,KSTP,KPER,ILVDA)
   ...
   &BOTM,NBOTM,DELR,DELC,IOUT,ITRSS
   !
   !      USE PARAMMODULE, ONLY:RMLT,IZON,NMLTAR,NZONAR
   USE PARAMMODULE
   USE GWFBASMODULE,ONLY:HDRY
   !
   ...
   !     ------------------------------------------------------------------
   !
   ! Check for cells that GO DRY/REWET
   DO 5 K=1,NLAY
...
115         CONTINUE
   ...
   !
   !---Populate HGU arrays for given i,j depending on parameter type
         ...
         ENDIF
   !
   !---Populate HANI and VANI from input file if not already defined by
   !     a parameter
         DO 120 NU=1,NHUF
...
120      CONTINUE
   ...
   !
            ...
            &KSTP.EQ.0) THEN
               TOPU = HUFTHK(J,I,NU,1)
               BOTU = TOPU - THCKU
   !-----Determine which layer(s) unit applies to, assuming for the sake of
   !       computing SC1 that every layer is confined
               IFLG=0
               CALL SGWF2HUF7HSRCH(NCOL,NROW,NLAY,BOTM,NBOTM,I,J,TOPU,BOTU,&
               &HNEW,IBOUND,KT,KB,IFLG)
   !-----Note that a check for IFLG=1 is not needed here, as such a
   !        condition would have been caught after the earlier call
   !        to subroutine SGWF2HUF7HSRCH
   !-----Populate SC1 array
...
100 CONTINUE
   ...
   &KSTP.EQ.0) THEN
   !
   !-------CHECK FOR SYTP PARAMETERS
      CALL SGWF2HUF7SYTP(SC1,IBOUND,NCOL,NROW,NLAY)
   !
   !-------MULTIPLY BY DELR*DELC TO GET SC1
   ...
   !-----Print HUF data
   IF(KITER.EQ.0) THEN
   !-----Print unit arrays depending on flags
      DO 200 NU=1,NHUF
         DO 210 IP=1,5
            IF(IHGUFLG(IP,NU).GT.0) THEN
               ...
               ELSE
                  IA=IP
               ENDIF
   !-----Populate HUFHK as a temporary holding array
               DO 220 J=1,NCOL
                  DO 220 I=1,NROW
                     HUFHK(NU)=0
...
&HKCC,HUFHK,HUFHANI,LAMBDA,NHUF,NU,HNEW,GS)
   ...
   !---Adjust top elevation for water-table layers
   ...
   !            THCK=TOPL-BOTL
         ENDIF
         THCK=TOP1-BOT1
   !---Check for small or negative thickness
...
&VK,KL,NCOL,NROW,NLAY,BOTM,NBOTM,HNEW,IBOUND,IOUT,GS,HUFTHK,NHUF)
   !
   !     ******************************************************************
   !     Calculate VK for a given layer
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
      ...
      DO 200 I=1,NROW
         DO 210 J=1,NCOL
            IF(IBOUND(J,I,KL).EQ.0) THEN
               ...
               GOTO 210
            ENDIF
   !           FIND TOP AND BOTTOM LAYERS THIS UNIT APPLIES TO
            TOPU=HUFTHK(J,I,NU,1)
            THCKU=HUFTHK(J,I,NU,2)
   ...
   !           IF IN THIS LAYER, CONTRIBUTE TO VK
   !           GET THICKNESS OF UNIT IN THIS LAYER
...
&HNEW,IBOUND,KT,KB,IFLG)
   !
   !     ******************************************************************
   !     Search for top and bottom layer the unit applies to.
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   IFLG=1
   !
   ! Loop through layers to determine where unit applies
   !
   ! First, search for top
   DO 100 KT=1,NLAY-1
      IF(IBOUND(J,I,KT).EQ.0.OR.IBOUND(J,I,KT+1).EQ.0) GOTO 100
      TOP1=BOTM(J,I,LBOTM(KT)-1)
   !---Adjust top of model layer for unconfined layer
      IF(LTHUF(KT).NE.0.AND.HNEW(J,I,KT).LT.TOP1) TOP1=HNEW(J,I,KT)
      TOP2=BOTM(J,I,LBOTM(KT))
      TOP3=BOTM(J,I,LBOTM(KT)+1)
      RMID1=(TOP1+TOP2)/2.
      RMID2=(TOP2+TOP3)/2.
   !---If unit bottom is above the middle of the top layer, return
   !        IF(KT.EQ.1.AND.TOPU.GT.RMID1.AND.BOTU.GE.RMID1) THEN
      IF(TOPU.GT.RMID1.AND.BOTU.GE.RMID1) THEN
         RETURN
      ENDIF
   !---If unit top is above middle of top layer, exit loop
   !        IF(KT.EQ.1.AND.TOPU.GT.RMID1.AND.BOTU.LT.RMID1) GOTO 110
      IF(TOPU.GT.RMID1.AND.BOTU.LT.RMID1) GOTO 110
   !---If unit top is between the middle of this layer and the middle of the
   !     next layer, exit loop
      IF(TOPU.LE.RMID1.AND.TOPU.GE.RMID2) GOTO 110
100 CONTINUE
   RETURN
110 CONTINUE
   ! Now search for bottom
   ...
   !---Adjust top of model layer for unconfined layer
      ...
      RMID1=(TOP1+TOP2)/2.
      RMID2=(TOP2+TOP3)/2.
   !---If unit bottom is between the middle of this layer and the middle of
   !     the next layer, return
      IF(BOTU.LE.RMID1.AND.BOTU.GE.RMID2) THEN
         KB=KKB
         IFLG=0
         RETURN
      ENDIF
   !---If entire unit is below the middle of the bottom layer, return
      IF(KKB.EQ.NLAY-1.AND.TOPU.LE.RMID2.AND.BOTU.LT.RMID2) THEN
         RETURN
      ENDIF
   !---If unit bottom is below the middle of the bottom layer, return
...
&HNEW,IBOUND,KT,KB,IFLG)
   ...
   !     ******************************************************************
   !     Search for top and bottom layer the unit applies to.
   !     Values for IFLG on input:
   !       IFLG = 0, Do not adjust top of unconfined layer to HNEW
   !       IFLG <> 0, Adjust top of unconfined layer to HNEW
   !     Values for IFLG on output:
   !       IFLG = 0, Unit successfully found
   !       IFLG = 1, Unit not found
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !---Adjust top of model layer for unconfined layer (if IFLG<>0 on input)
      IF(IHNEW.NE.0.AND.LTHUF(KT).NE.0.AND.HNEW(J,I,KT).LT.TOP)&
      &TOP=HNEW(J,I,KT)
   !---If unit top is in this layer, exit loop
      IF(TOPU.LE.TOP.AND.TOPU.GT.BOTM(J,I,LBOTM(KT))) GOTO 110
   !---If unit top is above model top, adjust unit top elevation, exit loop
      IF(TOPU.GT.TOP) THEN
         TOPU=TOP
         GOTO 110
      ENDIF
100 CONTINUE
   RETURN
110 CONTINUE
   !---If unit top has been adjusted to below unit bottom, return
   ...
   !---Adjust top of model layer for unconfined layer (if IFLG<>0 on input)
      ...
      &TOP=HNEW(J,I,KKB)
   !---If unit bottom is in this layer, set KB=KKB, return
      IF(BOTU.LE.TOP.AND.BOTU.GE.BOTM(J,I,LBOTM(KKB))) THEN
         KB=KKB
         IFLG=0
         RETURN
      ENDIF
   !---If top of model layer has been adjusted for unconfined unit and
   !    unit bottom is in the gap, set KB=KKB-1, return
      IF(BOTU.GT.TOP) THEN
         KB=KKB-1
         ...
         RETURN
      ENDIF
   !---If unit bottom is below model bottom, set KB=NLAY, return
      ...
      ENDIF
200 CONTINUE
   !---If KB has not been found due to inactive cells, search again from bottom
   DO 300 KKB=NLAY,1,-1
...
&NROW,NLAY,IOUT,WETDRY,BOTM,NBOTM)
   ...
   IZFLG=0
   !
   !-------CONVERT CELL TO NO FLOW IF CELL THICKNESS IS 0.
   DO 30 K=1,NLAY
      KB=LBOTM(K)
      KT=KB-1
      DO 30 I=1,NROW
...
30 CONTINUE
   !
   !1------INSURE THAT EACH ACTIVE CELL HAS AT LEAST ONE NON-ZERO
   !1------TRANSMISSIVE PARAMETER.
   DO 60 K=1,NLAY
      IF(LAYWT(K).NE.0) THEN
   !
   !2------WETTING IS ACTIVE.
   ...
   !
   !2A-----CHECK HORIZONTAL HYDRAULIC CONDUCTIVITY (HK).
               IF(HK(J,I,K).NE.ZERO) GO TO 40
   !
   !2C-----ALL TRANSMISSIVE TERMS ARE ALL 0, SO CONVERT CELL TO NO FLOW.
               IBOUND(J,I,K)=0
               HNEW(J,I,K)=HCNV
...
40       CONTINUE
   ...
   !3B-----VERTICAL HYDRAULIC CONDUCTIVITY.
               ...
               END IF
   !
   !3C-----ALL TRANSMISSIVE TERMS ARE 0, SO CONVERT CELL TO NO FLOW.
               IZFLG=1
               IBOUND(J,I,K)=0
               HNEW(J,I,K)=HCNV
               WRITE(IOUT,43) K,I,J
...
&KITER,KSTP,KPER)
   ...
   !     ------------------------------------------------------------------
   !
   !4------CALCULATE HOR. CONDUCTANCE(CR AND CC) FOR CONSTANT T LAYERS.
   DO 70 K=1,NLAY
   !-------CONVERT HK TO HYDRAULIC CONDUCTVITY AND SC1 TO SPECIFIC STORAGE
      DO 71 I=1,NROW
         DO 71 J=1,NCOL
...
&NCOL,NROW,NLAY)
   !
   !     ******************************************************************
   !     CALCULATE VERTICAL CONDUCTANCES
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE SGWF2HUF7SYTP(SC1,IBOUND,NCOL,NROW,NLAY)
   !
   !     ******************************************************************
   !     REPLACE SC1 WITH SY FOR TOPMOST ACTIVE CELL FOR ANY I,J
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !
   !
   ! Loop through parameters to determine if there are SYTP parameter(s)
   IPFLG=0
   DO 10 NP=1,MXPAR
...
15 CONTINUE
   ...
   ! Loop through and zero out SC1 in appropriate spots to prepare for
   !   additive parameters
   DO 20 I=1,NROW
      DO 30 J=1,NCOL
         DO 40 K=1,NLAY
...
20 CONTINUE
   ...
   !         SEARCH DOWN TO FIND HIGHEST ACTIVE CELL
                  DO 250 K=1,NLAY
                     IF(IBOUND(J,I,K).NE.0) THEN
...
SUBROUTINE SGWF2HUF7SC(IBOUND,SC1,DELR,DELC,NCOL,NROW,NLAY)
   ...
   !
   DO 70 K=1,NLAY
   !-------CONVERT SC1 TO SPECIFIC STORAGE
      DO 70 I=1,NROW
         DO 70 J=1,NCOL
...
SUBROUTINE GWF2HUF7AD(KPER,IGRID)
   !
   !%%%%%
   !  PLEASE NOTE: THIS SUBROUTINE WAS COPIED DIRECTLY FROM THE LPF
   ! PACKAGE.  THERE IS NOTHING SPECIFIC TO THE HUF PACKAGE.
   !%%%%%
   !     ******************************************************************
...
50          CALL SGWF2HUF7WDMSG(2,NCNVRT,ICNVRT,JCNVRT,ACNVRT,IHDCNV,&
            &IOUT,KITER,J,I,K,KSTP,KPER)
   !
   !7A-----USE EQUATION 3A IF IHDWET=0; ISE EQUATION 3B IF IHDWET IS NOT 0.
            IF(IHDWET.EQ.0) THEN
...
SUBROUTINE GWF2HUF7FM(KITER,KSTP,KPER,ILVDA,IGRID)
   ...
   &CR,CC,CV,HNEW,RHS,HCOF,HOLD,ISSFLG,IOUT
   ...
   ISS=ISSFLG(KPER)
   !
   !1------IF ANY LAYER IS CONVERTIBLE REPOPULATE ARRAYS AND CALCULATE
   !          BRANCH CONDUCTANCES
   KLAYFLG=0
   DO 100 K=1,NLAY
      IF(LTHUF(K).NE.0) KLAYFLG=1
100 CONTINUE
   ...
   !3------SEE IF THIS LAYER IS CONVERTIBLE OR NON-CONVERTIBLE.
         IF(LTHUF(K).EQ.0) THEN
   !4------NON-CONVERTIBLE LAYER, SO USE PRIMARY STORAGE
            DO 140 I=1,NROW
               DO 140 J=1,NCOL
...
140         CONTINUE
   ...
   !5------WHEN TO USE PRIMARY AND SECONDARY STORAGE
            DO 180 I=1,NROW
               DO 180 J=1,NCOL
   !
   !5A-----IF THE CELL IS EXTERNAL THEN SKIP IT.
...
200   CONTINUE
   END IF
   !
   !6------FOR EACH LAYER DETERMINE IF CORRECTION TERMS ARE NEEDED FOR
   !6------FLOW DOWN INTO PARTIALLY SATURATED LAYERS.
   DO 300 K=1,NLAY
   !
   !7------SEE IF CORRECTION IS NEEDED FOR LEAKAGE FROM ABOVE.
      IF(LTHUF(K).NE.0 .AND. K.NE.1) THEN
   !
   !7A-----FOR EACH CELL MAKE THE CORRECTION IF NEEDED.
         DO 220 I=1,NROW
            DO 220 J=1,NCOL
   !
   !7B-----IF THE CELL IS EXTERNAL(IBOUND<=0) THEN SKIP IT.
               IF(IBOUND(J,I,K).LE.0) GO TO 220
               HTMP=HNEW(J,I,K)
   !
   !7C-----IF HEAD IS ABOVE TOP THEN CORRECTION NOT NEEDED
               TOP=BOTM(J,I,LBOTM(K)-1)
               IF(HTMP.GE.TOP) GO TO 220
   !
   !7D-----WITH HEAD BELOW TOP ADD CORRECTION TERMS TO RHS.
               RHS(J,I,K)=RHS(J,I,K) + CV(J,I,K-1)*(TOP-HTMP)
220      CONTINUE
      END IF
   !
   !8------SEE IF THIS LAYER MAY NEED CORRECTION FOR LEAKAGE TO BELOW.
   ...
   !8A-----FOR EACH CELL MAKE THE CORRECTION IF NEEDED.
         DO 280 I=1,NROW
            DO 280 J=1,NCOL
   !
   !8B-----IF CELL IS EXTERNAL (IBOUND<=0) THEN SKIP IT.
               IF(IBOUND(J,I,K).LE.0) GO TO 280
   !
   !8C-----IF HEAD IN THE LOWER CELL IS LESS THAN TOP ADD CORRECTION
   !8C-----TERM TO RHS.
               HTMP=HNEW(J,I,K+1)
...
&HUFTHK,NCOL,NROW,NHUF,AREA,IOUT)
   !
   !     ******************************************************************
   !     Compute contributions to HCOF and RHS for convertible cell
   !     Enter subroutine when HO and/or HN are below TOP
   !     Values for IFLG:
   !       IFLG = 0, Calculate contributions to HCOF and RHS
   !       IFLG = 1, Calculate contributions to flow within cell
   !       IFLG = 2, Calculate contributions to sensitivity calculations
   !     Subroutine will halt execution if Sy is not defined.
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   DO 100 NP=1,MXPAR
      IF(PARTYP(NP).EQ.'SY') THEN
         BNP=B(NP)*AREA*TLED
   !---------Loop through units for this parameter to determine if they apply
   !         to this layer
         DO 200 ND=IPLOC(1,NP),IPLOC(2,NP)
            NU=IPCLST(1,ND)
            NM=IPCLST(2,ND)
            ...
            THCKU=HUFTHK(J,I,NU,2)
            BOTU=TOPU-THCKU
   !-----------Skip this unit if it is not present in this layer
   ...
   !-----------------Old head is above and new head is below this unit
                        CRHS=CRHS+RMLT0*BNP*(TOPU-BOTU)
                     ENDIF
                  ELSE
   !---------------Water table is going up
                     IF(HO.LT.TOPU.AND.HO.GT.BOTU .AND.&
                     &HN.LT.TOPU.AND.HN.GT.BOTU) THEN
   !-----------------Old and new heads are both in this unit
                        CHCOF=CHCOF+RMLT0*BNP
   ...
   !-----------------Old head is below and new head is abov this unit
                        CRHS=CRHS+RMLT0*BNP*(BOTU-TOPU)
                     ENDIF
                  ENDIF
               ENDIF
...
SUBROUTINE GWF2HUF7BDS(KSTP,KPER,IGRID)
   !     ******************************************************************
   !     COMPUTE STORAGE BUDGET FLOW TERM FOR HUF.
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   USE GLOBAL,      ONLY:NCOL,NROW,NLAY,ISSFLG,IBOUND,HNEW,HOLD,BOTM,&
   &LBOTM,DELR,DELC,BUFF,IOUT
   ...
   CALL SGWF2HUF7PNT(IGRID)
   !
   !2------INITIALIZE BUDGET ACCUMULATORS AND 1/DELT.
   ISS=ISSFLG(KPER)
   ZERO=0.
   STOIN=ZERO
   STOUT=ZERO
   !
   !1------SKIP STORAGE BUDGET CALCULATIONS IF STEADY STATE.
...
7     CONTINUE
   ...
   !
   !7B-----CALCULATE FLOW THROUGH THE LEFT FACE.
            IF(ILVDA.GT.0) THEN
               CHCH1 = -DFL
            ELSE
               HDIFF=HNEW(J,I,K)-HNEW(J-1,I,K)
...
SUBROUTINE GWF2HUF7OT(KSTP,KPER,ICNVG,ISA,IGRID)
   !
   !     ******************************************************************
   !     PRINT AND RECORD HEADS AND FLOWS INTERPOLATED TO HGU'S
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
17       FORMAT(1X,/11X,'****FAILED TO CONVERGE IN TIME STEP',I6,& !gsf
   ...
   IF(IOHUFHDS.GT.0) THEN
      ...
      DO 100 NU=1,NHUF
   !
   !-------CALL ULAPRS OR ULAPRW TO PRINT HEAD.
...
72       FORMAT('1',&
         ...
         &NCOL,NROW,0,IHEDFM,IOUT)
   !
   !5------FOR EACH UNIT: DETERMINE IF HEAD SHOULD BE SAVED ON DISK.
...
&IBD,KSTP,KPER,DELT,PERTIM,TOTIM,ICHFLG)
   !
   !     ******************************************************************
   !     CALCULATE FLOWS WITHIN A GIVEN HYDROGEOLOGIC UNIT
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
100   CONTINUE
   ...
   !        WRITE(IOUT,1) TEXT(ICNT),IHUFCB,KSTP,KPER
   !    1   FORMAT(1X,'SGWF2HUF7FLOT SAVING "',A16,'" ON UNIT',I3,
...
&HUFTHK,BOTM,NBOTM,CV,DELR,DELC,HNOFLO,IOUT,GS)
   !
   !     ******************************************************************
   !     CALCULATE HEADS WITHIN A GIVEN HYDROGEOLOGIC UNIT
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
210      CONTINUE
   !
   !---Populate HGU arrays depending on parameter type
         CALL UHUF7POP(HUFHK,'HK  ',I,J,0,IOUT)
         CALL UHUF7POP(HUFVK,'VK  ',I,J,0,IOUT)
         CALL UHUF7POP(HUFVK,'VANI',I,J,0,IOUT)
         CALL UHUF7POP(HUFKDEP,'KDEP',I,J,0,IOUT)
   !
   !---Populate VANI from input file if not already defined by
   !     a parameter
         DO 220 NU=1,NHUF
            IF(HGUVANI(NU).GT.0..AND.HUFVK(NU).EQ.0.)&
            &HUFVK(NU)=HGUVANI(NU)
220      CONTINUE
   ...
   !
            IF(KT.EQ.KB) THEN
               ...
               ENDIF
            ELSE
   !---------FIRST, PROPOGATE HEAD FROM MIDDLE OF CELL TO MIDDLE OF UNIT
               DO 400 KL=KT,KB
                  ...
                  ENDIF
400            CONTINUE
   !---------NOW, PROPOGATE CELL TO BOTTOM OF UNIT
...
&IBD,KSTP,KPER,ICHFLG)
   !     ******************************************************************
   !     COMPUTE FLOW FROM CONSTANT-HEAD CELLS IN HGU's
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !5----------IF CELL IS NOT CONSTANT HEAD SKIP IT & GO ON TO NEXT CELL.
               ...
               H0 = HNEW(J,I,K)
   !7----------CALCULATE FLOW THROUGH THE LEFT FACE.
   !           IF THERE IS NO FLOW TO CALCULATE THROUGH THIS FACE, THEN GO ON
   !           TO NEXT FACE.  NO FLOW OCCURS AT THE EDGE OF THE GRID, TO AN
   !           ADJACENT NO-FLOW CELL, OR TO AN ADJACENT CONSTANT-HEAD CELL.
               IF(J.EQ.1) GO TO 30
               IF(IBOUND(J-1,I,K).EQ.0) GO TO 30
               IF(IBOUND(J-1,I,K).LT.0 .AND. ICHFLG.EQ.0) GO TO 30
   !
...
152            HDIFF=HNEW(J,I,K)-HD
               CHCH6=HDIFF*CV(J,I,K)
   !
   !13---------SUM THE FLOWS THROUGH SIX FACES OF CONSTANT HEAD CELL
180            RATE=CHCH1+CHCH2+CHCH3+CHCH4+CHCH5+CHCH6
400         CONTINUE
   !
   !13-------STORE SUM IN BUFFER.
...
&IPT,JPT,KPT,GPT,CRL,CRR,CCT,CCB,THK0,THKL,&
&THKR,THKT,THKB,HNEW,DELR,DELC,BOTM,NCOL,NROW,NLAY,&
&IOUT,NHUF,NBOTM,HUFTHK,GS)
   !
   !     ******************************************************************
   !     CALCULATE HORIZONTAL BRANCH CONDUCTANCES FOR SINGLE HGU OR FOR
   !     LAYER WITHIN AN HGU.  FOR UNITS THAT PINCH OUT, THE EQUIVALENT
   !     THICKNESS IN THE ADJACENT UNIT IS USED
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   CCB = 0.0
   !
   ! Loop through 5 times and calculate conductances
   DO 100 ICNT=1,5
      I = IPT
      J = JPT
      IF(ICNT.EQ.2) THEN
   ...
   ! Zero out arrays
      ...
      HUFHANI(GPT)=0.
      HUFKDEP(GPT)=0.
   !---Get thickness of unit within cell
   ...
   !---Calculate transmissivity for cell that pinches out
...
200      CONTINUE
         THK = THK0
      ELSE
   !---Otherwise, calculate transmissivities of cell
...
SUBROUTINE SGWF2HUF7IND(NCOL,NROW,NHUF,HUFTHK,JJ,II,INDX)
   !
   !     ******************************************************************
   !     INDEX HYDROGEOLOGIC UNITS FOR A GIVEN ROW/COLUMN LOCATION
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
&IN,IOUT,ITERP,NPKDEP,IFKDEP,NROW,NCOL,GS,TOP,NHUF)
   !
   !     ******************************************************************
   !     READ PARAMETERS FOR KDEP CAPABILITY
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
47 FORMAT(&
   ...
   &/1X,75('-'))
   !
   !-------READ GROUND SURFACE OR TRANSFER FROM BOTTOM ARRAY
   IF(IFKDEP.GT.0) THEN
...
&LAMBDA,TOP,BOT,GS,MULTKDEP)
   !
   !     ******************************************************************
   !     Calculate KDEP multiplier for interval
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
&IN,IOUT,ITERP,NHUF,NPLVDA,NLAY,ISEN)
   !
   !     ******************************************************************
   !     READ PARAMETERS FOR LVDA CAPABILITY
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
20    CONTINUE
   END IF
   !
   !-----CHECK TO SEE IF THE SEN PROCESS IS ACTIVE AND THERE ARE CONVERTIBLE
   !     LAYERS
   IF(ISEN.GT.0) THEN
      KLAYFLG=0
      DO 30 K=1,NLAY
...
&HNEW,IBOUND,CR,CC,VDHT,RHS,NCOL,NROW,NLAY,A9)
   !
   !     ******************************************************************
   !     Calculate stiffness matrix and RHS for LVDA
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   DO 600 K=1,NLAY
      DO 610 I=1,NROW
         DO 620 J=1,NCOL
   !-------Skip calculation if cell is inactive
            IF(IBOUND(J,I,K).EQ.0) THEN
               CR(J,I,K)=ZERO
               CC(J,I,K)=ZERO
            ELSE
   !
   !-------Grab transmissivities
   !---------Local cell 1, I,J
               TAA1 = VDHT(J,I,K,1)
               TBB1 = VDHT(J,I,K,3)
   !-------Calculate CR equivalent
               IF(J.NE.NCOL) THEN
                  IF(IBOUND(J+1,I,K).NE.0) THEN
                     TAA2 = VDHT(J+1,I,K,1)
   ...
   !-------and Calculate CC equivalent
               ...
               END IF
   !
   !-------and subtract elements from RHS
...
&HUFTHK,NHUF,BOTM,NBOTM,HNEW,GS)
   !
   !     ******************************************************************
   !     Populate the Hydraulic Conductivity Arrays for LVDA.
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
&IBOUND,HK,HKCC,VDHD,VDHT,NCOL,NROW,NLAY,DELR,DELC)
   !
   !     ******************************************************************
   !     Populate the Hydraulic Conductivity Tensor for LVDA.
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !
   PI=3.14159265
   !-------Loop through all cells, calculate Taa, Tab, and Tbb
   DO 500 K=1,NLAY
      DO 510 I=1,NROW
         DC=DELC(I)
         DO 520 J=1,NCOL
            ...
            IF(IBOUND(J,I,K).NE.0) THEN
               ...
               TAA=TAA*DC/DR
   !---Minus sign deleted from beginning of following line 060502, according
   !   to Roseanna Neupauer
...
&FL,FR,FT,FB)
   !     ******************************************************************
   !     COMPUTE FLOW THROUGH CELL FACES USING THE 9-POINT STENCIL
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE SGWF2HUF7VDA9F9(I,J,K,A9,HNEW,IBOUND,NLAY,NROW,NCOL,F)
   !     ******************************************************************
   !     COMPUTE FLOW THROUGH CELL FACES USING THE STORED 9-POINT STENCIL
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE SGWF2HUF7VDA9(VDHT,A9,IBOUND,NLAY,NROW,NCOL)
   !     ******************************************************************
   !     COMPUTE AND STORE STIFFNESS-COEFFICIENT MATRIX FOR 9-POINT STENCIL
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
130         CONTINUE
   ...
   !
   !
   !-------CALCULATE COEFFICIENT FOR I,J,K (LOCAL 1)
            C = 0.0
   !--FL
            IF(J.GT.1) THEN
   ...
   !            C = C - (CVB+CVT)
            A9(J,I,K,1) = C
   !
   !-------CALCULATE COEFFICIENT FOR I,J+1,K (LOCAL 2)
            IF(J.LT.NCOL) THEN
               IF(IBOUND(J+1,I,K).NE.0) THEN
                  C =     (TAA1+TAB1)*AS21234
            ...
            ENDIF
   !
   !-------CALCULATE COEFFICIENT FOR I+1,J-1,K (LOCAL 5)
            IF(I.LT.NROW) THEN
               IF(J.GT.1) THEN
               ...
               ENDIF
   !
   !-------CALCULATE COEFFICIENT FOR I+1,J,K (LOCAL 4)
               IF(IBOUND(J,I+1,K).NE.0) THEN
                  C =     (TAA1+TAB1)*AS41234
               ...
               ENDIF
   !
   !-------CALCULATE COEFFICIENT FOR I+1,J+1,K (LOCAL 3)
               IF(J.LT.NCOL) THEN
                  IF(IBOUND(J+1,I+1,K).NE.0) THEN
                     C =     (TAA1+TAB1)*AS31234
...
&IBD2,IBD3,IBD4,IBD5,IBD6,IBD7,IBD8,IBD9)
   !
   !     ******************************************************************
   !     Grab heads and ibound for all local cells
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
&TAA1,TAB1,TBB1)
   !
   !     ******************************************************************
   !     Calculate the A coefficients for Quadrant I
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
&AE16145,AE26145,AE36145,AE46145)
   !
   !     ******************************************************************
   !     Calculate the A coefficients for Quadrant II
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
&AE17816,AE27816,AE37816,AE47816)
   !
   !     ******************************************************************
   !     Calculate the A coefficients for Quadrant III
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
&AW18921,AW28921,AW38921,AW48921)
   !
   !     ******************************************************************
   !     Calculate the A coefficients for Quadrant IV
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE SGWF2HUF7PNT(IGRID)
   !
   !     ******************************************************************
   !     POINT TO HUF DATA FOR A SPECIFIC GRID
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
