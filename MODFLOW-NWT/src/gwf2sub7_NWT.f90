...
SUBROUTINE GWF2SUB7AR(IN,IGRID)
   !     ******************************************************************
   !     ALLOCATE ARRAY STORAGE FOR SUBSIDENCE PACKAGE.
   !     READ SUBSIDENCE PACKAGE DATA.
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   &AC1,AC2,NDF,NNDF,ISBOCF,ISBOCU,&
   &OCFLGS,OCLAY,ILSYS,NTSSUM,LN,LDN,NZ,RNB,&
   &DH,DHP,DHC,DZ,HC,SCE,SCV,DCOM,A1,A2,BB,&
   &SUB,DP,DVB
   !
   DIMENSION IFL(13)
...
1  FORMAT(/,'SUB7 -- SUBSIDENCE PACKAGE, VERSION 7,',&
   &' 03/31/2006',' INPUT READ FROM UNIT',I3)
   !
...
12 CONTINUE
   !
   !3------ALLOCATE SPACE FOR ARRAY NTSSUM, WHICH WILL CONTAIN THE TOTAL
   !3------NUMBER OF TIME STEPS PRIOR TO THE CURRENT TIME STEP.
...
50 FORMAT(/,'         NUMBER OF SYSTEMS OF NO-DELAYED INTERBEDS:',&
   &I3,/,'              NUMBER OF SYSTEMS OF DELAY INTERBEDS:',&
   &I3,/,'                          NUMBER OF MATERIAL ZONES:',&
   &I3,/,'                    NUMBER OF NODES IN EACH STRING:',I3)
   IF(IDSAVE.GT.0) THEN
...
55    FORMAT(' RESTART INFORMATION WILL NOT BE READ FOR DELAY',&
   ...
   ENDIF
   !
   !4A-----ABORT IF NO LAYERS ARE SPECIFIED FOR INTERBED STORAGE
...
90 FORMAT(1X,'OUTPUT CONTROL RECORDS FOR SUB PACKAGE WILL BE ',&
   &'READ.')
   !
   !6------READ IN MODEL LAYER NUMBERS FOR EACH SYSTEM OF INTERBEDS,
   !6------FOR LAYERS WITHOUT DELAY.
   IF(NNDF) THEN
      ALLOCATE(LN(NNDB))
      WRITE(IOUT,100) NNDB
...
120   CONTINUE
   ...
   ENDIF
   !
   !7------READ IN MODEL LAYER NUMBERS FOR EACH SYSTEM OF INTERBEDS,
   !7------FOR LAYERS WITH DELAY.
   IF(NDF) THEN
      ALLOCATE(LDN(NDB))
      WRITE(IOUT,135) NDB
...
140   CONTINUE
   ...
   ND2=0
   !
   !9-----READ IN ARRAY RNB TO SEE HOW MANY STRINGS OF NN CELLS ARE NEEDED.
   IF(NDF) THEN
      ALLOCATE(RNB(ND1))
      NNSUM=0
      DO 190 KQ=1,NDB
...
190   CONTINUE
   ...
   ANNI=0.5/(FLOAT(NN)-.5)
   !
   !12-----READ RESTART RECORDS IF THIS SIMULATION CONTINUES FROM A
   !12-----PREVIOUS SIMULATION
   IF(NDF) THEN
      IF(IDREST.GT.0) THEN
         READ(IDREST) NND2
         IF(NND2.EQ.ND2) THEN
...
242         FORMAT(' HEAD AND PRECONSOLIDATION HEAD FOR DELAY BEDS ARE',&
            &' BEING READ FROM RESTART RECORDS')
...
252         FORMAT(' HEAD AND PRECONSOLIDATION HEAD FOR DELAY BEDS ',&
            &'CANNOT BE READ FROM RESTART RECORDS',/,&
            &' SIMULATION ABORTING')
            CALL USTOP(' ')
         ENDIF
      ENDIF
   ENDIF
   !
   !13-----READ IN ARRAYS FOR SYSTEMS OF NO-DELAY INTERBEDS.
   IF(NNDF) THEN
      DO 260 KQ=1,NNDB
         K=LN(KQ)
         LOC1=1+(KQ-1)*NCR
...
260   CONTINUE
   !
   !14-----INITIALIZE ARRAYS FOR SYSTEMS OF NO-DELAY INTERBEDS.
      DO 280 KQ=1,NNDB
         K=LN(KQ)
         NQ=(KQ-1)*NCR
   !       NK=(K-1)*NCR
   ...
   !       LOC2H=NKR+IC
   !
   !15------MULTIPLY STORAGE BY AREA TO GET STORAGE CAPACITY.
               AREA=DELR(IC)*DELC(IR)
               SCE(LOC2)=SCE(LOC2)*AREA
               SCV(LOC2)=SCV(LOC2)*AREA
   !
   !16-----MAKE SURE THAT PRECONSOLIDATION HEAD VALUES
   !16-----ARE CONSISTANT WITH STARTING HEADS.
...
280   CONTINUE
   ...
   IF(NDF) THEN
   !
   !17-----READ IN TABLE OF MATERIAL PROPERTIES: K, Sse, Ssv FOR EACH
   !17-----OF NMZ ZONES.
      WRITE(IOUT,295)
295   FORMAT(/,' MATERIAL PROPERTIES OF INTERBEDS WITH DELAY PROPERTIES'&
      &,//,'   ZONE        HYDRAULIC           ELASTIC            INEL',&
      &'ASTIC       ',/,'  NUMBER      CONDUCTIVITY     SPECIFIC STORA',&
      &'GE    SPECIFIC STORAGE   ',/,' ',69('-'))
      DO 300 N=1,NMZ
...
305   FORMAT(I5,4X,G15.5,5X,G15.5,5X,G15.5)
      ...
      DO 380 KQ=1,NDB
         ...
         LOC1=1+(KQ-1)*NCR
   !
   !18-----READ IN ARRAYS FOR SYSTEMS OF DELAY INTERBEDS.
         IF(IDREST.LE.0) THEN
            WRITE(IOUT,308) KQ
...
380   CONTINUE
   ENDIF
   !
   !20-----SET ALL FLAGS FOR OUTPUT CONTROL TO "FALSE".
   DO 390 I=1,NSTPT
      DO 385 N=1,13
         OCFLGS(N,I)=.FALSE.
385   CONTINUE
390 CONTINUE
   ! the following initialization of the NTSSUM array was removed from the
   ! block IF construct immediatly below and placed here so that it would
   ! be executed even if ISUBOC is not greater than zero.
   ! Stan Leake, July 14, 2010
   NTSSUM(1)=0
   IF(NPER.GT.1) THEN
      DO 415 N=2,NPER
...
415   CONTINUE
   !
   !21-----READ FORMATS AND UNIT NUMBERS OUTPUT FLAGS.
      IF(ISUBOC.GT.0) THEN
         CALL URDCOM(IN,IOUT,LINE)
         LLOC=1
...
SUBROUTINE GWF2SUB7ST(KPER,IGRID)
   !     ******************************************************************
   !        SET PRECONSOLIDATION HEAD (HC AND DHC) EQUAL TO THE STEADY-
   !        STATE HEAD IF HEAD IS LOWER THAN PRECONSOLIDATION HEAD.
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !2-----MAKE SURE THAT NO-DELAY PRECONSOLIDATION HEAD VALUES ARE CONSISTENT
   !2-----WITH STEADY-STATE HEADS.
   NCR=NROW*NCOL
   IF(NNDF) THEN
      DO 20 KQ=1,NNDB
         K=LN(KQ)
...
20    CONTINUE
   ENDIF
   !3-----MAKE SURE THAT DELAY PRECONSOLIDATION HEAD VALUES ARE CONSISTENT
   !3-----WITH STEADY-STATE HEADS. ALSO, SET HEAD (DH) AND HEAD FOR
   !3-----PREVIOUS TIME STEP (DHP) EQUAL TO THE AQUIFER HEAD (HNEW).
   IF(NDF) THEN
      LOC3=0
      DO 40 KQ=1,NDB
         K=LDN(KQ)
...
SUBROUTINE GWF2SUB7FM(KPER,KITER,ISIP,IGRID)
   !     ******************************************************************
   !        ADD INTERBED STORAGE TERMS TO RHS AND HCOF
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   USE GLOBAL,    ONLY: RHS,HCOF,HNEW,HOLD,IBOUND,DELR,DELC,&
   &NCOL,NROW,ISSFLG,BOTM,LBOTM
      ...
      DO 420 KQ=1,NDB
         ...
         DO 410 IR=1,NROW
            ...
            DO 410 IC=1,NCOL
               ...
               END IF
   !
   !5------ASSEMBLE COEFFICIENTS FOR DIRECT SOLUTION OF HEAD IN INTERBED
               HAQ=HNEW(IC,IR,K)+VV*AC1
               SSE=DP(NZONE,2)
               SSV=DP(NZONE,3)
               NEND=LOC3+NN-1
               CALL SGWF2SUB7A(HAQ,TLED,CI,SSE,SSV,DZZ,&
               &DH(LOC3:NEND),DHP(LOC3:NEND),DHC(LOC3:NEND),NN)
   !
   !6------SOLVE FOR HEAD CHANGES IN STRING USING GAUSSIAN ELIMINATION.
   !6------ADD CHANGES TO HEAD VALUES TO GET HEAD AT CURRENT ITERATION.
               CALL SGWF2SUB7S(NN)
               DO 200 N=1,NN
                  DH(LOC3+N-1)=DH(LOC3+N-1)+BB(N)*AC2
200            CONTINUE
   !
   !7------CALCULATE STORAGE CHANGE IN INTERBEDS
205            AREA=DELR(IC)*DELC(IR)
               ...
               DO 210 LOC4=L1,L2
                  ...
                  HHC=DHC(LOC4)
   !
   !8------GET STORAGE CAPACITIES AT BEGINNING AND END OF TIME STEP.
                  SBGN=DP(NZ(LOC2),2)
                  SEND=SBGN
                  IF(HHNEW.LT.HHC) SEND=DP(NZ(LOC2),3)
   !
   !9------CALCULATE VOLUME CHANGE IN INTERBED STORAGE FOR TIME STEP.
                  STR1=(HHC*(SEND-SBGN)+SBGN*HHOLD-&
                  &SEND*HHNEW)*DZ(LOC2)*RNB(LOC2)*2.
                  IF(LOC4.EQ.L2) STR1=STR1*.5
                  STRGS=STRGS+STR1
210            CONTINUE
               RATES=STRGS*AREA*TLED
   !
   !10-----ADD APPROPRIATE TERMS TO RHS AND HCOF
...
SUBROUTINE GWF2SUB7BD(KSTP,KPER,IGRID)
   ...
   &BUFF,DELR,DELC,ISSFLG,BOTM,LBOTM
   ...
   !1------SET IF CELL-BY-CELL FLOW TERMS ARE NEEDED.
   IBD=0
   IF(ICBCFL.NE.0 .AND. IIBSCB.GT.0 ) IBD=1
   NCR=NCOL*NROW
   !
   !2------RUN THROUGH EVERY CELL IN THE GRID WITH INTERBED STORAGE.
   IF(NNDF) THEN
   !
...
90       CONTINUE
      ...
      STOUT=ZERO
   !
   !4------IF THIS IS A STEADY-STATE STRESS PERIOD, SKIP CALCULATIONS
      IF(ISSFLG(KPER).EQ.1) GO TO 111
   !
   !5------CALCULATE NO-DELAY INTERBED STORAGE CHANGE FOR EACH CELL
      DO 110 KQ=1,NNDB
         K=LN(KQ)
         NQ=(KQ-1)*NCR
         DO 100 IR=1,NROW
            ...
            DO 100 IC=1,NCOL
               LOC2=NQR+IC
   !
   !6------CALCULATE FLOW FROM STORAGE (VARIABLE HEAD CELLS ONLY)
               IF(IBOUND(IC,IR,K).LE.0) GO TO 100
               HHOLD=HOLD(IC,IR,K)
               HHNEW=HNEW(IC,IR,K)
               BOT=BOTM(IC,IR,LBOTM(K))
               ...
               HHC=HC(LOC2)
   !
   !7------GET STORAGE CAPACITIES AT BEGINNING AND END OF TIME STEP.
               SBGN=SCE(LOC2)
               SEND=SBGN
               IF(HHNEW.LT.HHC) SEND=SCV(LOC2)
   !
   !8------CALCULATE VOLUME CHANGE IN INTERBED STORAGE FOR TIME STEP.
   ...
   !       END IF
               ...
               STRGE=STRG-STRGV
   !
   !9------ACCUMULATE SUBSIDENCE ASSOCIATED WITH CHANGE IN STORAGE
               SUB(LOC2)=SUB(LOC2)+STRG/(DELR(IC)*DELC(IR))
   !
   !10-----IF C-B-C FLOW TERMS ARE TO BE SAVED THEN ADD RATE TO BUFFER.
               IF(IBD.EQ.1) BUFF(IC,IR,K)=BUFF(IC,IR,K)+STRG*TLED
   !
   !11-----SEE IF FLOW IS INTO OR OUT OF STORAGE.
...
110   CONTINUE
   !
   !12-----IF C-B-C FLOW TERMS WILL BE SAVED CALL UBUDSV TO RECORD THEM.
111   IF(IBD.EQ.1) CALL UBUDSV(KSTP,KPER,TEXT(1),IIBSCB,BUFF,NCOL,&
      &NROW,NLAY,IOUT)
   !
   !13-----MOVE RATES,VOLUMES & LABELS INTO ARRAYS FOR PRINTING.
   ...
   !14-----INCREMENT BUDGET TERM COUNTER
      MSUM=MSUM+1
   !
   !15-----UPDATE PRECONSOLIDATION HEAD ARRAY
      DO 310 KQ=1,NNDB
         K=LN(KQ)
         LOCT=(KQ-1)*NCR
         N=0
...
390      CONTINUE
      ...
      RATOUT=ZERO
   !
   !16-----IF THIS IS A STEADY-STATE STRESS PERIOD, SKIP CALCULATIONS
      IF(ISSFLG(KPER).EQ.1) GO TO 421
   !
   !17-----CALCULATE NO-DELAY INTERBED STORAGE CHANGE FOR EACH CELL
      LOC3=1-NN
      DO 420 KQ=1,NDB
         K=LDN(KQ)
         NQ=(KQ-1)*NCR
   ...
   !
   !18-----CALCULATE CONDUCTANCE BETWEEN AQUIFER AND FIRST CELL IN INTERBED
   !18-----ACCOUNTING FOR BOTH HALVES OF RNB(LOC2) BEDS IN SYSTEM
               COND=4.*RNB2*DP(NZ(LOC2),1)*AREA/DZ(LOC2)
   !
   !19-----CALCULATE THE FLOW RATE INTO THE CELL
               ...
               DO 401 LOC4=L1,L2
                  ...
                  HHC=DHC(LOC4)
   !
   !20-----GET STORAGE CAPACITIES AT BEGINNING AND END OF TIME STEP.
                  SBGN=DP(NZ(LOC2),2)
                  SEND=SBGN
                  IF(HHNEW.LT.HHC) SEND=DP(NZ(LOC2),3)
   !
   !21-----CALCULATE VOLUME CHANGE IN INTERBED STORAGE FOR TIME STEP.
                  ...
                  STRGS=STRGS+STR1
   !
   !22-----ACCUMULATE SUBSIDENCE ASSOCIATED WITH CHANGE IN STORAGE
...
420   CONTINUE
   !
   !23-----IF C-B-C FLOW TERMS WILL BE SAVED CALL UBUDSV TO RECORD THEM.
421   IF(IBD.EQ.1) CALL UBUDSV(KSTP,KPER,TEXT(2),IIBSCB,BUFF,&
      &NCOL,NROW,NLAY,IOUT)
   !
   !24-----MOVE RATES,VOLUMES & LABELS INTO ARRAYS FOR PRINTING.
      VBVL(3,MSUM)=RATIN
      VBVL(4,MSUM)=RATOUT
      ...
      VBNM(MSUM)=TEXT(2)
   !
   !25-----INCREMENT BUDGET TERM COUNTER
      MSUM=MSUM+1
   !
   !26-----UPDATE PRECONSOLIDATION HEAD ARRAY, PREVIOUS HEAD ARRAY FOR
   !26-----SYSTEMS OF DELAY INTERBEDS.
      DO 500 N=1,ND2
...
SUBROUTINE GWF2SUB7OT(KSTP,KPER,IN,IGRID)
   ...
   !1------SAVING ARRAYS. SET FLAG FOR PRINTING BUDGET FOR DELAY INTERBEDS.
...
70       CONTINUE
   ...
   !7------INCLUDING DELAY AND NO-DELAY INTERBEDS.
...
76             FORMAT(/,1X,' SYSTEM',I4,' OF NO-DELAY BEDS:')
               ...
               &KPER,NCOL,NROW,K,-ISBOCF(3),IOUT)
...
90       CONTINUE
   ...
   ENDIF
   !
   !8------SUM COMPACTION IN EACH LAYER IN THE BUFF ARRAY FOR SAVING
   !8------OR PRINTING COMPACTION OR VERTICAL DISPLACEMENT BY MODEL
   !8------LAYER. FIRST, CLEAR OUT BUFF.
...
95    CONTINUE
   !
   !9-------SUM NO-DELAY COMPACTION IN ALL MODEL LAYERS.
      IF(NNDF) THEN
         DO 105 KQ=1,NNDB
            K=LN(KQ)
            OCLAY(K)=.TRUE.
...
105      CONTINUE
      ENDIF
   !
   !10------SUM DELAY COMPACTION IN ALL MODEL LAYERS.
      IF(NDF) THEN
         DO 115 KQ=1,NDB
            K=LDN(KQ)
            OCLAY(K)=.TRUE.
...
125   CONTINUE
   ENDIF
   !
   !13----CALCULATE VERTICAL DISPLACEMENT.
...
140      CONTINUE
      ENDIF
   !
   !14-----PRINT VERTICAL DISPLACEMENT FOR ALL MODEL LAYERS.
      IF(OCFLGS(7,NNSTP)) THEN
         DO 145 KL=1,NLAY
            KKL=KL
            IF(ISBOCF(4).LT.0) CALL ULAPRS(BUFF(:,:,KL),TEXT(5),KSTP,KPER,&
            &NCOL,NROW,KKL,-ISBOCF(4),IOUT)
            IF(ISBOCF(4).GE.0) CALL ULAPRW(BUFF(:,:,KL),TEXT(5),KSTP,KPER,&
            &NCOL,NROW,KKL,ISBOCF(4),IOUT)
145      CONTINUE
      ENDIF
   !
   !15-----SAVE VERTICAL DISPLACEMENT FOR ALL MODEL LAYERS.
...
150      CONTINUE
   ...
   ENDIF
   !
   !16-----PRINT CRITICAL HEAD FOR SYSTEMS OF NO-DELAY INTERBEDS.
   !16-----STORAGE.
...
160      CONTINUE
   ...
   ENDIF
   !
   !17-----PRINT CRITICAL HEAD FOR ALL SYSTEMS OF DELAY INTERBED.
...
190      CONTINUE
   ...
   ENDIF
   !
   !18-----PRINT VOLUMETRIC BUDGET FOR SYSTEMS OF DELAY INTERBEDS
...
230 FORMAT(/,31X,'VOLUMETRIC BUDGET FOR SYSTEMS OF DELAY INTERBEDS',&
   &/,42X,'AT END OF TIME STEP',I3,' IN ','STRESS PERIOD',I3,&
   &//,'          |   C U M U L A T I V E   ',&
   &'V O L U M E S   L**3           | R A T E S   F O R  T H I S',&
   &'  T I M E  S T E P   L**3/T  |',/,'   SYSTEM |    CHANGE IN',&
   &'     BOUNDARY                    PERCENT   |    CHANGE IN  ',&
   &'   BOUNDARY                    PERCENT   |',/,'   NUMBER | ',&
   &'    STORAGE        FLOW           SUM      DISCREPANCY |   ',&
   &'  STORAGE        FLOW           SUM      DISCREPANCY |',/,2X,&
   &8('-'),'|',56('-'),'|',56('-'),'|')
...
SUBROUTINE SGWF2SUB7A(HAQ,TLED,CI,SSE,SSV,DZ,DH,DHP,DHC,NN)
   !     ******************************************************************
   !        ASSEMBLE COEFFICIENTS FOR SOLVING FOR HEAD DISTRIBUTION
   !        IN ONE STRING OF CELLS REPRESENTING ONE-HALF OF A DOUBLY
   !        DRAINING INTERBED
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   ...
   DS=DZ*TLED
   NN1=NN-1
   !2------SET COEFFICIENTS FOR CELL BORDERING AQUIFER
   SS=SSE
   A2(1)=CI
   HD=DH(1)
   HC=DHC(1)
   ...
   !3------SET COEFFICIENTS FOR INTERIOR CELLS
   BB(2)=-CI*HD
   DO 10 N=2,NN1
      SS=SSE
      A2(N)=CI
...
10 CONTINUE
   !4------SET COEFFICIENTS FOR CELL BORDERING MIDPLANE OF INTERBED
   SS=SSE
   A2(NN)=CI
   HD=DH(NN)
   HC=DHC(NN)
...
SUBROUTINE SGWF2SUB7S(NN)
   !     ******************************************************************
   !        SOLVE SYSTEM OF EQUATIONS WITH A SYMMETRICAL TRI-DIAGONAL
   !        COEFFICIENT MATRIX
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
...
SUBROUTINE GWF2SUB7SV(IGRID)
   !     ******************************************************************
   !     SAVE INTERBED STORAGE DATA FOR FUTURE RESTART
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !2-----WRITE OUT NUMBER OF NODAL HEAD AND PRECONSOLIDATION HEAD VALUES
   !2-----THAT ARE BEING SAVED TO DISK
      WRITE(IDSAVE) ND2
   !3-----WRITE ARRAYS FOR EACH SYSTEM OF INTERBEDS
...
SUBROUTINE GWF2SUB7DA(IGRID)
   !
   !     ******************************************************************
   !     DEALLOCATE DYNAMIC STORAGE FOR SUB PACKAGE
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
