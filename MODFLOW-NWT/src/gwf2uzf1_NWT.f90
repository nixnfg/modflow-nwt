...
END FUNCTION smoothuz
   ...
   !
   !-------SUBROUTINE GWF2UZF1AR
SUBROUTINE GWF2UZF1AR(In, Iunitbcf, Iunitlpf, Iunithuf,&
&Iunitupw, Igrid)
   !     ******************************************************************
   !     ALLOCATE ARRAY STORAGE FOR UNSATURATED FLOW, RECHARGE, AND ET
   !     READ AND CHECK VARIABLES THAT REMAIN CONSTANT
...
9001 FORMAT (1X, /' UZF1 -- UNSATURATED FLOW PACKAGE, VERSION 1.5',&
   ...
   NSETS = 1
   !
   !2------READ UNSATURATED FLOW FLAGS WHEN IUZFOPT IS GREATER THAN ZERO.
   ...
   ! EDM a better way is to check if LMT is active, then set RTSOLUTE=1
...
9007  FORMAT (//' VERTICAL FLOW THROUGH UNSATURATED ZONE IS ',&
   ...
   !      END IF
   !
   !4------ALLOCATE SPACE FOR UNSATURATED FLOW.
   ALLOCATE (IUZFBND(NCOL,NROW))
   IUZFBND = 0
   !
   !10-----READ IN BOUNDARY ARRAY FOR UNSATURATED FLOW.
   ...
   IF ( IUZFOPT.EQ.1 .OR. IUZFOPT.EQ.2 ) THEN
      ...
      NUZRW = NROW
   !
   !5------SET DIMENSIONS FOR UNSATURATED ZONE ARRAYS TO 1 IF NO
   !      UNSATURATED ZONE
   ELSE
      IUZM = 1
      NWAV = 1
   ...
   !6------CALCULATE SPACE USED FOR LISTING UNSATURATED MOISTURE PROFILES.
   IF ( NUZGAG.GT.0 ) THEN
      NUZGAGAR = NUZGAG
   ELSE
      NUZGAGAR = 1
   END IF
   !
   !7------ALLOCATE SPACE FOR ARRAYS AND INITIALIZE.
   ...
   UZTOTBAL = 0.0D0
   !rgn changed allocation 10/23/06
   ...
   DELSTOR = 0.0D0
   !dep changed allocation of UZOLSFLX 7/30/08
   ...
   !
   !8------PRINT OPTION CODE WHEN NUZTOP IS WITHIN SPECIFIED RANGE.
   ...
   END IF
   !
   !8b-----Set flag for determining if FINF will be provided by PRMS.
   !       A value of zero means that FINF will not be set by PRMS.
   IGSFLOW = 0
...
9008  FORMAT (' OPTION 4 -- RECHARGE IN UZF TO HIGHEST CELL ',&
      &'WITH WATER TABLE')
   !
   !9------STOP SIMULATION IF NUZTOP IS NOT WITHIN SPECIFIED RANGE.
   ELSE
...
9012  FORMAT (1X, 'ILLEGAL RECHARGE OPTION CODE IN UZF (NUZTOP = ',&
   ...
   END IF
   !
   !11-----READ STREAM AND LAKE ARRAY FOR ROUTING OVERLAND FLOW.
   ...
   END IF
   !
   !5B------READ AND SET VALUES FOR SOLUTE ROUTING IN UNSATURATED ZONE
   !
   IF ( RTSOLUTE.GT.0 ) THEN
      WRITE(IOUT, 8051)                       !EDM
8051  FORMAT(' LMT ACTIVE: WILL WRITE UZF FLUXES TO FILE SPECIFIED ',&
   ...
   END IF
   !
   !12-----READ VERTICAL HYDRAULIC CONDUCTIVITY FROM UZF INPUT FILE.
   IF ( abs(IUZFOPT).EQ.1 ) THEN    !RGN 2/6/17
      CALL U2DREL(VKS, aname(6), NROW, NCOL, 0, In, IOUT)
   !
   !13-----CHECK FOR ERRORS IN VERTICAL HYDRAULIC CONDUCTIVITY
      DO nrck = 1, NROW
         DO ncck = 1, NCOL
            iflgbnd = 1
...
9013              FORMAT (1X/, 'SATURATED VERTICAL K FOR CELL AT ROW ',&
   ...
   END IF
   !
   !12b-----READ SURFACE CONDUCTIVITY FROM UZF INPUT FILE.
   IF ( Ireadsurfk > 0 ) THEN
      CALL U2DREL(SURFK, aname(9), NROW, NCOL, 0, In, IOUT)
   !
   !13b-----CHECK FOR ERRORS IN SURFACE K
      DO nrck = 1, NROW
         DO ncck = 1, NCOL
            iflgbnd = 1
            IF ( IUZFBND(ncck, nrck).NE.0 ) THEN
...
9030              FORMAT (1X/, 'LAND SURFACE K FOR CELL AT ROW ',&
   ...
   IF ( iuzflg.GT.0 ) THEN
   !
   !14-----READ BROOKS-COREY EPSILON ASSUMING IT IS CONSTANT THROUGHOUT
   !        VERTICAL COLUMN.
      CALL U2DREL(EPS, aname(5), NUZRW, NUZCL, 0, In, IOUT)
   !
   !15-----READ SATURATED WATER CONTENT FOR UNSATURATED ZONE ASSUMING IT
   !       IS CONSTANT THROUGHOUT VERTICAL COLUMN.
   ...
   !
      ...
      END IF
   !
   !15-----READ AIR ENTRY PRESSURE FOR UNSATURATED ZONE ASSUMING IT
   !       IS CONSTANT THROUGHOUT VERTICAL COLUMN.
   ...
   !
   !16-----READ INITIAL WATER CONTENT FOR UNSATURATED ZONE ASSUMING IT
   !         IS CONSTANT THROUGHOUT VERTICAL COLUMN. DO NOT READ
   !         INITIAL WATER CONTENT IF PERIOD IS STEADY STATE.
      IF ( ISSFLG(1).EQ.0 .OR. ITHTIFLG.GT.0 )&
      &CALL U2DREL(THTI, aname(8), NUZRW, NUZCL, 0, In, IOUT)
   !
   !17-----CHECK FOR ERRORS IN EPS, THTS, AND THTI ARRAYS.
      DO nrck = 1, NUZRW
         DO ncck = 1, NUZCL
            iflgbnd = 1
...
9016              FORMAT (1X/, 'BROOKS-COREY EPSILON FOR CELL AT ROW ',&
   ...
   !              END IF
      ...
      END DO
   !
   !18-----COMPUTE RESIDUAL WATER CONTENT (THTR) FOR ALL UNSATURATED CELLS
   !       AS THE DIFFERENCE BETWEEN SATURATED WATER CONTENT AND SPECIFIC
   !       YIELD OF UPPERMOST ACTIVE LAYER.
         ...
         DO nrth = 1, NROW
            ...
            DO ncth = 1, NCOL
               ...
               IF ( iuzflg.GT.0 ) THEN
                  ...
                  END DO
   !
   !19-----SPECIFIC YIELD IS STORAGE CAPACITY DIVIDED BY AREA OF MODEL CELL.
                  IF ( ibndflg.GT.0 ) THEN
                     sy = 0.0
                     IF ( Iunitlpf.GT.0 ) THEN
                        IF ( LAYHDT(nlth).GT.0 ) THEN
   ! use LPF SC2, Iunitlpf>0
...
9028           FORMAT (1X/, 'SATURATED WATER CONTENT FOR UPPERMOST ACTIVE ',&
   ...
   !
   !20-----IF SPECIFIC YIELD IS 0 FOR UPPERMOST ACTIVE CELL AT COLUMN J
   !       AND ROW I OR IF ALL LAYERS AT CELL ARE INACTIVE, SET
   !       UNSATURATED FLOW AT CELL INACTIVE.
               IF ( isyflg.EQ.0 ) THEN
...
9018              FORMAT (1X/, 'SPECIFIC YIELD FOR UPPERMOST ACTIVE ',&
   ...
   END IF
   !
   !21-----READ FILES FOR PRINTING TIME SERIES WATER CONTENT PROFILES.
   IF ( NUZGAG.GT.0 ) THEN
      WRITE (IOUT, 9019)
...
9020        FORMAT (1X, I7, 7X, I7, 12X, I7, 11X, I7)
   !
   !22-----DETERMINE IF ROW AND COLUMN NUMBERS ARE IN ACTIVE AREA OF
   !        UNSATURATED FLOW.
...
9023              FORMAT (1X/, 'WARNING--- RECORD ', I7, ' IS NOT IN ',&
      ...
      END DO
   !
   !23-----WRITE HEADER FILES FOR CELLS WITH SELECTED OUTPUT.
      DO igage = 1, NUZGAG
         iuzrow = IUZLIST(1, igage)
         iuzcol = IUZLIST(2, igage)
         ...
         IF ( igunit.GT.0 ) THEN
   !
   !24----GET VARIABLE OUTTYPE.
            SELECT CASE (iuzopt)
   !
   !25-----PRINT HEADER WHEN WRITING ONLY VOLUMES.
             CASE (1)
               WRITE (igunit, 9024) igage, iuzrow, iuzcol, iuzlay
   !
   !26-----PRINT HEADER WHEN WRITING VOLUMES AND RATES.
             CASE (2)
               WRITE (igunit, 9025) igage, iuzrow, iuzcol, iuzlay
   !
   !27-----PRINT HEADER FOR UNSATURATED-ZONE MOISTURE PROFILES
             CASE (3)
...
9027 FORMAT (1X,'"UNSATURATED MASS BALANCE COMPONENTS FOR ENTIRE ',&
   ...
   !
   !29-----PRINT WARINING WHEN UNITS ARE UNDEFINED IN MODFLOW.
   ...
   !
   !31-----SAVE POINTERS FOR GRID AND RETURN.
   CALL SGWF2UZF1PSV(Igrid)
   RETURN
END SUBROUTINE GWF2UZF1AR
   !
   !-------SUBROUTINE PARSEUZFOPTIONS
...
SUBROUTINE SGWF2UZF1VKS(Iunithuf, Iunitlpf, Iunitupw)
   !     ******************************************************************
   !     ASSIGN SATURATED VERTICAL HYDRAULIC CONDUCTIVITY ARRAY
   !     (VKS) IN UZF TO EQUAL VERTICAL HYDRAULIC CONDUCTIVITY IN LAYER-
   !     PROPERTY FLOW PACKAGE
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     VERSION 1.0.5:  April 5, 2012
   !     ******************************************************************
   ...
   nlayp1 = NLAY + 1
   !
   !1------CHECK TO SEE IF UPPERMOST ACTIVE CELL IS CONVERTABLE AND
   !       SET VKS EQUAL TO VKALPF FOR CORRESPONDING MODEL CELL.
   krck = 1
   DO nrck = 1, NROW
      DO ncck = 1, NCOL
         ...
         ELSE IF ( NUZTOP.EQ.3 ) THEN
            ...
            DO WHILE ( ill.LT.nlayp1 )
               ...
               ELSE IF ( IBOUND(ncck, nrck, ill).LT.0 ) THEN
                  EXIT
               END IF
   !RGN made il = 0 when all layers for column are inactive 2/21/08
               ill = ill + 1
            END DO
...
END SUBROUTINE SGWF2UZF1VKS
   !
   !-------SUBROUTINE GWF2UZF1RP
SUBROUTINE GWF2UZF1RP(In, Kkper, Kkstp, Iunitsfr, Igrid)
   !     ******************************************************************
   !     READ AND CHECK VARIABLES EACH STRESS PERIOD
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     VERSION 1.0.5:  April 5, 2012
   !     ******************************************************************
   USE GWFUZFMODULE
   ...
   !
   !2------READ INFILTRATION RATES FOR UZF CELLS AT THE BEGINNING OF EACH
   !       STRESS PERIOD.
   iss = ISSFLG(Kkper)
   iflginit = 0
   ...
   ELSE
   !
   !3------READ IN ARRAY FOR INFILTRATION RATE.
      CALL U2DREL(FINF, aname(1), NROW, NCOL, 0, In, IOUT)
   !
   !4------CHECK FOR NEGATIVE INFILTRATION RATES.
      DO nrck = 1, NROW
         DO ncck = 1, NCOL
            IF ( IUZFBND(ncck, nrck).NE.0 ) THEN
               fks = VKS(ncck, nrck)
...
9002              FORMAT (1X/, 'INFILTRATION RATE FOR CELL AT ROW ', I5,&
                  ...
                  FINF(ncck, nrck) = 0.0
   !
   !5------SET INFILTRATION RATE TO SATURATED VERTICAL K WHEN RATE IS
...
9004     FORMAT (1X/, 'USING ET RATE FROM PREVIOUS STRESS ',&
      ...
      ELSE
   !
   !6------READ IN ARRAY FOR ET RATE.
         CALL U2DREL(PETRATE, aname(2), NROW, NCOL, 0, In, IOUT)
   !
   !7-----CHECK FOR NEGATIVE ET RATES.
         DO nrck = 1, NROW
            DO ncck = 1, NCOL
   !8----ZERO ET ARRAY THAT IS PASSED TO PRMS.
...
9006     FORMAT (/1x, 'USING ET EXTINCTION DEPTH FROM PREVIOUS ',&
      ...
      ELSE
   !
   !9------READ IN ARRAY FOR ET EXTINCTION DEPTH.
         CALL U2DREL(ROOTDPTH, aname(3), NROW, NCOL, 0, In, IOUT)
   !
   !10-----CHECK FOR NEGATIVE ET EXTINCTION DEPTH.
         DO nrck = 1, NROW
            DO ncck = 1, NCOL
               IF ( IUZFBND(ncck, nrck).EQ.0 ) THEN
...
222   FORMAT('***WARNING*** ET EXTINCTION DEPTH IS BELOW LAYER BOTTOM',/&
      ...
      &' COLUMN ',I7)
   !
   !11-----SKIP READING OF EXTINCTION WATER CONTENT ARRAY WHEN
   !         IUZFOPT IS ZERO.
      IF ( IUZFOPT.GT.0 ) THEN
         READ (In, *) nuzf
         IF ( nuzf.LT.0 ) THEN
...
9012        FORMAT (/1X, 'USING EXTINCTION WATER CONTENT FROM PREVIOUS',&
         ...
         ELSE
   !
   !12-----READ IN ARRAY FOR ET WATER CONTENT.
            CALL U2DREL(WCWILT, aname(4), NROW, NCOL, 0, In, IOUT)
   !
   !13-----CHECK FOR EXTINCTION WATER CONTENT LESS THAN RESIDUAL WATER
   !         CONTENT.
            DO nrck = 1, NUZRW
               DO ncck = 1, NUZCL
...
9014                    FORMAT (1X/, 'EXTINCTION WATER CONTENT FOR ',&
         ...
         END IF
   !
   !11-----READ AIR ENTRY PRESSURE HEAD IF CAPILLARYUZET IS SPECIFIED
   !         IN OPTIONS BLOCK.
...
9013           FORMAT (/1X, 'USING AIR ENTRY PRESSURE FROM PREVIOUS',&
            ...
            ELSE
   !
   !12-----READ IN ARRAY FOR AIR ENTRY PRESSURE.
...
9015           FORMAT (/1X, 'USING ROOT PRESSURE HEAD FROM PREVIOUS',&
               &' STRESS PERIOD. CURRENT PERIOD IS: ', I7)
            ELSE
   !12-----READ IN ARRAY FOR ROOT PRESSURE HEAD.
...
9016           FORMAT (/1X, 'USING ROOT ACTIVITY FROM PREVIOUS',&
   ...
   !12-----READ IN ARRAY FOR ROOT PRESSURE HEAD.
      ...
      END IF
   END IF
   !13B-----SEARCH FOR UPPERMOST ACTIVE CELL.
   ...
   END IF
   !
   !14------INITIALIZE UNSATURATED ZONE IF ACTIVE.
   !
   !15------SET FLAGS FOR STEADY STATE OR TRANSIENT SIMULATIONS.
   ...
   !
   !--------NUZTOP EQUAL 4 SO SET LAYNUM ARRAY
   !
      ...
      DO ll = 1, NUMCELLS
         ...
         IF ( IUZFBND(ic,ir).GT.0 ) THEN
            l = l + 1
   !
   !16-----SEARCH FOR UPPERMOST ACTIVE CELL.
            IF ( IUZFBND(ic, ir).GT.0 ) THEN
               il = 0
               IF ( NUZTOP.EQ.1 .OR. NUZTOP.EQ.2 ) THEN
                  il = IUZFBND(ic, ir)
               ...
               END IF
   !
   !16B-----SEARCH FOR UPPER MOST ACTIVE CELL WITH A WATER LEVEL.
               ilay = il
               IF ( il.GT.0 ) THEN
   ...
   !rgn changed HNEW(ic, ir, il) to h in next line.
                  HLDUZF(ic, ir) = h
                  IF ( IBOUND(ic, ir, il).LT.0 ) IUZFBND(ic, ir) = 0
                  IF ( IUZFOPT.GT.0 ) THEN
   !
   !17-----SET CELL TOP, LENGTH, WIDTH AND WATER TABLE ELEVATION.
                     slen = DELC(ir)
                     width = DELR(ic)
   ! RGN changed BOTM(ic, ir, 0) to BOTM(ic, ir, land-1) 1/28/2010
                     ...
                     END IF
   !
   !18-----SKIP IF CELL IS OUTSIDE ACTIVE BOUNDARY OR IS NOT WATER TABLE.
   ! commented next line out to simulate unsat. flow over a portio of area.
   !                  IF ( il.LT.1 ) IUZFBND(ic, ir) = 0
   !
   !19-----INITIALIZE UZTHST ARRAY TO RESIDUAL WATER CONTENT.
                     ...
                     END DO
   !
   !20-----INITIALIZE UNSATURATED ZONE ARRAYS FOR FIRST STRESS PERIOD.
                     IF ( iflginit.EQ.1 ) THEN
                        IF ( celtop.GT.h ) THEN
                           UZDPST(1, l) = (celtop-h)
   !
   !21-----CALCULATE INITIAL WATER CONTENT AND FLUX IF STEADY STATE.
                           IF ( iss.NE.0 ) THEN
                              ...
                              &UZTHST(1, l) = thtrcell
   !
   !22-----SET INITIAL WATER CONTENT TO THTI AND CALCULATE FLUX IF
   !         TRANSIENT.
                           ELSE
                              UZTHST(1, l) = THTI(ic, ir)
                              top = UZTHST(1, l) - thtrcell
                           ...
                           &= thtrcell
   !
   !23-----CALCULATE VOLUME OF WATER STORED IN UNSATURATED ZONE.
                           IF ( top.GT.0.0 ) THEN
                           ...
                           END IF
   !
   !24-----IF NO UNSATURATED ZONE, SET ARRAY VALUES TO ZERO EXEPT WHEN
   !         STEADY STATE, THEN SET UZFLST ARRAY TO INFILRATION RATE.
                        ELSE
                           IF ( iss.NE.0 ) THEN
                              fcheck = FINF(ic, ir)
                              if ( fcheck > VKS(ic,ir) ) fcheck = VKS(ic,ir)
   ...
   !     +                        (BOTM(ic,ir,uzlay-1)-BOTM(ic,ir,uzlay))
                        ...
                        END IF
   !
   !25-----INITIALIZE ARRAYS FOR A TRANSIENT PERIOD THAT FOLLOWS A
   !         STEADY STATE PERIOD IN STRESS PERIOD 1.
                     ELSE IF ( iflginit.EQ.2 ) THEN
                        IF ( celtop.GT.h ) THEN
                           UZDPST(1, l) = celtop - h
                           HLDUZF(ic,ir) = h
   !
   !26-----CALCULATE INITIAL WATER CONTENT AND FLUX FROM STEADY STATE
   !         SIMULATION.
                           ...
                           IF ( top.GT.1.0E-5 ) THEN
                              ...
                              UZOLSFLX(ic, ir) = UZFLST(1, l)
   !
   !27-----IF NO UNSATURATED ZONE, SET ARRAYS VALUES TO ZERO.
                           ELSE
                              UZSTOR(ic, ir) = 0.0D0
                              UZFLST(1, l) = 0.0D0
                              UZSPST(1, l) = 0.0D0
...
END SUBROUTINE GWF2UZF1RP
   !
   !
   !-------SUBROUTINE GWF2UZF1AD
SUBROUTINE GWF2UZF1AD(In, KKPER, KKSTP, Igrid)
   !     ******************************************************************
   !     SET LAYER FOR GROUNDWATER RECHARGE AND DISCHARGE
   !     VERSION 1.3.0:  March 1, 2020
   !     ******************************************************************
   USE GWFUZFMODULE
   !      USE GLOBAL,       ONLY: NCOL, NROW, NLAY, IOUT, ISSFLG, IBOUND,
   !     +                        HNEW, BOTM, LBOTM
   ...
   CALL SGWF2UZF1PNT(Igrid)
   !
   !2------CALL SETLAY TO SET GWF LAYER
   !       FOR EACH TIME STEP.
   !
...
SUBROUTINE SETLAY(kkstp)
   ...
   !     -----------------------------------------------------------------
   !
   !1------SET LAYER NUMBER FOR UZF RECHARGE/DISCHARGE
   !       FOR BEGINNING OF EACH TIME STEP
...
END SUBROUTINE SETLAY
   !
   !--------SUBROUTINE GWF2UZF1FM
   ! SWR - JDH ADDED Iunitswr
SUBROUTINE GWF2UZF1FM(Kkper, Kkstp, Kkiter, Iunitsfr, Iunitlak,&
&Iunitnwt, Iunitswr, Igrid)
   !     ******************************************************************
   !     COMPUTE UNSATURATED ZONE FLOW AND STORAGE, RECHARGE, ET, AND
   !     SURFACE LEAKAGE AND ADD OR SUBTRACT TERMS RHS AND HCOF
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     VERSION 1.0.5:  April 5, 2012
   !     ******************************************************************
   USE GWFUZFMODULE
   ...
   CALL SGWF2UZF1PNT(Igrid)
   !
   !2------LOOP THROUGH UNSATURATED ZONE FLOW CELLS.
   iss = ISSFLG(Kkper)
   fact = 1.0D0
   zero = 0.0d0
   deltinc = DELT
   ...
   IF ( deltinc.GT.DELT ) THEN
      ...
      idelt = 1
   END IF
   ! set excess precipitation to zero for integrated (GSFLOW) simulation
   ...
   !
         ...
         finfact = finfhold
   !
   !3------SEARCH FOR UPPERMOST ACTIVE CELL.
         il = 0
         IF ( NUZTOP.EQ.1 .OR. NUZTOP.EQ.2 ) THEN
            il = ABS(ibnd)
            IF ( il.GT.0 ) THEN
            ...
            IF ( htest1.LT.-CLOSEZERO .OR. htest2.LT.-CLOSEZERO ) THEN
               IF ( IUZFOPT.GT.0 .AND. iss.EQ.0 ) THEN
                  IF ( ibnd.GT.0 ) THEN
                     ...
                     totetact = 0.0d0
   !
   !4------RESET ALL UNSATRATED ZONE CELLS TO PREVIOUS CONDITIONS.
                     iset = 1
                     ...
                     END DO
   !
   !5------CALL UZFLOW TO ROUTE WAVES FOR LATEST ITERATION.
   ...
   !
   !6------GROUNDWATER IS DISCHARGING TO LAND SURFACE.
   ...
   !7------CALCULATE ET DEMAND LEFT FOR GROUND WATER.
            IF ( IETFLG.GT.0 .AND. lakflginf.NE.1 ) THEN
               s = celtop
               x = ROOTDPTH(ic, ir)
   ...
   END DO
   !
   !8------ADD OVERLAND FLOW TO STREAMS, LAKES AND CONDUITS.
...
END SUBROUTINE GWF2UZF1FM
   ! SWR - JDH CHANGED SUBROUTINE TO ADD OVERLAND FLOW TO SWR
   !
   !--------SUBROUTINE SGWF2UZF1OLF
SUBROUTINE SGWF2UZF1OLF(Iunitsfr, Iunitlak, Iunitswr, Igrid)
   ...
   !     -----------------------------------------------------------------
   !1-----INITIALIZE OVERLAND RUNOFF TO STREAMS AND LAKES TO ZERO.
   gwrunof = 0.0
   TOTRUNOFF = 0.0
   !
   !2-----OVERLAND RUNOFF TO STREAMS FROM UZF SEPARATE FROM
   !        OVERLAND RUNOFF SPECIFIED FOR STREAMS.
   ...
   IF ( Iunitlak.GT.0 ) THEN
   !
   !3------OVERLAND RUNOFF TO LAKES SET TO ZERO EVEN IF
   !        VALUE IS SPECIFIED FOR A LAKE.
      DO i = 1, NLAKES
         OVRLNDRNF(i) = 0.0
      END DO
   END IF
   !
   !3A----IF PRMS ACTIVE THEN RETURN
   IF ( IGSFLOW.NE.0 )RETURN
   !
   !4------LOOP THROUGH IRUNBND ARRAY AND ADD SEEPOUT PLUS EXCESSPP TO
   !         CORRECT STREAM SEGMENT OR LAKE.
   DO ir = 1, NROW
      DO ic = 1, NCOL
   ...
   END DO
   !
   !temp------add runoff only to last reach in a segment for P.Juckem
   !IF ( Iunitsfr.GT.0 ) THEN
   !  irchcheck = 0
   !  nsegold = 1
   !  DO i = 1, NSTRM
   ...
   !END IF
   !
   !5------PROPORTION RUNOFF TO REACHES ON BASIS OF STREAM LENGTH.
...
END SUBROUTINE SGWF2UZF1OLF
   !
   !------SUBROUTINE GWF2UZF1BD
SUBROUTINE GWF2UZF1BD(Kkstp, Kkper, Iunitlak, Iunitsfr, Igrid)
   !     ******************************************************************
   !     CALCULATE VOLUMETRIC BUDGETS FOR RECHARGE, ET, AND SURFACE LEAKAGE
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     VERSION 1.0.5:  April 5, 2012
   !     ******************************************************************
   USE GWFUZFMODULE
   ...
   CALL SGWF2UZF1PNT(Igrid)
   !
   !2------INITIALIZE CELL BY CELL FLOW TERM FLAG (IBD) AND
   ...
   IUZFCB2 = ABS(IUZFCB2)
   !
   !3------WRITE HEADER IF UNSATURATED STORAGE TERMS ARE SAVED.
   !      IF(IBDUZF.EQ.1)
   !      IF(IBDUZF.EQ.2)
   !
   !4------CLEAR BUFFERS.
   ...
   ! set excess precipitation to zero for integrated (GSFLOW) simulation
         ...
         SEEPOUT(ic, ir) = 0.0
   !
   !5-----SEARCH FOR UPPERMOST ACTIVE CELL.
         IF ( NUZTOP.EQ.1 ) THEN
            il = 1
            IF ( ibnd.EQ.0 ) THEN
               il = 0
         ...
         END IF
   !
   !6------PRINT WARNING WHEN NUZTOP IS 1 OR 2 AND ALL LAYERS ARE INACTIVE.
         IF ( il.EQ.0 ) THEN
            IF ( ibnd.NE.0 ) THEN
               IF ( NUZTOP.LT.2 ) THEN
         ...
         IF ( NUZTOP.EQ.3 ) THEN
            ...
            IF ( ibnd.EQ.0 ) il = 0
   !
   !7------PRINT WARNING WHEN NUZTOP IS 3 AND ALL LAYERS ARE INACTIVE.
   ...
   ! Suppress seepout and ET beneath a lake
         ...
         IF ( il.GT.0 .AND. VKS(ic, ir).GT.NEARZERO ) THEN
            ...
            etact = 0.0D0
   !
   !8------SET NWAVES TO 1 WHEN IUZFOPT IS NEGATIVE.
            IF ( IUZFOPT.GT.0 ) THEN
               thr = THTR(ic, ir)
               epsilon = EPS(ic, ir)
               ths = THTS(ic, ir)
   ...
   ! Suppress SEEPOUT beneath a lake
               ...
               END IF
   !
   !9------CALCULATE ET FROM GROUND WATER.
   ...
   !
   !10-----REMOVE ALL UNSATURATED ZONE WAVES AND CALCULATE CHANGE IN
   !         STORAGE WHEN WATER TABLE RISES TO LAND SURFACE.
   ...
   !11-----RESET WAVE CHARACTERISTICS.
               ...
               UZOLSFLX(ic, ir) = finfact
   !
   !12-----CALCULATE ET DEMAND LEFT FOR GROUND WATER.
   ! Suppress ET beneath a lake
               IF ( IETFLG.GT.0 .AND. lakflginf.NE.1 ) THEN
                  trhs = 0.0d0
                  thcof = 0.0d0
               ...
               END IF
   !
   !13-----UPDATE UNSATURATED ZONE WAVES WHEN THE WATER TABLE REMAINS
   !         BELOW LAND SURFACE AND CALCULATE CHANGE IN STORAGE.
               ...
               IF ( IUZFOPT.GT.0 ) THEN
                  IF ( ibnd.GT.0 ) THEN
                     IF ( iss.EQ.0 ) THEN
                        ...
                        IF ( loop.GT.0 ) THEN
   !
   !15-----CALCULATE CHANGE IN UNSATURATED ZONE STORAGE WHEN WATER TABLE
   !          RISES.
                           IF ( h.GT.hld ) THEN
                              fm = 0.0D0
                              ...
                              jk = iset + 1
   !
   !16-----WATER TABLE RISES THROUGH WAVES.
                              IF ( jj.GE.jk ) THEN
                                 ...
                                 END DO
   !
   !17-----LOOP THROUGH NUMBER OF TRAIL WAVES INTERSECTED BY WATER TABLE.
                                 eps_m1 = EPS(ic, ir) - 1.0
                                 DO j = iset, jj + 1
                                    IF ( j.EQ.jj+1 ) THEN
                                       IF ( ITRLSTH(j).GT.0 ) THEN
   !18-----LEAD TRAIL WAVE BELOW WATER TABLE AND FIRST TRAIL WAVE IS
   !         ABOVE WATER TABLE.
                                          ...
                                          ELSE
   !
   !19-----LEAD TRAIL WAVE BELOW WATER TABLE AND MULTIPLE TRAIL WAVES
   !         ABOVE WATER TABLE.
   ...
   !
   !20-----MULTIPLE TRAIL WAVES BELOW AND ABOVE WATER TABLE.
                                       ...
                                       END IF
   !
   !21-----ONLY ONE LEAD TRAIL AND ONE TRAIL WAVE BELOW WATER TABLE
   !         AND THERE ARE MUTIPLE TRAIL WAVES IN SET ABOVE WATER TABLE.
                                 ...
                                 END DO
   !
   !22-----DETERMINE VOLUME OF WATER IN WAVES BELOW WATER TABLE.
                                 fm = 0.0D0
                                 j = iset
   ...
   !
   !23-----COMPUTE VOLUME OF WATER BELOW WATER TABLE WHEN
   !         NO WAVES ARE INTERSECTED.
                              ELSE
   ...
   !
   !24------CALCULATE CHANGE IN UNSATURATED ZONE STORAGE WHEN WATER
   !          TABLE DECLINES.
               ...
               END IF
   !
   !25-----CALCULATE ET DEMAND LEFT FOR GROUND WATER.
   ! Suppress ET beneath a lake
               IF ( IETFLG.GT.0 .AND. lakflginf.NE.1 ) THEN
                  s = celtop
                  x = ROOTDPTH(ic, ir)
               ...
               UZOLSFLX(ic, ir) = finfact
   !
   !26------UPDATE ALL VADOSE ZONE WAVES WHEN WATER TABLE
   !          DROPS BELOW LAND SURFACE.
            ELSE IF ( htest1.LT.0.0D0 .AND. htest2.GE.0.0D0 ) THEN
               ...
               UZOLSFLX(ic, ir) = finfact
   !
   !27-----CALCULATE ET DEMAND LEFT FOR GROUND WATER.
   !
   ! Suppress ET beneath a lake
               IF ( IETFLG.GT.0 .AND. lakflginf.NE.1 ) THEN
                  trhs = 0.0d0
            ...
            END IF
   !
   !28-----COMPUTE UNSATURATED ERROR FOR EACH CELL.
   ...
   !
   !29-----ACCUMULATE INFLOW AND OUTFLOW VOLUMES FROM CELLS.
   ...
   !
   !30-----NO UNSATURATED ZONE AND GROUND WATER DISCHARGES TO SURFACE.
   ...
   ! IF SOLUTE ROUTING (MT3D) IS ACTIVE THEN CALCULATE INTERCELL FLUXES
   ! AND WATER CONTENTS
         IF ( RTSOLUTE.GT.0 ) THEN
            IF ( ibnd.NE.0 ) THEN
   ...
   ! SET UZ INTERCELL FLUX TO ZERO WHEN BELOW WATER TABLE
   ...
   !
   !
   !31-----UPDATE RATES AND BUFFERS WITH ET FOR UZF OR MODFLOW BUDGET ITEMS.
   !
   ! Print net recharge as ascii to a separate output file
   IF ( INETFLUX.GT.0 ) THEN
      ...
      txthold = netrchtext
   !
   !-----SAVE NET RECHARGE RATES TO UNFORMATTED FILE FOR UZF OR MODFLOW BUDGET ITEMS.
      ...
      END DO
   !
   !-----SAVE NET DISCHARGE RATES TO UNFORMATTED FILE FOR UZF OR MODFLOW BUDGET ITEMS.
      IF ( ICBCFL.GT.0 ) THEN
   ...
   END IF
   !
   !32-----SAVE ET RATES TO UNFORMATTED FILE FOR UZF OR MODFLOW BUDGET ITEMS.
   IF(IETFLG.NE.0) THEN
      IF ( IUZFB22.LT.0 .OR. IUZFB11.LT.0 ) THEN
   ...
   END IF
   !
   !33-----UPDATE RATES AND BUFFERS WITH GW ET FOR MODFLOW BUDGET ITEMS.
   IF ( IETFLG.GT.0 ) THEN
      IF ( ibd.GT.0 .OR. ibduzf.GT.0 ) THEN
         CALL INITARRAY(TOTCELLS,0.0,BUFF(:,:,1))
         DO ir = 1, NROW
   ...
   !              END DO
         ...
         txthold = textet
   !
   !34-----SAVE GW ET RATES TO UNFORMATTED FILE FOR UZF OR MODFLOW BUDGET ITEMS.
         IF ( ibd.GT.0 ) CALL UBUDSV(Kkstp, Kkper, txthold, IUZFCB1,&
         &BUFF, NCOL, NROW, NLAY, IOUT)
         IF ( ibduzf.GT.0 ) CALL UBDSV3(Kkstp, Kkper, txthold,&
         &IUZFCB2, BUFF, LAYNUM, NUZTOP,&
   ...
   END IF
   !
   !35-----UPDATE RATES AND BUFFERS FOR INFILTRATION.
   ...
   !             END DO
      ...
      END IF
   !
   !37-----SAVE INFILTRATION RATES TO UNFORMATTED FILE.
      IF ( IUZFB22.LT.0 .OR. IUZFB11.LT.0 ) THEN
   ...
   !--not sure if this is used for MODSIM-MODFLOW. Uncomment if yes.
   !35-----UPDATE RATES AND BUFFERS FOR SFR-DIVERTED INFILTRATION.
   !      IF ( Iunitsfr.GT.0 ) THEN
   !        IF ( IUZFB22.LT.0 .OR. IUZFB11.LT.0 ) THEN
   !          IF ( ibd.GT.0 .OR. ibduzf.GT.0 ) THEN
   !            CALL INITARRAY(TOTCELLS,0.0,BUFF(:,:,1))
   ...
   !        END IF
   !      END IF
   !--possible end here for MODSIM-MODFLOW
   !
   !--Uncomment for MODSIM-MODFLOW required output.
   !35-----UPDATE RATES AND BUFFERS FOR INFILTRATION.
   !      IF ( ibd.GT.0 .OR. ibduzf.GT.0 .AND. IETBUD.EQ.0 ) THEN
   !        IF ( IUZFB22.LT.0 .OR. IUZFB11.LT.0 ) THEN
   !          CALL INITARRAY(TOTCELLS,0.0,BUFF(:,:,1))
   ...
   !        END IF
   !      END IF
   !--Uncomment to here for MODSIM-MODFLOW
   !38-----UPDATE RATES AND BUFFERS FOR RECHARGE.
   IF ( ibd.GT.0 .OR. ibduzf.GT.0 .AND. IETBUD.EQ.0 ) THEN
      CALL INITARRAY(TOTCELLS,0.0,BUFF(:,:,1))
      DO ir = 1, NROW
         DO ic = 1, NCOL
   ...
   END IF
   !
   !39-----SAVE ACTUAL INFILTRATION RATES TO UNFORMATTED FILE.
   IF ( IETBUD.EQ.0 ) THEN
   ...
   END IF
   !
   !40-----UPDATE RATES AND BUFFERS FOR SURFACE LEAKAGE RATES.
   IF ( ibd.GT.0 .OR. ibduzf.GT.0 .AND. IETBUD.EQ.0 ) THEN
      CALL INITARRAY(TOTCELLS,0.0,BUFF(:,:,1))
      DO ir = 1, NROW
         DO ic = 1, NCOL
   ...
   !              END DO
      ...
      END DO
   END IF
   !41-----SAVE SURFACE LEAKAGE TO UNFORMATTED FILE.
   IF ( IETBUD.EQ.0 ) THEN
   ...
   END IF
   !
   !40-----UPDATE RATES AND BUFFERS FOR REJECTED INFILTRATON RATES.
   IF ( ibd.GT.0 .OR. ibduzf.GT.0 .AND. IETBUD.EQ.0 ) THEN
      CALL INITARRAY(TOTCELLS,0.0,BUFF(:,:,1))
      DO ir = 1, NROW
         DO ic = 1, NCOL
   ...
   !              END DO
      ...
      END DO
   END IF
   !41-----SAVE REJECTED INFILTRATON RATES TO UNFORMATTED FILE.
   IF ( IETBUD.EQ.0 ) THEN
   ...
   END IF
   !
   !40-----UPDATE RATES AND BUFFERS FOR STORAGE CHANGES.
   IF ( ibd.GT.0 .OR. ibduzf.GT.0 ) THEN
      CALL INITARRAY(TOTCELLS,0.0,BUFF(:,:,1))
      IF ( IUZFOPT.EQ.1 .OR. IUZFOPT.EQ.2 ) THEN
         DO ir = 1, NROW
   ...
   small = 0.1D0
   !
   !43------MOVE TOTAL RECHARGE RATE INTO VBVL FOR PRINTING BY BAS6OT.
   rout = zero
   rin = ratin
   VBVL(4, MSUM) = rout
   VBVL(3, MSUM) = rin
   !
   !44------ADD RECHARGE FOR TIME STEP TO RECHARGE ACCUMULATOR IN VBVL.
   VBVL(2, MSUM) = VBVL(2, MSUM) + rout*DELT
   VBVL(1, MSUM) = VBVL(1, MSUM) + rin*DELT
   !
   !45-----MOVE BUDGET TERM LABELS TO VBNM FOR PRINT BY MODULE BAS6OT.
   VBNM(MSUM) = textrch
   !
   !46-----INCREMENT BUDGET TERM COUNTER.
   MSUM = MSUM + 1
   !
   !47------MOVE TOTAL ET RATE INTO VBVL FOR PRINTING BY BAS6OT.
   rout = ratout
   rin = zero
   VBVL(4, MSUM) = rout
   VBVL(3, MSUM) = rin
   !
   !48------ADD ET FOR TIME STEP TO RECHARGE ACCUMULATOR IN VBVL.
   VBVL(2, MSUM) = VBVL(2, MSUM) + rout*DELT
   VBVL(1, MSUM) = VBVL(1, MSUM) + rin*DELT
   !
   !49-----MOVE BUDGET TERM LABELS TO VBNM FOR PRINT BY MODULE BAS6OT.
   VBNM(MSUM) = textet
   !
   !50-----INCREMENT BUDGET TERM COUNTER.
   MSUM = MSUM + 1
   !
   !51------MOVE TOTAL SURFACE LEAKAGE RATE INTO VBVL FOR PRINTING
   !          BY BAS6OT.
   rout = ratout2
   rin = zero
   VBVL(4, MSUM) = rout
   VBVL(3, MSUM) = rin
   !
   !52------ADD SURFACE LEAKAGE FOR TIME STEP TO RECHARGE ACCUMULATOR
   !          IN VBVL.
   VBVL(2, MSUM) = VBVL(2, MSUM) + rout*DELT
   VBVL(1, MSUM) = VBVL(1, MSUM) + rin*DELT
   !
   !53-----MOVE BUDGET TERM LABELS TO VBNM FOR PRINT BY MODULE BAS6OT.
   VBNM(MSUM) = textexfl
   !
   !54-----INCREMENT BUDGET TERM COUNTER.
   MSUM = MSUM + 1
   !
   !55-----UNSATURATED ZONE INFORMATION TO SEPARATE FILE WHEN REQUESTED
   !        (IUZFCB2>0).
   prcntercum = 0.0
   prcnterrat = 0.0
   IF ( IUZFOPT.GT.0 ) THEN
      IF ( IBUDFL.GT.0 ) THEN
   !
   !56-----WRITE BUDGET INFORMATION FOR UNSATURATED ZONE TO MAIN LIST FILE.
         WRITE (IOUT, 9002) Kkstp, Kkper
         WRITE (IOUT, 9003)
   !
   !57-----PRINT INFLOW AND OUTFLOW VOLUMES AND RATES.
   ...
   !
   !58----CALCULATE DIFFERENCE AND ERROR BETWEEN INFLOW AND OUTFLOW.
   ...
   !     +         (unsatrat.GE.bigvl1 .OR. unsatrat.LT.small) ) THEN
      ...
      END IF
   !dep changed conditional end if location 7/30/08
   END IF

   !
   !59----PRINT TIME SERIES OF UZ FLOW AND STORAGE OR MOISTURE CONTENT
   !        PROFILES FOR SELECTED CELLS.
   IF ( NUZGAG.GT.0 ) THEN
   !
   !60----LOOP OVER GAGING STATIONS.
      iog = 1
   ...
   !
   !61-----GET OUTTYPE FOR SPECIFIED UNSATURATED ZONE CELL.
            SELECT CASE (iuzopt)
   !
   !62-----CASE 1: WRITE VOLUMES IN, OUT, AND IN STORAGE FOR A CELL.
             CASE (1)
               WRITE (iftunit, 9011) il, TOTIM, ghnw, ghdif, gcumapl,&
               &gcumin, gcumrch, guzstore, gdelstor, gseep
   !
   !63-----CASE 2: WRITE VOLUMES AND RATES FOR CELL.
             CASE (2)
   ...
   !
   !64-----CASE 3: WRITE UNSATURATED-ZONE MOISTURE PROFILES.
             CASE (3)
   !
   !65-----TOTAL WATER CONTENT OVER SPECIFIED DEPTH.
               IF ( IUZFOPT.GT.0) THEN
               ...
               END IF
   !
   !66-----CASE 4: WRITE TOTAL INFILTRATION, RUNOFF, ET AND RATES FOR
   !                 ALL ACTIVE CELLS.
             CASE (4)
               WRITE (iftunit, 9016) TOTIM, cumapplinf, TOTRUNOFF,&
               &UZTSRAT(1), UZTSRAT(5), UZTSRAT(2), totbet,&
               &UZTSRAT(4), UZTSRAT(3)
            END SELECT
         END IF
         iog = iog + 1
...
&ic, ir, land, h, celtop, mtflg, kper)
   !     ******************************************************************
   !     COMPUTE WAVE INTERACTION WITHIN AN UNSATURATED FLOW CELL
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     VERSION 1.0.5:  April 5, 2012
   !     ******************************************************************
   ...
   !
   !2------CREATE A NEW WAVE WHEN SURFACE FLUX CHANGES.
   !         CALL TRAILWAVE2 IF SURFACE FLUX DECREASES.
   !         CALL LEADWAVE2 IF SURFACE FLUX INCREASES.
   ...
   END IF
   !
   !3------CALCULATE VOLUME OF WATER BELOW WATER TABLE.
   ...
   END IF
   !
   !4------CALCULATE UNSATURATED ZONE ET.
   IF ( IETFLG.NE.0 ) THEN
      numwavesd = Numwaves
...
&Feps2, Itrailflg, Delt, ffcheck)
   ...
   IF ( Itester.NE.1 ) THEN
      DO WHILE ( diff.GT.ZEROD6 )
         ...
         j = 2
   !
   !3------CALCULATE TIME UNTIL A WAVE WILL OVERTAKE A WAVE AHEAD.
         nwavp1 = Numwaves + 1
               ...
               IF ( lcheck ) THEN
                  DO WHILE ( lcheck )
                     ...
                     IF ( Numwaves.GT.kk ) THEN
                        ...
                        &Itrwave(Jpnt+j).GT.0)
   !
   !4------LEAD WAVE INTERSECTS A TRAIL WAVE.
                        fhold = 0.0D0
   ...
   !
   !9------CONVERT TRAIL WAVES TO LEAD TRAIL WAVES.
   ...
   !11-----ROUTE TRAIL WAVES.
            ...
            END DO
   !
   !12-----REMOVE WAVES THAT HAVE BEEN INTERCEPTED AND UPDATE SPEED
   !         OF COMBINED WAVE.
            j = 3
            l = j
            iflag = 0
            DO WHILE ( iflag.EQ.0 )
               IF ( MORE(j).EQ.1 ) THEN
                  l = j
   !
   !13-----CHECK IF INTERCEPTED WAVES ARE TRAIL WAVES.
                  IF ( Ltrail(jpntm1+j).NE.1 ) THEN
                     ...
                     END IF
   !
   !14-----CONVERT REMAINING TRAIL WAVES TO LEAD TRAIL WAVES WHEN
   !         WHEN LEAD TRAIL WAVE INTERSECTS A LEAD WAVE.
   ...
   !
   !15-----NON TRAIL WAVES INTERCEPTS A NON TRAIL WAVE.
                  ELSE
                     Ltrail(jpntm1+j) = 0
                     Itrwave(Jpnt+j) = 0
                     IF ( j.EQ.3 ) THEN
   ...
   !
   !17-----CALCULATE TOTAL FLUX TO WATER TABLE FOR CONSTANT FLUX
   ...
   !18-----ROUTE TRAILING WAVES.
         ...
         END IF
   !
   !19-------REMOVE WAVES THAT WERE INTERCEPTED OR REACHED WATER TABLE.
...
&Surflux, Jpnt)
   ...
   !     ******************************************************************
   !     INITIALIZE NEW SET OF TRAIL WAVES WHEN SURFACE FLUX DECREASES
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     VERSION 1.0.5:  April 5, 2012
   !     ******************************************************************
   IMPLICIT NONE
...
&celtop, mtflg, kper)
   !     ******************************************************************
   !     REMOVE WATER FROM UNSATURATED ZONE CAUSED BY EVAPOTRANSPIRATION
   !     ******************************************************************
   ...
   itest = 0
   k=0
   ! MASTER LOOP FOR REDUCING ACTUAL ET TO POTENTIAL ET.
   do while ( itest == 0 )
      k = k + 1
   !RESET AND TRY AGAIN
      DO ii = 1, Nwv
      ...
      Etout = 0.0D0
   !
   !2------ONE WAVE IN PROFILE THAT IS SHALLOWER THAN ET EXTINCTION DEPTH.
      IF ( Numwaves.EQ.1 .AND. Depth(Jpnt).LE.Rootdepth ) THEN
         ...
         END IF
   !
   !3------MULTIPLE WAVES BUT SHALLOWEST WAVE IS DEEPER THAN ET EXTINCTION
   !         DEPTH.
      ...
      &Rootdepth) THEN
         ...
         END IF
   !
   !4------ONLY ONE WAVE IS DEEPER THAN ET EXTINCTION DEPTH.
      ELSE IF ( Numwaves.EQ.1 ) THEN
         IF ( ETOFH_FLAG.GT.0 ) THEN
            THO = Theta(jpntp1)
            FKTHO = Flux(jpntp1)
      ...
      ELSE
   !
   !5------MULTIPLE WAVES AND ET EXTINCTION DEPTH FALL BETWEEN DEEPEST
   !         AND SHALLOWEST WAVE.
         IF ( Depth(Jpnt).GT.Rootdepth ) THEN
            j = Jpnt + Numwaves - 2
            jk = 0
   !
   !6------LOCATE ET EXTINCTION DEPTH BETWEEN WAVES.
            DO WHILE ( jk.EQ.0 )
            ...
            IF ( Theta(j).GT.Thetar+Wiltwc ) THEN
               ...
               END DO
   !
   !7------CREATE A NEW WAVE AT ET EXTINCTION DEPTH.
   ...
   !
   !8------CONVERT TRAIL WAVES SPLIT BY ET EXTINCTION DEPTH TO
   !         LEAD TRAIL WAVES IF WATER CONTENT ABOVE EXTINCTION WATER
   !         CONTENT.
   !              DO kj = j + 1, j + 1 + ihold - Itrwave(itrwaveyes)
         ...
         END IF
   !
   !9------ALL WAVES SHALLOWER THAN ET EXTINCTION DEPTH.
   ...
   !10-----CALCULATE ACTUAL ET.
      ...
      END DO
   !
   !11-----SET ETOUT TO ZERO WHEN ET DEMAND LESS THAN ROUNDOFF ERROR.
      Etout = st - fm
      fmp = Etout/Etime
...
&Iret, Finfact, Thr, land, iss )
   !     ******************************************************************
   !     AVEARGE WATER CONTENT AND FLUX FOR MT3DMS
   !     ******************************************************************
   ...
   !     ------------------------------------------------------------------
   !
   !65-----TOTAL WATER CONTENT AND FLUX OVER SPECIFIED DEPTH.
   !        IF ( il.GT.0 ) THEN
   ZEROD9 = 1.0d0-9
   ghdif = celtop - H
   totalwc = 0.0
...
END SUBROUTINE GWF2UZF1DA
   !
   !-------SUBROUTINE SGWF2UZF1PNT
SUBROUTINE SGWF2UZF1PNT(Igrid)
   ! Set UZF pointers for grid.
   USE GWFUZFMODULE
   !     ------------------------------------------------------------------
   !     ARGUMENTS
   !     ------------------------------------------------------------------
...
END SUBROUTINE SGWF2UZF1PNT
   !
   !-------SUBROUTINE SGWF2UZF1PSV
SUBROUTINE SGWF2UZF1PSV(Igrid)
   ! Save UZF pointers for grid.
   USE GWFUZFMODULE
   !     ------------------------------------------------------------------
   !     ARGUMENTS
   !     ------------------------------------------------------------------
