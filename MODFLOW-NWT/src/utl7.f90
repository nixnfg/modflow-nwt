...
SUBROUTINE URDCOM(IN,IOUT,LINE)
   !     ******************************************************************
   !     READ COMMENTS FROM A FILE AND PRINT THEM.  RETURN THE FIRST LINE
...
10 READ(IN,'(A)') LINE
   !
   !2------If the line does not start with "#", return.
   IF(LINE(1:1).NE.'#') RETURN
   !
   !3------Find the last non-blank character.
   L=LEN(LINE)
   DO 20 I=L,1,-1
      IF(LINE(I:I).NE.' ') GO TO 30
20 CONTINUE
   !
   !4------Print the line up to the last non-blank character if IOUT>0.
...
114            FORMAT(1X,'(THE SCALE FACTOR WAS APPLIED TO FIELDS',&
   ...
   ELSE
   !
   !5------CHECK FOR AUXILIARY VARIABLE "AUXSFAC" AND STORE LOCATION
...
240   CONTINUE
   ...
   DO 250 II=LSTBEG,N
      ...
      J=RLIST(3,II)
   !
   !6A------Scale fields ISCLOC1-ISCLOC2 by SFAC and AUXSFAC (if present)
      DO 204 ILOC=ISCLOC1,ISCLOC2
         RLIST(ILOC,II)=RLIST(ILOC,II)*SFAC
         IF (JAUX .NE. 0) RLIST(ILOC,II)=RLIST(ILOC,II)*RLIST(JAUX,II)
204   CONTINUE
   !
   !6C-----Write the values that were read if IPRFLG is 1.
      NN=II-LSTBEG+1
      IF(IPRFLG.EQ.1)&
      &WRITE(IOUT,206) NN,K,I,J,(RLIST(JJ,II),JJ=4,NREAD2)
206   FORMAT(1X,I6,I7,I7,I7,26G16.4)
   !
   !6D-----Check for illegal grid location
...
40 CONTINUE
   !
   !7------PRINT A LINE OF DOTS (FOR AESTHETIC PURPOSES ONLY).
50 NTOT=NTOT
...
SUBROUTINE ULAPRS(BUF,TEXT,KSTP,KPER,NCOL,NROW,ILAY,IPRN,IOUT)
   ...
   !     ------------------------------------------------------------------
   !
   !1------MAKE SURE THE FORMAT CODE (IP OR IPRN) IS BETWEEN 1
   !1------AND 21.
   IP=IPRN
   IF(IP.LT.1 .OR. IP.GT.21) IP=12
   !
   !2------DETERMINE THE NUMBER OF VALUES (NCAP) PRINTED ON ONE LINE.
   NCAP=10
   IF(IP.EQ.1) NCAP=11
   IF(IP.EQ.2) NCAP=9
   ...
   IF(IP.EQ.21) NCAP=7
   !
   !3------CALCULATE THE NUMBER OF STRIPS (NSTRIP).
   NCPF=129/NCAP
   ...
   J2=0
   !
   !4------LOOP THROUGH THE STRIPS.
   DO 2000 N=1,NSTRIP
   !
   !5------CALCULATE THE FIRST(J1) & THE LAST(J2) COLUMNS FOR THIS STRIP
      J1=J1+NCAP
      J2=J2+NCAP
      IF(J2.GT.NCOL) J2=NCOL
   !
   !6-------PRINT TITLE ON EACH STRIP DEPENDING ON ILAY
      IF(ILAY.GT.0) THEN
...
2        FORMAT('1',/1X,A,' FOR CROSS SECTION AT END OF TIME STEP',I3,&
      ...
      END IF
   !
   !7------PRINT COLUMN NUMBERS ABOVE THE STRIP
      CALL UCOLNO(J1,J2,ISP,NCAP,NCPF,IOUT)
   !
   !8------LOOP THROUGH THE ROWS PRINTING COLS J1 THRU J2 WITH FORMAT IP
      DO 1000 I=1,NROW
...
&NROW,ILAY,ICHN,FMTOUT,LBLSAV,IBOUND)
   !     ******************************************************************
   !     SAVE 1 LAYER ARRAY ON DISK USING FORMATTED OUTPUT
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
&NROW,ILAY,ICHN,FMTOUT,LBLSAV)
   !     ******************************************************************
   !     SAVE 2-D (LAYER) INTEGER ARRAY ON DISK USING FORMATTED OUTPUT
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
&NCOL,NROW,NLAY,NLIST,IOUT,DELT,PERTIM,TOTIM,IBOUND)
   !     ******************************************************************
   !     WRITE HEADER RECORDS FOR CELL-BY-CELL FLOW TERMS FOR ONE COMPONENT
...
SUBROUTINE UMESPR(TEXT1,TEXT2,IOUT)
   !     ******************************************************************
   !     PRINT A LINE CONSISTING OF TWO TEXT VARIABLES.
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
