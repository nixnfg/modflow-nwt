...
SUBROUTINE GWF2NWT1AR(In, Mxiter, Iunitlak, Igrid)
   !
   !------NEWTON SOLVER VERSION NUMBER 1.3.0, 7/01/2022
   !      RICHARD G. NISWONGER
...
21    FORMAT(1X,'SIMPLE OPTION:',/,&
      &1X,'DEFAULT SOLVER INPUT VALUES REFLECT NEARLY LINEAR MODEL')
...
9012 FORMAT(1X,'  BACKTRACKING IS ACTIVE ',&
   ...
   !     MBLACK = NODES
   !     NJAF = 7 * Numnonzero
   !     liwrk = 3*NODES + 3*mblack + njaf + 1    ! ldcomb = .false.
   !     lrwrk = 7/2*mblack + 2*(14+1)*mblack + 14
   !mi


   mblack = nodes
   !     if (iredsys.eq.1) mblack = nodes * 0.5 + 1
   !     njaf = 7 * nja
   njaf = 7 * numnonzero
   !     liwrk = 3*nodes + 3*mblack + njaf + 1    ! ldcomb = .false.
   !     if (ldcomb) liwrk = 3*nodes + 4*mblack + 2*njaf + 1  !        = .true.
   liwrk = 3*nodes + 4*mblack + 2*njaf + 1  !        = .true.

   !     lrwrk = 3*mblack + 2*(north+1)*mblack + north
...
&Iunitchd, agconverge, Igrid)
   ...
   !         INPUT DATA
   !            n=number of cells
   !            nja=number of non-zero entries in the linear system
   !            ia,ja,a matrix in CRS format
   !            BB= right hand side of the linear system
   !            Hchange= first guess / solution of the linear system
   !            matrix=22 if not symmetric else matrix=12
   !             matrix=22
   !            Stop_tol_samg = closure criterion L2-norm
   !               if eps<0 :: absolute residual
   !               else     :: relative reduction compared to first guess
   !            Maxitr_samg=maximal number of iterations
   !            control=1 no setup reuse
   !            control=2 automatic setup reuse
   !             control=1
   !            iout  Controls print output related to SAMG’s solution phase.
   !                <0 No printout, except for warnings and errors24.
   !                =0 Minimal output on results and timings.
   !                >0 Additional print output specified by the individual digits:
   !                   iout[1:1]
   !                      1 Table of input data and work statistics.
   !                      2 In addition: history of cycling process.
   !                      3 Extended history: including all levels, full smoothing steps.
   !                      4 Extended history: including all levels, partial smoothing steps.
   !                    iout[2:2]
   !                      0 No action.
   !                      1 Display most relevant hidden parameters.
   !                      2 Display all hidden parameters.
   !                      3 Display all hidden parameters in a single list.
   !              iout_samg=2

   !
   !         OUTPUT DATA
   !            res_out=output residual L2-Norm (Norm can be changed)
   !            ncyc_done=iterations performed
   !            ierr=error flag



   ! !Diagonals have to be positive!
   !             do i=1,n
   !                if (a(ia(i)).lt.0) then
   !                   do j=ia(i),ia(i+1)-1
   !                      a(j)=-a(j)
