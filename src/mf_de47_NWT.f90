...
SUBROUTINE DE47AR(INDE4,MXITER,IGRID)
   !     ******************************************************************
   !     ALLOCATE STORAGE IN X ARRAY FOR D4 ARRAYS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !
   !1------PRINT A MESSAGE IDENTIFYING DE4 PACKAGE.
   WRITE(IOUT,1) INDE4
1  FORMAT(1X,/1X,'DE4 -- D4 DIRECT SOLUTION PACKAGE',&
   &', VERSION 7, 5/2/2005  INPUT READ FROM UNIT ',I4)
   !
   !2------CALCULATE DEFAULT VALUES FOR MXUP, MXLOW, AND MXBW.
   !2------ALSO SET VALUES FOR ID4DIR (DIRECTION OF EQUATION ORDERING) AND
   !2------ID4DIM (MAXIMUM NUMBER OF HEAD COEFFICIENTS FOR ONE EQUATION).
   !2------ID4DIM=5 for a 2-D problem; ID4DIM=7 for 3-D.
   ...
   END IF
   !
   !3------READ AND PRINT COMMENTS, ITMX, MXUP, MXLOW, MXBW.  FOR ANY
   !3------ZERO OR NEGATIVE VALUES, SUBSTITUE THE DEFAULT VALUE.
...
4  FORMAT(1X,'MAXIMUM EQUATIONS IN UPPER PART OF [A]:',I7,/&
   &1X,'MAXIMUM EQUATIONS IN LOWER PART OF [A]:',I7,/&
   &1X,'MAXIMUM BAND WIDTH OF [AL] PLUS 1:',I5)
   !
   !4------ALLOCATE SPACE FOR THE DE4 ARRAYS.
   ...
   !
   !1------READ IFREQ, MUTD4, ACCL, HCLOSE, AND IPRD4
   ONE=1.
   ZERO=0.
...
11    FORMAT(1X,/1X,'INVALID VALUE FOR IFREQ PARAMETER:',I8)
   ...
   END IF
   !
   !2------CHECK TO SEE IF THERE IS ITERATION (ITMX>1).
   IF(ITMX.GT.1) THEN
   !
   !3------THERE IS ITERATION -- DETERMINE TYPE OF ITERATION BASED ON IFREG
   !3------VALUE.
      IF(IFREQ.EQ.3) THEN
   !
   !3A-----EXTERNAL ITERATION -- SET ITERATION VARIABLES AND PRINT A
   !3A-----MESSAGE.
         MXITER=ITMX
         NITERDE4=1
         WRITE(IOUT,51)
51       FORMAT(1X,/21X,'SOLUTION BY D4 DIRECT SOLVER WITH EXTERNAL',&
         &' ITERATION',/21X,52('-'),/)
   !
   !3B-----INTERNAL ITERATION -- SET ITERATION VARIABLES AND PRINT A
   !3B-----MESSAGE.
      ELSE
         MXITER=1
         NITERDE4=ITMX
...
81       FORMAT(1X,/21X,'SOLUTION BY D4 DIRECT SOLVER WITH INTERNAL',&
      ...
      END IF
   !
   !3C-----PRINT ITERATION INFORMATION.
      WRITE(IOUT,91) ITMX,ACCLDE4,HCLOSEDE4,IPRD4
91    FORMAT(1X,'MAXIMUM ITERATIONS =',I3/&
      &1X,'RELAXATION-ACCELERATION PARAMETER =',F10.6/&
      &1X,'HEAD CHANGE CRITERION FOR CLOSURE =',G15.5/&
      &1X,'D4 PRINTOUT INTERVAL =',I4)
      IF(MUTD4.EQ.1) WRITE(IOUT,92)
92    FORMAT(1X,&
      &'CONVERGENCE PRINTOUT WILL SHOW ONLY THE NUMBER OF ITERATIONS')
      IF(MUTD4.EQ.2) WRITE(IOUT,93)
93    FORMAT(1X,'CONVERGENCE PRINTOUT WILL BE SUPPRESSED')
   !
   !4------NO ITERATION -- SET ITERATION VARIABLES AND PRINT A MESSAGE.
   ELSE
      MXITER=1
      NITERDE4=1
      ACCLDE4=ONE
...
&MXEQ,MXBW,CR,CC,CV,HCOF,RHS,ACCLDE4,KITER,ITMX,MXITER,NITERDE4,&
&HCLOSEDE4,IPRD4,ICNVG,NCOL,NROW,NLAY,IOUT,LRCHDE4,HDCGDE4,&
&IFREQ,KSTP,KPER,DELT,NSTP,ID4DIR,ID4DIM,MUTD4,DELTL,NBWL,NUPL,&
&NLOWL,NLOW,NEQ,NUP,NBW,IERR)
   !     ******************************************************************
   !     SOLVE FINITE-DIFFERENCE EQUATIONS FOR ONE EXTERNAL ITERATION.
   !     MULTIPLE SOLUTIONS ARE MADE WHEN INTERNALLY ITERATING.
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !     ------------------------------------------------------------------
   !
   !1------INITIALIZE VARIABLES AND SET FLAG THAT INDICATES IF [A] REQUIRES
   !1------ELIMINATION.
   ZERO=0.
   ICNVG=0
   NIT=0
...
10 NIT=NIT+1
   ...
   KBIG=0
   !
   !3------GO TO STATEMENT 100 IF [A] DOES NOT REQUIRE ELIMINATION.
   IF (ITYPE.EQ.1) GO TO 100
   !
   !4------[A] REQUIRES ELIMINATION.
   !4A-----CALL MODULE SDE47N TO ORDER EQUATIONS.  IF MXUP OR MXLOW ARE
   !4A-----TOO SMALL, PRINT A MESSAGE AND STOP.
...
41    FORMAT(1X,'INSUFFICIENT MEMORY FOR DE4 SOLVER:',/&
      &1X,'MXUP MUST BE AT LEAST',I8,/&
      &1X,'MXLOW MUST BE AT LEAST',I8)
      IERR = 1
      RETURN
   END IF
   !
...
50 CONTINUE
   !
   !5------LOOP THROUGH ALL CELLS CALCULATING COEFFICIENTS AND LOADING
   !5------ARRAYS FOR SOLUTION.  THE RESIDUAL MUST ALWAYS BE CALCULATED
   !5------AND LOADED IN D4B; IUPPNT, AU, AND AL(1,n) ARE CALCULATED AND
   !5------LOADED ONLY IF [A] REQUIRES ELIMINATION.
100 DO 310 K=1,NLAY
      DO 310 I=1,NROW
         DO 310 J=1,NCOL
            IR=IEQPNT(J,I,K)
...
110         CONTINUE
   ...
   !
   !5B-----IF [A] REQUIRES ELIMINATION, LOAD AL(1,N) AND AU
...
310 CONTINUE
   !
   !6------IF [A] DOES NOT REQUIRE ELIMINATION, SKIP TO STATEMENT 380.
   IF(ITYPE.EQ.1) GO TO 380
   !6A-----[A] REQUIRES ELIMINATION -- DETERMINE BAND WIDTH + 1.
   MNN=999999
   MXN=0
   DO 350 I=1,NUP
      L=IUPPNT(1,I)
...
350 CONTINUE
   NBW=MXN-MNN+1
   !
   !6B-----WRITE BAND WIDTH + 1 AND NUMBER OF EQUATIONS IF ANY HAVE CHANGED.
   IF(NUP.NE.NUPL .OR. NLOW.NE.NLOWL .OR. NBW.NE.NBWL) THEN
      WRITE(IOUT,351) NUP,NLOW,NBW
351   FORMAT(1X,/1X,I7,' UPPER PART EQS.',&
      &I10,' LOWER PART EQS.    BAND WIDTH + 1 =',I5)
      NUPL=NUP
      NLOWL=NLOW
      NBWL=NBW
   END IF
   !
   !6C-------STOP IF BAND WIDTH EXCEEDS USER-SPECIFIED SIZE.
   IF(NBW.GT.MXBW) THEN
      WRITE(IOUT,353) NBW
353   FORMAT(1X,'INSUFFICIENT MEMORY FOR DE4 SOLVER:',/&
      &1X,'MXBW MUST BE AT LEAST',I5)
      IERR = 1
      RETURN
   END IF
   !
   !6D-------INITIALIZE OFF DIAGONAL PART OF AL.
   DO 360 I=1,NLOW
      DO 360 J=2,NBW
         AL(J,I)=ZERO
360 CONTINUE
   !
   !7------CALL MODULE SDE47S TO SOLVE EQUATIONS FOR HEAD CHANGE.
380 CALL SDE47S(AU,AL,IUPPNT,D4B,NUP,NLOW,NEQ,MXBW,NBW,ITYPE,ID4DIM)
   !
   !8------CALCULATE NEW HEAD FROM HEAD CHANGE AND FIND MAXIMUM CHANGE.
   DO 400 K=1,NLAY
      DO 400 I=1,NROW
         DO 400 J=1,NCOL
            L=IEQPNT(J,I,K)
...
400 CONTINUE
   !
   !9------IF THE NUMBER OF INTERNAL ITERATIONS IS 1, GO TO STATEMENT 500
   IF(NITERDE4.EQ.1) GO TO 500
   !
   !10-----THE NUMBER OF INTERNAL ITERATIONS IS GREATER THAN 1, SO MUST BE
   !10-----ITERATING INTERNALLY.  KEEP TRACK OF MAXIMUM HEAD CHANGE AND
   ...
   !
   !10A----IF NOT CONVERGED AND MAXIMUM ITERATIONS HAS NOT BEEN REACHED,
   !10A----GO BACK AND DO ANOTHER INTERNAL ITERATION.  SET ITYPE=1 TO
   !10A----AVOID REFORMULATION OF [A].
   ITYPE=1
   IF(ICNVG.EQ.0 .AND. NIT.NE.NITERDE4) GO TO 10
   !
   !10B----INTERNAL ITERATION IS DONE EITHER BECAUSE CONVERGENCE IS REACHED
   !10B----OR BECAUSE MAX. ITERATIONS EXCEEDED.  PRINT CONVERGENCE
   !10B----INFORMATION UNLESS IMUTD4=2.
   IF(MUTD4.LT.2) THEN
   !10B1---PRINT A BLANK LINE IF THIS IS THE FIRST TIME STEP.
      IF(KSTP.EQ.1) WRITE(IOUT,601)
   !10B2---PRINT NUMBER OF ITERATIONS.
      WRITE(IOUT,751) NIT,KSTP,KPER
751   FORMAT(1X,I5,' INTERNAL ITERATIONS FOR TIME STEP',I6,& !gsf
      &' IN STRESS PERIOD ',I4)
   !10B3---IF MUTD4=0 AND
   !10B3---IF FAILED TO CONVERGE OR LAST TIME STEP IN STRESS PERIOD OR
   !10B3---IPRD4 INTERVAL IS MET, CALL MODULE SDE47P TO PRINT HEAD CHANGE.
      IF((MUTD4.EQ.0) .AND.&
      &(ICNVG.EQ.0 .OR. KSTP.EQ.NSTP .OR. MOD(KSTP,IPRD4).EQ.0))&
   ...
   RETURN
   !
   !11-----THERE ARE NO INTERNAL ITERATIONS, SO THERE MUST EITHER BE
   !11-----EXTERNAL ITERATION OR NO ITERATION.  IF THERE IS EXTERNAL
   !11-----ITERATION, GO TO STATEMENT 600.
500 IF(MXITER.NE.1) GO TO 600
   !
   !12-----NO ITERATION (NITERDE4=1 AND MXITER=1).  SET FLAG TO INDICATE
   !12-----CONVERGENCE HAS OCCURRED, AND PRINT MAXIMUM HEAD CHANGE UNLESS
   !12-----MUTD4<2.
   ICNVG=1
   IF(MUTD4.LT.2) WRITE(IOUT,501) KSTP,KPER,BIG,KBIG,IBIG,JBIG
501 FORMAT(1X,/1X,'MAXIMUM HEAD CHANGE IN TIME STEP',I6,& !gsf
   &' OF STRESS PERIOD ',I4,' =',G15.5,&
   &' AT LAYER =',I3,', ROW =',I5,', COL=',I5)
   RETURN
   !
   !13-----EXTERNAL ITERATION.  KEEP TRACK OF MAXIMUM HEAD CHANGE AND SET
   !13-----CONVERGENCE FLAG IF CONVERGENCE OCCURRED.
600 LRCHDE4(1,KITER)=KBIG
   LRCHDE4(2,KITER)=IBIG
   LRCHDE4(3,KITER)=JBIG
   HDCGDE4(KITER)=BIG
   IF(ABS(BIG).LE.HCLOSEDE4) ICNVG=1
   !13A----RETURN IF NO CONVERGENCE AND MAX. ITERATIONS NOT EXCEEDED.
   IF(ICNVG.EQ.0 .AND. KITER.NE.MXITER) RETURN
   !13B----EXTERNAL ITERATION IS DONE EITHER BECAUSE CONVERGENCE IS REACHED
   !13B----OR BECAUSE MAX. ITERATIONS EXCEEDED.  PRINT CONVERGENCE
   !13B----INFORMATION UNLESS IMUTD4=2.
   IF(MUTD4.LT.2) THEN
   !13B1---PRINT A BLANK LINE IF THIS IS THE FIRST TIME STEP.
      IF(KSTP.EQ.1) WRITE(IOUT,601)
601   FORMAT(1X)
   !13B2---PRINT NUMBER OF ITERATIONS.
      WRITE(IOUT,602) KITER,KSTP,KPER
602   FORMAT(1X,I5,' EXTERNAL ITERATIONS FOR TIME STEP',I6,& !gsf
      &' IN STRESS PERIOD ',I4)
   !13B3---IF MUTD4=0 AND
   !13B3---IF FAILED TO CONVERGE OR LAST TIME STEP IN STRESS PERIOD OR
   !13B3---IPRD4 INTERVAL IS MET, CALL MODULE SDE47P TO PRINT HEAD CHANGE.
      IF((MUTD4.EQ.0) .AND.&
      &(ICNVG.EQ.0 .OR. KSTP.EQ.NSTP .OR. MOD(KSTP,IPRD4).EQ.0))&
...
&NEQ)
   !     ******************************************************************
   !     ORDER EQUATIONS USING D4 ORDERING
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
20 CONTINUE
   NEQ=0
   !
   !2------ORDER EQUATIONS BASED ON DIRECTION FLAG, ID4DIR
   !2------Ordering is done as described by Price, H.S. and Coats, K.H.,
   !2------1974,Direct methods in reservoir simulation: Soc. Petrol. Eng.
   !2------Jour., June 1974, p. 295-308.
   GO TO (100,200,300,400,500,600) ID4DIR
   CALL USTOP(' ')
   !
   !3------DIRECTION 1 -- NCOL>or=NROW>or=NLAY
   !3A-----Order equations with odd plane numbers.
...
130 CONTINUE
   NUP=NEQ
   !
   !3B-----Order equations with even plane numbers.
   DO 140 N=4,NPLANE,2
      K1=N-2
      IF(K1.GT.NLAY) K1=NLAY
      K2=N-NCOL-NROW
...
140 CONTINUE
   ...
   RETURN
   !
   !4------DIRECTION 2 NROW>NCOL>or=NLAY
   !4A-----Order equations with odd plane numbers.
...
230 CONTINUE
   NUP=NEQ
   !
   !4B-----Order equations with even plane numbers.
   DO 240 N=4,NPLANE,2
      K1=N-2
      IF(K1.GT.NLAY) K1=NLAY
      K2=N-NCOL-NROW
...
240 CONTINUE
   ...
   RETURN
   !
   !-------DIRECTION 3 NCOL>or=NLAY>NROW
   !5A-----Order equations with odd plane numbers.
...
330 CONTINUE
   NUP=NEQ
   !
   !5B-----Order equations with even plane numbers.
   DO 340 N=4,NPLANE,2
      K1=N-2
      IF(K1.GT.NROW) K1=NROW
      K2=N-NCOL-NLAY
...
340 CONTINUE
   ...
   RETURN
   !
   !6------DIRECTION 4 NLAY>NCOL>or=NROW
   !6A-----Order equations with odd plane numbers.
...
430 CONTINUE
   NUP=NEQ
   !
   !6B-----Order equations with even plane numbers.
   DO 440 N=4,NPLANE,2
      K1=N-2
      IF(K1.GT.NROW) K1=NROW
      K2=N-NCOL-NLAY
...
440 CONTINUE
   ...
   RETURN
   !
   !7------DIRECTION 5 NROW>or=NLAY>NCOL
   !7A-----Order equations with odd plane numbers.
...
530 CONTINUE
   NUP=NEQ
   !
   !7B-----Order equations with even plane numbers.
   DO 540 N=4,NPLANE,2
      K1=N-2
      IF(K1.GT.NCOL) K1=NCOL
      K2=N-NROW-NLAY
...
540 CONTINUE
   ...
   RETURN
   !
   !-------DIRECTION 6 NLAY>NROW>NCOL
   !8A-----Order equations with odd plane numbers.
...
630 CONTINUE
   NUP=NEQ
   !
   !8B-----Order equations with even plane numbers.
   DO 640 N=4,NPLANE,2
      K1=N-2
      IF(K1.GT.NCOL) K1=NCOL
      K2=N-NROW-NLAY
...
&ID4DIM)
   !     ******************************************************************
   !     SOLVE EQUATIONS USING GAUSS ELIMINATION ASSUMING D4 ORDERING
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   NLOWM1=NLOW-1
   !
   !2------DON'T ELIMINATE UNLESS NECESSARY.
   IF (ITYPE.EQ.1) GO TO 80
   !
   !2A-----MUST ELIMINATE -- DO THIS IN TWO PARTS.
   !2A-----ELIMINATE THE LEFT SIDE OF THE LOWER PART OF [A] TO FILL [AL].
   DO 40 I=1,NUP
      JJ=IUPPNT(1,I)
      C1=ONE/AU(1,I)
      DO 30 J=2,JJ
...
70 CONTINUE
   !
   !3------D4B MUST ALWAYS BE MODIFIED -- MODIFY D4B IN TWO PARTS.
   !3A-----MODIFY D4B DUE TO ELIMINATION TO FILL [AL].
80 DO 100 I=1,NUP
      JJ=IUPPNT(1,I)
      DO 90 J=2,JJ
         LR=IUPPNT(J,I)
...
100 CONTINUE
   !
   !3B-----MODIFY D4B DUE TO ELIMINATION OF [AL].
   DO 120 I=1,NLOWM1
      IR=I+NUP
      LR=IR
      DO 110 J=2,NBW
...
SUBROUTINE SDE47P(HDCGDE4,LRCHDE4,NUMIT,IOUT,NCOL,NROW)
   !     ******************************************************************
   !     PRINT MAXIMUM HEAD CHANGE DURING EACH D4 ITERATION FOR A TIME STEP
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
5     FORMAT(1X,/1X,'MAXIMUM HEAD CHANGE FOR EACH ITERATION:',/&
      &1X,/1X,3('  HEAD CHANGE  LAY,ROW,COL'),/1X,79('-'))
...
15    FORMAT(1X,/1X,'MAXIMUM HEAD CHANGE FOR EACH ITERATION:',/&
      &1X,/1X,2('    HEAD CHANGE  LAY,ROW,COL  '),/1X,60('-'))
