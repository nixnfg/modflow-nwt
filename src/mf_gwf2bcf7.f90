...
SUBROUTINE GWF2BCF7AR(IN,IGRID)
   !     ******************************************************************
   !     ALLOCATE ARRAYS AND READ DATA FOR BLOCK-CENTERED FLOW PACKAGE
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !     ------------------------------------------------------------------
   !1------ALLOCATE SCALAR VARIABLES IN FORTRAN MODULE.
...
1  FORMAT(1X,/1X,'BCF -- BLOCK-CENTERED FLOW PACKAGE, VERSION 7',&
   ...
   !
   !3------READ AND PRINT IBCFCB (FLAG FOR PRINTING
   !3------OR UNIT# FOR RECORDING CELL-BY-CELL FLOW TERMS), HDRY
   !3------(HEAD AT CELLS THAT CONVERT TO DRY), AND WETTING PARAMETERS.
   IF(IFREFM.EQ.0) THEN
...
52 FORMAT(1X,5X,'LAYER  LAYER-TYPE CODE     INTERBLOCK T',&
   &/1X,5X,44('-'))
   !
   !5------LOOP THROUGH LAYERS CALCULATING LAYAVG, PRINTING THE LAYER-TYPE
   !5------CODE, AND COUNTING LAYERS THAT NEED TOP & BOT ARRAYS.
   NBOT=0
   NTOP=0
   DO 100 I=1,NLAY
...
56       FORMAT(1X,'INVALID LAYER TYPE:',I4)
      ...
      END IF
   !
   !5A-----SET GLOBAL HEAD-DEPENDENT THICKNESS FLAGS.
      IF (L.EQ.0) THEN
         LAYHDT(I)=0
         LAYHDS(I)=0
      ELSEIF (L.EQ.1) THEN
      ...
      ENDIF
   !
   !5B-----ONLY THE TOP LAYER CAN BE UNCONFINED(LAYCON=1).
      IF(L.NE.1 .OR. I.EQ.1) GO TO 70
      WRITE(IOUT,57)
57    FORMAT(1X,/1X,'LAYER TYPE 1 IS ONLY ALLOWED IN TOP LAYER')
      CALL USTOP(' ')
   !
   !5C-----LAYER TYPES 1 AND 3 NEED A BOTTOM. ADD 1 TO KB.
70    IF(L.EQ.1 .OR. L.EQ.3) NBOT=NBOT+1
   !
   !5D-----LAYER TYPES 2 AND 3 NEED A TOP. ADD 1 TO KT.
...
100 CONTINUE
   ...
   !
   !8------READ ARRAYS FOR EACH LAYER.
   KT=0
   KB=0
   DO 200 K=1,NLAY
      KK=K
   !
   !8A-----FIND ADDRESS OF EACH LAYER IN THREE DIMENSION ARRAYS.
      IF(LAYCON(K).EQ.1 .OR. LAYCON(K).EQ.3) KB=KB+1
      IF(LAYCON(K).EQ.2 .OR. LAYCON(K).EQ.3) KT=KT+1
   !
   !8B-----READ PRIMARY STORAGE COEFFICIENT INTO ARRAY SC1 IF TRANSIENT.
      IF(ISS.EQ.0)CALL U2DREL(SC1(:,:,K),ANAME(1),NROW,NCOL,KK,IN,IOUT)
   !
   !8C-----READ TRANSMISSIVITY INTO ARRAY CC IF LAYER TYPE IS 0 OR 2.
      IF(LAYCON(K).EQ.3 .OR. LAYCON(K).EQ.1) GO TO 105
      CALL U2DREL(CC(:,:,K),ANAME(2),NROW,NCOL,KK,IN,IOUT)
      GO TO 110
   !
   !8D-----READ HYDRAULIC CONDUCTIVITY(HY) IF LAYER TYPE IS 1 OR 3.
105   CALL U2DREL(HY(:,:,KB),ANAME(3),NROW,NCOL,KK,IN,IOUT)
   !
   !8E-----READ VERTICAL HYCOND/THICK INTO ARRAY CV IF NOT BOTTOM LAYER;
   !2E-----MULTIPLIED BY CELL AREA TO CONVERT TO CONDUCTANCE LATER.
110   IF(K.EQ.NLAY) GO TO 120
      CALL U2DREL(CV(:,:,K),ANAME(4),NROW,NCOL,KK,IN,IOUT)
   !
   !8F-----READ SECONDARY STORAGE COEFFICIENT INTO ARRAY SC2 IF TRANSIENT
   !8F-----AND LAYER TYPE IS 2 OR 3.
120   IF(LAYCON(K).NE.3 .AND. LAYCON(K).NE.2) GO TO 130
      IF(ISS.EQ.0)CALL U2DREL(SC2(:,:,KT),ANAME(5),NROW,NCOL,KK,IN,IOUT)
   !
   !8H-----READ WETDRY CODES IF LAYER TYPE IS 1 OR 3 AND WETTING
   !8H-----CAPABILITY HAS BEEN INVOKED (IWDFLG NOT 0).
...
SUBROUTINE GWF2BCF7AD(KPER,IGRID)
   ...
   ISS=ISSFLG(KPER)
   !
   !1------RETURN IF STEADY STATE OR IF NOT USING WETTING CAPABILITY
...
SUBROUTINE GWF2BCF7FM(KITER,KSTP,KPER,IGRID)
   ...
   &LBOTM,CV,HNEW,RHS,HCOF,HOLD,ISSFLG
   ...
   !
   !1------FOR EACH LAYER: IF T VARIES CALCULATE HORIZONTAL CONDUCTANCES
   DO 100 K=1,NLAY
      KK=K
   !
   !1A-----IF LAYER TYPE IS NOT 1 OR 3 THEN SKIP THIS LAYER.
      IF(LAYCON(K).NE.3 .AND. LAYCON(K).NE.1) GO TO 100
      KB=KB+1
   !
   !1B-----FOR LAYER TYPES 1 & 3 CALL SGWF2BCF7H TO CALCULATE
   !1B-----HORIZONTAL CONDUCTANCES.
      CALL SGWF2BCF7H(KK,KB,KITER,KSTP,KPER)
100 CONTINUE
   !
   !2------IF THE SIMULATION IS TRANSIENT ADD STORAGE TO HCOF AND RHS
...
150   KT=KT+1
   ...
   !5A-----IF THE CELL IS EXTERNAL THEN SKIP IT.
            ...
            RHO1=SC1(J,I,K)*TLED
   !
   !5B-----FIND STORAGE FACTOR AT START OF TIME STEP.
            SOLD=RHO2
...
250   IF(K.EQ.NLAY) GO TO 300
   ...
   !8C-----TERM TO RHS.
            ...
            &- CV(J,I,K)*(BOTM(J,I,LBOTM(K+1)-1)-HTMP)
280   CONTINUE
300 CONTINUE
   !
   !9------RETURN
...
500 CONTINUE
   ...
   IF(IBD.EQ.1)&
   &CALL UBUDSV(KSTP,KPER,TEXT(2),IBCFCB,BUFF,NCOL,NROW,NLAY,IOUT)
...
600 CONTINUE
   ...
   IF(IBD.EQ.1)&
   &CALL UBUDSV(KSTP,KPER,TEXT(3),IBCFCB,BUFF,NCOL,NROW,NLAY,IOUT)
...
SUBROUTINE SGWF2BCF7C(K)
   ...
   DO 40 I=1,NROW
      DO 40 J=1,NCOL
         T1=CC(J,I,K)
   !
   !2------IF T=0 THEN SET CONDUCTANCE EQUAL TO 0. GO ON TO NEXT CELL.
         IF(T1.NE.ZERO) GO TO 10
         CR(J,I,K)=ZERO
         GO TO 40
   !
   !3------IF THIS IS NOT THE LAST COLUMN(RIGHTMOST) THEN CALCULATE
   !3------BRANCH CONDUCTANCE IN THE ROW DIRECTION (CR) TO THE RIGHT.
10       IF(J.EQ.NCOL) GO TO 30
         T2=CC(J+1,I,K)
         CR(J,I,K)=TWO*T2*T1*DELC(I)/(T1*DELR(J+1)+T2*DELR(J))
   !
   !4------IF THIS IS NOT THE LAST ROW(FRONTMOST) THEN CALCULATE
   !4------BRANCH CONDUCTANCE IN THE COLUMN DIRECTION (CC) TO THE FRONT.
...
SUBROUTINE SGWF2BCF7H(K,KB,KITER,KSTP,KPER)
   ...
   DO 200 I=1,NROW
      DO 200 J=1,NCOL
   !
   !2------IF CELL IS ACTIVE, THEN SKIP TO CODE THAT CALCULATES SATURATED
   !2------THICKNESS.
         IF(IBOUND(J,I,K).NE.0) GO TO 20
   !
   !3------DETERMINE IF THE CELL CAN CONVERT BETWEEN CONFINED AND
   !3------UNCONFINED.  IF NOT, SKIP TO CODE THAT SETS TRANSMISSIVITY TO 0.
   ...
   !
   !3A-----CHECK HEAD IN CELL BELOW TO SEE IF WETTING THRESHOLD HAS BEEN
   !3A-----REACHED.
         IF(K.EQ.NLAY)GO TO 2
         HTMP=HNEW(J,I,K+1)
         IF(IBOUND(J,I,K+1).GT.0.AND.HTMP.GE.TURNON)GO TO 9
   !
   !3B-----CHECK HEAD IN ADJACENT HORIZONTAL CELLS TO SEE IF WETTING
   !3B-----THRESHOLD HAS BEEN REACHED.
2        IF(WETDRY(J,I,KB).LT.ZERO) GO TO 6
         IF(J.EQ.1)GO TO 3
         HTMP=HNEW(J-1,I,K)
         IF(IBOUND(J-1,I,K).GT.0.AND.IBOUND(J-1,I,K).NE.30000.AND.&
         &HTMP.GE.TURNON)GO TO 9
...
5        IF(I.EQ.NROW)GO TO 6
   ...
   !
   !3C-----CELL IS DRY AND STAYS DRY.  SET TRANSMISSIVITY TO 0, SET
   !3C-----SATURATED THICKNESS (BUFF) TO 0, AND SKIP TO THE NEXT CELL.
6        CC(J,I,K)=ZERO
         IF(LAYAVG(K).EQ.30) BUFF(J,I,K)=ZERO
         GO TO 200
   !
   !4------CELL BECOMES WET.  SET INITIAL HEAD AND VERTICAL CONDUCTANCE.
9        IF(IHDWET.NE.0) HNEW(J,I,K)=BOTM(J,I,LBOTM(K))+WETFCT*WD
         IF(IHDWET.EQ.0) HNEW(J,I,K)=BOTM(J,I,LBOTM(K))+WETFCT*&
         &(HTMP-BOTM(J,I,LBOTM(K)))
...
14       IBOUND(J,I,K)=30000
   !
   !4A-----PRINT MESSAGE SAYING CELL HAS BEEN CONVERTED TO WET.
         NCNVRT=NCNVRT+1
         ICNVRT(NCNVRT)=I
         JCNVRT(NCNVRT)=J
         ACNVRT(NCNVRT)='WET'
...
18       FORMAT(1X,3X,5(A,'(',I3,',',I3,')   '))
         NCNVRT=0
   !
   !5------CALCULATE SATURATED THICKNESS.
20       HD=HNEW(J,I,K)
...
50       THCK=HD-BBOT
   !
   !6------CHECK TO SEE IF SATURATED THICKNESS IS GREATER THAN ZERO.
         IF(THCK.LE.ZERO) GO TO 100
   !
   !6A-----IF SATURATED THICKNESS>0 THEN EITHER CALCULATE TRANSMISSIVITY
   !6A-----AS HYDRAULIC CONDUCTIVITY TIMES SATURATED THICKNESS OR STORE
   !6A-----K IN CC AND SATURATED THICKNESS IN BUFF.
         ...
         GO TO 200
   !
   !6B-----WHEN SATURATED THICKNESS < 0, PRINT A MESSAGE AND SET
   !6B-----TRANSMISSIVITY, IBOUND, AND VERTICAL CONDUCTANCE =0
100      NCNVRT=NCNVRT+1
         ICNVRT(NCNVRT)=I
         JCNVRT(NCNVRT)=J
         ACNVRT(NCNVRT)='DRY'
...
200 CONTINUE
   !
   !7------PRINT ANY REMAINING CELL CONVERSIONS NOT YET PRINTED
   ...
   NCNVRT=0
   !
   !8------CHANGE IBOUND VALUE FOR CELLS THAT CONVERTED TO WET THIS
   !8------ITERATION FROM 30000 to 1.
...
SUBROUTINE SGWF2BCF7N(ISS)
   ...
   !     ------------------------------------------------------------------
   !
   !1------MULTIPLY VERTICAL LEAKANCE BY AREA TO MAKE CONDUCTANCE.
   ZERO=0.
...
10 CONTINUE
   !
   !2------IF WETTING CAPABILITY IS ACTIVATED, SAVE CV IN CVWD FOR USE WHEN
   !2------WETTING CELLS.
...
15 CONTINUE
   !
   !3------IF IBOUND=0, SET CV=0 AND CC=0.
20 DO 30 K=1,NLAY
      DO 30 I=1,NROW
         DO 30 J=1,NCOL
...
30 CONTINUE
   !
   !4------INSURE THAT EACH ACTIVE CELL HAS AT LEAST ONE NON-ZERO
   !4------TRANSMISSIVE PROPERTY.
   HCNV=HNOFLO
   KB=0
   DO 60 K=1,NLAY
      IF(LAYCON(K).EQ.1 .OR. LAYCON(K).EQ.3) GO TO 50
   !
   !4A-----WHEN LAYER TYPE IS 0 OR 2, TRANSMISSIVITY OR CV MUST BE NONZERO.
      DO 45 I=1,NROW
         DO 45 J=1,NCOL
...
43          FORMAT(1X,'NODE (LAYER,ROW,COL)',3I4,&
            &' ELIMINATED BECAUSE ALL CONDUCTANCES TO NODE ARE 0')
45    CONTINUE
      GO TO 60
   !
   !4B-----WHEN LAYER TYPE IS 1 OR 3, HY OR CV MUST BE NONZERO.
50    KB=KB+1
      DO 59 I=1,NROW
         DO 59 J=1,NCOL
   !
   !4B1----IF WETTING CAPABILITY IS ACTIVE, CHECK CVWD.
...
51          IF(K.EQ.1) GO TO 57
            ...
            GO TO 57
   !
   !4B2----WETTING CAPABILITY IS INACTIVE, SO CHECK CV AT ACTIVE CELLS.
...
56          IF(K.EQ.1) GO TO 57
   ...
   !
   !4B3----CHECK HYDRAULIC CONDUCTIVITY.
57          IF(HY(J,I,KB).NE.ZERO) GO TO 59
   !
   !4B4----HY AND CV ARE ALL 0, SO CONVERT CELL TO NO FLOW.
            IBOUND(J,I,K)=0
            HNEW(J,I,K)=HCNV
...
70 CONTINUE
   !
   !6------IF TRANSIENT, LOOP THROUGH LAYERS AND CALCULATE STORAGE
   !6------CAPACITY.
   IF(ISS.NE.0) GO TO 100
   KT=0
   DO 90 K=1,NLAY
   !
   !6A-----MULTIPLY PRIMARY STORAGE COEFFICIENT BY DELR & DELC TO GET
   !6A-----PRIMARY STORAGE CAPACITY.
      DO 80 I=1,NROW
         DO 80 J=1,NCOL
            SC1(J,I,K)=SC1(J,I,K)*DELR(J)*DELC(I)
80    CONTINUE
   !
   !6B-----IF LAYER IS CONF/UNCONF MULTIPLY SECONDARY STORAGE COEFFICIENT
   !6B-----BY DELR AND DELC TO GET SECONDARY STORAGE CAPACITY(SC2).
...
10       IF(J.EQ.NCOL) GO TO 30
         T2=CC(J+1,I,K)
   !3A-----ARITHMETIC MEAN INTERBLOCK TRANSMISSIVITY
         IF(T2.EQ.ZERO) THEN
            CR(J,I,K)=ZERO
         ELSE
...
10       IF(J.EQ.NCOL) GO TO 30
         T2=CC(J+1,I,K)
         IF(T2.EQ.ZERO) THEN
   !3A-----SET TO ZERO AND EXIT IF T2 IS ZERO
            CR(J,I,K)=ZERO
            GO TO 30
         END IF
   !3B-----LOGARITHMIC MEAN INTERBLOCK TRANSMISSIVITY
         RATIO=T2/T1
...
SUBROUTINE SGWF2BCF7U(K)
   !     ******************************************************************
   !-------COMPUTE CONDUCTANCE USING ARITHMETIC MEAN SATURATED THICKNESS
   !-------AND LOGARITHMIC MEAN HYDRAULIC CONDUCTIVITY
   !-------NODE HYDRAULIC CONDUCTIVITY IS IN CC,
   !-------NODE SATURATED THICKNESS IS IN BUFF
   !-------ACTIVATED BY LAYAVG=30
   !     ******************************************************************
   !
   !      SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
10       IF(J.EQ.NCOL) GO TO 30
   ...
   !3A-----SET TO ZERO AND EXIT IF T2 IS ZERO
            ...
            GO TO 30
         END IF
   !3B-----LOGARITHMIC MEAN HYDRAULIC CONDUCTIVITY
         RATIO=T2/T1
         IF(RATIO.GT.FRAC1.OR.RATIO.LT.FRAC2) THEN
            T=(T2-T1)/LOG(RATIO)
         ELSE
            T=HALF*(T1+T2)
         END IF
   !3C-----MULTIPLY LOGARITHMIC K BY ARITHMETIC SAT THICK
