...
SUBROUTINE GWF2DRT7AR(IN,IGRID)
   !     ******************************************************************
   !     ALLOCATE ARRAY STORAGE AND READ PARAMETERS FOR DRAINS AND
   !     RETURN FLOWS
   !     ******************************************************************
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !
   !1------IDENTIFY PACKAGE AND INITIALIZE NDRTCL.
   WRITE(IOUT,1)IN
1  FORMAT(1X,/&
...
14    FORMAT(1X,'LISTS OF DRAIN-RETURN CELLS WILL NOT BE PRINTED')
   ...
   !
   !4------ALLOCATE SPACE FOR THE DRTF ARRAY.
   IDRTPB=MXADRT+1
   MXDRT=MXADRT+MXL
...
500 FORMAT(1X,//1X,I5,' Drain-return parameters')
   ...
   !5B-----ASSIGN STARTING INDEX FOR READING INSTANCES
...
SUBROUTINE GWF2DRT7RP(IN,IGRID)
   !     ******************************************************************
   !     READ DRAIN HEAD, CONDUCTANCE AND BOTTOM ELEVATION.  IF THE
   !     RETURNFLOW OPTION IS SELECTED, READ RECIPIENT CELL AND PROPORTION.
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   ENDIF
   !
   !2------DETERMINE THE NUMBER OF NON-PARAMETER DRAIN-RETURN CELLS.
   IF (ITMP.LT.0) THEN
      WRITE(IOUT,7)
7     FORMAT(1X,/,&
   ...
   ENDIF
   !
   !3------IF THERE ARE NEW NON-PARAMETER DRAIN-RETURN CELLS, READ THEM.
   MXADRT=IDRTPB-1
   IF (ITMP.GT.0) THEN
...
500      FORMAT(1X,/1X,'THE NUMBER OF ACTIVE DRT DRAINS (',I6,&
   ...
   NDRTCL=NDRTNP
   !
   !1C-----IF THERE ARE ACTIVE DRT PARAMETERS, READ THEM AND SUBSTITUTE
   CALL PRESET('DRT')
   IF (NP.GT.0) THEN
      NREAD=NDRTVL-2
      DO 30 N=1,NP
...
30    CONTINUE
   ENDIF
   !
   !     COUNT NUMBER OF DRAIN-RETURN CELLS THAT CAN HAVE RETURN FLOW
   NRFLOW = 0
   IF (IDRTFL.GT.0) THEN
      DO 40 I=1,NDRTCL
...
40    CONTINUE
   ENDIF
   !
   !3------PRINT NUMBER OF DRAIN-RETURN CELLS IN CURRENT STRESS PERIOD.
...
SUBROUTINE GWF2DRT7FM(IGRID)
   !     ******************************************************************
   !     ADD DRAIN-RETURN FLOW TO SOURCE TERMS FOR BOTH DRAIN-RETURN CELLS
   !     AND RECIPIENT CELLS
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   ...
   CALL SGWF2DRT7PNT(IGRID)
   !
   !1------IF NDRTCL<=0 THERE ARE NO DRAINS. RETURN.
   IF (NDRTCL.LE.0) RETURN
   !
   !2------PROCESS EACH CELL IN THE DRAIN-RETURN CELL LIST.
   DO 100 L=1,NDRTCL
   !
   !3------GET COLUMN, ROW AND LAYER OF CELL CONTAINING DRAIN.
      IL=DRTF(1,L)
      IR=DRTF(2,L)
      IC=DRTF(3,L)
   !
   ...
   !
   !5-------IF THE CELL IS INTERNAL GET THE DRAIN DATA.
      EL=DRTF(4,L)
      EEL=EL
   !
   !6------IF HEAD IS LOWER THAN DRAIN THEN SKIP THIS CELL.
      IF (HNEW(IC,IR,IL).LE.EEL) GOTO 100
   !
   !7------HEAD IS HIGHER THAN DRAIN. ADD TERMS TO RHS AND HCOF.
      C=DRTF(5,L)
...
SUBROUTINE GWF2DRT7BD(KSTP,KPER,IGRID)
   !     ******************************************************************
   !     CALCULATE VOLUMETRIC BUDGET FOR DRAIN-RETURN CELLS
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
30 CONTINUE
   !
   !4------IF THERE ARE NO DRAIN-RETURN CELLS THEN DO NOT ACCUMULATE FLOW.
   IF (NDRTCL.LE.0) GOTO 200
   !
   !5------LOOP THROUGH EACH DRAIN-RETURN CELL, CALCULATING FLOW.
   DO 100 L=1,NDRTCL
   !
   !5A-----GET LAYER, ROW & COLUMN OF CELL CONTAINING DRAIN.
      IL=DRTF(1,L)
      IR=DRTF(2,L)
      IC=DRTF(3,L)
      Q=ZERO
      ...
      END IF
   !
   !5B-----IF CELL IS NO-FLOW OR CONSTANT-HEAD, IGNORE IT.
      IF (IBOUND(IC,IR,IL).LE.0) GOTO 99
   !
   !5C-----GET DRAIN PARAMETERS FROM DRAIN-RETURN LIST.
      EL=DRTF(4,L)
      EEL=EL
      C=DRTF(5,L)
      HHNEW=HNEW(IC,IR,IL)
   !
   !5D-----IF HEAD HIGHER THAN DRAIN, CALCULATE Q=C*(EL-HHNEW).
   !5D-----SUBTRACT Q FROM RATOUT.
      ...
      ENDIF
   !
   !5E-----PRINT THE INDIVIDUAL RATES IF REQUESTED(IDRTCB<0).
      IF (IBD.LT.0) THEN
...
550         FORMAT(1X,'DRAIN ',I6,' RETURN:  LAYER ',I3,'   ROW ',I5,&
   ...
   !
   !5G-----IF SAVING CELL-BY-CELL FLOWS IN A LIST, WRITE FLOW.  OR IF
   !5G-----RETURNING THE FLOW IN THE DRTF ARRAY, COPY FLOW TO DRTF.
99    IF (IBD.EQ.2) THEN
...
&NCOL,NROW,NLAY,ITERP,IDRTFL)
   !     ******************************************************************
   !     Read and print a list of drain and optional associated
   ...
   !
   !  Check for and decode EXTERNAL and SFAC records.
   READ(IN,'(A)') LINE
   SFAC=1.
   LLOC=1
...
550      FORMAT(1X,'(THE SCALE FACTOR WAS APPLIED TO FIELDS ',&
   ...
   N=NLIST+LSTBEG-1
   DO 100 II=LSTBEG,N
   !  Read a line into the buffer.  (The first line has already been read
   !  in order to scan for EXTERNAL and SFAC records.)
      IF (II.NE.LSTBEG) READ(IN,'(A)') LINE
   !
   !  Read the non-optional values from the line.
      IF (IDRTFL.EQ.0) THEN
         IF (IFREFM.EQ.0) THEN
...
50    CONTINUE
   !
   !  Read the optional values from the line
      IF (NAUX.GT.0) THEN
...
60       CONTINUE
      ENDIF
   !
   !  Write the values that were read and that are not related to
   !  return flow.
      NN=II-LSTBEG+1
      IF (IDRTFL.EQ.0) THEN
...
100 CONTINUE
   !
   !     Check and write data related to return-flow recipient cells
   IF (IDRTFL.GT.0) THEN
...
600      FORMAT(1X,I6,3I7,3I7,2X,F8.6)
   ...
   !  Check for illegal grid location
         IF (KR.NE.0) THEN
            ...
            ENDIF
   !
   !  Check for invalid return-flow proportion
...
590            FORMAT(' ERROR: Proportion must be between 0.0 and 1.0')
         ...
         ENDIF
   !
   !  If the proportion = 0 or KR = 0, set all indices and proportion to 0
...
&NDRTCL,DRTAUX,NCAUX,NAUX,IDRTFL)
   ...
   !     ******************************************************************
   !     Modified 11/8/2001 to support parameter instances - ERB
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   USE PARAMMODULE
...
600 FORMAT(1X,I6,3I7,3I7,2X,F8.6)
   ...
   DO 100 IP=1,MXPAR
      ...
      IF(CTMP1.EQ.CTMP2) THEN
         ...
         ENDIF
   !
   !         DESIGNATE CELLS CORRESPONDING TO CORRECT PARAMETER INSTANCE
...
120 CONTINUE
   !
   !     WRITE DATA RELATED TO RETURN-FLOW RECIPIENT CELLS
