   !  The hydrograph lables are established by the SE subroutines the
   !  first time they are called.  SE is called the first time by the
   !  AR subroutine for the BAS, IBS, and SUB Packages.  SE is called
   !  the first time by the RP subroutine for the STR and SFR Packages.
   !  Subsequent calls to SE subroutines in the Main must be made in
   !  the same order as the initial calls.  A HYDMOD package that
   !  has an RP subroutine will always have the initial SE call after
   !  the initial SE calls for all of the packages that have the
   !  inital SE call in the AR subroutine.
MODULE HYDBASMODULE
...
SUBROUTINE GWF2HYD7BAS7AR(IN,IGRID)
   !     ******************************************************************
   !     ALLOCATE ARRAY STORAGE FOR HYDROGRAPH DATA FOR BAS PACKAGE
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
1  FORMAT(1X,/1X,'HYD -- HYDROGRAPH DATA FOR BAS PACKAGE,',&
   &' VERSION 7, 07/14/2006',/&
   &1X,'        INPUT READ FROM UNIT',I3)
   !
   !4------READ NUMBER OF HYDROGRAPHS AND UNIT FOR SAVING UNFORMATTED
   !4------HYDROGRAPH FILE AND NUMERIC FLAG FOR DRY/INACTIVE CELLS
   READ(IN,'(A)') LINE
   LLOC=1
   !  Number of hydrographs (NHYDM) specified by the user is ignored --
   !    the program initially counts the number of hydrographs (NTOT).
   !    Note that there may be less than NHTOT hydrograps because some
   !    may be eliminated due to invalid values.
...
5  FORMAT(1X,'HYDROGRAPH VALUES WILL BE SAVED ON UNIT:',I4,&
   &/,1X,'HYDROGRAPH VALUES AT DRY CELLS WILL BE:',1PG14.5)
   !
   !4------COUNT NUMBER OF BAS PACKAGE AND OVERALL HYDROGRAPHS.
   NTOT=0
   NHYDBAS=0
   REWIND(IN)
...
10 READ(IN,'(A)',END=19) LINE
   ...
   GO TO 10
   !
   !  ALLOCATE MEMORY FOR ALL TYPES OF HYDROGRAPHS
19 IF(NTOT.GT.0) THEN
      ALLOCATE(HYDVAL(NTOT,2))
      ALLOCATE(HYDLBL(NTOT))
      WRITE(IOUT,17)
17    FORMAT(1X,/1X,&
      &'ARR INTYP  KLAY     XL            YL         HYDLBL',&
      &/1X,64('-'))
   ELSE
      ALLOCATE(HYDVAL(1,2))
      ALLOCATE(HYDLBL(1))
   END IF
   !
   !  ALLOCATE MEMORY FOR BAS HYDROGRAPH DATA
...
23 FORMAT(1X,A,1X,A,I7,1PE14.5,E14.5,2X,A)
   ...
   !
   ! Determine if drawdown
   ELSE IF(HYDBASARR(NHYDBAS).EQ.'DD') THEN
      IBHYDBAS(NHYDBAS)=.TRUE.
   !
   !  Not head or drawdown, so error.
   ELSE
      WRITE(IOUT,25) LINE
25    FORMAT(' Invalid array type was found on the following',&
   ...
   ENDIF
   !
   !  Find the grid coodrdinates for the cell.
   CALL SGWF2HYD7GRDLOC(XL,YL,NR1,NC1,NR2,NC2,X1,X2,Y1,Y2)
   !
   !  Check if interpolating between nodes.
   IF(INTYP.EQ.'C') THEN
   !
   !  Do not interpolate
...
27    FORMAT(' Invalid interpolation type was found on the ',&
   ...
   ENDIF
   !
   !  If computing drawdown, save the starting head
   ...
   ENDIF
   !
   !  Save the hydrograph label and continue with the next record.
   HYDLBL(NHYDBAS)=HYDBASLBL
   GO TO 20
   !
   !  End of file after all BAS HYDROGRAPH data have been processed
   !  Note that NHYDTOT is accumulated by each package from this point on.
   !  NHYDTOT is the total number of valid hydrographs after data are
   !  checked for errors.
99 NHYDTOT=NHYDBAS
   !
...
SUBROUTINE GWF2HYD7IBS7AR(IN,IGRID)
   !     ******************************************************************
   !     READ IBS PACKAGE DATA FOR HYDROGRAPHS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
10 READ(IN,'(A)',END=19) LINE
   ...
   GO TO 10
   !
   !  ALLOCATE MEMORY FOR IBS HYDROGRAPH DATA
...
15 READ(IN,'(A)',END=99) LINE
   ...
   !c    TIME SERIES FROM THE CRITICAL HEAD ARRAY
   IF (ARR.EQ.'HC') THEN
      HYDIBSARR(NHYDIBS)='HC'
      IBHYDIBS(NHYDIBS)=.TRUE.
   !c    TIME SERIES FROM THE COMPACTION ARRAY
   ELSE IF (ARR.EQ.'CP') THEN
      HYDIBSARR(NHYDIBS)='CP'
      IBHYDIBS(NHYDIBS)=.TRUE.
   !c    TIME SERIES ACCUMULATED OVER ALL LAYERS OF THE COMPACTION ARRAY
   ELSE IF (ARR.EQ.'SB') THEN
      HYDIBSARR(NHYDIBS)='SB'
      IBHYDIBS(NHYDIBS)=.FALSE.
   !  Change the layer number to the number of subsidence layers
      NQ=0
      DO 20 K=1,KLAY
...
23 FORMAT(1X,A,1X,A,I7,1PE14.5,E14.5,2X,A)
   ...
   CALL SGWF2HYD7GRDLOC(XL,YL,NR1,NC1,NR2,NC2,X1,X2,Y1,Y2)
   IF(INTYP.EQ.'C') THEN
   !  Use cell value without interpolation
...
26       FORMAT(' Coordinates of the following record are ',&
   ...
   ELSE IF(INTYP.EQ.'I') THEN
   !  Interpolate value between nodes
...
27    FORMAT(' Invalid interpolation type was found on the ',&
   ...
   ENDIF
   !
   !  Save the hydrograph label and process the next line in input file
   HYDLBL(NHYDTOT+NHYDIBS)=HYDIBSLBL
   GO TO 15
   !
   !  End of file after all IBS hydrograph data have been processed
...
SUBROUTINE GWF2HYD7SUB7AR(IN,IGRID)
   !     ******************************************************************
   !     READ SUB PACKAGE DATA FOR HYDROGRAPHS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
10 READ(IN,'(A)',END=19) LINE
   ...
   GO TO 10
   !
   !  ALLOCATE MEMORY FOR SUB HYDROGRAPH DATA
...
27    FORMAT(' Invalid interpolation type was found on the ',&
   ...
   GO TO 15
   !
   !  End of file after all SUB hydrograph data have been processed
...
SUBROUTINE GWF2HYD7STR7AR(IN,IGRID)
   !     ******************************************************************
   !     READ STREAM PACKAGE DATA FOR HYDROGRAPHS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !     ------------------------------------------------------------------
   ALLOCATE (NHYDSTR)
   !  Count number of Stream hydrographs
   NHYDSTR=0
   REWIND(IN)
...
SUBROUTINE GWF2HYD7STR7RP(IN,KPER,IGRID)
   !     ******************************************************************
   !     READ HYDROGRAPH RECORDS FOR STREAMS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
8     FORMAT(' No Active Streams in this Model')
      WRITE(IOUT,'(1X,A)')&
      &'Stream hydrograph records will be ignored.'
      NHYDSTR=0
      RETURN
   END IF
   !
   !
   ! ------Read STR hydrograph data
   ! ------Reading is done here (rather than in AR) because stream data are
   ! ------not available until the first time step is initiated.
   ! ---- Reading could be done in a special RP routine right after STRRP
   IF(KPER.EQ.1) THEN
      NUMSTR=0
      REWIND(IN)
      READ(IN,'(A)',END=99) LINE
20    READ(IN,'(A)',END=99) LINE
   ...
   !
   ! ------PROCESS A STR HYDROGRAPH RECORD
      NUMSTR=NUMSTR+1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,IOUT,IN)
   ...
   !
      ...
      &HYDSTRARR(NUMSTR).NE.'SO' .AND.&
      &HYDSTRARR(NUMSTR).NE.'SI' .AND.&
      &HYDSTRARR(NUMSTR).NE.'SA') THEN
         WRITE(IOUT,25) LINE
25       FORMAT(' Invalid streamflow array was found on the following'&
      ...
      ENDIF
   !
   ! ------Look through all reaches to find the hydrograph reach.
      DO 30 N=1,NSTREM
26       ISEG=ISTRM(4,N)
         IRCH=ISTRM(5,N)
   !    XL contains the SEGMENT number and YL contains the REACH number.
...
30    CONTINUE
      WRITE(IOUT,*)&
      &' Hydrograph specified for non-existent stream reach'
      WRITE(IOUT,*) 'Hydrograph Record will be ignored.'
      NUMSTR=NUMSTR-1
      GO TO 20
   !
   ! ------The interpolation type must be 'C'
...
39       FORMAT(' Invalid interpolation type was found on the ',&
      ...
      ENDIF
   !
   ! ------SAVE THE LABEL AND PROCESS NEXT RECORD.
...
100   FORMAT(' A total of ',I3,' hydrographs have been added ',&
      ...
      NHYDSTR=NUMSTR
   !
   !  Create initial values for stream hydrographs
...
SUBROUTINE GWF2HYD7SFR7AR(IN,IGRID)
   !     ******************************************************************
   !     READ SFR2 PACKAGE DATA FOR HYDROGRAPHS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
8     FORMAT(' No Active Streams in this Model')
   ...
   !
   !
   ! ------Read SFR hydrograph data
   ! ------Reading is done here (rather than in AR) because stream data are
   ! ------not available until the first time step is initiated.
   ! ---- Reading could be done in a special RP routine right after SFRRP
   IF(KPER.EQ.1) THEN
      NUMSFR=0
      REWIND(IN)
      READ(IN,'(A)',END=99) LINE
20    READ(IN,'(A)',END=99) LINE
   ...
   !
   ! ------PROCESS A SFR HYDROGRAPH RECORD
      NUMSFR=NUMSFR+1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,IOUT,IN)
   ...
   !
      ...
      &HYDSFRARR(NUMSFR).NE.'SO' .AND.&
      &HYDSFRARR(NUMSFR).NE.'SI' .AND.&
      &HYDSFRARR(NUMSFR).NE.'SA') THEN
         WRITE(IOUT,25) LINE
...
SUBROUTINE GWF2HYD7BAS7SE(IHYDLOC,IGRID)
   !     ******************************************************************
   !     COMPUTE HYDROGRAPH RECORDS FOR BAS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !
   ! -----Calculate value for each hydrograph point.
   DO 50 N=1,NHYDBAS
      NN=N
   ! -----Determine interpolation type, word length, number of weights,
   ! -----and locations of IBOUND codes at or around hydrograph point.
      J=JIKHYDBAS(1,N)
      I=JIKHYDBAS(2,N)
      K=JIKHYDBAS(3,N)
   !
   ! -----If IBOUND is to be checked for inactive cells, retrieve values
   ! -----at or around hydrograph point.
      IF(IBHYDBAS(N)) THEN
         IB1=IBOUND(J,I,K)
         IBFACT=IB1
      ...
      ENDIF
   !
   ! -----Check if hydrograph value is HEAD.
      IF(HYDBASARR(N).EQ.'HD') THEN
         ...
         ENDIF
   !
   ! -----Hydrograph value is DRAWDOWN if NOT HEAD
      ELSE
         IF(IBHYDBAS(N) .AND. IBFACT.EQ.0) THEN
            HYDVAL(N,IHYDLOC)=HYDNOH
         ELSE
...
SUBROUTINE GWF2HYD7IBS7SE(IHYDLOC,IGRID)
   !     ******************************************************************
   !     COMPUTE HYDROGRAPH RECORDS FOR IBS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   ! -----at or around hydrograph point.
      ...
      ENDIF
   !
   ! -----Check if hydrograph value is Preconsolidation Head.
      IF(HYDIBSARR(N).EQ.'HC') THEN
         ...
         ELSE
            ...
            &HC(J,I,K)*W1+HC(J+1,I,K)*W2+HC(J+1,I-1,K)*W3+HC(J,I-1,K)*W4
         ENDIF
   !
   ! -----Check if hydrograph value is compaction
      ELSE IF(HYDIBSARR(N).EQ.'CP') THEN
         ...
         ELSE
            ...
            W4=HYDIBSWT(4,N)
            HYDVAL(NHYDTOT+N,IHYDLOC)=&
            &SUB(J,I,K)*W1+SUB(J+1,I,K)*W2+SUB(J+1,I-1,K)*W3+SUB(J,I-1,K)*W4
         ENDIF
   !
   ! -----Hydrograph value must be subsidence (not HC and not CP)
      ELSE
         KLAY=JIKHYDIBS(3,N)
         TOTL=0.
         DO 40 K=1,KLAY
...
SUBROUTINE GWF2HYD7SUB7SE(IHYDLOC,IGRID)
   !     ******************************************************************
   !     COMPUTE HYDROGRAPH RECORDS FOR SUB
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   ! -----Check if hydrograph value is Preconsolidation Head.
      IF(HYDSUBARR(N).EQ.'HC') THEN
         ...
         ELSE
            ...
            HYDVAL(NHYDTOT+N,IHYDLOC)=&
            &HC(icell1)*W1+HC(icell2)*W2+HC(icell3)*W3+HC(icell4)*W4
         ENDIF
   !
   ! -----Check if hydrograph value is compaction
...
SUBROUTINE GWF2HYD7STR7SE(IHYDLOC,IGRID)
   !     ******************************************************************
   !     COMPUTE HYDROGRAPH RECORDS FOR STREAMS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   CALL SGWF2HYD7STR7PNT(IGRID)
   !
   ! ------Return if no STR hydrographs.
   IF(NHYDSTR.LE.0) RETURN
   !
   ! ------Calculate value for each stream hydrograph.
   DO 50 N=1,NHYDSTR
      ISTR=ISTRHYD(N)
   !
   ! ------Check for the hydrograph type.
   !c    ARR/IRR designate the streamflow option with
   !c    ST==> stream stage, SO==> out of reach,
   !c    SI==> into reach, and SA==> into aquifer.
...
SUBROUTINE GWF2HYD7BAS7OT(KSTP,KPER,IGRID)
   !     ******************************************************************
   !     WRITE HYDROGRAPH DATA FOR ONE TIME STEP
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !
   !2------IF THIS IS THE FIRST TIME IN THE SIMULATION, WRITE HEADER RECORD.
...
130   FORMAT(1X,'A TOTAL OF ',I3,' HYDROGRAPH POINTS HAVE BEEN ',&
   ...
   ENDIF
   !
   !3------WRITE HYDROGRAPH RECORD FOR ONE TIME STEP.
...
SUBROUTINE SGWF2HYD7GRDLOC(XL,YL,NR1,NC1,NR2,NC2,XX1,XX2,YY1,YY2)
   !     ******************************************************************
   !     LOCATE CELLS FOR HYDROGRAPH POINTS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
FUNCTION SHYD7WTAVG(N)
   !     ******************************************************************
   !     COMPUTE WEIGHTED AVERAGE OF HEAD
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE GWF2HYD7DA(IGRID)
   !     ******************************************************************
   !     SUBROUTINE TO DEALLOCATE ALL HYDMOD VARIABLES
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   &CALL SGWF2HYD7IBS7DA(IGRID)
   IF(IUNIT(43).GT.0 .AND. IUNIT(54).GT.0)&
   &CALL SGWF2HYD7SUB7DA(IGRID)
   IF(IUNIT(43).GT.0 .AND. IUNIT(18).GT.0)&
   &CALL SGWF2HYD7STR7DA(IGRID)
   IF(IUNIT(43).GT.0 .AND. IUNIT(44).GT.0)&
   &CALL SGWF2HYD7SFR7DA(IGRID)
   !3
   RETURN
END
