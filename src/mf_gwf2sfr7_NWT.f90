   ...
   !
   !     ******************************************************************
   !     CHECK FOR STEAMBED BELOW CELL BOTTOM. RECORD REACHES FOR PRINTING
   !     ******************************************************************
...
END MODULE ICHKSTRBOT_MODULE
   !
   !-------SUBROUTINE GWF2SFR7AR
SUBROUTINE GWF2SFR7AR(In, Iunitbcf, Iunitlpf, Iunithuf, Iunitgwt,&
&Nsol, Iouts, Iunitupw, Iunituzf, Igrid)
   !     ******************************************************************
   !     ALLOCATE ARRAY STORAGE FOR STREAMS
   !     INITIALIZE VARIABLES FOR SFR PACKAGES
   !     READ STREAM DATA THAT IS CONSTANT FOR ENTIRE SIMULATION:
   !     REACH DATA AND PARAMETER DEFINITIONS
   ...
   !1------IDENTIFY PACKAGE AND INITIALIZE NSTRM.
   WRITE (IOUT, 9001) In
9001 FORMAT (1X, /, ' SFR7 -- STREAMFLOW ROUTING PACKAGE, '&
   &,'VERSION 1.3.0, 2022-07-01', /, 9X,&
   &'INPUT READ FROM UNIT', I4)
   !
   !2------READ COMMENT RECORDS, NSTRM, NSS, NSFRPAR, NPARSEG, CONST,
   !         DLEAK, ISTCB1, ISTCB2.
   ISFROPT = 0
   IUZT = 0
   IRTFLG = 0
   ...
   SFRUZRECH = 0.0
   !
   !2A------COMMENTS/FIRST LINE.
   CALL URDCOM(In, IOUT, line)
   CALL UPARLSTAL(IN,IOUT,LINE,NPP,MXVL)
   !
   !2B------CHECK FOR KEYWORDS.
   ...
   END IF
   !
   !3------READ ISFROPT FLAGS WHEN NSTRM IS LESS THAN ZERO.
   IF ( IRFG.GT.0 ) THEN
...
9002     FORMAT (//, 'KEYWORD "REACHINPUT" IS SPECIFIED AND NSFRPAR IS ',&
   ...
   !
   !4------READ UNSATURATED FLOW VARIABLES WHEN ISFROPT GREATER THAN 1.
...
9045     FORMAT (//, 'If the BCF Package is used and unsaturated ',&
   ...
   END IF
   !
   !5------CALCULATE SPACE NEEDED FOR TABULATED DISCHARGE VERSUS FLOW
   !         AND WIDTH RELATIONS.
   MAXPTS = 3*50
   !
   !     ******************************************************************
   ...
   !dep  allocate space for stream outflow derivatives for lake package
   ...
   END IF
   !
   !6------PRINT INFORMATION THAT WAS READ.
...
9012 FORMAT (//, ' *** WARNING ***   DLEAK IS LESS THAN OR EQUAL',&
   ...
   IF ( IUZT.EQ.1 ) THEN
   !
   !8------ALLOCATE SPACE FOR UNSATURATED FLOW.
      NUZST = NSTRM
   ...
   ELSE
   !
   !9------ALLOCATE ONLY ONE ARRAY ELEMENT IF UNSATURATED FLOW IS INACTIVE.
      NUZST = 1
      NSTOTRL = 1
      NUMAVE = 1
      ISUZN = 1
   ...
   SFRUZBD = 0.0
   !
   !10-----READ AND PRINT DATA FOR EACH STREAM REACH.
   !IFACE
   ndash=50
   face=' '
   if(iface.eq.1) then
...
9016 FORMAT (1X, //, 3X, 'STREAM NETWORK DESCRIPTION: ', //, 3X,&
   ...
   !
   !11-----READ AND WRITE DATA FOR EACH REACH ON BASIS OF ISFROPT.
   nseg = 0
   nreach = 0
...
9018  FORMAT (5X, '**WARNING** CELL BENEATH STREAM REACH IS INACTIVE',&
   ...
   !
   !12-----CALCULATE RESIDUAL WATER CONTENT FROM SATURATED WATER CONTENT
   !        AND SPECIFIC YIELD WHEN UNSATURATED FLOW IS ACTIVE.
...
9022  FORMAT (3(1X, I5), 1X, I5, 3X, I5, 1X, 5(1X, 1PE11.4),&
   ...
   !
   !13-----CHECK RANGE AND ORDER FOR SEGMENTS AND REACHES.
...
9025  FORMAT (' EACH SEGMENT MUST START WITH REACH 1, AND', /,&
   ...
   !       Number of reaches in segment added to ISEG
      ISEG(4, jseg) = ireach
   ...
   END DO
   !
   !14-----READ SEGMENT INFORMATION FOR FIRST STRESS PERIOD.
   ...
   END IF
   !
   !15-----COMPUTE UNSATURATED VARIABLE WHEN SPECIFIED BY SEGMENT.
   IF ( IUZT.EQ.1 ) THEN
      irch = 1
      ksfropt = 0
      DO nseg = 1, NSS
         ...
         DO ii = 1, ISEG(4, nseg)
            IF ( icalc.EQ.1 .OR. icalc.EQ.2 ) THEN
               ...
               END IF
   !
   !16-----CALCULATE RESIDUAL WATER CONTENT FROM SATURATED WATER CONTENT
   !         AND SPECIFIC YIELD WHEN UNSATURATED FLOW IS ACTIVE.
   ! RGN 5/8/09 Fixed calculation of THTR to include HUF
               ...
               END IF
   !
   !17-----CHECK THAT RESIDUAL WATER CONTENT IS LESS THAN
   !         SATURATED WATER CONTENT.
               IF ( IUZT.EQ.1 ) THEN
                  ...
                  IF ( THTI(irch).GT.THTS(irch) ) THEN
                     ...
                     CALL USTOP(' ')
                  END IF
   !dep  Added check that THTI is greater than THTR.
...
9028 FORMAT (' INITIAL WATER CONTENT IS LESS THAN RESIDUAL ',&
   ...
   !
   !18-----CHECK IF STREAM REACH IS IN ACTIVE CELL.
   kkptflg = 0
   DO ichk = 1, NSTRM
      krck = ISTRM(1, ichk)
      irck = ISTRM(2, ichk)
...
9032 FORMAT (/, ' *** WARNING *** A TOTAL OF', I6, 'STREAM REACHES ',&
   ...
   !
   !20-----ASSIGN STARTING INDEX FOR READING INSTANCES.
         ...
         END IF
   !
   !21-----READ LIST(S) OF CELLS, PRECEDED BY INSTANCE NAME IF NUMINST>0.
   !dep  Revised to change ib loop counter
         lb = lstbeg
         DO i = ib, numinst
...
9033 FORMAT (//)
   !
   !22-----INITIALIZE VARIABLES AND LISTS FOR UNSATURATED FLOW BENEATH STREAM.
   !         NWAVS INITIALLY SET TO 1.
   ...
   WAT1 = 0.0
   !
   !22B-----INITIALIZE VARIABLES FOR STREAM DEPTH, LEAKAGE, AND
   !         PREVIOUS HEAD BENEATH STREAM.
   ...
   !
   !23-----SAVE POINTERS FOR GRID AND RETURN.
...
SUBROUTINE PARSESFROPTIONS(In,Iout,IRFG,NPP,MXVL,TEXT,line)
   !     ******************************************************************
   !     INTERPRET KEYWORDS TO DETERMINE ACTIVE OPTIONS. COMBINES
   !     OLD APPROACH AND NEW APPROACH. FOR NEW APPROACH KEYWORDS
   !     MUST BE SURROUNDED BY "OPTIONS" AND "END"
   !     ******************************************************************
   USE GWFSFRMODULE
...
SUBROUTINE SGWF2SFR7UHC(Iunitlpf, Iunitupw)
   !     ******************************************************************
   !     SETS UNSATURATED VERTICAL HYDRAULIC CONDUCTIVITY TO VERTICAL
   !     HYDRAULIC CONDUCTIVITY IN THE LAYER-PROPERTY FLOW PACKAGE.
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     NEW VERSION NUMBER 1.0.6:  December 5, 2012
   !     ******************************************************************
...
END SUBROUTINE SGWF2SFR7UHC
   !
   !-------SUBROUTINE GWF2SFR7RP
SUBROUTINE GWF2SFR7RP(In, Iunitgwt, Iunitlak, Kkper, Kkstp, Nsol,&
&Iouts, Iunituzf, Igrid)
   !     ******************************************************************
   !     READ STREAM DATA FOR STRESS PERIOD
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     Compute three new tables for lake outflow
   !     ******************************************************************
   USE GWFSFRMODULE
   ...
   IFLG = 0
   !
   !1------READ ITMP FLAG TO REUSE NON-PARAMETER DATA, 2 PRINTING FLAGS,
   !         AND NUMBER OF PARAMETERS BEING USED IN CURRENT STRESS PERIOD.
   iss = ISSFLG(Kkper)
   zero = 1.0E-7
   !dep added NSFRPAR to IF statement
   IF ( Kkper.GT.1 ) THEN
      IF ( NSFRPAR.EQ.0 ) THEN
         READ (In, *) ITMP, IRDFLG, IPTFLG
         NP = 0
   ...
   END IF
   !
   !2------CHECK FOR TOO MANY SEGMENTS.
   IF ( ITMP.GT.NSS ) THEN
      WRITE (IOUT, 9001)
      CALL USTOP(' ')
   END IF
   !
   !5b-----RESET SPECIFIED INFLOWS IN CASE AG PACKAGE NEEDS THEM.
   IF ( KKPER > 1 ) THEN
      DO kss = 1, NSS
   ...
   END IF
   !
   !3------REUSE NON-PARAMETER DATA FROM LAST STRESS PERIOD IF ITMP<0.
   IF ( ITMP.GE.0 ) THEN
   !
   !4------NOT REUSING DATA -- INITIALIZE NSEGCK LIST TO ZERO FOR ALL
   !         SEGMENTS. Moved NSEGCK below ELSE IF 6/9/2005 dep
      IF ( Kkper.GT.1 ) THEN
         DO kss = 1, NSS
            NSEGCK(kss) = 0
         END DO
   ...
   !
   !5------INITIALIZE NSEGCK TO 0 FOR SEGMENTS THAT ARE DEFINED BY
...
9003 FORMAT (/, ' REUSING STREAM SEGMENT DATA FROM LAST STRESS PERIOD')
   !
   !6------READ NON-PARAMETER STREAM SEGMENT DATA.
   IF ( ITMP.GT.0 ) THEN
      lstbeg = 1
      ichk = 1
      IF ( ISFROPT.GT.0 ) THEN
         ...
         &Nsol)
   !rgn 10/16/06 fixed logic for calls to RDSEG
   ...
   END IF
   !
   !7------DEACTIVATE ANY PREVIOUSLY USED STREAM PARAMETERS, AND
   !         ACTIVATE PARAMETERS BEING USED IN CURRENT STRESS PERIOD.
   ...
   END IF
   !
   !8------CHECK FOR ERRORS IN SEGMENT DATA.
   IF ( ITMP.GT.0 .OR. NSFRPAR.NE.0 ) THEN
      DO nseg = 1, NSS
         ...
         END IF
   !
   !9------READ DATA ACCORDING TO VARIABLE ISFROPT.
         isoptflg = 0
...
9010  FORMAT (/, 5X, '*** WARNING *** IPRIOR = -2 FOR NSEG =', I7,&
   ...
   !
   !10-----PLACE STREAM SEGMENT IDENTITY NUMBERS IN ISEG ARRAY.
   !         5 ASSIGNED TO SEGMENTS NOT RECEIVING TRIBUTARY FLOW.
   !         6 ASSINGED TO SEGMENTS THAT DIVERT FLOW.
   !         7 ASSIGNED TO SEGMENTS RECEIVING TRIBUTARY FLOW.
      k5 = 0
      k6 = 0
      k7 = 0
      DO nseg = 1, NSS
         ...
         END IF
   !
   !11-----IDENTIFY SEGMENTS THAT DIVERT FLOW.
         IF ( IDIVAR(1, nseg).NE.0 ) THEN
            ISEG(3, nseg) = 6
            k6 = k6 + 1
   !
   !12-----IDENTIFY SEGMENTS THAT DO NOT DIVERT FLOW.
         ELSE
            jj = 0
   !
   !13-----IDENTIFY SEGMENTS THAT RECEIVE TRIBUTARY FLOW.
            DO ii = 1, NSS
               IF ( IOTSG(ii).EQ.nseg ) jj = 1
            END DO
   !
   !14-----IDENTIFY SEGMENTS THAT DO NOT RECEIVE TRIBUTARY FLOW.
            IF ( jj.EQ.0 ) THEN
               ISEG(3, nseg) = 5
               k5 = k5 + 1
            ELSE
      ...
      END DO
   !
   !15-----TALLY DIFFERENT STREAM SEGMENT TYPES.
      ktot = k5 + k6 + k7
      WRITE (IOUT, 9012) k5, k6, k7
   !
   !16-----PRINT WARNING IF TALLIED SEGMENTS LESS THAN NSS.
      IF ( ktot.NE.NSS ) THEN
...
9013  FORMAT (/, 5X, '*** WARNING ***  INTERNAL ERROR SUMMING ',&
      ...
      &I6//)
   !
   !17-----PRINT INPUT DATA IF IRDFLG IS ZERO.
   !         SKIP IF INPUT READ BY REACHES (ISFROPT = 1, 3, OR 5)
      IF ( IRDFLG.LE.0 ) CALL SGWF2SFR7PRSEG(NSS, 1, Iunitgwt, Kkper,&
      &Nsol, Iouts)
   !
   !18-----COMPUTE STREAM REACH VARIABLES.
      irch = 1
      ksfropt = 0
      DO nseg = 1, NSS
         ...
         sumlen = 0.0
   !
   !--SET SOME VALUES NEEDED BY THE LMT PACKAGE
   !--AS EACH SEGMENT IS READ, DETERMINE IF ANY OF THE FOLLOWING ARE ACTIVE
   !  (1) STORAGE (TRANSIENT ROUTING); (2) PRECIP; (3) EVAP; (4) USER-SPECIFIED RUNOFF;
   !  (5) RUNOFF FROM UZF1 PACKAGE; (6) UNSATURATED FLOW BENEATH REACH (NOT AVAILABLE YET)
         IF(IUNIT(49).GT.0) THEN  !IUNIT(49): LMT
         ...
         ENDIF
   !
   !19-----COMPUTE VARIABLES NEEDED FOR STREAM LEAKAGE.
   ...
   !
               STRM(6, irch) = avhc
               STRM(8, irch) = avthk
   !20-----COMPUTE STREAMBED ELEVATION AND STREAM WIDTH FOR BEGINNING
   !         OF EACH STREAM SEGMENT FOR COMPUTATION OF LAKE OUTFLOW.
   !dep 4/26/2006
            ELSE
               IF ( ii.EQ.1) THEN
   ...
   !
   !21-----STOP IF ICALC LESS THAN 0 AND GREATER THAN 4.
            ELSE
      ...
      END DO
   !
   !22-----CHECK VALUES IN STREAM CROSS SECTION LIST (XSEC).
      DO nseg = 1, NSS
         icalc = ISEG(1, nseg)
         IF ( icalc.EQ.2 ) THEN
...
9016  FORMAT (1X, /, ' *** ERROR *** STREAM SEGMENT', I7,&
   ...
   !
   !23-----CHECK ROUGHNESS COEFFICIENTS WHEN ICALC = 1 OR 2.
      DO nseg = 1, NSS
         icalc = ISEG(1, nseg)
         IF ( icalc.EQ.1 ) THEN
            rough = SEG(16, nseg)
...
9019  FORMAT ('*** ERROR *** ROUGHNESS COEFFICIENT FOR BANK ',&
      ...
      &' VALUE IS', 1PE11.3, //, ' PROGRAM STOPPING')
   !
   !24-----CHECK VALUES IN TABLE OF FLOW VERSUS DEPTH AND WIDTH
   !         WHEN ICALC = 4.
      DO nseg = 1, NSS
         icalc = ISEG(1, nseg)
...
9026 FORMAT (//)
   !
   !25-----COMPUTE STREAMBED ELEVATIONS FOR TOP AND BOTTOM, AND STREAMBED
   !        SLOPE FROM LAND SURFACE ELEVATION WHEN SPECIFIED.
   !        MODIFIED BY WOLFGANG SCHMID FOR FARM PROCESS.
   !rgn---3/19/07 separated if statement to avoid referencing zero elements in array.
   ...
   !
   !26-----COMPUTE STREAMBED TOP ELEVATION FOR CANAL REACHES
   !        IN FARM PROCESS.
            seglen = SEG(1, nseg)
            sumlen = 0.0
            DO irch = 1, NSTRM
            ...
            END DO
   !
   !27-----COMPUTE STREAMBED SLOPE FOR CANAL REACHES IN FARM PROCESS.
   !       NOTE THAT FIRST AND LAST REACH CAN NOT BE CANAL REACHES.
            DO irch = 2, NSTRM-1
               IF( IDIVAR(1, ISTRM(4, irch)).EQ.IDIVAR(1, nseg) ) THEN
...
9028  FORMAT (//)
   END IF
   !
   !29-----SET FLAGS FOR STEADY STATE OR TRANSIENT SIMULATIONS.
   iflginit = 0
   IF ( Kkper.EQ.1 ) THEN
      iflginit = 1
   ...
   END IF
   !
   !30-----DETERMINE VARIABLES WHEN UNSATURATED FLOW IS ACTIVE.
   DO l = 1, NSTRM
      il = ISTRM(1, l)
      ir = ISTRM(2, l)
      ic = ISTRM(3, l)
      ...
      IF ( IUZT.EQ.1 .AND. iflginit.GE.1 ) THEN
         ...
         width = STRM(5, l)
   !
   !31-----SKIP IF CELL IS OUTSIDE ACTIVE BOUNDARY OR IS NOT WATER TABLE.
   !dep
   !31B-----SEARCH FOR UPPER MOST ACTIVE CELL IN STREAM REACH.
         ilay = il
         ...
         IF ( IBOUND(ic, ir, il).LE.0 ) THEN
            ...
            UZOLSFLX(1, l) = 0.0D0
   !
   !32-----BREAK CHANNEL INTO ISUZN WIDTHS FOR UNSATURATED FLOW
   !         WHEN ICALC IS 2 AND UNSATURATED FLOW IS ACTIVE.
         ELSE IF ( icalc.EQ.2 ) THEN
   !            CALL CHANNELAREA(istsg, l) moved to above
            ...
            END DO
   !
   !33-----INITIALIZE UNSATURATED ZONE ARRAYS WHEN GROUND WATER HEAD
   !         IS LESS THAN BOTTOM OF STREAMBED.
            IF ( sbot.GT.h ) THEN
               iwvcnt = 1
               DO i = 1, ISUZN
                  ...
                  NWAVST(i, l) = 1
   !
   !34-----INITIALIZE UNSATURATED ZONES ARRAYS FOR SECOND STRESS PERIOD
   !         WHEN FIRST STRESS PERIOD IS STEADY STATE.
                  IF ( iflginit.EQ.2 ) THEN
                     ...
                     END IF
   !
   !35-----INITIALIZE UNSATURATED ZONE ARRAYS WHEN FIRST STRESS PERIOD IS
   !         TRANSIENT.
                  ELSE IF ( iss.EQ.0 ) THEN
                     ...
                     END IF
   !
   !36-----INITIALIZE UNSATURATED ZONE ARRAYS WHEN FIRST STRESS PERIOD IS
   !         STEADY STATE.
                  ELSE
                     UZTHST(1, l) = THTR(l)
                     UZSTOR(1, l) = 0.0D0
               ...
               END DO
   !
   !37-----INITIALIZE UNSATURATED ZONE ARRAYS TO ZERO WHEN NO UNSATURATED
   !         ZONE.
            ELSE
               iwvcnt = 1
               istep = NSTOTRL/ISUZN
   ...
   !
   !38-----ONLY ONE UNSATURATED ZONE WIDTH WHEN ICALC IS 1.
         ELSE IF ( icalc.EQ.1 ) THEN
            WETPER(1, l) = width
   !
   !39-----INITIALIZE UNSATURATED ZONE ARRAYS WHEN GROUND WATER HEAD
   !         IS LESS THAN BOTTOM OF STREAMBED.
            IF ( sbot.GT.h ) THEN
               UZDPST(1, l) = sbot - h
               UZSPST(1, l) = 0.0D0
               NWAVST(1, l) = 1
   !
   !40-----INITIALIZE UNSATURATED ZONE ARRAYS FOR SECOND STRESS PERIOD
   !         WHEN FIRST STRESS PERIOD IS STEADY STATE.
               IF ( iflginit.EQ.2 ) THEN
                  ...
                  END IF
   !
   !41-----INITIALIZE UNSATURATED ZONE ARRAYS WHEN FIRST STRESS PERIOD IS
   !         TRANSIENT.
               ELSE IF ( iss.EQ.0 ) THEN
                  UZTHST(1, l) = THTI(l)
                  top = THTI(l) - THTR(l)
                  ...
                  END IF
   !
   !42-----INITIALIZE UNSATURATED ZONE ARRAYS WHEN FIRST STRESS PERIOD IS
   !         STEADY STATE.
               ELSE
                  UZTHST(1, l) = THTR(l)
                  UZSTOR(1, l) = 0.0D0
            ...
            ELSE
   !
   !43-----INITIALIZE UNSATURATED ZONE ARRAYS TO ZERO WHEN NO UNSATURATED
   !         ZONE.
               UZTHST(1, l) = THTR(l)
               UZDPST(1, l) = 0.0D0
               UZFLST(1, l) = 0.0D0
      ...
      END IF
   END DO
   !dep    Added new subroutine to compute tables for lake outflow
   !44-----COMPUTE VALUES FOR ARRAYS DKLOTFLW AND DLKSTAGE WHEN OUTFLOW FROM
   !        LAKES ARE COMPUTED IN THE LAKE PACKAGE.
   IF ( Iunitlak.GT.0 ) THEN
      CALL GWF2SFR7LAKOUTFLW(1)
   END IF
   !C45-----READ TABLES FOR SPECIFIED INFLOWS
   IF ( Kkper.EQ.1 ) THEN
      IF ( NUMTAB_SFR.GT.0 ) THEN
         DO i=1,NUMTAB_SFR
...
END SUBROUTINE GWF2SFR7RP
   !
   !-------SUBROUTINE GWF2SFR7FM
SUBROUTINE GWF2SFR7FM(Kkiter, Kkper, Kkstp, Iunitlak, Iunitnwt,&
&Iunitrch, Iunituzf, Igrid)
   !     *****************************************************************
   !     ADD STREAM TERMS TO RHS AND HCOF IF FLOW OCCURS IN MODEL CELL
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !rgn------REVISION NUMBER CHANGED TO BE CONSISTENT WITH NWT RELEASE
   ...
   numdelt = NUMTIM
   !
   !1------RETURN IF NO STREAMS (NSTRM<=0).
   IF ( NSTRM.LE.0 ) RETURN
   maxwav = NSFRSETS*NSTRAIL
   !
   !2------THERE ARE STREAMS.  INITIALIZE SEGMENT INFLOWS AND OUTFLOWS
   !         TO ZERO FOR LAKE PACKAGE.
   itstr = 0
   iprvsg = -1
   fact = FACTOR
   !      fin = 0.0
   ...
   IF ( Iunitlak.GT.0 ) THEN
      DO i = 1, NSS
         STRIN(i) = 0.0
   !dep commented out FXLKOT to allow it vary with lake stage in LAK7
   !dep          FXLKOT(i) = 0.0
      END DO
   !dep  Change time weighting factor for lake stage calculation.
   !dep   6/27/2005
      IF (iss.NE.1) THEN
         thet1 = THETA
      ELSE
   ...
   END IF
   !
   !2b-----START INTERNAL TIME LOOP FOR STREAMFLOW ROUTING.
   deltinc = DELT
   ...
   DO irt = 1, numdelt
   !
   !3------DETERMINE LAYER, ROW, COLUMN OF EACH REACH.
      DO l = 1, NSTRM
         lfold = l
   ...
   !dep changed to allow allocation of QA and QB from Steady State
         ...
         strtop = STRM(3, l)
   !
   !4------DETERMINE STREAM SEGMENT AND REACH NUMBER.
         istsg = ISTRM(4, l)
         nreach = ISTRM(5, l)
         icalc = ISEG(1, istsg)
   ...
   !
   !5------SET FLOWIN EQUAL TO STREAM SEGMENT INFLOW IF FIRST REACH.
         IF ( nreach.EQ.1 ) THEN
            ...
            END IF
   !
   !6------STORE OUTFLOW FROM PREVIOUS SEGMENT IN SGOTFLW LIST AND IN
   !         STRIN FOR LAKE PACKAGE.
            IF ( istsg.GT.1 ) THEN
               iprvsg = ISTRM(4, ll)
            ...
            END IF
   !
   !7------COMPUTE INFLOW OF A STREAM SEGMENT EMANATING FROM A LAKE.
            IF ( (Iunitlak.GT.0) .AND. (IDIVAR(1,istsg).LT.0) ) THEN
               lk = IABS(IDIVAR(1, istsg))
   ! DEP moved the following code to SFR7AD to initialize FXLKOT at every time step.
   ! DEP FXLKOT is now adjusted in LAK7FM for limiting to available lake water.
   !
   !8------CHECK IF LAKE OUTFLOW IS SPECIFIED AT A FIXED RATE.
   ...
   !              END IF
   !
   !9------SPECIFIED FLOW FROM LAKE IS ZERO AND ICALC IS ZERO.
   !              IF ( icalc.EQ.0 ) THEN
   !              END IF
   !
   !9B-----ESTIMATE LAKE OUTFLOW FOR FIRST ITERATION OF SIMULATION.
   ...
   !
   !10-----FLOW FROM LAKE COMPUTED USING MANNINGS FORMULA AND ASSUMING A
   ...
   !
   !11-----FLOW FROM LAKE COMPUTED USING MANNINGS FORMULA AND EIGHT POINT
   !         CROSS-SECTIONAL AREA.
                     ELSE IF ( dlkstr.GT.NEARZERO .AND. icalc.EQ.2 ) THEN
                        ...
                        &flowin, width)
   !
   !12-----FLOW FROM LAKE COMPUTED USING FORMULA-- Q=(DEPTH/CDPTH)**1/FDPTH).
                     ELSE IF ( dlkstr.GT.NEARZERO .AND. icalc.EQ.3 ) THEN
                        cdpth = SEG(9, istsg)
                        fdpth = SEG(10, istsg)
                        flowin = (dlkstr/cdpth)**(1.0D0/fdpth)
   !
   !13-----FLOW FROM LAKE COMPUTED USING TABULATED VALUES.
            ...
            END IF
   !
   !14-----COMPUTE ONE OR MORE DIVERSIONS FROM UPSTREAM SEGMENT.
   !rgn&dep   revised computation of diversions and added subroutine
            IF( istsg.GT.1 )THEN
               DO kss = 2, NSS
               ...
               END DO
   !
   !20-----SET FLOW INTO DIVERSION IF SEGMENT IS DIVERSION.
               IF( ISEG(3,istsg).EQ.6 ) THEN
                  IF( IDIVAR(1,istsg).GT.0 ) flowin = DVRSFLW(istsg)
               END IF
            END IF
   !
   !21-----SUM TRIBUTARY OUTFLOW AND USE AS INFLOW INTO DOWNSTREAM SEGMENT.
...
9002              FORMAT (//2X, '*** WARNING *** FLOW INTO TRIBUTARY ',&
            ...
            END IF
   !
   !22-----SET INFLOW EQUAL TO OUTFLOW FROM UPSTREAM REACH WHEN REACH
   !         IS GREATER THAN 1.
         ELSE IF ( nreach.GT.1 ) THEN
            flowin = STRM(9, ll)
         END IF
   !
   !23-----SEARCH FOR UPPER MOST ACTIVE CELL IN STREAM REACH.
         ilay = il
         TOPCELL: DO WHILE ( ilay.LE.NLAY )
   ...
   ! factor for losing streams
         ...
         END IF
   !
   !25-----INITIALIZE UNSATURATED ZONE VARIABLES.
         icalccheck = 0
         flobotold = 0.0D0
         areamax = 0.0
         IF ( IUZT.EQ.1 ) THEN
         ...
         END IF
   !
   !26-----SET STREAMBED HYDRAULIC CONDUCTIVITY AND STREAM LEAKAGE TO
   !         ZERO WHEN NOT AN ACTIVE CELL.
         IF ( IBOUND(ic, ir, il).LE.0 ) THEN
            avhc = 0.0
            strleak = 0.0D0
            h = hstr
         ...
         END IF
   !26a----CHECK IF LAKE INUNDATES STREAM. IF SO THEN SET avhc and cstr = 0.
         IF ( Iunitlak.GT.0 ) THEN
            illake = il - 1
            IF ( illake.GE.1 ) THEN
         ...
         END IF
   !
   !27-----BEGIN COMPUTATION OF STREAM DEPTH FOR ACTIVE CELL.
   !
   !28-----COMPUTE FLOW AT MIDPOINT OF REACH IGNORING STREAMBED LEAKAGE.
         IF ( icalc.EQ.0 ) THEN
            flwmpt = flowin + 0.5D0*(runof+runoff-etstr+precip)
   !rgn 10/23/06 initialize flowc
   ...
   ! RGN might need to correct depth for smooth width
         ...
         ELSE IF ( icalc.GE.2 ) THEN
            ...
            IF ( flowc.LT.NEARZERO ) flowc = 0.0D0
   !
   !29-----CALCULATE AN INITIAL ESTIMATE OF FLOW IN CHANNEL.
         ...
         END IF
   !
   !30-----ESTIMATE DEPTH USING BISECTION METHOD WHEN ICALC IS GREATER THAN 0.
         iflg = 1
   !
   !30b----SKIP NEWTON METHOD WHEN ICALC IS 1 AND SURFACE INFLOW IS ZERO.
   ! RGN 10/4/11 need newton to smooth width to zero for drying channels
         IF ( icalc.EQ.1 .AND. hstr.LE.strtop .AND. h.LE.strtop)&
         &iflg = 0
   !30c----SKIP NEWTON METHOD WHEN REACH OUTSIDE ACTIVE AREA AND ISKIP IS 1.
         IF ( iskip.NE.0 ) iflg = 0
         IF ( h.LE.strtop .AND. flowc.LT.NEARZERO ) iflg = 0
         IF ( icalc.GE.1 .AND. iflg.EQ.1 ) THEN
   !
   !31-----ESTIMATE INITIAL ENDPOINTS.
            enpt1 = 0.0D0
   ...
   ! RGN might need to smooth width for end points
            ...
            END IF
   !
   !33-----ESTIMATE DEPTH FOR ENDPOINTS WHEN ICALC IS 1.
            IF ( icalc.EQ.1 ) THEN
               IF ( h.GT.sbot ) THEN
   ...
   !
   !34-----ESTIMATE DEPTH, WIDTH AND WETTED PERIMETER WHEN
   !         ICALC IS GREATER THAN OR EQUAL TO 2.
            ELSE IF ( icalc.GE.2 ) THEN
               IF ( icalc.EQ.2 ) THEN
               ...
               END IF
   !
   !35-----ESTIMATE LEAKAGE THROUGH STREAMBED WHEN ICALC GREATER THAN OR
   !         EQUAL TO 2.
               ...
               END IF
   !
   !36-----SET DEPTH2 AND WIDTH2 TO ZERO, AND WETPERM2 TO ONE WHEN FLOW AT
   !         ENDPOINT2 IS LESS THAN OR EQUAL TO ZERO.
               IF ( flwen2.LT.NEARZERO ) THEN
                  depth2 = 0.0D0
                  width2 = 0.0D0
                  wetperm2 = 1.0D0
   !
   !37-----OTHERWISE CALCULATE DEPTH2, WIDTH2, AND WETPERM2.
               ELSE IF ( icalc.EQ.2 ) THEN
            ...
            END IF
   !
   !38-----DETERMINE ROOT FOR ENDPOINT 1 WHEN DEPTH IS GREATER THAN 0.
            ...
            ELSE
               ...
               f1 = enpt1 - 0.0D0
   !
   !39-----DETERMINE ROOT FOR ENDPOINT 2 WHEN DEPTH IS GREATER THAN 0.
            END IF
            ...
            iflg = 1
   !
   !40-----ITERATE THROUGH NEWTON METHOD TO FIND ESTIMATE OF STREAM DEPTH
   !         AND STREAMBED LEAKAGE WHEN ICALC IS GREATER THAN 0.
   ...
   !
   !41-----CALCULATE FLOBOT1 AND FLOBOT2 FOR ICALC EQUAL TO 1.
   !dep  Corrected depth1+dlh and depth2+dlh to be depth1 and depth2.
               IF ( icalc.EQ.1 ) THEN
   ! RGN 10/4/11 added smoothing for width when icalc=1
                  ...
                  END IF
   !
   !42-----USE BISECTION WHEN FLOBOT1 IS LIMITED BY FLOW IN CHANNEL.
   ...
   ! might be issue here using depthp for smoothing
               ...
               ELSE IF ( icalc.GE.2 ) THEN
   !
   !43-----CALCULATE NEWTON VARIABLES FOR ICALC EQUAL TO 2.
                  IF ( icalc.EQ.2 ) THEN
                     ...
                     &flwmdpt2, width2)
   !
   !44-----CALCULATE NEWTON VARIABLES FOR ICALC EQUAL TO 3.
                  ELSE IF ( icalc.EQ.3 ) THEN
                     ...
                     wetpermd = widthd
   !
   !45-----CALCULATE NEWTON VARIABLES FOR ICALC EQUAL TO 4.
                  ELSE IF ( icalc.EQ.4 ) THEN
                     deptha = depth1 - (deps*depth1)
                     IF ( deptha.LT.NEARZERO ) deptha = depth1
                     depthb = depth1 + (deps*depth1)
                  ...
                  END IF
   !
   !46-----CALCULATE DERIVATIVES FOR DEPTH DEPENDENT VARIABLES FOR ICALC
   !         GREATER THAN 1.
   ...
   !dep revised pp1,pp2,et1,and et2, wrong placment of parenthesis.
   ...
   !
   !47-----CALCULATE FLOBOT1 AND FLOBOT2 WHEN ICALC GREATER THAN 1.
                  IF ( h.GE.sbot ) THEN
                  ...
                  END IF
   !
   !48-----DETERMINE IF LEAKAGE LIMITED BY FLOW IN CHANNEL.
                  IF ( width1.GT.NEARZERO ) THEN
   !dep revised flwpet1, wrong placment of parenthesis.
                  ...
                  END IF
   !
   !49-----USE BISECTION WHEN LEAKAGE IS LIMITED BY FLOW IN CHANNEL.
   ...
   !rgn changed widthp to widthx 12/5/06
                     ...
                     END IF
   !
   !50-----CALCULATE LEAKAGE FROM NEWTON METHOD WHEN NOT LIMITED BY FLOW.
                  ELSE
               ...
               IF ( ibflg.EQ.0 ) THEN
                  ...
                  IF ( iflg.GT.0 ) THEN
                     ...
                     END IF
   !
   !51-----SET FLAGS TO DETERMINE IF NEWTON METHOD OSCILLATES OR
   ...
   !
   !52-----SWITCH TO BISECTION WHEN NEWTON METHOD OSCILLATES OR WHEN
   !         CONVERGENCE IS SLOW.
                     ...
                     END IF
   !
   !53-----COMPUTE FLOBOTP ON BASIS OF DEPTHP AND THEN ESTIMATE DEPTHX FROM
   !         FLOBOTP.
                     IF ( icalc.EQ.1 ) THEN
                        IF ( h.GT.sbot ) THEN
   ...
   ! RGN smooth width 10/4/11
                     ...
                     ELSE IF ( icalc.GE.2 ) THEN
                        ...
                        flwx = flwmpt - 0.5D0*flobotp
   !
   !54-----USE BISECTION WHEN LEAKAGE LIMITED BY FLOW FOR ICALC GREATER
   !         OR EQUAL TO 2.
                        ...
                        END IF
   !
   !55-----ESTIMATE DEPTHX WHEN ICALC IS GREATER THAN OR EQUAL TO 2.
                        IF ( flwx.LT.NEARZERO ) THEN
                           depthx = 0.0D0
                           widthx = 0.0D0
                           wetpermx = 1.0D0
               ...
               END IF
   !
   !56-----COMPUTE DIFFERENCE BETWEEN DEPTHP AND DEPTHX.
               fp = depthp - depthx
               IF ( ibflg.EQ.1 ) dlh = fp
   !
   !57-----CHANGE ENDPOINTS IF BISECTION HAS BEEN USED.
               IF ( ibflg.EQ.1 ) THEN
                  IF ( f1*fp.LT.0.0D0 ) THEN
   !
   !58-----ROOT IS BETWEEN F1 AND FP.
                     enpt2 = depthp
                     f2 = fp
                  ELSE
   !
   !59-----ROOT IS BETWEEN FP AND F2.
                     enpt1 = depthp
                     f1 = fp
   !
   !60-----CALCULATE ERROR.
               ...
               END IF
   !
   !61-----SET DEPTH TO DEPTHP AND FLOBOT TO FLOBOTP WHEN IS ERROR LESS
   !         THAN TOLERANCE.
   ...
   !dep  August 26, 2009  added ELSE to recalculate flwmpt for ICALC=0 or 1
               ...
               END IF
   !
   !62-----PRINT WARNING THAT REACH FAILED TO CONVERGE AFTER 100 ITERATIONS.
   !         AND SET DEPTH TO DEPTHP AND FLOBOT TO FLOBOTP.
               IF ( itot.GE.100 ) THEN
                  iflg = 0
...
9003              FORMAT (//5X, '*** WARNING *** SFR FAILED TO ',&
   ...
   !dep  August 26, 2009  added ELSE to recalculate flwmpt for ICALC=0 or 1
               ...
               SFRQ(4, l) = width
   !
   !63-----END OF NEWTON LOOP.
            END DO
   !
   !64-----DEFINE HSTR, CSTR, WIDTH, AND FLOWOT.
            hstr = depth + STRM(3, l)
            IF ( icalc.GE.2 ) cstr = (avhc*wetperm*strlen)/sbdthk
         END IF
   !
   !65-----ROUTE FLOW WITHOUT ANY STREAM LEAKAGE (FLOBOT IS ZERO) WHEN
   !          MODEL CELL IS INACTIVE. Revised dep 5/19/2005
   ...
   ! RGN 10/3/07 added check for flwmpt to avoid NaN values.
            ...
            ELSE IF ( icalc.GE.2 ) THEN
               depth = 0.0D0
               cstr = 0.0D0
   !rgn used width in calculations.
               IF ( icalc.EQ.2 ) THEN
   ...
   !dep  August 26, 2009 fixed flwmpt calculation  not half of flowc
            ...
            END IF
   !
   !66-----COMPUTE STREAM LEAKAGE IF STREAM DEPTH WAS NOT COMPUTED
   !         USING EITHER BISECTION OR NEWTON METHOD. revised dep
            IF ( iskip.EQ.0 ) THEN
               cstr = strleak*width/sbdthk
   !rgn added next 4 lines.
               IF (icalc.GT.1 ) THEN
                  etstr = etstr*width
                  precip = precip*width
               END IF
   ...
   !
   !67-----ROUTE FLOW DOWN CHANNEL WHEN ACTIVE.
         ...
         ELSE
   !
   !68-----STREAMFLOW OUT EQUALS STREAMFLOW IN MINUS LEAKAGE.
            flowot = flowc - flobot
            qc = flowc
            qd = flowot
   ...
   !
   !
   !69-----STORE STREAM INFLOW, OUTFLOW, LEAKAGE, STAGE, AND STREAMBED
   !          CONDUCTANCE FOR EACH REACH.
         STRM(9, l) = flowot
         STRM(10, l) = flowin
         STRM(11, l) = flobot
         STRM(15, l) = hstr
   ...
   !70-----STORE OUTFLOW FROM LAST REACH IN LAST SEGMENT IN STRIN
   !          LIST FOR LAKE PACKAGE.
         IF ( Iunitlak.GT.0 ) THEN
            IF ( l.EQ.NSTRM .AND. istsg.EQ.NSS ) STRIN(istsg) = flowot
         END IF
   !
   !71-----STORE OUTFLOW FOR LAST SEGMENT IN SGOTFLW LIST AND IN
   !         STRIN FOR LAKE PACKAGE.
         IF ( l.EQ.NSTRM ) THEN
   ...
   !
   !72-----CALCULATE SEEPAGE THROUGH UNSATURATED ZONE.
            ...
            END IF
   !
   !73-----ROUTE SEEPAGE THROUGH UNSATURATED ZONE.
            kerp = 0
   ...
   !74-----SUM SEEPAGE TO WATER TABLE.
         ...
         END IF
   !
   !75-----STORE FLOWS NEEDED FOR SENSITIVITIES. - ERB
         IF ( IRTFLG.EQ.0 ) THEN
            SFRQ(1, l) = flwmpt
            SFRQ(2, l) = flowc
            SFRQ(3, l) = flobot
         ...
         END IF
   !
   !76-----ADD TERMS TO RHS AND HCOF IF FLOBOT IS NOT ZERO.
         IF ( irt.EQ.numdelt ) THEN
            hstrave = 0.0D0
            DO i = 1, numdelt
   ...
   !
   !77-----ADD TERMS TO RHS AND HCOF WHEN GROUND-WATER HEAD LESS THAN
   !         STREAMBED BOTTOM ELEVATION.
               IF ( h.LT.sbot ) THEN
                  RHS(ic, ir, il) = RHS(ic, ir, il) - SUMRCH(l)
   !       fin=fin+sumrch(l)
   !dep  changed dbleak to -CLOSEZERO
               ELSE IF ( SUMLEAK(l)-flowc.LT.-CLOSEZERO ) THEN
   !
   !78-----STREAM LEAKAGE IS NOT HEAD DEPENDENT.
                  IF ( iss.EQ.0 ) THEN
   ...
   !      end if
               ...
               ELSE
   !
   !79-----CONSTANT STREAMBED LEAKAGE IS LIMITED BY STREAMFLOW OR
   !         STREAMBED CONDUCTANCE IN REACH.
                  IF ( iss.EQ.0 ) THEN
                     RHS(ic, ir, il) = RHS(ic, ir, il)&
                     &- SUMLEAK(l)- SUMRCH(l)
   ...
   !       fin=fin+sumleak(l)
                  END IF
   !
   !80-----ADD TERM ONLY TO RHS WHEN GROUND-WATER HEAD IS LESS THAN
   !         STREAMBED BOTTOM ELEVATION.
               END IF
   ...
   !
   !81-----END INTERNAL TIME LOOP FOR ROUTING STREAMFLOWS.
...
&Iunituzf, Nsol, Iunitrch, Igrid)
   ...
   END IF
   !
   !1b-----PRINT STREAM RESULTS WHENEVER BUDGET TERMS ARE PRINTED.
   !dep    revised to allow for compact and non compact budgets
   ...
   ibstlb = 0
   !
   !2------WRITE HEADER WHEN CELL-BY-CELL FLOWS WILL BE SAVED AS A LIST.
   !IFACE
   IF ( ibd.EQ.2 ) THEN
      naux=1
   ...
   END DO
   !
   !4------INITIALIZE SEGMENT INFLOWS AND OUTFLOWS TO ZERO FOR LAKE PACKAGE
   !         WHEN THERE ARE STREAMS.
   iprvsg = -1
   IF ( Iunitlak.GT.0 ) THEN
      DO i = 1, NSS
         STRIN(i) = 0.0
   ...
   END IF
   !
   !5-----START INTERNAL TIME LOOP FOR ROUTING STREAMFLOWS.
   deltinc = DELT
   IF ( IRTFLG .GT. 0 .AND. iss.EQ.0 ) THEN
      deltinc = DELT/FLOAT(numdelt)
   ELSE
   ...
   !
   !5b------DETERMINE LAYER, ROW, COLUMN OF EACH REACH.
      DO l = 1, NSTRM
         lfold = l
         IF ( Nfoldflbt==1 ) lfold = 1
         gwflow = 0.0D0
         ...
         ic = ISTRM(3, l)
   !
   !6------DETERMINE STREAM SEGMENT AND REACH NUMBER.
         istsg = ISTRM(4, l)
         nreach = ISTRM(5, l)
         icalc = ISEG(1, istsg)
         strtop = STRM(3, l)
   ! RGN 5/9/09 set slope for all icalc
         slope = STRM(2, l)
   !
   !7------SET FLOWIN EQUAL TO STREAM SEGMENT INFLOW IF FIRST REACH.
         IF ( nreach.EQ.1 ) THEN
            flowin = SEG(2, istsg)
   ...
   !
   !8------STORE OUTFLOW FROM PREVIOUS SEGMENT IN SGOTFLW LIST AND IN
   !         STRIN FOR LAKE PACKAGE.
            IF ( istsg.GT.1 ) THEN
               iprvsg = ISTRM(4, ll)
               SGOTFLW(iprvsg) = STRM(9, ll)
            ...
            END IF
   !
   !9------COMPUTE INFLOW OF A STREAM SEGMENT EMANATING FROM A LAKE.
            IF ( (Iunitlak.GT.0) .AND. (IDIVAR(1,istsg).LT.0) ) THEN
               lk = IABS(IDIVAR(1, istsg))
   !10-----OUTFLOW FROM LAKE COMPUTED IN LAKE PACKAGE
               IF ( SEG(2,istsg).LE.CLOSEZERO ) THEN
                  flowin = STROUT(istsg)
                  sfrbudg_in = sfrbudg_in + flowin
               ELSE
   !11-----SET SPECIFIED OUTFLOW FROM LAKE
                  flowin = FXLKOT(istsg)
                  sfrbudg_in = sfrbudg_in + FXLKOT(istsg)
               END IF
   !              NINTOT = NINTOT + 1    !EDM  WITH NEW FTL FORMAT, THIS INDEX SHOULD NO LONGER BE COUNTED AS 'CONNECT SFR LAK' WILL ACCOUNT FOR IT
            END IF
   !
   !14-----COMPUTE ONE OR MORE DIVERSIONS FROM UPSTREAM SEGMENT.
   !rgn&dep   revised computation of diversions and added subroutine
   ...
   !EDM - For LMT
            ...
            END IF
   !
   !22-----SUM TRIBUTARY OUTFLOW AND USE AS INFLOW INTO DOWNSTREAM SEGMENT.
            IF ( istsg.GE.1 .AND. ISEG(3, istsg).EQ.7 ) THEN
               itrib = 1
               flowin = 0.0D0
               ...
               END IF
   !
   !23-----CHECK IF SPECIFIED "FLOW" IS WITHDRAWAL (i.e., negative), THAT WATER IS AVAILABLE.
   ...
   !     +                 'FLOWIN RE-SET = 0.0'/)
            ...
            END IF
   !
   !24-----SET INFLOW EQUAL TO OUTFLOW FROM UPSTREAM REACH, WHEN REACH
   !         GREATER THAN 1.
         ELSE IF ( nreach.GT.1 ) THEN
            flowin = STRM(9, ll)
            IF(IUNIT(49).GT.0) THEN  !IUNIT(49): LMT
         ...
         END IF
   !
   !- EDM -IF OUTSEG=0 THEN SEGMENT IS A NETWORK SINK AND SHOULD BE COUNTED FOR LMT
         IF(IUNIT(49).GT.0) THEN  !IUNIT(49): LMT
         ...
         ENDIF
   !
   !25-----SEARCH FOR UPPER MOST ACTIVE CELL IN STREAM REACH. Revised ERB
         ilay = il
         TOPCELL: DO WHILE ( ilay.LE.NLAY )
            IF ( IBOUND(ic, ir, ilay).EQ.0 ) THEN
               ilay = ilay + 1
         ...
         IF ( ilay.LE.NLAY ) il = ilay
   !
   !26-----DETERMINE LEAKAGE THROUGH STREAMBED.
         hstr = HSTRM(l,irt)
         depth = hstr - STRM(3, l)
         IF ( icalc.EQ.0 ) THEN
   ...
   !26a----CHECK IF LAKE INUNDATES STREAM. IF SO THEN SET avhc and cstr = 0.
         ...
         END IF
   !
   !26b-----SET STREAMBED HYDRAULIC CONDUCTIVITY AND STREAM LEAKAGE TO
   !         ZERO WHEN NOT AN ACTIVE CELL.
         IF ( IBOUND(ic, ir, il).LE.0 ) THEN
            avhc = 0.0
            h = hstr
   ...
   !
   !27-----COMPUTE HEAD DIFFERENCE ACROSS STREAMBED.
            h = HNEW(ic, ir, il)
            hld = HLDSFR(l)
            avhc = STRM(6, l)
   ! Added code to test for BCF or LPF 11/19/07
            ...
            END IF
   !
   !28-----COMPUTE LEAKAGE ACROSS STREAMBED.
            flobot = cstr*(hdiff)
   !
   !29-----STREAMFLOW OUT EQUALS STREAMFLOW IN MINUS LEAKAGE.
         ELSE
            flobot = 0.0D0
            grad = 0.0D0
            hdiff = 0.0D0
         END IF
   !
   !30-----COMPUTE FLOW IN STREAM CHANNEL AND SET LEAKAGE EQUAL TO FLOW
   !         IF LEAKAGE MORE THAN FLOW. Revised DEP
         runof = STRM(12, l)
         runoff = STRM(24, l)
         IF ( icalc.LE.1 ) THEN
            precip = STRM(14, l)
         ...
         ELSE IF ( icalc.GE.2 .AND. icalc.LE.4 ) THEN
   !
   !31-----NO PRECIPITATION OR ET FROM CHANNEL WHEN WIDTH IS ZERO.
   ...
   !
   !32-----ROUTE FLOW IN CHANNELS WHEN IRTFLG IS NOT ZERO AND
   ...
   !                        X-sectional area should still be calculated and passed to ftl
         ...
         END IF
   !
   !33-----STORE OUTFLOW FROM LAST REACH IN LAST SEGMENT IN STRIN
   !         LIST FOR LAKE PACKAGE.
         IF ( Iunitlak.GT.0 ) THEN
            IF ( l.EQ.NSTRM .AND. istsg.EQ.NSS ) STRIN(istsg) = flowot
         END IF
   !
   !34-----STORE STREAM INFLOW, OUTFLOW, AND LEAKAGE FOR EACH REACH.
         STRM(9, l) = flowot
         STRM(10, l) = flowin
         STRM(11, l) = flobot
         ...
         STRM(19, l) = h
   !
   !35-----ADD FLOW TO STREAM BUDGET OUTFLOW IF FLOW LEAVES ACTIVE MODEL.
         IF ( irt.EQ.numdelt ) THEN
            IF ( l .LT. NSTRM ) THEN
         ...
         END IF
   !
   !36-----STORE OUTFLOW FOR LAST SEGMENT IN SGOTFLW LIST AND IN STRIN FOR
   !         LAKE PACKAGE.
         IF ( l.EQ.NSTRM ) THEN
            SGOTFLW(istsg) = STRM(9, l)
            IF ( Iunitlak.GT.0 ) STRIN(istsg) = STRM(9, l)
   ...
   !
   !38-----SUBTRACT FLOBOT FROM RATOUT WHEN GROUND WATER DISCHARGES
   !         TO STREAM REACH.
            IF ( flobot.LT.0.0D0 ) THEN
            ...
            END IF
   !
   !39-----ADD FLOBOT TO RATIN WHEN STREAM RECHARGES GROUND WATER.
            IF ( flobot.GT.0.0D0 ) THEN
         ...
         END IF
   !
   !40-----PRINT STREAMFLOWS AND RATES FOR EACH REACH TO MAIN LIST IF
   !         REQUESTED (ISTCB1<0 and IBD<0)AND NO UNSATURATED FLOW.
         IF ( IUZT.EQ.0 ) THEN
            IF ( irt.EQ.numdelt ) THEN
               ...
               END IF
   !
   !41-----PRINT STREAMFLOWS AND RATES FOR EACH REACH TO STREAM LIST
   !         IF REQUESTED (ISTCB2>0).
            ...
            END IF
   !
   !42-----PRINT STREAMLFOWS AND RATES FOR EACH REACH TO MAIN LIST
   !         WHEN UNSATURATED FLOW IS ACTIVE.
         ELSE
            IF ( irt.EQ.numdelt ) THEN
               ...
               END IF
   !
   !43-----PRINT STREAMFLOWS AND RATES FOR EACH REACH TO STREAM LIST
   !         WHEN UNSATRATED FLOW IS ACTIVE.
            ...
            END IF
         END IF
   !43B----SAVE SEEPAGE TO ARRAY FOR PRINTING NET SEEPAGE IN UZF
         FNETSEEP(IC,IR) = FNETSEEP(IC,IR) + gwflow    !RGN multiple reaches in a cell
   !
   !44-----SAVE FLOW TO AND FROM GROUND WATER IN A LIST FILE WHEN
   !         IBD IS EQUAL TO 2. revised dep 5/10/2006--fixed 9/15/2006
   !DEP   need to fix for unsaturated flow
   !IFACE
   ...
   !
   !45-----RECORD STREAM GAGING STATION DATA (IF SOLUTE TRANSPORT NOT ACTIVE).
      IF ( Iunitgwt.LE.0 .AND. Iunitgage.GT.0 ) THEN
         CALL SGWF2GAG7SO(Iunitgwt, Iunituzf, rtime, BUFF, SFRQ,&
         &ibd, Nsol)
      END IF
   !
   !46-----END OF INTERNAL TIME LOOP FOR ROUTING FLOW IN CHANNELS.
   END DO
   !
   !46Bdep----New loop to compute specified flows from external sources
   !added 6/27/2008
   DO istsg = 1, NSS
...
9010 FORMAT (1X, I3, I5, I5, 2I6, 3X, 1PE11.4, 1X, E11.4, 2X, E11.4,&
   ...
   !
   !47-----PRINT MASS BALANCE OF UNSATURATED ZONE IF IUZT GREATER THAN 0.
   IF ( IBUDFL.GT.0 .and. icalccheck.EQ.1)&
   &CALL GWF2SFR7UZOT(Kkstp, Kkper)
   !
   !48-----SAVE FLOW TO AND FROM GROUND WATER AS A 3-D ARRAY WHEN
   !         IBD IS EQUAL TO 1. revised dep 5/10/2006
   IF ( ibd.EQ.1 ) CALL UBUDSV(Kkstp, Kkper, text, iout1, BUFF, NCOL,&
   &NROW, NLAY, IOUT)
   !
   !49-----MOVE RATES, VOLUMES, AND LABELS INTO ARRAYS FOR PRINTING.
   rin = ratin
   rout = ratout
   VBVL(3, MSUM) = rin
   VBVL(4, MSUM) = rout
   ...
   VBNM(MSUM) = text
   !
   !50-----INCREMENT BUDGET TERM COUNTER.
   MSUM = MSUM + 1
   !
   !51-----STREAMFLOW OUT OF EACH REACH IS SAVED TO A LIST FILE
   !       WHEN COMPACT BUDGET REQUESTED OR TO A 3-D ARRAY
   !       WHEN STANDARD UNFORMATTED BUDGET. revised dep 5/10/2006
   IF ( ibdst.GT.0 ) THEN
      DO il = 1, NLAY
         DO ir = 1, NROW
            DO ic = 1, NCOL
   ...
   !dep   added compact budget option for streamflow out of reach
...
END SUBROUTINE GWF2SFR7BD
   !
   !-------SUBROUTINE GWF2SFR7LAKOUTFLW
SUBROUTINE GWF2SFR7LAKOUTFLW(kkiter)
   !     *****************************************************************
   !     CALCULATE ARRAYS OF LAKE STAGE, FLOW, AND THE DERIVATIVE OF
   !     FLOWS FOR STREAM SEGMENTS THAT HAVE INFLOWS DETERMINED BY
   !     LAKE STAGE
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     *****************************************************************
   USE GWFSFRMODULE
   ...
   !
   !1------LOOP THROUGH ALL STREAM REACHES.
   !
   ...
   DO l = 1, NSTRM
      ...
      IF ( icalc.EQ.4 ) nstrpts = ISEG(2, istsg)
   !
   !2------DETERMINE SEGMENTS THAT GET THEIR INFLOWS FROM A LAKE.
      IF ( nreach.EQ.1 .AND. IDIVAR(1, istsg).LT.0 ) THEN
         finc = DPMAXLK/200.0D0
         strbdtop = SEG(8, istsg)
   !
   !dep    Added tables for computing lake outflow in Lake Package
   !3------CALCUATE TABLES FOR LAKE STAGE AND CHANGE IN LAKE OUTFLOW.
         DO lk = 1, 200
            IF ( lk.EQ.1 )THEN
   ...
   !
   !3-----ICALC EQUALS 3 USING FORMULA-- Q=(DEPTH/CDPTH)**1/FDPTH).
            ELSE IF ( icalc.EQ.3 ) THEN
               cdpth = SEG(9, istsg)
               fdpth = SEG(10, istsg)
   ...
   !
   !4-----FLOW FROM LAKE COMPUTED USING TABULATED VALUES.
            ELSE IF ( icalc.EQ.4 ) THEN
...
END SUBROUTINE GWF2SFR7LAKOUTFLW
   !
   !-------SUBROUTINE GWF2SFR7DIVERS
SUBROUTINE GWF2SFR7DIVERS(Iprior, Upflw, Dvrsn)
   !     ******************************************************************
   !     COMPUTES DIVERSIONS FROM AN UPSTREAM SEGMENT
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
   IMPLICIT NONE
   !     SPECIFICATIONS:
   ...
   IF ( Iprior.EQ.0 ) THEN
      ...
      END IF
   !
   !2------IF IPRIOR IS -1 THEN FLOW DIVERTED ONLY IF SPECIFIED FLOW
   !         AVAILABLE OTHERWISE NO FLOW DIVERTED.
   ELSE IF ( Iprior.EQ.-1 ) THEN
      ...
      END IF
   !
   !3------IF IPRIOR IS -2 THEN FLOW DIVERTED IS PERCENTAGE OF
   !         AVAILABLE STREAMFLOW.
   ELSE IF ( Iprior.EQ.-2 ) THEN
      prcnt = Dvrsn
      Dvrsn = Upflw*prcnt
   !
   !4------IF IPRIOR IS -3 THEN FLOW DIVERTED ONLY WHEN STREAMLFOW
   !         EXCEEDS SPECIFIED FLOW (FLOOD CONTROL DIVERSION).
...
END SUBROUTINE GWF2SFR7DIVERS
   !
   !-------SUBROUTINE GWF2SFR7UZOT
SUBROUTINE GWF2SFR7UZOT(Kkstp, Kkper)
   !     ******************************************************************
   !     PRINTS MASS BALANCE FOR ENTIRE UNSATURATED ZONE
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
   ...
   WRITE (IOUT, 9002)
   !
   !1------PRINT INDIVIDUAL INFLOW RATES AND VOLUMES AND THEIR TOTALS.
   !
   !1a-----STREAM LEAKAGE.
   ...
   END IF
   !
   !3------PRINT TOTALS AND RATES TO GROUND WATER.
   ...
   !
   !4------PRINT TOTALS AND RATES FROM GROUND WATER.
   ...
   !
   !5------PRINT DIFFERENCES AND PERCENT DIFFERENCES BETWEEN INPUT
...
&Iprndpth)
   ...
   Wetperm = 0.0D0
   !
   !2------FIND LOWEST POINT IN CHANNEL.
   ymin = XSEC(9, Istsg)
   DO i = 9, 16
      y0 = XSEC(i, Istsg)
      ymin = DMIN1(ymin, y0)
   END DO
   !
   !3------ESTIMATE INITIAL DEPTH ASSUMING WIDE RECTANGULAR CHANNEL.
   depth1 = 0.0D0
   xnum = Flow*Roughch
   ...
   IF ( depth1.GT.0.0D0 ) THEN
      ...
      f2 = Flow - flow2
   !
   !4------ESTIMATE NEW DEPTH USING EITHER BISECTION OR SECANT METHOD
   !         FOR SOLVING ROOTS.
      depth3 = 0.0D0
   ...
   !6------USE SECANT METHOD.
      ...
      IF ( err1.LT.0.000001 .AND. err2.LT.0.000001 ) THEN
         depth3 = (depth1+depth2)*0.5D0
   !
   !7------CONTINUE RECALCULATING DEPTH3 UNTIL ERROR LESS THAN 0.000001
   ...
   !
   !8------ROOT IS BETWEEN DEPTH2 AND DEPTH3.
               IF ( f1*f2.LT.0.0D0 ) THEN
   !
   !9------ROOT IS BETWEEN DEPTH1 AND DEPTH2.
                  depth1 = depth3
                  f1 = f3
               ELSE
   !
   !10-----DEPTH1 AND DEPTH2 ON SAME SIDE OF ROOT;
   !          REPLACE WITH FARTHER ROOT.
               ...
               END IF
   !
   !11-----DEPTH2 AND DEPTH3 ARE ON THE SAME SIDE OF ROOT.
            ELSE IF ( f1*f2.LT.0.0D0 ) THEN
   !
   !12-----ROOT IS BETWEEN DEPTH1 AND DEPTH2.
               depth2 = depth3
               f2 = f3
            ELSE
   !
   !13-----DEPTH1,DEPTH2, AND DEPTH3 ARE ON SAME SIDE OF ROOT;
...
9001           FORMAT (1X/, 'ITSTR,IFLG,FLOW,DEPTH1,DEPTH2,DEPTH3,F1,F2,'&
            ...
            END IF
   !
   !14-----PRINT WARNING MESSAGE IF SECANT METHOD FAILED TO FIND A DEPTH.
...
9002           FORMAT (1X/, 'SECANT METHOD FAILED TO FIND SOLUTION FOR',&
      ...
      END IF
   !
   !15-----CALCULATE WETTED PERIMETER AND WIDTH FROM FINAL DEPTH.
      Depth = depth3
      stage = Depth + ymin
      flow1 = Flow
      CALL GWF2SFR7FLW(stage, Istsg, Roughch, Roughbnk, Slope,&
...
END SUBROUTINE GWF2SFR7DPTH
   !
   !-------SUBROUTINE GWF2SFR7FLW
SUBROUTINE GWF2SFR7FLW(Depth, Istsg, Roughch, Roughbnk, Slope,&
&Wetperm, Flow, Totwdth)
   !     *******************************************************************
   !     COMPUTE FLOW IN STREAM GIVEN DEPTH USING 8-POINT CROSS SECTION
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     *******************************************************************
   ...
   j = 1
   !
   !2------INITIALIZE X AND Y POINTS; START AT I = 2.
   DO i = 2, 8
      ii = i + 8
      x0 = XSEC(i-1, Istsg)
      y0 = XSEC(ii-1, Istsg)
      ...
      IF ( Depth.GT.y0 ) THEN
   !
   !3------LEFT ENDPOINT IS SUBMERGED.
         xleft = x0
         yleft = y0
         dpthleft = Depth - yleft
         IF ( Depth.GT.y1 ) THEN
   !
   !4------LEFT AND RIGHT ENDPOINTS ARE SUBMERGED.
            xright = x1
            yright = y1
            dpthrght = Depth - yright
         ELSE
   !
   !5------LEFT ENDPOINT IS SUBMERGED AND RIGHT IS NOT.
            dpthrght = 0.0D0
            yright = Depth
   ...
   !
   !6------LEFT ENDPOINT IS ABOVE WATER AND RIGHT IS SUBMERGED.
         dpthleft = 0.0D0
         yleft = Depth
      ...
      ELSE
   !
   !7------LEFT AND RIGHT ENDPOINTS ARE ABOVE WATER.
         dpthleft = 0.0D0
         dpthrght = 0.0D0
         width = 0.0D0
      END IF
   !
   !8------ADD AREA AND WETPERM FOR SUBSECTION OF CHANNEL.
      ...
      END IF
   !
   !9------COMPUTE FLOW FOR EACH SUBSECTION OF CHANNEL.
   ...
   END DO
   !
   !10-----SUM FLOW, WETTED PERIMETER, AND WIDTH FOR SUBSECTIONS.
...
END SUBROUTINE GWF2SFR7FLW
   !
   !-------SUBROUTINE GWF2SFR7TBD
SUBROUTINE GWF2SFR7TBD(Flow, Depth, Width, Nstrpts, Istsg)
   !     *******************************************************************
   !     COMPUTE DEPTH AND WIDTH IN STREAM GIVEN FLOW USING RATING TABLES.
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     *******************************************************************
   ...
   !
   !2------USE A LINEAR INTERPOLATION TO ESTIMATE DEPTH AND WIDTH WHEN
   !         FLOW IS LESS THAN LOWEST VALUE IN TABLE.
   IF ( Flow.LE.flwlw ) THEN
      Depth = (stglw/flwlw)*Flow
      Width = (wthlw/flwlw)*Flow
   !
   !3------OTHERWISE USE A LOG INTERPOLATION TO ESTIMATE DEPTH AND WIDTH.
   ELSE IF ( Flow.GT.flwlw ) THEN
   !
   !4------FIND NEAREST VALUES OF FLOW, DEPTH, AND WIDTH IN TABLE.
      istp = 2
   ...
   !
   !5------COMPUTED FLOW EXCEEDS HIGHEST FLOW IN TABLE.
      ...
      END IF
   !
   !6------COMPUTE DEPTH AND WIDTH FROM LOG INTERPOLATION.
      dstglw = DLOG10(stglw)
      dstghi = DLOG10(stghi)
      dwthlw = DLOG10(wthlw)
      dwthhi = DLOG10(wthhi)
...
END SUBROUTINE GWF2SFR7TBD
   !
   !-------SUBROUTINE GWF2SFR7TBF
SUBROUTINE GWF2SFR7TBF(Flow, Depth, Width, Nstrpts, Nreach, Istsg,&
&Kkiter, Itb)
   !     *******************************************************************
   !     COMPUTE FLOW AND WIDTH IN STREAM GIVEN DEPTH USING RATING TABLES.
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     *******************************************************************
   USE GWFSFRMODULE, ONLY: QSTAGE
   ...
   wthhi = QSTAGE(3*Nstrpts, Istsg)
   !
   !2------USE A LINEAR INTERPOLATION TO ESTIMATE FLOW AND WIDTH WHEN
   !         DEPTH IS LESS THAN LOWEST VALUE IN TABLE.
   IF ( Depth.LE.stglw ) THEN
      Flow = (flwlw/stglw)*Depth
      Width = (wthlw/flwlw)*Flow
   !
   !3------OTHERWISE USE A LOG INTERPOLATION TO ESTIMATE FLOW AND WIDTH.
   ...
   !
   !5------PRINT WARNING IF COMPUTED DEPTH EXCEEDS HIGHEST DEPTH IN TABLE.
...
&Ischk, Nischk, Ichk, Kkper, Nsol)
   ...
   !     ------------------------------------------------------------------
   !
   !1------READ STREAM SEGMENT DATA.
   lstend = Lstbeg + Nlst - 1
   DO iqseg = Lstbeg, lstend
   !
   !2------ONLY READ FIRST 4 VARIABLES TO DETERMINE VALUE OF IUPSEG.
...
9003        FORMAT (1X, 'READING ENTRY ', I6, ' OF ITEM 4A')
      ...
      END IF
   !
   !3------DETERMINE WHERE DATA ARE STORED.
      IF ( Ichk.NE.0 ) THEN
   !  Store data in active segment area
         nseg = n
         Ischk(n) = Ischk(n) + 1
      ELSE
   !  Store data in parameter area
         nseg = iqseg
         ISEG(3, iqseg) = n
         SEG(1, nseg) = SEG(1, n)
      END IF
      BACKSPACE In
   !
   !4------READ DATA SET 4B FOR SEGMENTS THAT ARE NOT DIVERSIONS.
      IF ( iupseg.LE.0 ) THEN
         IF ( icalc.LE.0 ) THEN
         ...
         END IF
   !
   !5------READ DATA 4B FOR SEGMENTS THAT ARE DIVERSIONS FROM STREAMS.
   ...
   !7------READ DATA SET 4D.
      ...
      END IF
   !
   !8------READ DATA SET 4E FOR SEGMENT WHEN ICALC IS 2.
      IF ( icalc.EQ.2 ) THEN
   !       ADDED CONDITIONAL IF WHEN UNSATURATED FLOW INACTIVE DEP
      ...
      END IF
   !
   !9------READ DATA SET 4F FOR SEGMENT WHEN ICALC IS 4.
      IF ( icalc.EQ.4 ) THEN
         nstrpts = ISEG(2, nseg)
...
9005        FORMAT (/1X, 'FOR SEGMENT ', I6, ' NUMBER OF POINTS',&
      ...
      END IF
   !
   !10-----READ DATA SET 4G FOR SEGMENT IF SOLUTES SPECIFIED.
      IF ( Iunitgwt.GT.0 ) THEN
         DO isol = 1, Nsol
...
END SUBROUTINE SGWF2SFR7RDSEG
   !
   !-------SUBROUTINE SGWF2SFR7PARMOV
SUBROUTINE SGWF2SFR7PARMOV(In, Iunitgwt, Nsol)
   !     ******************************************************************
   !     MOVE STREAM PARAMETER DATA INTO ACTIVE SEGMENTS
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
   ...
   package = 'SFR '
   !
   !1------READ PARAMETER NAME AND FIND IT IN THE PARAMETER LIST.
   READ (In, '(A)') line
   lloc = 1
...
9001 FORMAT (/, ' Parameter:  ', A)
   ...
   !
   !2------DESIGNATE CELLS CORRESPONDING TO CORRECT PARAMETER INSTANCE.
...
9005  FORMAT (/, 1X, '*** ERROR: PARAMETER "', A,&
   ...
   IACTIVE(ip) = ni
   !
   !3------MOVE EACH ENTRY FOR THE PARAMETER.
   lstbeg = IPLOC(1, ip) + (ni-1)*nlst
   ! changed iqseg to lstbeg in the following line. 4/25/06
   lstend = lstbeg + nlst - 1
   DO iqseg = lstbeg, lstend
   !
   !4------DETERMINE VALUES OF ICALC, NSEG, AND IUPSEG.
      icalc = ISEG(1, iqseg)
      nseg = ISEG(3, iqseg)
      iupseg = IDIVAR(1, iqseg)
   !
   !5------COUNT THE NUMBER OF TIMES A SEGMENT IS DEFINED.
   ...
   !
   !9------MOVE DATA SET 4D FOR SEGMENT WHEN ICALC IS 2.
      IF ( icalc.EQ.2 ) THEN
         DO jj = 1, 16
      ...
      END IF
   !
   !10-----MOVE DATA SET 4E FOR SEGMENT WHEN ICALC IS 4.
      IF ( icalc.EQ.4 ) THEN
         nstrpts = ISEG(2, nseg)
         DO jj = 1, nstrpts*3
      ...
      END IF
   !
   !11-----MOVE DATA SET 4F FOR SEGMENT IF SOLUTES SPECIFIED.
      IF ( Iunitgwt.GT.0 ) THEN
         DO isol = 1, Nsol
            IF ( IDIVAR(1, nseg).EQ.0 ) THEN
...
END SUBROUTINE SGWF2SFR7PARMOV
   !
   !-------SUBROUTINE SGWF2SFR7PRSEG
SUBROUTINE SGWF2SFR7PRSEG(Nlst, Lstbeg, Iunitgwt, Kkper, Nsol,&
&Iouts)
   !     ******************************************************************
   !     PRINT STREAM SEGMENT DATA -- parameters or non parameters
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
...
9001 FORMAT (1X, //20X, 'INPUT DATA FOR EACH STREAM SEGMENT', /1X,&
   &93('-')/)
   !
   !1------PRINT INPUT FLOW RATES FOR EACH STREAM SEGMENT.
   WRITE (IOUT, 9002)
...
9003  FORMAT (1X, I6, 1X, 1P5E10.3, 2X, I3, 3X, I6, 3X, I6, 4X, I5)
   END DO
   !
   !2------PRINT STREAMBED PROPERTIES AND STREAM DIMENSIONS.
   IF ( Lstbeg.EQ.1 ) THEN
      IF ( ISFROPT.EQ.0 ) THEN
         WRITE (IOUT, 9004)
      ...
      ELSE IF ( ISFROPT.EQ.5 ) THEN
         ...
         END IF
      END IF
   ! rgn added else and write statement.
   ELSE
      WRITE(IOUT,210)
210   FORMAT (1X,//9X,'STREAMBED PROPERTIES AND STREAM ',&
      &'DIMENSIONS',//1X,'SEGMENT  BED HYD. COND. FACTOR',2X,&
...
9024        FORMAT (I6, 1X, 1P6E10.3)
   ...
   END DO
   !
   !4------PRINT CROSS-SECTIONAL DATA FOR SEGMENTS WITH ICALC=2.
   iflg = 0
   DO nseg = Lstbeg, lstend
      IF ( Lstbeg.EQ.1 ) THEN
         nn = nseg
...
9028     FORMAT (7X, I6, 5X, 8(1PE10.3))
   ...
   END DO
   !
   !5------PRINT STREAMFLOW, DEPTH AND WIDTH RELATIONS FOR SEGMENTS
   !         WITH ICALC=3.
   iflg = 0
   DO nseg = Lstbeg, lstend
      IF ( Lstbeg.EQ.1 ) THEN
...
9030     FORMAT (5X, I6, 1P4E10.3)
   ...
   END DO
   !
   !6------PRINT TABULATED VALUES FOR COMPUTING STREAM WIDTH AND DEPTH
   !         FROM STREAMFLOW FOR SEGMENTS WITH ICALC=4.
   iflg = 0
   DO nseg = Lstbeg, lstend
      IF ( Lstbeg.EQ.1 ) THEN
         nn = nseg
...
9032        FORMAT (5X, I6, 2X, 3(1PE13.4)) !gsf
   ...
   END DO
   !
   !7------PRINT SOLUTE DATA FOR EACH STREAM SEGMENT.
   IF ( Iunitgwt.GT.0 ) THEN
      DO isol = 1, Nsol
...
END SUBROUTINE SGWF2SFR7PRSEG
   !
   !-------FUNCTION CALCUNSATFLOBOT written by RGN, MAY 24, 2004
...
END FUNCTION CALCUNSATFLOBOT
   !
   !-------SUBROUTINE CALC_UNSAT_INFIL written by RGN, MAY 24, 2004
...
&Strlen, Iwidthcheck, Icalc)
   !     ******************************************************************
   !     DEFINE UNSATURATED CELLS TO ACCOMMODATE STREAM LOSS.
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
...
END SUBROUTINE CALC_UNSAT_INFIL
   !
   !-------SUBROUTINE UZMASSBAL written MAY 24, 2004
...
&Iunitgage, Gwflow)
   !     ******************************************************************
   !     COMPUTE INFLOW, OUTFLOW, AND CHANGE IN STORAGE IN UNSATURATED
   !     ZONE BENEATH STREAMBED.
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
   ...
   IF ( htest1.GE.-CLOSEZERO .AND. htest2.GE.-CLOSEZERO ) THEN
      ...
      END IF
   !
   !2------REMOVE ALL UNSATURATED ZONE WAVES AND CALCULATE CHANGE IN
   !         STORAGE WHEN WATER TABLE RISES TO ELEVATION OF STREAMBED.
   ELSE IF ( htest1.GE.-CLOSEZERO .AND. htest2.LT.-CLOSEZERO ) THEN
      ...
      END IF
   !
   !3------CALCULATE CHANGE IN STORAGE AND UPDATE UNSATURATED ZONE WAVES
   !         WHEN WATER TABLE REMAINS BELOW STREAMBED ELEVATION.
   ...
   ELSE IF ( htest1.LT.-CLOSEZERO .AND. htest2.LT.-CLOSEZERO ) THEN
      ...
      DO i = 1, iuznhold
         IF ( loop(i).GT.0 ) THEN
   !
   !4------CALCULATE CHANGE IN UNSATURATED ZONE STORAGE WHEN WATER TABLE
   !         RISES.
            IF ( H.GT.Hld ) THEN
               fm = 0.0D0
               ...
               jk = iset + 1
   !
   !5------WATER TABLE RISES THROUGH WAVES.
               IF ( jj.GE.jk ) THEN
                  ...
                  END DO
   !
   !6------LOOP THROUGH NUMBER OF TRAIL WAVES INTERSECTED BY WATER TABLE.
                  DO j = iset, jj + 1
                     IF ( j.EQ.jj+1 ) THEN
                        IF ( ITRLSTH(j).GT.0 ) THEN
   !
   !7------LEAD TRAIL WAVE BELOW WATER TABLE AND FIRST TRAIL WAVE IS
   !         ABOVE WATER TABLE.
                           ...
                           ELSE
   !
   !8------LEAD TRAIL WAVE BELOW WATER TABLE AND MULTIPLE TRAIL WAVES
   !         ABOVE WATER TABLE.
   ...
   !
   !9------MULTIPLE TRAIL WAVES BELOW AND ABOVE WATER TABLE.
                        ...
                        END IF
   !
   !10-----ONLY ONE LEAD TRAIL AND ONE TRAIL WAVE BELOW WATER TABLE
   !         AND THERE ARE MUTIPLE TRAIL WAVES IN SET ABOVE WATER TABLE.
                  ...
                  END DO
   !
   !11-----DETERMINE VOLUME OF WATER IN WAVES BELOW WATER TABLE.
                  fm = 0.0D0
                  j = iset
   ...
   !
   !12-----COMPUTE VOLUME OF WATER BELOW WATER TABLE WHEN
   !         WHEN NO WAVES INTERSECTED.
               ELSE
                  fm = 0.0D0
                  j = iset
                  Uzdpst(iset) = Uzdpst(iset) - (H-Hld)
               ...
               uzinfiltot = uzinfiltot + Uzseep(i)
   !
   !13-----CALCULATE CHANGE IN UNSATURATED ZONE STORAGE WHEN GROUND-
   !         WATER LEVEL DROPS.
      ...
      END IF
   !
   !14-----UPDATE ALL UNSATURATED ZONE WAVES WHEN WATER TABLE DROPS
   !         BELOW STREAMBED.
   ...
   END IF
   !
   !16-----TOTAL WATER CONTENT OVER SPECIFIED DEPTH
   !         FOR PRINTING WATER CONTENT PROFILES.
   IF ( Ibd.NE.0 .AND. Iunitgage.GT.0 ) THEN
      ...
      END IF
   END IF
   !17-----STORE UNSATURATED FLOW RATES FOR GAGE PACKAGE.
...
&Uzflit, Uzdpit, Uzthit, Deltinc, Sbot)
   !     ******************************************************************
   !     ROUTE UNSATURATED ZONE WAVES DURING MODEL ITERATIONS
   !     CALLED FROM SUBROUTINE GWF2SFR7FM
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
   !
...
END SUBROUTINE ROUTWAVESIT
   !
   !-------SUBROUTINE ROUTWAVESST
SUBROUTINE ROUTWAVESST(L, Seep, H, Hld, Thr, Thetas, Fks, Epsilon,&
&Iwidthcheck, Sbot, Icalc, Nwavst, Uzwdth,&
&Uzflwt, Uzolsflx, Uzseep, Itrlst, Ltrlst,&
&Uzspst, Uzflst, Uzdpst, Uzthst, Deltinc)
   !     ******************************************************************
   !     ROUTE UNSATURATED-ZONE WAVES AFTER FINAL ITERATION
   !     CALLED FROM SUBROUTINE GWF2SFR7BD
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
...
&Jpnt, Deltinc)
   !     ******************************************************************
   !     WAVE INTERACTION WITHIN AN UNSATURATED FLOW COMPARTMENT
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
   ...
   itrailflg = 0
   !
   !       FEPS IS USED TO SUPPRESS A NEW WAVE WHEN CHANGES IN WATER TABLE
   !       ARE NEGLIGIBLE. FEPS2 IS USED TO SUPPRESS A NEW WAVE WHEN
   !       CHANGES IN FLUX ARE NEGLIGIBLE.
   IF ( feps.LT.1.0D-8 ) feps = 1.0D-8
   IF ( feps2.LT.1.0D-8 ) feps2 = 1.0D-8
   fm = 0.0D0
   Oldsflx = Flux(Jpnt+Numwaves-1)
   !
   !1------DETERMINE IF WATER TABLE IS RISING OR FALLING.
   ...
   !
   !2------CREATE A NEW WAVE IF SURFACE FLUX CHANGES.
   !         CALL TRAILWAVE IF SURFACE FLUX DECREASES.
   !         CALL LEADWAVE IF SURFACE FLUX INCREASES.
   ...
   END IF
   !
   !3------CALCULATE VOLUME OF WATER IN UNSATURATED ZONE LOST WHEN
   !         WATER TABLE ROSE AND ADD AS RECHARGE TO GROUND WATER.
...
&Feps2, Itrailflg, Deltinc)
   !     ******************************************************************
   !     CREATE LEAD WAVE WHEN THE SURFACE FLUX INCREASES AND ROUTE WAVES.
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
   !
   ...
   END IF
   !
   !2------ROUTE ALL WAVES AND INTERCEPTION OF WAVES OVER TIME STEP.
   diff = 1.0
   iflx = 0
   FLUXHLD2 = Flux(Jpnt)
   ...
   DO WHILE ( diff.GT.1.0E-7 .AND. Itester.NE.1 )
      ...
      j = 2
   !
   !3------CALCULATE TIME UNTIL A WAVE WILL OVERTAKE NEXT WAVE BELOW.
   ...
   !
   !4------LEAD WAVE INTERSECTING TRAIL WAVE.
                     fhold = 0.0D0
      ...
      END DO
   !
   !5------CALCULATE HOW LONG IT WILL TAKE BEFORE DEEPEST WAVE REACHES
   !         WATER TABLE.
      IF ( Numwaves.GT.1 ) THEN
      ...
      ELSE
         bottomtime = big
      END IF
   !6------CALCULATE SHORTEST TIME FOR WAVE INTERCEPTION.
   ...
   !
   !7------CHECK IF DEEPEST WAVE REACHES WATER TABLE BEFORE WAVES
   ...
   !dep check to see if theta = thetar do not route?
   ...
   !
   !9------MAKE ALL TRAIL WAVES LEAD TRAIL WAVES.
         ...
         Speed(Jpnt) = 0.0D0
   !
   !10-----CHECK IF WAVES INTERCEPT BEFORE TIME STEP ENDS.
   ...
   !dep check to see if theta = thetar do not route?
         ...
         END DO
   !
   !12-----REMOVE WAVES THAT HAVE BEEN INTERCEPTED AND COMPUTE SPEED OF
   !         COMBINED WAVE.
         j = 3
         l = j
         iflag = 0
   ...
   !
   !13-----CONVERT TRAIL WAVES TO LEAD TRAIL WAVES WHEN LEAD TRAIL
   !         WAVE INTERSECTS A LEAD WAVE.
   ...
   !
   !15-----CALCULATE TOTAL FLUX TO WATER TABLE DURING REMAINING TIME IN
   !         STEP.
      ELSE
         j = 2
         DO WHILE ( j.LE.Numwaves )
   ...
   !dep check to see if theta = thetar do not route?
      ...
      IF ( iflx.EQ.1 ) THEN
         ...
         iflx = 0
      END IF
   !17-----REMOVE ARRAY ELEMENTS RESULTING FROM INTERCEPTED WAVES.
...
END SUBROUTINE LEADWAVE
   !
   !-------SUBROUTINE TRAILWAVE
SUBROUTINE TRAILWAVE(Numwaves, I, Flux, Theta, Speed, Depth,&
&Itrwave, Ltrail, Fksat, Eps, Thetas, Thetar,&
&Surflux, Jpnt)
   !     ******************************************************************
   !     INITIALIZE A NEW SET OF TRAIL WAVES WHEN SURFACE FLUX DECREASES.
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
   ...
   !1------INITIALIZE TRAIL WAVES WHEN SURFACE FLUX DECREASES.
   kk = 1
   FLUXHLD2 = Flux(Jpnt)
...
END SUBROUTINE TRAILWAVE
   !
   !-------SUBROUTINE CHANNELAREA
SUBROUTINE CHANNELAREA(Istsg, L)
   !     ******************************************************************
   !     COMPARTMENTALIZE UNSATURATED ZONE BENEATH STREAMBED ON BASIS OF
   !     EIGHT POINT CROSS SECTION WHEN ICALC IS 2
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !     ******************************************************************
   ...
   wetted = 0.0
   !
   !1------CALCULATE THE MAXIMUM AND MINIMUM CHANNEL ELEVATIONS.
   !
   fmin = XSEC(9, Istsg)
   fmax = XSEC(9, Istsg)
   DO i = 2, 8
   ...
   END DO
   !
   !2------CALCULATE WETTED PERIMETERS FOR INCREMENTAL RIVER DEPTHS.
   DO i = 1, ISUZN
      stage = dpthinc(i, L)
      area = 0.0
      wetted = 0.0
   !
   !3------DETERMINE POINTS THAT ARE BELOW STREAM STAGE.
      k = 0
      DO j = 2, 8
      ...
      END DO
   !
   !4------BREAK CHANNEL UP INTO A SERIES OF LINES BETWEEN POINTS
   !       AND CALCULATE EQUATION OF EACH LINE.
      DO ll = 1, k
   ...
   !
   !5------WETTED AREA ASSUMING A FLAT CHANNEL BOTTOM.
         IF ( ABS(chap).LT.1.0E-30 ) THEN
            area1 = (XSEC(mark(ll), Istsg)-XSEC(mark(ll)-1, Istsg))&
            &*(stage-ffmin)
         ELSE
   !
   !6------DETERMINE IF STREAM STAGE IS BETWEEN POINTS.
            ...
            ELSE
               ...
               xmid = (stage-b)/slope
   !
   !7------MOVE DOWN THE CHANNEL BANK OR UP OTHER SIDE.
   ...
   !8------CALCULATE WETTED PARIMETER.
            xx = ABS(xmid-XSEC(mark(ll), Istsg))
            yy = ABS(ffmax-ffmin)
            wetted = wetted + SQRT((xx**2)+(yy**2))
   !9------BREAK AREA UP INTO TRAPAZOIDS.
            DO ii = 1, 50
               y1 = slope*xmid + b
...
&Chanstor,depth)
   !***********************************************************************
   !     IMPLICIT FINITE-DIFFERENCE SCHEME TO ROUTE FLOW DOWN CHANNELS
   !--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
   !***********************************************************************
   ...
   END IF
   !
   !1------CHANGE SIGN OF FLOBOT BECAUSE IT IS A DISCHARGE FROM GROUND
   !         WATER AND IS STORED AS A NEGATIVE VALUE.
   flobot2 = -Flobot/Strlen
   !
   !2------MAKE AN INITIAL GUESS AT Qd.
   Qd = (Qc+Qb)/2.0
   !
   ...
   END IF
   !
   !4------CALCULATE FLOW IN CHANNELS--MAXIMUM ITERATIONS IS 50.
   CONVERGE: DO WHILE( i.LT.maxiter )
   !
   !5------LOOP THROUGH UNTIL delq LESS THAN TOLERANCE.
      IF ( DABS(delq).GT.tol .OR. i.EQ.1 ) THEN
         qd2 = Qd + dq
         qd3 = Qd + 2.0*dq
   !
   !6------CALCULATE VARIABLES ad1 and ad2 ON BASIS OF Icalc.
         IF ( Icalc.EQ.1 ) THEN
            IF ( Qcnst.GT.0 ) THEN
   ...
   !7------CALCULATE FLOW.
         ...
         END IF
   !
   !8------EXIT LOOP IF delq LESS THAN TOLERANCE.
      ELSE
         EXIT CONVERGE
      END IF
      i = i + 1
...
&Slope,Istsg,Nreach,Itstr,Width,depth)
   !***********************************************************************
   !     CALCULATE CHANNEL CROSS-SECTIONAL AREA FOR USE BY LMT
   !--------EDM
   !***********************************************************************
...
SUBROUTINE GWF2SFR7AD(In, Iunitlak, kkstp, kkper, Igrid)
   !     ******************************************************************
   !     DETERMINE SPECIFIED INFLOWS FOR TIME STEP BASED ON TABULAR VALUES
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   END IF
   !
   ! DEP moved the from SFR7FM
   ! DEP FXLKOT is now adjusted in LAK7FM for limiting to available lake water.
   !
   !2------COMPUTE INFLOW OF A STREAM SEGMENT EMANATING FROM A LAKE.
   DO istsg = 1, NSS
      IF ( (Iunitlak.GT.0) .AND. (IDIVAR(1,istsg).LT.0) ) THEN
         IF ( SEG(2,istsg).GT.CLOSEZERO ) THEN
            lk = IABS(IDIVAR(1, istsg))
   !3------CHECK IF LAKE OUTFLOW IS SPECIFIED AT A FIXED RATE.
            FXLKOT(istsg) = SEG(2, istsg)
...
REAL FUNCTION FLOWTERP (TIME,INUM)
   ...
   !     OF TIME TO CACULATE SPECIFIED INFLOW TO SEGMENTS.
...
END FUNCTION FLOWTERP
   !
   !------FUNCTION FXLKOT_TERP FOR SMOOTHING STREAM WIDTH DURING CHANNEL DRYING.
   !
...
&Timestep, KITER)
   !     *******************************************************************
   !-------- MARCH 8, 2017
   !     COMPUTE NET ACCRETION/DEPLETION OVER A SEGMENT FOR MODSIM.
   !     CAUTION: DOES NOT WORK WITH TRANSIENT ROUTING.
   !-------- May 28, 2017
   !     ADDING CODE TO RETURN WATER-LIMITED RELEASES FROM RESERVOIRS
   !     *******************************************************************
   ...
   FLOWOUT = 0.0D0
   !
   !1------LOOP OVER REACHES TO SET ACCRETIONS/DEPLETIONS
   !
   DO L = 1, NSTRM
   !
   !2------DETERMINE STREAM SEGMENT NUMBER.
      REACHNUMINSEG = REACHNUMINSEG + 1
      ISTSGOLD = ISTSG
      ISTSG = ISTRM(4, L)
   !
   !3------DIFFERENCE FLOW IN AND FLOW OUT OF SEGEMENT.
      IF( ISTSG /= ISTSGOLD ) THEN
         REACHNUMINSEG = 1
      END IF
   !
   !4------IF FIRST REACH IN SEGMENT THEN SET FLOWIN
      IF ( REACHNUMINSEG == 1 ) FLOWIN = STRM(10,L)
   !
   !5------IF LAST REACH IN SEGMENT THEN SET FLOWOT
   ...
   END DO
   !
   !6----GENERATE SOME DEBUG 'WATCHER' FILES
...
END SUBROUTINE SFR2MODSIM
   !
   !-------SUBROUTINE MODSIM2SFR
   !
SUBROUTINE MODSIM2SFR(Diversions)
   !     *******************************************************************
   !     APPLY DIVERSIONS/LAKE RELEASES CALCULATED BY MODSIM TO DIVERSION
   !     SEGMENTS.
   !--------MARCH 8, 2017
   !     *******************************************************************
   ...
   DO ISEG = 1, NSS
   !
   !4------APPLY DIVERSION AMOUNT TO SFR SEGMENT INFLOW.
   !
...
END SUBROUTINE OUTSEGFLOWS
   !-------SUBROUTINE GWF2SFR7DA
SUBROUTINE GWF2SFR7DA(IGRID)
   !  Save SFR data for a grid.
   USE GWFSFRMODULE
   !     ------------------------------------------------------------------
   !     ARGUMENTS
   !     ------------------------------------------------------------------
...
END SUBROUTINE GWF2SFR7DA
   !
   !-------SUBROUTINE GWF2SFR7PNT
SUBROUTINE SGWF2SFR7PNT(IGRID)
   !  Change SFR data to a different grid.
   USE GWFSFRMODULE
   !     ------------------------------------------------------------------
   !     ARGUMENTS
   !     ------------------------------------------------------------------
