...
!#define MFNWT
!#ifdef MFNWT
!#define DOUBLEPRECISION DOUBLEPRECISION
!#endif
!#ifndef MFNWT
!#define DOUBLEPRECISION REAL
!#endif
MODULE GWFSWIMODULE
   TYPE TSWIDE4
   ...
   !---------STORAGE FOR ADAPTIVE SWI TIME STEP SUMMARY
   ...
   !---------POINTERS WITH STORAGE VALUES FROM BCF, LPF, HUF, OR UPW
...
SUBROUTINE GWF2SWI2AR(In,Ibcf,Ilpf,Ihuf,Iupw,Igrid)
   !
   !     ******************************************************************
   !     ALLOCATE ARRAY STORAGE FOR SEA WATER INTRUSION (SWI2) PACKAGE
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
...
1  FORMAT(//1X,'SWI2 -- SWI PACKAGE, VERSION 2.0.0, 07/22/2013',&
   &/1X,50('-'),&
   &/1X,'SWI2 INPUT READ FROM UNIT',I3,//)
7  FORMAT(//1X,'SWI2 ERROR: ',&
   &'THE TOTAL NUMBER OF SURFACES LESS THAN 1')
2200  FORMAT(//1X,'SWI2 DATASET 1',/,1X,52('-'),&
   &/1X,'NUMBER OF SURFACES (NSRF):                    ',1X,I5,&
   &/1X,'TOTAL NUMBER OF ZONES:                        ',1X,I5,&
   &/1X,'DENSITY DISTRIBUTION OPTION (ISTRAT):         ',1X,I5,&
   &/1X,'  VARIABLE DENSITY FLOW -- ISTRAT = 0',&
   &/1X,'  STRATIFIED FLOW       -- ISTRAT = 1',&
   &/1X,'NUMBER OF SWI OBSERVATIONS (NOBS):            ',1X,I5,&
   &/1X,'ZETA OUTPUT FILE UNIT NUMBER (ISWIZT):        ',1X,I5,&
   &/1X,'BUDGET OUTPUT FILE UNIT NUMBER (ISWICB):      ',1X,I5,&
   &/1X,'OBSERVATION OUTPUT FILE UNIT NUMBER (ISWIOBS):',1X,I5,&
   &/1X,52('-'))
2300  FORMAT(//1X,'SWI2 DATASET 1 KEYWORD OPTIONS',/,1X,52('-'))
2310  FORMAT(1X,A50)
2320  FORMAT(1X,52('-'))
111 FORMAT('        ZETA SURFACE ',I2)
2100  FORMAT(//1X,'SWI2 PROCESS REQUIRES USE OF THE BCF, LPF,'&
   &1X,'OR HUF FLOW PACKAGES',//)
2110  FORMAT(//1X,16X,'SWI2 OBSERVATION LOCATIONS',&
   &/1X,' OBSERVATION',&
   &1X,'     LAYER',1X,'       ROW',1X,'    COLUMN',&
   &1X,'      OBSNAM',&
   &/1X,58('-'))
2120  FORMAT(1X,I12,3(1X,I10),1X,A12)
2140  FORMAT(//1X,13X,'SWI2 INVALID OBSERVATION LOCATIONS',&
   &/1X,59('-'))
2150  FORMAT(1X,' OBSERVATION',1X,I5,1X,A6,1X,I5,&
   &1X,'NOT BETWEEN',1X,I5,1X,'AND',1X,I5)
2160  FORMAT(1X,'        NONE')
   !     ------------------------------------------------------------------
   !
   !       + + + CODE + + +
   !
   !---------ALLOCATE VARIABLES - INITIALIZE IF POSSIBLE
   ...
   !              IF ( LEN(line(istart:istop)).EQ.0 ) EXIT
         ...
         &'UNRECOGNIZED KEYWORD:', line(istart:istop)
      END SELECT
   END DO

   !
   !---------CHECK TO SEE THAT NUMBER OF SURFACES IS AT LEAST 1
   IF ( NSRF.LT.1 ) THEN
      WRITE (IOUT,7)
   ...
   &IFIXED.NE.0 ) THEN
      ...
      IF ( NSWIOPT.NE.0 ) THEN
         WRITE (IOUT,2310)&
         &'DEBUG OPTION - ADDITION DATA DURING TIP/TOE     '
      END IF
      IF ( iadptflg.NE.0 ) THEN
         WRITE (IOUT,2310)&
         &'SWI2 ADAPTIVE TIME STEP OPTION (ADAPTIVE)       '
      END IF
      IF ( IFIXED.NE.0 ) THEN
         WRITE (IOUT,2310)&
         &'FIXED ZETA SURFACES OPTION   '
      END IF
      WRITE (IOUT,2320)
   END IF
   !
   !---------READ DATASET 2A - SOLVER DATA FOR BOTH SOLVERS
   CALL URDCOM(In, IOUT, line)
   lloc = 1
...
2210  FORMAT(//1X,'SWI2 DATASET 2A',/,1X,52('-'),&
   &/1X,'ERROR SPECIFYING NSOLVER - VALID VALUES ARE',&
   &100(:/1X,I2,1X,'=',A40))
2220  FORMAT(//1X,'SWI2 DATASET 2A',/,1X,52('-'),&
   &/1X,'SOLVER (NSOLVER):                             ',1X,I5,&
   &/1X,'  SOLVER =',1X,A40,&
   &/1X,'SOLVER PRINTOUT INTERVAL (IPRSOL):            ',1X,I5,&
   &/1X,'SOLVER OUTPUT FLAG (MUTSOL):                  ',1X,I5,&
   &/1X,'  0 = PRINTING EVERY ITERATION',&
   &/1X,'  1 = LIMITED (TOTAL NUMBER OF ITERATIONS)',&
   &/1X,'  2 = NO PRINTING',&
   &/1X,'  3 = ONLY IF CONVERGENCE FAILS',&
   &/1X,52('-'))
   !
   !---------ALLOCATE STORAGE FOR SOLVER DATA
   ALLOCATE(SWIDE4,SWIPCG)
   !
   !---------READ DATASET 2B IF NECESSARY
   SELECT CASE (NSOLVER)
    CASE (2)
      CALL URDCOM(In, IOUT, line)
      lloc = 1
      ...
      END IF
   !             WRITE DATASET 2B FOR PCG SOLVER
      WRITE (IOUT,2230) SWIPCG%MXITER, SWIPCG%ITER1,&
      &SWIPCG%NPCOND,&
      &SWIPCG%ZCLOSEPCG, SWIPCG%RCLOSEPCG,&
      &SWIPCG%RELAXPCG, SWIPCG%NBPOL,&
      &SWIPCG%DAMPPCG, SWIPCG%DAMPPCGT
   END SELECT
2230  FORMAT(//1X,'SWI2 DATASET 2B',&
   &/1X,'PRECONDITIONED CONJUGATE GRADIENT PARAMETERS',&
   &/1X,52('-'),&
   &/1X,'MAXIMUM NUMBER OF CALLS TO PCG (MXITER):      ',1X,I5,&
   &/1X,'MAXIMUM ITERATIONS PER CALL TO PCG (ITER1):   ',1X,I5,&
   &/1X,'MATRIX PRECONDITIONING TYPE (NPCOND):         ',1X,I5,&
   &/1X,'  1 = MODIFIED INCOMPLETE CHOLESKY',&
   &/1X,'  2 = NEUMAN POLYNOMIAL - THE MATRIX WILL BE SCALED',&
   &/1X,'ZETA CLOSURE CRITERION (ZCLOSE):    ',1X,G15.5,&
   &/1X,'RESIDUAL CLOSURE CRITERION (RCLOSE):',1X,G15.5,&
   &/1X,'RELAXATION FACTOR (RELAX):          ',1X,G15.5,&
   &/1X,'  ONLY USED WITH NPCOND = 1',&
   &/1X,'POLYNOMIAL PRECONDITIONER PARAMETER (NBPOL):  ',1X,I5,&
   &/1X,'  INTERNALLY CALCULATED IF NPCOND.NE.2',&
   &/1X,'STEADY-STATE DAMPING (DAMPPCG):     ',1X,G15.5,&
   &/1X,'TRANSIENT DAMPING (DAMPPCGT):       ',1X,G15.5,&
   &/1X,52('-'))
   !
   !---------ALLOCATE ARRAYS
   IF ( NOBS.GT.0 ) THEN
   ...
   !---------ALLOCATE AND INITIALIZE SWI ZONE BUDGET DATA
   ALLOCATE(NBDITEMS)
   NBDITEMS = 5
   ...
   END IF
   !
   !-------CONFIRM THAT VALID ALPHA AND BETA VALUES ARE SPECIFIED
   ...
   END IF
2240  FORMAT(//1X,'SWI2 ERROR:',1X,A,1X,'(',G10.3,')',1X,&
   &'MUST BE GREATER THAN 0.0 AND LESS THAN OR ',&
   &'EQUAL TO 1.0')
   !
   !-------READ DATASET 3B PARAMETERS
   NADPTMX  = 1
   NADPTMN  = 1
   ADPTFCT  = 1.0
   IF ( iadptflg.NE.0 ) THEN
      ...
      END IF
   END IF
2245  FORMAT(//1X,'SWI2 ERROR: NADPTMN (',I10,')',1X,&
   &'EXCEEDS NAPTMX (',I10,')')
2250  FORMAT(//1X,'SWI2 ERROR: ADPTFCT (',G10.3,')',1X,&
...
2255  FORMAT(//1X,'SWI2 DATASET 3A',/,1X,52('-'),&
   &/1X,'MAXIMUM TOE SURFACE SLOPE (TOESLOPE):    ',1X,G10.3,&
   &/1X,'MAXIMUM TIP SURFACE SLOPE (TIPSLOPE):    ',1X,G10.3,&
   &/1X,'EXCESS SURFACE SLOPE FRACTION (ALPHA):   ',1X,G10.3,&
   &/1X,'MINIMUM ZONE THICKNESS FRACTION (BETA):  ',1X,G10.3,&
   &/1X,52('-'))
2260  FORMAT(//1X,'SWI2 DATASET 3B',/,1X,52('-'),&
   &/1X,'MINIMUM SWI SUB-TIME STEPS  (NADPTMN):   ',1X,I5,&
   &/1X,'MAXIMUM SWI SUB-TIME STEPS  (NADPTMX):   ',1X,I5,&
   &/1X,'MAXIMUM ZETA SLOPE FRACTION (ADPTFCT):   ',1X,G10.3,&
   &/1X,52('-'))
2270  FORMAT(1X,'Invalid ISOURCE value (',I4,&
   &') at cell (layer,row,col)',I4,',',I4,',',I4)
   !
   !---------ALLOCATE AND INITIALIZE STORAGE FOR SWI ADAPTIVE TIME STEP SUMMARY
   ...
   !
   !         DATASET 4
   !---------READ DENSITY DATA BASED ON USER-SPECIFIED DENSITY MODEL
   !         CONSTANT ZONE DENSITY MODEL
   CALL SSWI2_RD_COMM(In)
   DENSITYMODEL: IF (ISTRAT.EQ.1) THEN
   !
   !-----------READ NU FOR EACH ZONE (NZONES)
      CALL U1DREL(NUS,ANAME(1),NZONES,In,IOUT)
   !-----------SET EPS EQUAL TO ZERO
      DO iz = 1, NZONES
         EPS(iz) = 0.0
      END DO
   !         LINEAR ZONE DENSITY MODEL
   ELSEIF (ISTRAT.EQ.0) THEN
   !
   !-----------READ NUSRF FOR EACH SURFACE (NZONES+1)
      CALL U1DREL(NUSRF,ANAME(2),NZONES+1,In,IOUT)
   !
   !-----------CALCULATE NU AND EPS FROM NUSRF
      DO iz = 1, NZONES
   ...
   !
   !---------DATASET 5
   !---------READ ZETA FOR EACH ACTIVE SURFACE (ZONES 2 TO NZONES)
   CALL SSWI2_RD_COMM(In)
   IZ_ZETA: DO iz=2,NZONES
      K_ZETA: DO k=1,NLAY
         ...
         &NROW,NCOL,kk,In,IOUT)
   !
   !-------------LOOP THROUGH EACH ROW AND COLUMN
   !             RESET ZETA IF SPECIFIED ZETA IS GREATER THAN THE TOP
   !             OF THE CURRENT CELL OR LESS THAN THE BOTTOM OF THE CURRENT
   !             CELL - IF ZETA IS RESET TO THE TOP OR BOTTOM BASED ON IF
   !             THE SPECIFIED ZETA VALUE IS CLOSER TO THE TOP OR BOTTOM
         d = SWISMALL !0.001
         I_ZETA: DO i = 1, NROW
            J_ZETA: DO j = 1, NCOL
   ...
   K_IZONENR: DO k = 1, NLAY
      ...
      &NROW,NCOL,kk,In,IOUT)
   !
   !-----------CHECK FOR INVALID ZONE NUMBERS
      DO i = 1, NROW
         DO j = 1, NCOL
   ...
   !-------------READ OBSERVATION DATA
         ...
         SWIOBS(n)%JCOL = j
      END DO
   !-------------WRITE OBSERVATION DATA TO IOUT
      WRITE (IOUT,2110)
      DO n = 1, NOBS
         WRITE (IOUT,2120)&
         &n, SWIOBS(n)%KLAY, SWIOBS(n)%IROW, SWIOBS(n)%JCOL,&
         &ADJUSTR(TRIM(SWIOBS(n)%OBSNAM))
      END DO
   !-----------CHECK FOR INVALID OBSERVATION LOCATIONS
      ierr = 0
      WRITE(IOUT,2140)
      DO n = 1, NOBS
         k = SWIOBS(n)%KLAY
   ...
   END IF
   !
   !---------ALLOCATE SPACE FOR THE SOLVER ARRAYS
   !         DIRECT SOLVER (DE4)
   SOLVERPARAM: IF ( NSOLVER.EQ.1 ) THEN
      SWIDE4%ITMX = 1
   !-----------SPECIFY DEFAULT DE4 SOLVER PARAMETERS
   ...
   !           INITIALIZE FLOAT ARRAYS
      ...
      DO i = 1, SWIDE4%MXEQ
         SWIDE4%D4B(i) = 0.0
      END DO
   !         PRECONDITIONED CONJUGATE GRADIENT SOLVER (PCG4)
   ...
   DO k = 1, NLAY
      ...
      ELSE IF ( Ihuf.GT.0 ) THEN
         ...
         ELSE
            WRITE (IOUT,'(A)')&
            &'CONVERTIBLE HUF LAYER INCONSISTENT WITH SWI2'
      ...
      ELSE
         WRITE (IOUT,'(A)')&
         &'FLOW PACKAGE SPECIFIED INCONSISTENT WITH SWI2'
         CALL USTOP('FLOW PACKAGE SPECIFIED INCONSISTENT WITH SWI2')
      END IF
      ICONV(k) = ic
   END DO
   !-------SET SC1 AND SC2 USED BY SWI2 TO CALCULATE MODFLOW STORAGE CHANGES
   IF ( Ibcf.GT.0 ) THEN
      SC1 => SC1B
      SC2 => SC2B
   ELSE IF ( Ilpf.GT.0 ) THEN
...
SUBROUTINE GWF2SWI2AD(Kkstp,Kkper,Igrid)
   !
   !     ******************************************************************
   !     SET FIRST AND LAST SURFACE TO TOP AND BOTOM OF LAYER ON FIRST TIME
   !     STEP OF THE FIRST STRESS PERIOD AND ADVANCE ZETAOLD AND ZETASWITS0
   !     TO ZETA EVERY TIME STEP.
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !--------SET FIRST AND LAST SURFACE EQUAL TO TOP AND BOTTOM OF THE AQUIFER
   ierr = 0
   IF ( Kkstp.EQ.1 .AND. Kkper.EQ.1 ) THEN
      DO k = 1, NLAY
         DO i = 1, NROW
            DO j = 1, NCOL
   !                 SET ZETA SURFACE FOR FIRST AND LAST SURFACE
               ttop=BOTM(j,i,LBOTM(k)-1)
               bbot=BOTM(j,i,LBOTM(k))
      ...
      END DO
   !
   !-----------SET FLAG FOR LOCATION OF THE ZETA SURFACE RELATIVE
   !           TO THE TOP AND BOTTOM OF A CELL
   !           IPLPOS=1 AT TOP, IPLPOS=2 AT BOTTOM, IPLPOS=0 IN BETWEEN, AND
   !           IPLPOS=3 IN INACTIVE CELLS
...
36 FORMAT(1X,'Top elevation, bottom elevation:',1P,2G13.5)
   !---------TERMINATE IF ANY CELL THICKNESSES ARE NEGATIVE
   IF ( ierr.NE.0 ) THEN
      CALL USTOP('ERROR SWI: NEGATIVE CELL THICKNESSES')
   END IF
   !
   !---------UPDATE ZETA FOR UPPER SURFACE FOR CONVERTIBLE LAYERS
   DO k = 1, NLAY
      IF ( ICONV(k).EQ.0 ) CYCLE
      CALL SSWI2_UPZ1(k,1)
   END DO
   !
   !----------COPY ZETA TO ZETAOLD AND ZETASWITS0
   DO k = 1, NLAY
      iz0 = 2
...
SUBROUTINE GWF2SWI2FM(Kkstp,Kkper,Kkiter,Igrid)
   !
   !     ******************************************************************
   !     ADD SWI2 PACKAGE TERMS TO RHS AND HCOF
   !
   !     SWI2 FORMULATE (GWF2SWI2FM) NEEDS TO BE THE LAST PACKAGE ENTRY
   !     SINCE SWI2 SAVES THE RHS (RHSPRESWI) PRIOR TO ADDING SWI TERMS
   !     RHSPRESWI IS USED TO CALCULATE BOUNDARY CONDITION FLUXES
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   &LBOTM,BOTM,&
   &CR,CC,CV,HCOF,RHS,BUFF,&
   &DELR,DELC,IBOUND,HNEW
   ...
   CALL SGWF2SWI2PNT(Igrid)
   !
   !---------COPY RHS TO RHSPRESWI FOR CALCULATING BOUNDARY FLUXES WHEN UPDATING ZETA
   KRHS: DO k=1,NLAY
      IRHS: DO i=1,NROW
         JRHS: DO j=1,NCOL
   ...
   END DO KRHS
   !
   !-------SET ZETA TO ZETA VALUE AT THE END OF THE LAST SWI TIME STEP IN THE
   !       LAST MODFLOW TIME STEP (ZETAOLD). ZETAOLD IS ASSGNED IN GWF2SWI2AD
   DO k = 1, NLAY
      DO i = 1, NROW
         DO j = 1, NCOL
   ...
   END DO
   !
   !-------EVALUATE IF THE NUMBER OF SWI TIME STEPS PER MODFLOW
   !       TIME STEP SHOULD BE REDUCED. REDUCTION IN THE NUMBER
   !       OF SWI TIME STEPS PER MODFLOW TIME STEP CAN ONLY
   !       OCCUR ON THE FIRST CALL TO GWF2SWI2FM IN EACH MODFLOW
   !       TIME STEP
      ...
      IF ( IADPT.GT.1 .AND. Kkiter.EQ.1 ) THEN
         IF ( IADPT.GT.NADPTMN ) THEN
            IF ( IADPTMOD.LT.0 ) THEN
               WRITE (IOUT,2000)&
               &'PREVIOUS SWI DELT:  ', IADPT, DELT, SWIDELT
               r = REAL( IADPT, 4 ) / 2.0
               ...
               SWIDELT  = DELT * switfact
               WRITE (IOUT,2000)&
               &'INCREASING SWI DELT:', IADPT, DELT, SWIDELT
            END IF
         END IF
      END IF
   END IF
2000 FORMAT(1X,A,&
   &1X,'IADPT      :',1X,I10,&
   &1X,'DELT       :',1X,G10.3,&
   &1X,'SWI DELT   :',1X,G10.3)
   !
   !-------ADAPTIVE TIME STEP LOOP
   iswi = 1
   ADAPTIVE: DO
   !
   !-----------COMPUTE HORIZONTAL (CUMULATIVE) CONDUCTANCE FOR EACH ZONE
   ...
   !           IPLPOS=3 IN INACTIVE CELLS
      CALL SSWI2_SET_IPLPOS()
   !
   !-----------SET ZONE CONDUCTANCES TO ZERO IF NOT CELL ON EITHER SIDE HAS ACTIVE ZONE
      KACTIVE: DO k=1,NLAY
         IACTIVE: DO i=1,NROW
            JACTIVE: DO j=1,NCOL
   ...
   !-----------COMPUTE NUTOP AND NUBOT
      ...
      END DO KCALCNU
   !
   !-----------CALCULATE QREXTRA and QFEXTRA and QLEXTRA
      KQEX: DO k=1,NLAY
         IQEX: DO i=1,NROW
            JQEX: DO j=1,NCOL
               QREXTRA(j,i,k)=0
               ...
               IF (k.GT.1) THEN
                  ...
                  END IF
               END IF
   !           ADD QREXTRA, QFEXTRA and QLEXTRA TO QREXTRACUM, QFEXTRA and QLEXTRA
               IF (k.GT.1) THEN
                  QLEXTRACUM(j,i,k-1) = QLEXTRACUM(j,i,k-1) +&
                  &QLEXTRA(j,i,k-1) * switfact
               END IF
               QFEXTRACUM(j,i,k) = QFEXTRACUM(j,i,k) +&
               &QFEXTRA(j,i,k) * switfact
               QREXTRACUM(j,i,k) = QREXTRACUM(j,i,k) +&
               &QREXTRA(j,i,k) * switfact
            END DO JQEX
         END DO IQEX
      END DO KQEX

   !
   !-------ADD DENSITY CORRECTIONS TO RHS
      KDDIFF: DO k=1,NLAY
         IDDIFF: DO i=1,NROW
            JDDIFF: DO j=1,NCOL
   ...
   !---------------LOWER FACE
      ...
      END DO KDDIFF
   !
   !---------ADAPTIVE SWI TIME STEP UPDATE OF ZETA SURFACE
   !         OR SIMULATING STEADY STATE ZETA SURFACE FOR A TIME STEP
   !         IN A STEADY-STATE STRESS PERIOD
      ADPUPZTST: IF ( NADPTFLG.NE.0 .AND. IFIXED.NE.1 ) THEN
   !
   !-----------STORE TOTAL CONSTANT HEAD FLUXES in BUFF
         CALL SSWI2_BDCH(1)
   !
   !-----------UPDATE ZETA SURFACE BASED ON CURRENT HEAD SOLUTION
   !           PRIOR TO SOLVING FOR NEW HEAD SOLUTION IN SELECTED
   !           GWF PROCESS SOLVER
         CALL SSWI2_UPDZ(Kkstp,Kkper)
   !
   !-----------EVALUATE IF THE NUMBER OF SWI TIME STEPS PER MODFLOW
   !           TIME STEP SHOULD BE INCREASED AND THE SWI SOLUTION
   !           RESTARTED AT THE FIRST SWI TIME STEP IN THE CURRENT
   !           MODFLOW TIME STEP.
         IF ( IADPTMOD.GT.0 .AND. IADPT.NE.NADPTMX ) THEN
            IF ( ADPTVAL.GT.1.0E0 ) THEN
               WRITE (IOUT,2010)&
               &'PREVIOUS SWI DELT:  ', IADPT, DELT, SWIDELT, ADPTVAL
               r = REAL( IADPT, 4 )
               r = r * ADPTVAL
               IF ( r.GT.REAL( NADPTMX, 4 ) ) THEN
                  r = REAL( NADPTMX, 4 )
               END IF
   !---------------RESET RHS AND ZETA IF CHANGE IN ZETA EXCEEDS
               DO k = 1, NLAY
                  DO i = 1, NROW
                     DO j = 1, NCOL
                        IF ( ABS( IBOUND(j,i,k) ).EQ.0 ) CYCLE
                  ...
                  END DO
               END DO
   !---------------UPDATE IADPT, IADPTMOD, AND SWIDELT
               IADPT    = INT( CEILING( r ) )
               IADPTMOD = 0
               switfact = 1.0 / REAL( IADPT, 4 )
               SWIDELT  = DELT * switfact
               WRITE (IOUT,2000)&
               &'DECREASING SWI DELT:', IADPT, DELT, SWIDELT
   !---------------RESTART SWI CALCULATIONS AT THE BEGINNING
               iswi = 1
               ADPTVAL = 1.0
               CYCLE ADAPTIVE
            END IF
         END IF
   !           RESET IADPTMOD AND ADPTVAL
         IADPTMOD = -1
         ADPTVAL  = 1.0
   !
   !-----------MOVE TIPS AND TOES OF ZETA SURFACES
...
SUBROUTINE GWF2SWI2BD(Kkstp,Kkper,Igrid)
   !
   !     ******************************************************************
   !     CALCULATE GLOBAL SWI2 BUDGET TERMS FOR MODFLOW, CALCULATE ZONE
   !     BUDGETS, SAVE ZETA, SAVE SWI2 CBC DATA, AND SAVE SWI2 OBSERVATIONS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   &LBOTM,BOTM,&
   &CR,CC,CV,HCOF,RHS,&
   &DELR,DELC,IBOUND,HNEW,HOLD,&
   &BUFF,ISSFLG,NSTP
   USE GWFBASMODULE, ONLY: DELT,PERTIM,TOTIM,HDRY,&
   &IHDDFL,IBUDFL,ICBCFL,IOFLG,&
   &MSUM,VBVL,VBNM
   USE GWFSWIMODULE
   IMPLICIT NONE
   !       + + + DUMMY ARGUMENTS + + +
...
3000 FORMAT(//22X,'VOLUMETRIC SWI ZONE BUDGET FOR ENTIRE MODEL',/,&
   &21X,'AT END OF TIME STEP',I5,' IN STRESS PERIOD',I4,/,&
   &38X,'ZONE',1X,I5,/,2X,83('-'))
...
3100 FORMAT(1X,83('-'),//)
   ...
   dzero  = 0.0D0
   !
   !---------FILL STORAGE FOR SWI ADAPTIVE TIME STEP SUMMARY
   IF ( NADPTFLG.NE.0 .AND. IFIXED.NE.1 ) THEN
      NADPTSUM(IADPTSUM) = IADPT
      RADPTSUM(IADPTSUM) = SWIDELT
      IADPTSUM  = IADPTSUM + 1
   !           WRITE SUMMARY OF SWI ADAPTIVE TIME STEPS
      IF ( Kkper.EQ.NPER .AND.&
      &Kkstp.EQ.NSTP(Kkper) ) THEN
         WRITE (IOUT,3080)
         n = 0
         DO i = 1, NPER
            DO j = 1, NSTP(i)
   ...
   END IF
   !
   !-------DETERMINE IF ZETA VALUES SHOULD BE OUTPUT
   iozeta = 0
   IF ( IHDDFL.GT.0 ) THEN
      DO k = 1, NLAY
   ...
   ibdlbl = 0
   !
   !-------IF CELL-BY-CELL TERMS WILL BE SAVED AS A 3-D ARRAY, THEN CALL
   !       UTILITY MODULE UBUDSV TO SAVE THEM.
   !
   !---------STORE CONSTANT HEAD CORRECTION FLUXES in BUFF
   CALL SSWI2_BDCH(0)
   ...
   !-------------PRINT THE INDIVIDUAL RATES IF REQUESTED(IGHBCB<0).
            IF( ibd.LT.0 ) THEN
   ...
   !---------------FLOW IS OUT OF AQUIFER SUBTRACT RATE FROM RATOUT.
               ratout = ratout - rrate
            ELSE
   !---------------FLOW IS INTO AQUIFER; ADD RATE TO RATIN.swiaddtofrf
               ratin = ratin + rrate
            END IF
         END DO
      END DO
   END DO
   !
   !-------MOVE RATES, VOLUMES AND LABELS INTO ARRAYS FOR PRINTING.
   rin          = ratin
   rout         = ratout
   VBVL(3,MSUM) = rin
   ...
   VBNM(MSUM)   = textch(1)
   !
   !-------INCREMENT THE BUDGET TERM COUNTER.
   MSUM = MSUM + 1
   !
   !---------REINITIALIZE BUFF
   DO k = 1, NLAY
   ...
   CALL SSWI2_BDCH(1)
   !
   !---------UPDATE ZETA SURFACE IF NON-ADAPATIVE
   !         SWI TIME STEPPING USED
   IF ( NADPTFLG.EQ.0 .AND. IFIXED.NE.1 ) THEN
      CALL SSWI2_UPDZ(Kkstp,Kkper)
   END IF
   !
   !---------MOVE TIP AND TOES IF ORIGINAL NON-ADAPTIVE SWI TIME STEP
   !         APPROACH USED AND NOT SOLVING FOR A STEADY STATE ZETA
   !         SURFACE
   IF ( NADPTFLG.EQ.0 .AND. IFIXED.NE.1 ) THEN
   !
   !-----------WRITE ZETA TO UNFORMATTED FILE PRIOR TO TIP AND TOE TRACKING
   ...
   END IF
   !
   !---------WRITE FINAL ZETA TO UNFORMATTED FILE
   ...
   !                   CONSTANT HEAD
               ...
               END IF
   !
   !                  TIP-TOE ADJUSTMENT CHANGE IN ZONE VOLUME
               qtt  = REAL((ZONECHG2(j,i,k,iz)-ZONECHG1(j,i,k,iz)),8)
   !
   !                 FLOW-BASED CHANGE IN THE ZONE VOLUME
               qint = REAL( ZONECHG1(j,i,k,iz), 8 )
   !
   !                  INSTANTANEOUS ZONE MIXING
               qmix = REAL( ZONEIMIX(j,i,k,iz), 8 )
   !
   !                 DETERMINE ZONE NUMBER FOR BOUNDARY CONDITION
               ...
               IF ( (iusezone.LT.0) .AND. (q.GT.zero) ) THEN
                  iusezone = 1
               ENDIF
   !-----------------FIND HIGHEST ACTIVE ZONE IF THICKNESS IS ZERO
   ...
   !-----------------ADJUST qbnd USING qint TO REMOVE TOTAL ZONE CHANGE FROM
   !                 COMBINED BOUNDARY AND STORAGE TERM IF TRANSIENT SOLUTION
   ...
   !-----------MOVE RATES, VOLUMES AND LABELS INTO ARRAYS FOR PRINTING.
      ...
      END DO ZBUDGET
   !
   !-----------WRITE BUDGET FOR EACH ZONE TO LIST FILE IF NECESSARY
   ...
   END DO CZBUDGET
   !
   !---------WRITE OBSERVATION DATA TO OBSERVATION FILE
   IF ( NOBS.GT.0 .AND. ISWIOBS.NE.0 ) THEN
      iu = ABS( ISWIOBS )
   !           WRITE HEADER FOR ASCII FILE
      IF ( ISWIOBS.GT.0 ) THEN
         IF ( IOBSHEADER.LT.1 ) THEN
            ...
            DO n = 1, NOBS
               DO iz = 2, NZONES
                  ...
                  &ADJUSTR(TRIM(SWIOBS(n)%OBSNAM)), iz-1
               END DO
            END DO
         ...
         END IF
      END IF
   !           WRITE DATA TO OBSERVATION OUTPUT FILE
      BINOBS: DO n = 1, NOBS
         k = SWIOBS(n)%KLAY
         i = SWIOBS(n)%IROW
         j = SWIOBS(n)%JCOL
...
&Q,HNEW,ZETA,DELNUS,EPS,SWICUMCR,SWICR)
   ...
   !     ******************************************************************
   !     SWI2 PACKAGE - CALCULATE FLUX IN THE ROW DIRECTION
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !       + + + CODE + + +
   !
   !---------ENSURE HEAD IN CELL J-1 AND J IS GREATER THAN CELL BOTTOM - FOR MODFLOW-NWT
   kk = LBOTM(K)
   b1 = BOTM(J-1,I,kk)
   b2 = BOTM(J,I,kk)
   h1 = HNEW(J-1,I,k)
...
&Q,HNEW,ZETA,DELNUS,EPS,SWICUMCC,SWICC)
   !
   !     ******************************************************************
   !     SWI2 PACKAGE - CALCULATE FLUX IN THE COLUMN DIRECTION
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !       + + + CODE + + +
   !
   !---------ENSURE HEAD IN CELL I-1 AND I IS GREATER THAN CELL BOTTOM - FOR MODFLOW-NWT
   kk = LBOTM(K)
   b1 = BOTM(J,I-1,kk)
   b2 = BOTM(J,I,kk)
   h1 = HNEW(J,I-1,k)
   ...
   END IF
   !
   !---------CALCULATE Q IN COLUMN DIRECTION
...
SUBROUTINE SSWI2_SD(J,I,NCOL,NROW,IPOS,FAC,B,CRLAY,CCLAY,VAR)
   !
   !     ******************************************************************
   !     SWI2 PACKAGE - CALCULATE FLUX IN THE COLUMN AND ROW DIRECTIONS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !       + + + CODE + + +
   !
   !---------CALCULATE FLUX IN COLUMN DIRECTION
   !         (LEFT FACE)
   IF (J.NE.1) THEN
   ...
   END IF
   !
   !---------CALCULATE FLUX IN ROW DIRECTION
   !         (BACK FACE)
   IF (I.NE.1) THEN
...
SUBROUTINE SSWI2_ZETAADJ(Kkstp,Kkper)
   !
   !     ******************************************************************
   !     POST FLOW SOLUTION AND ZETA SOLUTION ADJUSTMENT OF ZETA SURFACES
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !     + + + CODE + + +
   !
   !---------CALCULATE INSTANTANEOUS ZONE MIXING
   CALL SSWI2_IMIX(ZONEIMIX)
   !
   !---------CALCULATE PRE TIP TOE TRACKING CHANGE IN ZONE THICKNESS (ZONECHG1)
   CALL SSWI2_ZCHG(ZONECHG1)
   !
   MOVETIPTOE: IF ( ALPHA.GT.0.0 ) THEN
   !
   !-----------VERTICAL MOVEMENT OF SURFACES WHEN A POSITIVE ALPHA IS SPECIFIED
      CALL SSWI2_VERTMOVE(Kkstp,Kkper)
   !
   !-----------HORIZONTAL MOVEMENT OF SURFACES WHEN A POSITIVE ALPHA IS SPECIFIED
      CALL SSWI2_HORZMOVE(Kkstp,Kkper)
   !
   !-----------CHECK WHETHER ANYWHERE THE THICKNESS GETS TOO THIN
      CALL SSWI2_ZETACLIP()
   !
   !-----------MODIFY ZETA ANYWHERE THE SURFACES ARE CROSSING
      CALL SSWI2_ZETACROSS()
   !
   !-----------ADJUST TIP AND TOE FOR CELLS WERE CURRENT CELL IS AT THE TOP OR BOTTOM
   !           AND THE ADJACENT CELL IS AT THE BOTTOM OR TOP, RESPECTIVELY.
      CALL SSWI2_ANTILOCKMIN(Kkstp,Kkper)
   !
   END IF MOVETIPTOE ! this loop done only when ALPHA > 0
   !
   !---------CALCULATE POST TIP TOE TRACKING CHANGE IN ZONE THICKNESS (ZONECHG2)
   CALL SSWI2_ZCHG(ZONECHG2)
   !
   !---------RESET ZETASWITS0 TO ZETA
...
SUBROUTINE SSWI2_VERTMOVE(Kkstp,Kkper)
   ...
   !-------------------SET TEMPORARY ZETA VARIABLES
               zt    = ZETA(j,i,k,1)
               zb    = ZETA(j,i,k,NZONES+1)
               zetac = ZETA(j,i,k,iz)
   !-------------------SKIP CELLS WHERE HEAD IS BELOW THE BOTTOM  OF THE LAYER
               ht1 = HNEW(J,I,K)
   ...
   !-------------------CALCULATE qztop
               ...
               IF ( k.GT.1 ) THEN
                  IF ( IBOUND(J,I,K-1).NE.0 ) THEN
   !-----------------------CALCULATE qztop IF HEAD IN OVERLYING LAYER (k-1) ABOVE THE CELL BOTTOM
   !                       AND HEAD IN LAYER k ABOVE THE CELL BOTTOM - FOR MODFLOW-NWT
                     kk1 = LBOTM(k-1)
                     kk2 = LBOTM(k)
                     bt1 = BOTM(J,I,kk1)
                     bt2 = BOTM(J,I,kk2)
                     ...
                     IF ( ht1.GE.bt1 .AND. ht2.GE.bt2 ) THEN
                        qztop = CV(j,i,k-1) *&
                        &(HNEW(j,i,k)-HNEW(j,i,k-1)) -&
                        &QLEXTRA(j,i,k-1)
                     END IF
                  END IF
               END IF
   !-------------------MOVE TO UPPER OR LOWER LAYER
      ...
      END DO VIZTIPTOE
   END DO VZTIPTOE
   !-----------WRITE ZETA TO UNFORMATTED FILE
   IF ( IHDDFL.GT.0 .AND. ISWIZT.LT.0 ) THEN
      DO iz = 2, NZONES
...
SUBROUTINE SSWI2_HORZMOVE(Kkstp,Kkper)
   !
   !     ******************************************************************
   !     HORIZONTAL MOVEMENT OF SURFACES
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !
   !-------------------ONLY EVALUATE TIP AND TOE MOVEMENT TO ADJACENT CELLS FOR CELLS WHERE
   !                   ZETA SURFACE IS NOT AT THE TOP (ILPOS=1) OR BOTTOM (ILPOS=2) OF THE LAYER
               LACTIVE: IF (IPLPOS(j,i,k,iz).EQ.0) THEN
   !--------------------ADJUST TIPS AND TOES FOR EACH COLUMN BUT THE FIRST AND LAST
   !                    d1, s1: for the current (active) tip- or toe-cell
   !                    d2, s2: for the adjacent inactive cell (left, right, back, front)
   !                    d0, s0: for the (possibly active) cell on the other side (right, left, front, back)
   ...
   !-----------------------RIGHT FACE
                  ...
                  END IF LCOL
   !
   !---------------------ADJUST TIPS AND TOES FOR EACH ROW BUT THE FIRST AND LAST
...
SUBROUTINE SSWI2_ZETACLIP()
   !
   !     ******************************************************************
   !     IF ZETA IS ABOVE TOP OR BELOW BOTTOM THEN CLIP THIS WATER VOLUME
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE SSWI2_ZETACROSS()
   !
   !     ******************************************************************
   !     CORRECT IF SURFACES HAVE CROSSED
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE SSWI2_ANTILOCKMIN(Kkstp,Kkper)
   ...
   !
   !-------------------ONLY EVALUATE TIP AND TOE MOVEMENT TO ADJACENT CELLS FOR CELLS WHERE
   !                   ZETA SURFACE IS AT THE TOP (ILPOS=1) OR BOTTOM (ILPOS=2) OF THE LAYER
...
SUBROUTINE GWF2SWI2DA(Igrid)
   !
   !     ******************************************************************
   !     DEALLOCATE SWI2 PACKAGE DATA FOR A GRID
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE SGWF2SWI2PNT(Igrid)
   !
   !     ******************************************************************
   !     SET POINTERS TO SWI2 PACKAGE DATA FOR A GRID
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE SGWF2SWI2PSV(Igrid)
   !
   !     ******************************************************************
   !     SAVE POINTERS TO SWI2 PACKAGE DATA FOR A GRID
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE SSWI2_BDCH(ITOTAL)
   !     ******************************************************************
   !     COMPUTE SWI2 FLOW CORRECTION FOR CONSTANT-HEAD CELLS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !
   !
   !-------LOOP THROUGH EACH CELL AND CALCULATE FLOW INTO MODEL FROM EACH
   !-------CONSTANT-HEAD CELL.
   KCH: DO k = 1, NLAY
      ICH: DO i = 1, NROW
         JCH: DO j = 1,NCOL
   !
   !-------------IF CELL IS NOT CONSTANT HEAD SKIP IT & GO ON TO NEXT CELL.
            IF (IBOUND(J,I,K).GE.0) CYCLE JCH
   !
   !-------------CLEAR VALUES FOR FLOW RATE THROUGH EACH FACE OF CELL.
            chch1 = zero
            chch2 = zero
            chch3 = zero
            chch4 = zero
   ...
   !-------------DO NOT CALCULATE FLOW THROUGH THE LEFT FACE IF HEAD IN CELL J-1
   !             IS LESS THAN THE CELL BOTTOM IN CELL J-1 - FOR MODFLOW-NWT
            kk = LBOTM(k)
            b1 = BOTM(J-1,I,kk)
            h1 = HNEW(J-1,I,k)
            IF ( h1.LT.b1 ) GOTO 30
   !
   !-------------CALCULATE FLOW THROUGH THE LEFT FACE INTO THE ADJACENT CELL.
...
30          IF ( j.EQ.NCOL ) GO TO 60
   ...
   !-------------DO NOT CALCULATE FLOW THROUGH THE RIGHT FACE IF HEAD IN CELL J+1
   !             IS LESS THAN THE CELL BOTTOM IN CELL J+1 - FOR MODFLOW-NWT
            kk = LBOTM(k)
            b1 = BOTM(J+1,I,kk)
            h1 = HNEW(J+1,I,k)
            IF ( h1.LT.b1 ) GOTO 60
   !
   !-------------CALCULATE FLOW THROUGH THE RIGHT FACE INTO THE ADJACENT CELL.
            hdiff = REAL( ( HNEW(j,i,k) - HNEW(j+1,i,k) ), 4 ) * rtotal
            chch2 = ( hdiff * CR(j,i,k) ) + QREXTRACUM(j,i,k)
   !
   !-------------CALCULATE FLOW THROUGH THE BACK FACE.
   !-------------IF THERE IS NO FLOW TO CALCULATE THROUGH THIS FACE, THEN GO ON
   !-------------TO NEXT FACE.  NO FLOW OCCURS AT THE EDGE OF THE GRID, TO AN
   !-------------ADJACENT NO-FLOW CELL, OR TO AN ADJACENT CONSTANT-HEAD CELL.
60          IF ( i.EQ.1 ) GO TO 90
   ...
   !-------------DO NOT CALCULATE FLOW THROUGH THE BACK FACE IF HEAD IN CELL I-1
   !             IS LESS THAN THE CELL BOTTOM IN CELL I-1 - FOR MODFLOW-NWT
            kk = LBOTM(k)
            b1 = BOTM(J,I-1,kk)
            h1 = HNEW(J,I-1,k)
            IF ( h1.LT.b1 ) GOTO 90
   !
   !-------------CALCULATE FLOW THROUGH THE BACK FACE INTO THE ADJACENT CELL.
            hdiff = REAL( ( HNEW(j,i,k) - HNEW(j,i-1,k) ), 4 ) * rtotal
            chch3 = ( hdiff * CC(j,i-1,k) ) - QFEXTRACUM(j,i-1,k)
   !
   !-------------CALCULATE FLOW THROUGH THE FRONT FACE.
   !-------------IF THERE IS NO FLOW TO CALCULATE THROUGH THIS FACE, THEN GO ON
   !-------------TO NEXT FACE.  NO FLOW OCCURS AT THE EDGE OF THE GRID, TO AN
   !-------------ADJACENT NO-FLOW CELL, OR TO AN ADJACENT CONSTANT-HEAD CELL.
90          IF ( i.EQ.NROW ) GO TO 120
   ...
   !-------------DO NOT CALCULATE FLOW THROUGH THE FRONT FACE IF HEAD IN CELL I+1
   !             IS LESS THAN THE CELL BOTTOM IN CELL I+1 - FOR MODFLOW-NWT
            kk = LBOTM(k)
            b1 = BOTM(J,I+1,kk)
            h1 = HNEW(J,I+1,k)
   ...
   !
   !-------------CALCULATE FLOW THROUGH THE FRONT FACE INTO THE ADJACENT CELL.
            hdiff = REAL( ( HNEW(j,i,k) - HNEW(j,i+1,k) ), 4 ) * rtotal
            chch4 = ( hdiff * CC(j,i,k) ) + QFEXTRACUM(j,i,k)
   !
   !-------------CALCULATE FLOW THROUGH THE UPPER FACE.
   !-------------IF THERE IS NO FLOW TO CALCULATE THROUGH THIS FACE, THEN GO ON
   !-------------TO NEXT FACE.  NO FLOW OCCURS AT THE EDGE OF THE GRID, TO AN
   !-------------ADJACENT NO-FLOW CELL, OR TO AN ADJACENT CONSTANT-HEAD CELL.
120         IF ( k.EQ.1 ) GO TO 150
   ...
   !-------------DO NOT CALCULATE FLOW THROUGH THE UPPER FACE IF HEAD IN CELL K-1
   !             IS LESS THAN THE CELL BOTTOM IN CELL K-1 - FOR MODFLOW-NWT
            kk = LBOTM(k-1)
            b1 = BOTM(J,I,kk)
            h1 = HNEW(J,I,k-1)
            IF ( h1.LT.b1 ) GOTO 150
   !
   !-------------CALCULATE FLOW THROUGH THE UPPER FACE INTO THE ADJACENT CELL.
            hd = HNEW(j,i,k)
...
122         hdiff = REAL( ( hd - HNEW(j,i,k-1) ), 4 ) * rtotal
   ...
   !
   !-------------CALCULATE FLOW THROUGH THE LOWER FACE.
   !-------------IF THERE IS NO FLOW TO CALCULATE THROUGH THIS FACE, THEN GO ON
   !-------------TO NEXT FACE.  NO FLOW OCCURS AT THE EDGE OF THE GRID, TO AN
   !-------------ADJACENT NO-FLOW CELL, OR TO AN ADJACENT CONSTANT-HEAD CELL.
150         IF ( k.EQ.NLAY ) GO TO 180
   ...
   !-------------DO NOT CALCULATE FLOW THROUGH THE LOWER FACE IF HEAD IN CELL K+1
   !             IS LESS THAN THE CELL BOTTOM IN CELL K+1 - FOR MODFLOW-NWT
            kk = LBOTM(k+1)
            b1 = BOTM(J,I,kk)
            h1 = HNEW(J,I,k+1)
   ...
   !
   !-------------CALCULATE FLOW THROUGH THE LOWER FACE INTO THE ADJACENT CELL.
            hd = HNEW(j,i,k+1)
...
152         hdiff = REAL( ( HNEW(j,i,k) - hd ), 4 ) * rtotal
   ...
   !
   !-------------SUM THE FLOWS THROUGH SIX FACES OF CONSTANT HEAD CELL, AND
   !-------------STORE SUM IN BUFFER.
...
SUBROUTINE SSWI2_UPDZ(Kkstp,Kkper)
   !
   !     ******************************************************************
   !     UPDATE ACTIVE ZETA SURFACES FOR SWI2 PACKAGE
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
2030 FORMAT('      ZETASRF ',I2)
   !
   !---------UPDATE THE UPPER ZETA SURFACE TO THE TOP OF THE WATER
   !         TABLE FOR UNCONFINED CONDITIONS PRIOR TO SOLVING FOR
   !         ZETA SURFACES
   DO k = 1, NLAY
      IF ( ICONV(k).EQ.0 ) CYCLE
      CALL SSWI2_UPZ1(k,0)
   ...
   !               ACTIVE CELL
            IF ( IBOUND(J,I,K).GT.0 ) THEN
   !                 DETERMINE IF THE HEAD IN CELL IS LESS THAN THE
   !                 BOTTOM OF THE CELL - FOR MODFLOW-NWT
               kk = LBOTM(k)
               dt = BOTM(J,I,kk)
               ht = HNEW(J,I,K)
               IF ( ht.LT.dt ) THEN
   ...
   !               CONSTANT HEAD
            ELSE IF ( IBOUND(J,I,K).LT.0 ) THEN
               q = -BUFF(j,i,k)
            END IF
   !               SET ZONE NUMBER FOR BOUNDARY CONDITIONS
            iusezone = IZONENR(j,i,k)
   ...
   !---------------------FRONT FACE
                  ...
                  END DO
   !
   !-------------------SPECIFY BOUNDARY CONDITION OF FLOW AT TIPS AND TOES
                  IF (IPLPOS(j,i,k,iz).EQ.0) THEN
   !---------------------LEFT FACE
                     IF (j.NE.1) THEN
   ...
   !---------------------LOWER FACE
      ...
      END DO IBRHS
   !
   !-----------SOLVE EQUATIONS FOR 1 LAYER
      IZSOLVZ4LAY: DO iz = 2, NZONES
         iactive = 0
   !             CALCULATE SWI CONDUCTANCES FOR CURRENT ZETA SURFACE
   !             IN CURRENT LAYER
   ...
   !
   !                 CALCULATE SWISOLCV AND SWIHCOF (STORAGE)
               SWISOLCV(j,i) = 0.
               SWIHCOF(j,i)   = -SSZ(j,i,k)*DELR(j)*DELC(i)/SWIDELT

               DUM(j,i)       = ZETA(j,i,k,iz)
   !
   !                 DETERMINE IF CURRENT CELL HAS ZETA VALUE GREATER THAN
   !                 THE BOTTOM AND LESS THAN THE TOP (IPLPOS=0)
               IF (IPLPOS(j,i,k,iz).EQ.0 .AND.&
               &IBOUND(j,i,k).NE.0 ) THEN
                  IBO(j,i) = 1
                  iactive = iactive + 1
               ELSE
                  IBO(j,i) = 0
         ...
         END DO ISOLVZ4LAY
   !
   !-------------SOLVE FOR ZETA SURFACE
         IF ( iactive.GT.0 .AND. MUTSOL.NE.2 ) THEN
            WRITE (IOUT,2020) k, iz
         END IF
   !             DIRECT SOLVER
         SOLVEZETA: IF (NSOLVER.EQ.1) THEN
   !-------------INITIALIZE PARAMETERS FOR DIRECT SOLVING
            kksoliter = 1
            ierr=0
   !---------------CALL DE45AP TO SOLVE FOR ZETA
            ACTIVEDE4: IF ( iactive.GT.0 ) THEN
               CALL DE47AP(DUM,IBO,SWIDE4%AU,SWIDE4%AL,SWIDE4%IUPPNT,&
               &SWIDE4%IEQPNT,SWIDE4%D4B,SWIDE4%MXUP,SWIDE4%MXLOW,&
               &SWIDE4%MXEQ,SWIDE4%MXBW,&
               &SWISOLCR,SWISOLCC,SWISOLCV,&
               &SWIHCOF,BRHS(1:NCOL,1:NROW,iz),SWIDE4%ACCLDE4,&
               &kksoliter,SWIDE4%ITMX,SWIDE4%MXITER,SWIDE4%NITERDE4,&
               &SWIDE4%HCLOSEDE4,IPRSOL,icnvg,NCOL,NROW,NLAYSWI,&
               &IOUT,SWIDE4%LRCHDE4,SWIDE4%HDCGDE4,SWIDE4%IFREQ,&
               &Kkstp,Kkper,SWIDE4%DELTL,NSTP(Kkper),SWIDE4%ID4DIR,&
               &SWIDE4%ID4DIM,MUTSOL,SWIDE4%DELTL,SWIDE4%NBWL,&
               &SWIDE4%NUPL,SWIDE4%NLOWL,SWIDE4%NLOW,SWIDE4%NEQ,&
               &SWIDE4%NUP,SWIDE4%NBW,ierr)
   ...
   !             PRECONDITIONED CONJUGATE GRADIENT
         ELSE IF (NSOLVER.EQ.2) THEN
            kksoliter=1
            ierr = 0
   !---------------CALL PCG7AP TO SOLVE FOR ZETA
            ACTIVEPCG: IF ( iactive.GT.0 ) THEN
   !                 LINEAR EQUATION -- OUTER ITERATIONS ONLY NEEDED IF
   !                 HCLOSE AND/OR RCLOSE NOT ACHIEVED AFTER FIRST
   !                 INNER ITERATION
               rclose = ( 1.0 + DELNUS(iz) ) * SWIPCG%RCLOSEPCG
               OUTERPCG: DO ksoliter = 1, SWIPCG%MXITER
                  kksoliter = ksoliter
                  CALL PCG7AP(DUM,IBO,SWISOLCR,SWISOLCC,SWISOLCV,&
                  &SWIHCOF,BRHS(1:NCOL,1:NROW,iz),SWIPCG%VPCG,&
                  &SWIPCG%SS,SWIPCG%P,SWIPCG%CD,SWIPCG%HCHG,&
                  &SWIPCG%LHCH,SWIPCG%RCHG,SWIPCG%LRCHPCG,&
                  &kksoliter,SWIPCG%NITER,&
                  &SWIPCG%ZCLOSEPCG,rclose,icnvg,Kkstp,Kkper,IPRSOL,&
                  &SWIPCG%MXITER,SWIPCG%ITER1,SWIPCG%NPCOND,&
                  &SWIPCG%NBPOL,NSTP(Kkper),NCOL,NROW,NLAYSWI,&
                  &SWIPCG%NODES,SWIPCG%RELAXPCG,IOUT,MUTSOL,&
                  &SWIPCG%IT1,SWIPCG%DAMPPCG,BUFF,SWIPCG%HCSV,ierr,&
                  &SWIPCG%HPCG,SWIPCG%DAMPPCGT,ISSFLG(Kkper),HDRY,&
                  &SWIPCG%IHCOFADD)
         ...
         END IF SOLVEZETA
   !
   !-------------COPY REAL ARRAY DUM INTO DOUBLE ARRAY ZETA
   ...
   END DO KBRHS
   !
   !---------CHECK SLOPE AT TIPS AND TOES OF ZETA SURFACES
...
SUBROUTINE SSWI2_CHKSLOPE()
   !
   !     ******************************************************************
   !     CHECK TIP AND TOE SLOPE FOR SWI2 PACKAGE
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   ZTIPTOE: DO k = 1, NLAY
      ...
      IZTIPTOE: DO iz = 2, NZONES
         ITIPTOE: DO i = 1, NROW
            JTIPTOE: DO j = 1, NCOL
   !
   !-------------------SET TEMPORARY AQUIFER THICKNESS VARIABLES
   ...
   !                   ZETA SURFACE IS NOT AT THE TOP (ILPOS=1) OR BOTTOM (ILPOS=2) OF THE LAYER
               LACTIVE: IF (IPLPOS(j,i,k,iz).EQ.0) THEN
   !
   !--------------------EVALUATE CHANGE IN ZETA FOR ALL ACTIVE ZONES FOR CURRENT CELL
                  zcalc = rvl * aqb
                  b     = ABS( zetac - zoldc )
                  IF ( b.GT.zcalc ) THEN
                     IADPTMOD = 1
                  ...
                  END IF
   !--------------------CHECK TIPS AND TOES FOR EACH COLUMN BUT THE FIRST AND LAST
                  LCOL: IF ( (j.NE.1) .AND. (j.NE.NCOL) ) THEN
   !-----------------------LEFT FACE
                     IF ( IPLPOS(j-1,i,k,iz).EQ.1 ) THEN
   ...
   !-----------------------RIGHT FACE
                  ...
                  END IF LCOL
   !
   !---------------------CHECK TIPS AND TOES FOR EACH ROW BUT THE FIRST AND LAST
...
SUBROUTINE SSWI2_ZCHG(A)
   !
   !     ******************************************************************
   !     CALCULATE SWI2 ZONE CHANGE TERM FOR SWI2 PACKAGE ZONE BUDGETS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   USE GLOBAL,       ONLY: NCOL,NROW,NLAY,DELR,DELC,IBOUND,&
   &LBOTM,BOTM,HNEW,HOLD
   ...
   CZONECHG: DO iz = 1, NZONES
      DO k = 1, NLAY
         DO i = 1, NROW
            DO j = 1, NCOL
   !                 CURRENT AND ALCULATE CURRENT ZONE CHANGE
               h1    = HNEW(j,i,k)
               h0    = HOLD(j,i,k)
   !                 SKIP INACTIVE CELLS
               h1noflo = ABS( h1 - dhnoflo )
               IF ( h1noflo.LT.dsmall ) CYCLE
   !                 REAL AND DOUBLE PRECISION CELL BOTTOM
               rb = BOTM(j,i,LBOTM(k))
               db = BOTM(j,i,LBOTM(k))
   !                 CALCULATE CURRENT ZONE VOLUME
               t1    = ZETA(j,i,k,iz)
               b1    = ZETA(j,i,k,iz+1)
   !langevin
   !langevin -- potential problem here?  IF ( t1.LT.b1 ) t1 = b1
   !langevin
               IF ( t1.LT.b1 ) t1 = b1
   !                 RESET THE CURRENT ZONE THICKNESS TO ZERO IF THE CELL IS DRY
               h1dry = ABS( h1 - dhdry )
               IF ( h1dry.LT.dsmall ) THEN
                  t1 = db
                  b1 = db
               END IF
   !                 RESET THE CURRENT ZONE THICKNESS TO ZERO
   !                 IF HNEW IS LESS THAN THE BOTTOM OF THE LAYER - FOR MODFLOW-NWT
               IF ( h1.LT.db ) THEN
                  t1 = db
                  b1 = db
               END IF
   ...
   !langevin
   !langevin -- potential problem here?  IF ( t0.LT.b0 ) t0 = b0
   !langevin
               IF ( t0.LT.b0 ) t0 = b0
   !                 RESET THE PREVIOUS ZONE THICKNESS TO ZERO IF THE CELL IS DRY
               h0dry = ABS( h0 - HDRY )
               IF ( h0dry.LT.small ) THEN
                  t0 = db
                  b0 = db
               END IF
   !                 RESET THE PREVIOUS ZONE THICKNESS TO ZERO
   !                 IF HOLD IS LESS THAN THE BOTTOM OF THE LAYER - FOR MODFLOW-NWT
               IF ( h0.LT.rb ) THEN
                  t0 = db
                  b0 = db
               END IF
   !                 CALCULATE INCREMENTAL ZONE CHANGE FOR CURRENT SWI TIMESTEP AND
   !                 CUMULATIVE ZONE CHANGE FOR CURRENT MODFLOW TIMESTEP
...
SUBROUTINE SSWI2_IMIX(A)
   !
   !     ******************************************************************
   !     CALCULATE SWI2 PACKAGE INSTANTANEOUS MIXING TERMS FOR ZONE BUDGETS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
            ...
            IF (k.NE.NLAY) THEN
               ...
               IF ( IBOUND(j,i,k+1).NE.0 .AND. ht2.GT.db2 ) THEN
                  ...
                  IF ( nuupperbot.GT.nulowertop ) THEN
                     qzbot = CV(j,i,k)*(HNEW(j,i,k)-HNEW(j,i,k+1))+&
                     &QLEXTRA(j,i,k)
   !                     FIND ZONE NUMBER AT THE BOTTOM OF THE CURRENT LAYER
   !                     AND AT THE TOP OF THE UNDERLYING LAYER
...
SUBROUTINE SSWI2_MSTO(J,I,K,V,Vc)
   !
   !     ******************************************************************
   !     COMPUTE MODFLOW STORAGE FLOW TERM FROM SC1 AND SC2 TO ADJUST
   !     SWI2 PACKAGE BOUNDARY TERM FOR ZONE BUDGETS.
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   Vc    = zero
   tled  = one / DELT
   !---------DETERMINE SC2 POSITION
   kt = 0
   DO kk = 1, K
      IF( ICONV(kk).NE.0 ) kt = kt + 1
   END DO
   !
   !---------SET FLAG FOR CONVERTIBLE LAYER
   ic = ICONV(K)
   ...
   !             CONFINED PORTION OF THE STORAGE CHANGE
         IF ( rh0.LT.rt ) rh0 = rt
         IF ( rh.LT.rt )  rh = rt
...
SUBROUTINE SSWI2_UPZ1(Klay,Init)
   !
   !     ******************************************************************
   !     UPDATE THE UPPER ZETA SURFACE TO THE TOP OF THE WATER TABLE
   ...
   !             TOP OF LAYER LESS THAN HEAD
         IF ( dtop.GT.h ) THEN
            ...
            END IF
   !             ADJUST ZETA SURFACE 2 TO NZONES IF NECESSARY
   !             ASSUME THAT THE UPPER LAYERS ARE ADJUSTED IF THEY
   !             EXCEED THE MODIFIED UPPER SURFACE
            DO iz = 2, NZONES
               ...
               END IF
            END DO
   !             TOP OF LAYER LESS THAN OR EQUAL TO HEAD
         ELSE
...
SUBROUTINE SSWI2_SET_IPLPOS()
   !
   !     ******************************************************************
   !     SET FLAG FOR LOCATION OF THE ZETA SURFACE RELATIVE TO THE TOP AND
   !     BOTTOM OF A CELL
   !       IPLPOS=1 AT TOP
   !       IPLPOS=2 AT BOTTOM
   !       IPLPOS=0 IN BETWEEN, AND
   !       IPLPOS=3 IN INACTIVE CELLS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
         ...
         J_IPLPOS: DO j=1,NCOL
            ...
            IZ_IPLPOS: DO iz=2,NZONES
               ...
               ELSEIF (ZETA(j,i,k,iz).LE.ZETA(j,i,k,NZONES+1)) THEN
                  IPLPOS(j,i,k,iz) = 2
   !-----------------SET ILPOS TO THE BOTTOM OF THE LAYER (IPLPOS=2) IF THE
   !                 THE HEAD IN THE CELL IS LESS THAN THE BOTTOM OF THE
   !                 CELL - FOR MODFLOW-NWT
...
CHARACTER (LEN=17) FUNCTION SSWI2_BDCHAR(R) RESULT(value)
   !
   !     ******************************************************************
   !     CREATE SWI2 ZONE BUDGET TERMS CHARACTER STRINGS
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE SSWI2_RD_COMM(Iu)
   !
   !     ******************************************************************
   !     READ NON COMMENT AND NON BLANK LINE FROM SWI2 INPUT FILE
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
