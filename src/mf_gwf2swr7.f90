   !
   !     VERSION 1.02 SWR1 for MODFLOW NWT
   !     CHANGES
   !     o IF RTMIN IS ZERO SET RTMIN TO THE MINIMUM DELT FOR THE CURRENT MODFLOW
   !       STRESS PERIOD - ITSPMIN SET TO 1
   !     o IF RTMAX IS ZERO SET RTMAX TO DELT FOR THE CURRENT MODFLOW TIMESTEP -
   !       ITSPMAX VARIABLE SET TO 1
   !     o MINOR BUG FIXES IN:
   !         SSWR_P_QMFLOW
   !
   !     VERSION 1.03 SWR1 for MODFLOW NWT
   !     CHANGES
   !     o UPSTREAM-WEIGHTING OPTION FOR DIFFUSIVE-WAVE FLOW BETWEEN
   !       CONNECTED REACH GROUPS
   !     o SWR OBSERVATIONS
   !     o STAGE DEPENDENT EXTERNAL BOUNDARY STRUCTURE
   !     o ADDED DIRECT RUNOFF OPTION
   !     o GLOBAL AND ABSOLUTE TOLA OPTIONS
   !     o ADDED OPENMP FUNCTIONALITY TO GSOL BLAS-1 TYPE ROUTINES
   !     o L2 NORM AND FRACTIONAL TOLR OPTIONS
   !     o REVISED MODFLOW-SWR QAQ COMPARISON TO KEEP TRACK OF POSITIVE
   ...
   !     o ADDED OPTION FOR USING CURRENT STAGE (FROM THE LAST ITERATION) IN A SPECIFIED REACH
   !       TO DEFINE THE INITIAL STAGE IN A SPECIFIED REACH - A NEGATIVE REACH NUMBER IS
   !       SPECIFIED AND THE RSTAGE VALUE IS THE REACH USED TO DEFINE THE STAGE. ONLY APPLIES TO
   !       REACH STAGE DATA SPECIFIED USING LIST INPUT
   !
   !     o MINOR MODIFICATIONS TO EXISTING FUNCTIONALITY:
   !         - LATERAL FLOW FROM ACTIVE REACH GROUPS TO CONSTANT STAGE REACH GROUPS
   !           ARE REPORTED TO THE GLOBAL BUDGET AS EXTERNAL FLOW. THE REMAINDER OF
   !           CONSTANT STAGE REACH GROUP FLOW TERMS ARE STILL REPORTED AS CONSTANT
   !           FLOW IN THE GLOBAL BUDGET. THIS MODIFICATION WAS MADE TO MAKE IT EASIER
   !           TO DETERMINE LATERAL FLOW TO OR FROM EXTERNAL BOUNDARIES. OTHER SWR
   !           OUTPUT HAVE NOT BEEN MODIFIED.
   ...
   !         - SUMMARY CONVERGENCE INFORMATION WRITTEN TO SCREEN (ONLY WHEN ISWRSCRN=1) ONLY
   !           INCLUDES INFORMATION FOR ACTIVE REACH GROUPS
   !
   !     VERSION 1.04 SWR1 for MODFLOW-2005 (1.12) AND MODFLOW NWT (1.1.2)
   !     CHANGES
   !     o ADDED STATE FUNCTION SUBROUTINES - RETRIEVE STAGE, FLOW, OR STRUCTURE FLOW
   !       AT THE END OF THE PREVIOUS MODFLOW TIME STEP
   !     o ADDED OVERBANK STRUCTURE (ISTRTYPE=13) TO SIMPLIFY SPECIFICATION OF WEIR STRUCTURED
   !       FOR CONNECTIONS BETWEEN 1D AND 2D REACHES. INCLUDES OPTION TO SMOOTHLY TRANSITION
   !       WEIR WIDTH OVER A DEFINED DEPTH
   !     o REMOVED OPTION FOR USING CURRENT STAGE (FROM THE LAST ITERATION) IN A SPECIFIED REACH
   !       TO DEFINE THE INITIAL STAGE IN A SPECIFIED REACH - A NEGATIVE REACH NUMBER IS
   !       SPECIFIED AND THE RSTAGE VALUE IS THE REACH USED TO DEFINE THE STAGE. ONLY APPLIES TO
   !       REACH STAGE DATA SPECIFIED USING LIST INPUT
   !     o ADDED KINEMATIC-WAVE OPTION (IROUTETYPE = 4)
   !       *** THIS NEEDS TO BE REFINED FOR TWO-DIMENSIONAL PROBLEMS ***
   !     o ADDED MATRIX LEVEL PSEUDO-TRANSIENT CONTINUATION APPROACH FOR STEADY-STATE STRESS
   !       PERIODS (BASED ON KELLEY AND KEYES (1998). INITIAL PSEUDO-TRANSIENT CONTINUATION
   !       TIMESTEP (PTCDEL) IS CALCULATED USING THE COMPUTED WAVE CELERITY
   !       BASED ON SAVANT AND OTHERS (2011) - PTCDEL0 = DLEN / SQRT(g depth).
   !       ORIGINAL PSEUDO-TRANSIENT CONTINUATION APPROACH DESCRIBED IN THE DOCUMENTATION
   !       HAS BEEN DEPRECATED.
   !     o ADDED BOTTOM AVERAGING TO DAMPEN OSCILLATING SOLUTIONS - ALGORITHM IS IDENTICAL TO
   !       THE ALGORITHM USED IN MODFLOW-USG (PANDAY AND OTHERS, 2013).
   !     o ADDED ABILITY TO CONNECT A SWR REACH WITH MULTIPLE LAYERS STARTING FROM A SPECIFIC
   !       LAYER. LIKE THE PREVIOUS OPTION WHERE A NEGATIVE KRCH FOR A REACH INDICATED THE
   !       REACH SHOULD BE CONNECTED TO LAYERS 1 TO NLAY. SPECIFICATION OF A KRCH OF -2 FOR
   !       A REACH WILL CONNECT THE REACH TO LAYERS 2 TO NLAY.
   !
   !     o MINOR BUG FIXES IN:
   !         - CALCULATION OF QAQ EXCHANGE PERIMETER FOR EACH LAYER
   !           FIX IDENTIFIED BY JAN JEPPESEN
   !         - FIX TO WRITING GWET TO CBC FILE AS A LIST
   !         - MOVED SSWR_EXINIT() SUBROUTINE THAT RESETS QUZFLOW AND QEXTFLOW FROM
   !           SSWRBUDGET SUBROUTINE TO THE END OF GWF2SWR7BD SUBROUTINE. THIS
   !           ENSURES QUZFLOW AND QEXTFLOW TERMS ARE CORRECTLY WRITTEN IF SSWR_P_RGFLOW
   !           SUBROUTINE IS BEING USED
   !         - MODIFIED SSWR_SET_RCHOFF SO THAT LEVEL-POOL AND DIFFUSIVE-WAVE REACHES
   !           STAGES ARE SET TO GBELV WHEN STAGES SPECIFIED IN DATA SET 14 ARE
   !           LESS THAN GBELV.
   !
   !     VERSION 1.05 SWR1 for MODFLOW-2005 (1.12) AND MODFLOW NWT (1.2.1)
   !     CHANGES
   !     o MINOR BUG FIXES IN:
   !         NQAQCONN
   !
...
MODULE GWFSWRMODULE
   ...
   &6.283185307179586476925286766559005768394D+0
   !
   !---------SWR DATA TYPE DEFINITIONS
   ...
   !         CALCULATED STRUCTURE VARIABLES
   ...
   !         CALCULATED GATE ELEVATION FOR EACH SWR TIME STEP
   !         ONLY ALLOCATED FOR OPERABLE STRUCTURES
      DOUBLEPRECISION, DIMENSION(:), ALLOCATABLE :: OPRVAL
      TYPE(TSTRSOLN)  :: CURRENT
      TYPE(TSTRSOLN),  DIMENSION(:), ALLOCATABLE :: RSLTS
   !         SFR EXCHANGE STRUCTURES
   ...
   END TYPE TFLOWDATA
   !---------BASEFLOW SOLUTION DATA TYPE COMPONENTS
   TYPE TQAQTERMS
      ...
      DOUBLEPRECISION :: QAQFLOW         = DZERO
   END TYPE TQAQTERMS
   !---------NEWTON CORRECTION FOR SWR DATA TYPE
   TYPE TQAQNWT
   ...
   !           VARIABLE FOR CONVERSION TO STAGE SOLUTION FROM SOLUTION
   !           OF STAGE UPGRADE VECTOR
      DOUBLEPRECISION, DIMENSION(:),   ALLOCATABLE :: JSKM1
   !-----------TEMPORARY STORAGE FOR SOLVERS
   !-----------JACOBIAN CALCULATION STORAGE
   ...
   END TYPE TJAC
   !---------COMMON REACH DATA TYPE - INCLUDES NESTED TYPES
   TYPE TREACH
   !           INPUT DATA
   !           REACH LOCATION DATA
   ...
   !           AVERAGE QAQ RATE(BY LAYER), QEVT, AND IEVT - FOR MT3D LINK FILE
   ...
   !           NESTED DATA TYPES
      ...
      TYPE (TGEO)     :: GEO
   END TYPE TREACH
   !---------COMMON REACH GROUP DATA TYPE - CALCULATED
   TYPE TRCHGRP
   ...
   !           REACH GROUP SUMMARY INFORMATION
   ...
   !---------STORAGE MAPPING FOR COORDINATE INVARIANT FLOW (QM) IN MODEL-BASED CELLS
   INTEGER,SAVE,POINTER          :: IGEOCI
   INTEGER,SAVE,DIMENSION(:,:), POINTER  :: IQMCI
   DOUBLEPRECISION,SAVE,DIMENSION(:,:), POINTER  :: SFMCI
   !---------VARIABLES ACCESSED OUTSIDE SWR - SIMPLE ARRAYS TO SIMPLIFY CALLS
   INTEGER,SAVE,POINTER          :: IEXTFLOW
   REAL,SAVE,DIMENSION(:,:,:),POINTER :: HK
   !---------STORAGE OF REACH AND REACH GROUP STAGE
...
MODULE GWFSWRINTERFACE
   ...
   !-----------ASSUMED SHAPE INTERFACE FOR CREATING A VECTOR OF UNIQUE VALUES
   !           IN A PASSED VECTOR POSSIBLY CONTAINING DUPLICATES
      SUBROUTINE SSWR_UNIQVALUES(A)
         DOUBLE PRECISION, DIMENSION(:),&
         &ALLOCATABLE, INTENT(INOUT) :: A
      END SUBROUTINE SSWR_UNIQVALUES
   !-----------ASSUMED SHAPE INTERFACE FOR LINEARLY INTERPOLATING THE Y AT Z
   !           FROM VALUES IN X AND Y VECTORS
   ...
   !
   !-----------CALCULATE SMALL DEPTH SCALING FACTOR FOR DIFFUSIVE WAVE
   !           FLOW, QAQFLOW, AND EVAPORATION
   !           VALUE OF 1.0 (UNSCALED) ABOVE DUPDPTH
   !           S-CURVE FUNCTION BETWEEN DUPDPTH AND DMINDPTH
   !           VALUE OF 0.0 BELOW DMINDPTH
...
SUBROUTINE GWF2SWR7AR(In,Ibcf,Ilpf,Ihuf,Iupw,Isfr,Inwt,Igrid)
   !     ******************************************************************
   !     ALLOCATE ARRAY STORAGE FOR GROUP ROUTING ELEMENTS AND
   ...
   &(/ toptions('PRINT_SWR_TO_SCREEN                ', 0, 0,  0),&
   &toptions('SAVE_SWRDT                         ', 0, 1,  0),&
   &toptions('SAVE_CONVERGENCE_HISTORY           ', 0, 1,  0),&
   &toptions('SAVE_RIVER_PACKAGE                 ', 0, 1,  5),&
   &toptions('SAVE_RIVER_PACKAGE_ALL             ', 0, 1,  4),&
   &toptions('SAVE_AVERAGE_RESULTS               ', 0, 0,  0),&
   &toptions('USE_TABFILES                       ', 0, 0,  0),&
   &toptions('USE_NONCONVERGENCE_CONTINUE        ', 0, 0,  0),&
   &toptions('USE_UPSTREAM_WEIGHTING             ', 0, 0,  0),&
   &toptions('USE_INEXACT_NEWTON                 ', 0, 0,  0),&
   &toptions('USE_STEADYSTATE_STORAGE            ', 0, 0,  0),&
   &toptions('USE_LAGGED_OPR_DATA                ', 0, 0,  0),&
   &toptions('USE_LINEAR_DEPTH_SCALING           ', 0, 0,  0),&
   &toptions('USE_DIAGONAL_SCALING               ', 0, 0, 15),&
   &toptions('USE_L2NORM_SCALING                 ', 0, 0, 14),&
   &toptions('USE_RCMREORDERING                  ', 0, 0, 17),&
   &toptions('USE_RCMREORDERING_IF_IMPROVEMENT   ', 0, 0, 16),&
   &toptions('USE_STAGE_TRANSFORM                ', 0, 0,  0),&
   &toptions('USE_WEIGHTED_HEADS                 ', 0, 0,  0),&
   &toptions('USE_IMPLICIT_NEWTON_CORRECTION     ', 0, 0, 21),&
   &toptions('USE_EXPLICIT_NEWTON_CORRECTION     ', 0, 0, 20),&
   &toptions('USE_ORIGINAL_2D_QM_FORMULATION     ', 0, 0, 23),&
   &toptions('USE_IMPLICIT_INVARIATE_QM          ', 0, 0, 22),&
   &toptions('USE_SOURCECELL_INVARIATE_QM        ', 0, 0,  0),&
   &toptions('USE_FRACTIONAL_TOLR                ', 0, 0, 26),&
   &toptions('USE_L2NORM_TOLR                    ', 0, 0, 25),&
   &toptions('SAVE_SWROBSERVATIONS               ', 0, 1, 28),&
   &toptions('SAVE_SWROBSERVATIONS_ALL           ', 0, 1, 27),&
   &toptions('USE_MULTICORE                      ', 0, 1,  0),&
   &toptions('USE_MULTICORE_VECTOR               ', 0, 1,  0),&
   &toptions('USE_DIRECT_RUNOFF                  ', 0, 1,  0),&
...
2000 FORMAT(1X,/1X,'SWR -- SURFACE-WATER ROUTING PACKAGE,',/,1X,&
   &A64,/1X,'INPUT READ FROM UNIT ',I4)
2001  FORMAT(//1X,'SUMMARY OF SWR1 OPTIONS USED IN SIMULATION',&
   &/1X,'OPTION',1X,'OPTION KEYWORD',21X,1X,' FILE UNIT',&
   &/1X,6('-'),1X,35('-'),1X,10('-'))
2002  FORMAT(  1X,I6,1X,A35,1X,A10)
2003  FORMAT(  1X,53('-'))
2010  FORMAT(/1X,'NUMBER OF REACHES                         =',I5,&
   &/1X,'SURFACE WATER ONLY SIMULATION (>0=TRUE)   =',I5,&
   &/1X,'LENGTH CONVERSION FACTOR                  =',G15.7,&
   &/1X,'TIME CONVERSION FACTOR                    =',G15.7,&
   &/1X,'MINIMUM GRADIENT FOR FLOW                 =',G15.7,&
   &/1X,'MAXIMUM DEPTH FOR SCALING REACH OUTFLOW   =',G15.7,&
   &/1X,'MINIMUM DEPTH FOR REACH OUTFLOW           =',G15.7,&
   &/1X,'DIFFUSIVE-WAVE APPROX. SPATIAL WEIGHTING  =',1X,A20,&
   &/1X,'DEPTH SCALING                             =',1X,A20,&
   &/1X,'TWO-DIMENSIONAL COORD.-INVARIANT APPROACH =',1X,A20,&
   &/1X,'MINIMUM ROUTING TIME STEP LENGTH          =',G15.7,&
   &/1X,'MAXIMUM ROUTING TIME STEP LENGTH          =',G15.7,&
   &/1X,'INITIAL ROUTING TIME STEP LENGTH          =',G15.7,&
   &/1X,'OUTPUT PRINT INCREMENT                    =',G15.7,&
   &/1X,'ADAPTIVE TIME STEP INCREMENT (RTMULT)     =',G15.7,&
   &/1X,'ADAPTIVE TIME STEP INCREMENT FREQUENCY    =',I5,&
   &/1X,'MAXIMUM RAINFALL      PER ROUTING STEP    =',G15.7,&
   &/1X,'MAXIMUM STAGE  CHANGE PER ROUTING STEP    =',G15.7,&
   &/1X,'MAXIMUM INFLOW CHANGE PER ROUTING STEP    =',G15.7,/,&
   &/1X,'CBC OUTPUT UNIT (ISWRCBC)                 =',I5,&
   &/1X,'GROUP FLOW TERMS OUTPUT UNIT (ISWRPRGF)   =',I5,&
   &/1X,'REACH STAGE OUTPUT UNIT (ISWRPSTG)        =',I5,&
   &/1X,'REACH AQ-RCH FLOW OUTPUT UNIT (ISWRPQAQ)  =',I5,&
   &/1X,'REACH LATERAL FLOW OUTPUT UNIT (ISWRPQM)  =',I5,&
   &/1X,'STRUCTURE FLOW OUTPUT UNIT (ISWRPSTR)     =',I5,&
   &/1X,'SUMMARY VELOCITY AND FROUDE INFORMATION   =',L5)
2020 FORMAT(//15X,3('-'),1X,'SWR SOLVER PARAMETERS',1X,3('-'),&
   &/1X,'SOLVER =',1X,A40,&
   &/1X,'PRECONDITIONER                            =',1X,A20,&
   &/1X,'JACOBIAN SCALING                          =',1X,A20,&
   &/1X,'MAXIMUM NUMBER OF OUTER ITERATIONS        =',I5,&
   &/1X,'MAXIMUM NUMBER OF INNER ITERATIONS        =',I5,&
   &/1X,'NON-CONVERGENCE CONT.OF SWR AND MODFLOW   =',L5)
2025 FORMAT( 1X,'NUMBER OF LEVELS FOR ILUT (NLEVELS)       =',I5,&
   &/1X,'DROP TOLERANCE FOR ILUT (DROPTOL)         =',E15.7)
2030 FORMAT( 1X,'LU DECOMPOSITION STORAGE APPROACH         =',A20,&
   &/1X,'OFF DIAGONAL DIMENSION (JAC%NHALFB)       =',I5,&
   &/1X,'MAXIMUM BANDWIDTH (JAC%NBW)               =',I5)
2040 FORMAT( 1X,'MAXIMUM STAGE    TOLERANCE (TOLS)         =',E15.7,&
   &/1X,'MAXIMUM ROOT     TOLERANCE (TOLR)         =',E15.7,&
   &/1X,'INEXACT NEWTON APPROACH (INEXCTNWT)       =',L5,&
   &/1X,'MAXIMUM AQ-RCH FLOW TOLERANCE (TOLA)      =',E15.7,&
   &/1X,'STEADY-STATE DAMPENING PARAMETER (DAMPSS) =',E15.7,&
   &/1X,'TRANSIENT DAMPENING PARAMETER (DAMPTR)    =',E15.7,&
   &/1X,'PSEUDO-TRANSIENT CONTINUATION APPROACH    =',L5,&
   &/1X,'SOLVER OUTPUT FREQUENCY (IPRSWR)          =',I5,&
   &/1X,'CONVERGENCE PRINTING FLAG (MUTSWR)        =',I5,&
   &/1X,'  0 - MAXIMUM RESIDUAL EACH SWR TIMESTEP',&
   &/1X,'  1 - TOTAL NUMBER OF ITERATIONS EACH SWR TIMESTEP',&
   &/1X,'  2 - NO PRINTING',&
   &/1X,'  3 - PRINTING IF CONVERGENCE FAILS',&
   &/1X,'BACKTRACKING ALLOWED                      =',L5)
2050 FORMAT( 1X,'MAXIMUM BACKTRACKING ITERATIONS (IBT)     =',I5,&
   &/1X,'BACKTRACKING OUTPUT FREQUENCY  (IBTPRT)   =',I5)
2060 FORMAT(//1X,'TIME-INVARIANT SURFACE-WATER ROUTING REACH DATA',/,&
   &1X,' REACH',1X,'IROUTETYPE',1X,'SOLUTION METHOD     ',&
   &1X,'IRGNUM',1X,'   IRG',&
   &1X,'   LAY',1X,'   ROW',1X,'   COL',1X,'      DLEN',/&
   &1X,98('-'))
2070 FORMAT(1X,I6,1X,I10,1X,A20,1X,5(I6,1X),1(G10.3,1X))
2080 FORMAT(//1X,'SWR REACH GROUPS:',&
   &1X,'REACHES IN REACH GROUP',/1X,69('-'))
2090 FORMAT(1X,'REACH GROUP =',1X,I5,&
   &1X,'IRGNUM =',1X,I5,1X,'   REACHES',1X,4(I5,1X),&
   &100(:/1X,46X,:4(I5,1X)))
2100 FORMAT(//1X,'SURFACE WATER ONLY SIMULATION',&
   &/1X,'SETTING MODFLOW IBOUND TO ZERO FOR ALL CELLS')
2110 FORMAT(//1X,'SWR PROCESS REQUIRES LAYCON OF 1 OR 3 FOR'&
   &1X,'ALL LAYERS IF THE BCF PACKAGE IS USED',//)
2120 FORMAT(//1X,'SWR PROCESS REQUIRES USE OF THE BCF, LPF,'&
   &1X,'OR HUF FLOW PACKAGES',//)
2130 FORMAT(//1X,'COULD NOT PARSE NTABRCH FOR TABULAR DATA ITEM',&
   &1X,I5)
2140 FORMAT(//1X,'SWR TABULAR DATA:',&
   &/1X,'TYPE                ',1X,' FILE UNIT',&
   &1X,'INTERPOLATION       ',1X,'REACHES',&
   &/1X,82('-'))
2150 FORMAT(1X,A20,1X,I10,1X,A20,1X,5(I5,1X),&
   &100000(:/1X,53X,:5(I5,1X)))
2160 FORMAT(//1X,'SWR OBSERVATION DATA:',&
   &/1X,'OBSERVATION NAME    ',1X,'TYPE                ',&
   &1X,'  IOBSTYPE',1X,'   IOBSLOC',1X,'  IOBSLOC2',&
   &/1X,82('-'))
2170 FORMAT(1X,A20,1X,A20,1X,I10,1X,I10,1X,A10)
2180 FORMAT(//1X,'SWR PARALLEL SIMULATION',&
   &/1X,82('-'),&
   &/1X,'OPENMP CORES USED FOR MATRIX-VECTOR OPERATIONS:',I10,&
   &/1X,'OPENMP CORES USED FOR AXPY          OPERATIONS:',I10,&
   &/1X,'MAXIMUM NUMBER OF CORES AVAILABLE FOR OPENMP  :',I10,&
   &/1X,82('-'),//)
2190 FORMAT(//1X,'SWR DIRECT RUNOFF SIMULATION',&
   &/1X,82('-'),&
   &/1X,'DIRECT RUNOFF DATA WILL BE READ FROM FILE UNIT:',I10,&
   &/1X,82('-'),//)
   !     + + + CODE + + +
   ALLOCATE(SWREXETOT)
   ALLOCATE(ISFRUNIT)
   ...
   ALLOCATE(NPMAX)
   !
   !------INITIALIZE SWRTOTEXE TIMER
   SWREXETOT = DZERO
   !
   !-------START SWR TIMER FOR GWF2SWR7AR
   ...
   INWTUNIT = Inwt
   !
   !-------IDENTIFY OPTION AND INITIALIZE # OF GROUP ROUTING ELEMENTS
   WRITE(IOUT,2000) VERSION_SWR, IN
   !
   !-------READ AND PRINT DIMENSION AND OTHER ASSOCIATED DATA FOR
   !       SURFACE-WATER ROUTING PACKAGE
   !         PROBLEM DIMENSION AND OUTPUT CONTROL - INPUT ITEM 1
   CALL SSWRDSOUT('1A')
   lloc = 1
   ...
   !
   !-------READ AUXILIARY VARIABLES AND SWROPTIONS FLAG.
   NAUX    = 0
   IRDAUX  = 0
   ISWROPT = 0
   LAUX: DO
...
13       FORMAT(1X,'SWR1 OPTIONS SPECIFIED IN DATASET 1B')
   ...
   END DO LAUX
   !
   !-------EVALUATE ANY KEYWORDS IN DATASET 1A
   ISWRSCRN    = 0
   ISWRSDT     = 0
   ISWRCNV     = 0
   ISWRSRIV    = 0
   ...
   !               ONLY USE THE FIRST INSTANCE OF A PARTICULAR SWROPTION
               IF ( swroptions(i)%ioptionused.EQ.0 ) THEN
                  ...
                  END IF
   !                 RESET ANOTHER OPTION IF THE CURRENT OPTION INVALIDATES ANOTHER
                  ...
                  IF ( j.NE.0 ) THEN
                     ...
                     END IF
                  END IF
   !                 TERMINATE READING SWR1 OPTIONS SINCE "END" KEYWORD ENCOUNTERED
                  IF ( i.EQ.nopt ) THEN
                     icnt = icnt - 1
                     EXIT SWROPT
                  END IF
   !                 TERMINATE SEARCH SINCE THE KEYWORD HAS BEEN FOUND
                  EXIT ROPTIONSL
               END IF
            END IF
         END DO ROPTIONSL
         ...
         END IF
      END DO SWROPT
   !         PRINT SUMMARY OF OPTIONS SPECIFIED
      IF ( icnt.GT.0 ) THEN
         WRITE (IOUT,2001)
         POPTIONSL: DO i = 1, (nopt - 1)
            ...
            IF ( j.NE.0 ) THEN
               ...
               END IF
               WRITE ( IOUT,2002)&
               &i, swroptions(i)%coption, cunit
            END IF
         END DO POPTIONSL
         WRITE (IOUT,2003)
   !           SET APPROPRIATE VARIABLES BASED ON SWR1 OPTIONS
   !           PRINT_SWR_TO_SCREEN
   ...
   !           USE_RCMREORDERING_IF_IMPROVEMENT
   ...
   END IF
   !
   !         SOLUTION CONTROLS - INPUT ITEM 2
   CALL SSWRDSOUT('2')
   lloc = 1
   CALL SSWR_RD_COMM(In)
   CALL URDCOM(In,IOUT,line)
   ...
   IF ( RTIME.LT.RZERO )&
   &CALL USTOP('RTINI MUST BE GREATER THAN ZERO')
   ...
   !
   !       SET RTIME, RTMIN, AND/OR RTMAX IF ZERO VALUES ARE SPECIFIED FOR
   ...
   IF ( ITSPMAX.EQ.1 ) THEN
      ...
      END IF
   END IF
   !     SET RTSTMAX NOW THAT RTMAX IS FINAL FOR AT LEAST THE FIRST STRESS PERIOD
   RTSTMAX = RTMAX
   !       SET DEFAULT VALUES FOR ADAPTIVE TIME STEPPING
   IADTIME = 0
   TADMULT = DONE
   DMAXRAI = DZERO
   DMAXSTG = DZERO
   DMAXINF = DZERO
   DMAXSBZ = DZERO
   !       DETERMINE IF ADAPTIVE TIME STEPPING IS BEING USED
   IF (   RTMIN.LT.RTMAX .OR.&
   &( ITSPMIN.EQ.1 .AND. ITSPMAX.EQ.1 ) ) THEN
   ...
   END IF
   !
   !         SOLVER PARAMETERS - INPUT ITEM 3
   CALL SSWRDSOUT('3')
   lloc   = 1
   CALL SSWR_RD_COMM(In)
   CALL URDCOM(In,IOUT,line)
   ...
   CALL URWORD(line, lloc, istart, istop, 3, ival,      r, IOUT, In)
   DAMPSS = SSWR_R2D(r)
   !-------RESET DAMPSS IF .LE. ZERO
   IF (DAMPSS.LE.DZERO) DAMPSS = DONE
   CALL URWORD(line, lloc, istart, istop, 3, ival,      r, IOUT, In)
   DAMPTR = SSWR_R2D(r)
   !-------RESET DAMPTR IF .LE. ZERO
   ...
   IF (IBT.GT.1) THEN
      ...
      IBTPRT = ival
   END IF
   !       READ PTOLR IF USE_FRACTIONAL_TOLR OR USE_L2NORM_TOLR ARE SPECIFIED
   !       IN DATA SET 1B
   ...
   END IF
   !
   !-------OUTPUT SWR PROBLEM DIMENSION AND SOLVER DATA
   WRITE (IOUT,2010) NREACHES, ISWRONLY,&
   &DLENCONV, TIMECONV, DMINGRAD, DUPDPTH, DMINDPTH,&
   &cswght(ISWRUPWT), cdscale(IDPTHSCL),&
   &ciciqm(ICIQM),&
   &RTMIN, RTMAX, RTIME, RTPRN, RTMULT, NTMULT,&
   &DMAXRAI, DMAXSTG, DMAXINF,&
   &ISWRCB, ISWRPRGF, ISWRPSTG, ISWRPQAQ, ISWRPQM,&
   &ISWRPSTR, (ISWRPFRN>0)
   WRITE (IOUT,2020) SOLVERTEXT, cipc(IPC), cscale(IJACSCL),&
   &NOUTER, NINNER, (ISWRCONT>0)
   IF ( IPC.EQ.4 ) THEN
      WRITE (IOUT,2025) NLEVELS, DROPTOL
   END IF
   WRITE (IOUT,2040) TOLS, TOLR, (INEXCTNWT>0), TOLA,&
   &DAMPSS, DAMPTR, (ISSSTOR>0), IPRSWR, MUTSWR,&
   &(IBT>1)
   IF (IBT.GT.1) THEN
      WRITE (IOUT,2050) IBT, IBTPRT
   END IF
   !
   !-------RESET TOLA IF .LE. ZERO
   IF (TOLA.LT.EPSILON(DZERO)) TOLA = -999.D0
   !
   !-------SET QSCALE AND RSCALE
   QSCALE = DONE / ( DLENCONV * DLENCONV * DLENCONV * TIMECONV )
   RSCALE = DONE / ( DLENCONV * TIMECONV )
   !-------SCALE CONSISTENT TOLS, TOLR, AND TOLA TO INTERNAL UNITS (M AND SEC)
   TOLS  = TOLS / DLENCONV
   TOLR  = TOLR * QSCALE
   IF ( IFTOLR.EQ.0 ) PTOLR = PTOLR * QSCALE
   !      TOLA = TOLA * QSCALE
   !
   !-------RESET IBOUND IF SURFACE WATER ONLY SIMULATION
   ...
   END IF
   !
   !-------ALLOCATE SPACE FOR DATA OF KNOWN SIZE
   ...
   !         USER DEFINED REACH GROUP NUMBER
   ...
   !         SET ROUTING TYPE
      SELECT CASE ( REACH(irch)%IROUTETYPE )
       ...
       CASE DEFAULT
         ...
         &'SPECIFIED - REVISE SWR1 DATASET 4A')
      END SELECT
   END DO
   !       CLEAN UP TEMPORARY LIST STORAGE
   DEALLOCATE (rlist)
   !
   !-------READ REACH DATA
   !       REACH DATA - INPUT ITEM 4B - FULL CONNECTIVITY
   CALL SSWRDSOUT('4B')
   CALL SSWR_RD_COMM(In)
   iclose = 0
   ...
   !         EXPLICIT QM MAGNITUDE
      IF ( ICIQM.GT.0 ) THEN
         ALLOCATE( REACH(irch)%SFM(nconn) )
   !         IMPLICIT QM MAGNITUDE
      ELSE
   ...
   !           EXPLICIT QM MAGNITUDE
      ...
      END DO
   END DO
   !       DETERMINE IF CONNECTIVITY DATA FOR 4B IS READ FROM AN
   !       EXTERNAL FILE SPECIFIED USING OPEN/CLOSE.  IF THIS IS
   !       TRUE, USE SSWREXTRD TO CLOSE FILE
   IF ( iclose.NE.0 ) CALL SSWREXTRD(In,IOUT,iut,iclose)
   !
   !---------CHECK REACH CONNECTIVITY
   CALL SSWR_CNCK_RC()
   !
   !---------ALLOCATE SPACE FOR REACH GROUP DATA
   CALL SSWR_ALLO_RG()
   !
   ALLOCATE ( FROUDE )
   !
   ...
   WRITE (IOUT,2060)
   DO i = 1, NREACHES
   !         DETERMINE IF DIFFUSIVE WAVE REACHES ARE BEING SIMULATED
      ...
      END IF
   !         ADDITIONAL ERROR CHECKING OF REACH DATA
   ...
   !         NO ERRORS FOR REACH - WRITE TO OUTPUT FILE
      WRITE (IOUT,2070) i,REACH(i)%IROUTETYPE,&
      &ADJUSTL(cmethod(REACH(i)%IROUTETYPE)),&
      &REACH(i)%IRGNUM,REACH(i)%IRG,&
      &REACH(i)%KRCH,REACH(i)%IRCH,REACH(i)%JRCH,REACH(i)%DLEN
   END DO
   !
   !-------RESET ISWRPFRN IF NO DIFFUSIVE WAVE REACHES
   !       NO NEED TO OUTPUT THE LOCATION AND MAGNITUDE OF THE MAXIMUM
   !       VELOCITY AND FROUDE NUMBER IF SIMULATION DOES NOT CONTAIN
   !       DIFFUSIVE WAVE REACHES OR REACH GROUPS
   IF ( idw.LT.1 ) ISWRPFRN = 0
   !
   !-------WRITE REACH GROUP REACH DATA
   WRITE (IOUT,2080)
   DO i = 1, NRCHGRP
      WRITE (IOUT,2090) i, RCHGRP(i)%IRGNUM,&
      &(RCHGRP(i)%REACH(j),j=1,RCHGRP(i)%NRGREACH)
   END DO
   !
   !-------GET TOTAL SIMULATION TIME FOR MODEL
   RTOTIM = RZERO
   DO n = 1, NPER
      DO i = 1, NSTP(n)
   ...
   END DO
   !
   !-------READ TABULAR DATA DIMENSIONS - DATASET 4C
   ITABTIME = 0
   IF ( NTABS.NE.0 ) THEN
      CALL SSWRDSOUT('4C')
      lloc = 1
   ...
   !---------DATASET 4D - TABULAR DATA SPECIFICATIONS
         CALL SSWRDSOUT('4D')
         DO j = 1, NTABS
            lloc = 1
            CALL SSWR_RD_COMM(In)
            ...
            SELECT CASE ( line(istart:istop) )
             ...
             CASE DEFAULT
               TABDATA(itab)%TSDATA%INTP = 1
            END SELECT
   !-------------READ ADDITIONAL TABULAR DATA ITEMS DEFINING REACHES TO APPLY
   !             TABULAR RAINFALL, EVAPORATION, LATERAL INFLOW, AND STAGE DATA
            IF ( TABDATA(itab)%ITABTYPE.GT.0 .AND.&
            &TABDATA(itab)%ITABTYPE.LT.5 ) THEN
               CALL URWORD(line, lloc, istart, istop, 0,ival,r,IOUT,In)
               SELECT CASE (line(istart:istop))
                CASE ('ALL')
                  ival = NREACHES
                ...
                CASE DEFAULT
                  ...
                  &ntabrch, (TABDATA(itab)%ITABRCH(i),i = 1,ntabrch)
               END SELECT
            END IF
   !
   !-------------IF READING STAGE DATA DETERMINE IF IT IS BEING READ
   !             FROM A SWR STAGE OUTPUT FILE
   ...
   !-------------READ TABULAR DATA INTO MEMORY
            ...
            END IF
   !-------------SET VARIABLES DEFINING USE OF A SPECIFIED SWR TIMESTEP
   ...
   END IF
   !
   !-------WRITE SUMMARY OF TABULAR DATA TO OUTPUT FILE
   IF ( NTABS.GT.0 ) THEN
      WRITE (IOUT,2140)
      DO n = 1, NTABS
         i = TABDATA(n)%ITABTYPE
         WRITE (IOUT,2150) ctab(i), TABDATA(n)%ITABUNIT,&
         &cintp(TABDATA(n)%TSDATA%INTP),&
         &( TABDATA(n)%ITABRCH(j),j=1,TABDATA(n)%NTABRCH )
      END DO
   END IF
   !
   !-------DETERMINE NTMAX
      ...
      DO n = 1, NPER
         DO i = 1, NSTP(n)
            ...
            IF ( RTMAX.GT.rt .AND. ITSPMAX.EQ.IZERO ) THEN
               ...
               &'MODFLOW STRESS PERIOD', n,&
               &'TIME STEP', i,&
               &': RTMAX (', RTMAX, ') EXCEEDS DELT (', rt, ')'
            END IF
            totim = totim + rt
   ...
   IF ( ierr.EQ.1 ) THEN
      ...
      &' ALL MODFLOW DELT VALUES'
      CALL USTOP('RTMAX EXCEEDS MODFLOW DELT')
   END IF
   !
   !-------DETERMINE THE MAXIMUM NUMBER OF PRINT TIMES PER MODFLOW TIME STEP
   NPMAX = 1
   IF ( RTPRN.NE.RZERO ) THEN
      ...
      END DO
   END IF
   !       NTMAX IS THE SUM OF RTMIN TIME STEPS PER MODFLOW
   !       TIME STEP, THE MAXIMUM NUMBER OF PRINT TIMES PER
   !       MODFLOW TIME STEP, AND 2 TO ACCOUNT FOR ADDITIONAL
   !       TIME SLIVERS AT THE BEGINNING AND END OF A MODFLOW
   !       TIME STEP
   NTMAX = NTMAX + NPMAX + 1
   !
   !---------ALLOCATE SPACE FOR SOLUTION DATA SIZED USING NTMAX
   !         CALLABLE SUBROUTINE TO ALLOW FOR EXPANSION OF DATA BY
   !         ADAPTIVE TIME STEP ALGORITHM
   CALL SSWR_ALLO_SOLN(NTMAX,0)
   !
   !-------ALLOCATE AND INITIALIZE SPACE FOR PRINTING REACH AND
   !       REACH GROUP STAGE DATA
   ...
   END DO
   !
   !-------READ OBSERVATION DATA DIMENSIONS - DATASET 4E
   NOBS     = 0
   IOBSSTG  = 0
   IOBSDEP  = 0
   IOBSRBOT = 0
   ...
   IF ( ISWRPOBS.NE.0 ) THEN
      ...
      ELSE
         ...
         DO i = 1, NOBS
            ...
            END DO
         END DO
   !---------DATASET 4F - OBSERVATION DATA SPECIFICATIONS
         CALL SSWRDSOUT('4F')
         DO j = 1, NOBS
            lloc = 1
            CALL SSWR_RD_COMM(In)
   ...
   IF ( NOBS.GT.0 ) THEN
      ...
      DO n = 1, NOBS
         ...
         &i, OBSDATA(n)%IOBSLOC, cobs
      END DO
   END IF
   !
   !-------POINT SWR1 HK TO APPROPRIATE HORIZONTAL CONDUCTIVITY
   IF ( Ibcf.GT.0 ) THEN
      HK => HYB
   !---------MAKE SURE CONFINED AQUIFER ARE NOT SIMULATED
      DO k = 1, NLAY
   ...
   END IF
   !
   !       SET SWRHEPS TO HEPS IF USING NEWTON
   ...
   END IF
   !
   !-------ALLOCATE SPACE FOR DIRECT RUNOFF VARIABLES
   ...
   END IF
   !
   !-------ALLOCATE QAQRATE FOR EACH REACH TO STORE AVERAGE AQUIFER
   !-------EXCHANGE RATE FOR THE TIME STEP - FOR MT3D LINK FILE
   DO i = 1, NREACHES
      ALLOCATE(REACH(i)%QAQRATE(NLAY))
   ENDDO
   !
   !-------FINISH SWR TIMER FOR GWF2SWR7AR
   CALL SSWRTIMER(1,tim1,tim2,SWREXETOT)
   !
   !-------SET POINTERS FOR GRID
...
SUBROUTINE GWF2SWR7RP(In,Kkper,Igrid)
   !     ******************************************************************
   !     READ STRESS PERIOD DATA FOR SWR
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   &'BANDED JACOBIAN STORAGE (NSOLRG X JAC%NBW)       '/
   ...
   &'EXTERNAL STRUCTURE BOUNDARY   ',&
   &'SFR PACKAGE INFLOW BOUNDARY   ',&
   &'STAGE-DEPENDENT FLUX BOUNDARY '/
...
2090 FORMAT(/1X,'SURFACE-WATER ROUTING REACH STRUCTURE DATA ',/1X,&
   &'     REACH',1X,' STRUCTURE',1X,'  ISTRTYPE',1X,&
   &'  ISTRCONN',1X,'     STRCD',1X,'    STRCD2',1X,&
   &'    STRCD3',1X,'    STRINV',1X,'   STRINV2',1X,&
   &'    STRWID',1X,'   STRWID2',1X,'    STRLEN',1X,&
   &'    STRMAN',1X,'    STRVAL',1X,' FLOW DIR.',1X,&
   &'BOUNDARY TYPE',&
   &/1X,194('-'))
...
2121 FORMAT(//1X,91('*'),/,35X,&
   &'- - - WARNING - - -',/4X,'STRUCTURE ITEM =',I5,1X,&
   &'IN REACH =',I5,1X,&
   &'ALLOWS FOR UPSTREAM OR BI-DIRECTIONAL FLOW ',/,4X,&
   &'(ISTRDIR <= 0) AND DOWNSTREAM REACH =',I5,1X,&
   &'HAS A SPECIFIED WATER SLOPE = ',G10.3,//,7X,&
   &'THIS COMBINATION CAN RESULT IN UNREALISTIC UPSTREAM ',&
   &'FLOW OF WATER DRIVEN BY THE',/,8X,'SPECIFIED WATER ',&
   &'SURFACE SLOPE INSTEAD OF DRIVEN BY SIMULATED STAGE ',&
   &'GRADIENTS',/,1X,91('*'))
...
2143 FORMAT(/1X,'WARNING -- ',&
   &'REACH GROUP =',1X,I5,1X,'IS DEFINED AS A',1X,&
   &'CONSTANT STAGE ELEMENT',/,12X,'BUT REACH =',1X,I5,1X,&
   &'IS DEFINED WITH A ISWRBND (',I5,') GREATER THAN -1.',/,12X,&
   &'TO RESOLVE THE INCONSISTENCY ISWRBND IS BEING RESET TO -999.')
2150 FORMAT(//1X,'REACH OFFSETS FOR STRESS PERIOD',&
   &/1X,10(I10,1X),100(:/1X,10(I10,1X)))
2160 FORMAT(1X,109('-'),/1X,10(F10.4,1X),100(:/1X,10(F10.4,1X)))
2170 FORMAT(//1X,'DOWNSTREAM REACH FOR EACH REACH GROUP ',&
   &'(RCHGRP) FOR STRESS PERIOD',/1X,83('-'),&
   &100(:/,6(1X,A2,I4.4,1X,'=',1X,I4)))
2171 FORMAT(//1X,'REACH RATING CURVE DATA FOR STRESS PERIOD',&
   &/1X,90('-'))
2172 FORMAT(/1X,'REACH   =',1X,I5,/,1X,&
   &'LAYSTR  =',1X,I5,/,1X,'LAYEND  =',1X,I5,/,1X,&
   &'         N',1X,'  ELEVATION [L]',1X,&
   &' XSECTAREA [L2]',1X,'    VOLUME [L3]',1X,&
   &'WET. PERIM. [L]',1X,'  TOP WIDTH [L]',1X,&
   &'SURF. AREA [L2]',/1X,10('-'),1X,6(15('-'),1X))
...
2190 FORMAT(/1X,'REACH GROUP =',1X,I5,&
   &1X,'GROUP =',1X,I5,//1X,&
   &'         N',1X,'  ELEVATION [L]',1X,&
   &'      DEPTH [L]',1X,'    VOLUME [L3]',&
   &/1X,10('-'),1X,3(15('-'),1X))
2200 FORMAT(1X,I10,2(1X,G15.7),1X,G15.7)
2400 FORMAT(//1X,'STEADY-STATE STRESS PERIOD')
4100 FORMAT( /8X,'    SURFACE-WATER ROUTING     ',&
   &/8X,' STRUCTURES USING TIMESERIES  ',&
   &/8X,'     DATA FOR OPERATION       ',&
   &/1X,43('-'),&
   &/1X,'     REACH',1X,' STRUCTURE',1X,&
   &' DATA TYPE',1X,' TAB. FILE',&
   &/1X,43('-'))
...
4223 FORMAT(1X,I5,20(:7X,G10.3))
   ...
   !
   !-------START SWR TIMER FOR GWF2SWR7RP
   CALL SSWRTIMER(0,tim1,tim2,SWREXETOT)
   !
   !-------CONFIRM THAT THERE ARE ACTIVE SURFACE-WATER ROUTING REACHES
   IF( NREACHES.LT.1 ) RETURN
   !
   !-------SET STEADY-STATE SWR FLAG FOR CURRENT STRESS PERIOD
   !         PSEUDO-TRANSIENT CONTINUATION APPROACH USED IF STEADY-STATE
   ISWRSS = 0
   ...
   END IF
   !
   !-------READ ITMP(FLAG TO REUSE DATA) - INPUT ITEM 5
   CALL SSWRDSOUT('5')
   CALL SSWR_RD_COMM(In)
   lloc = 1
   CALL URDCOM(In,IOUT,LINE)
   ...
   !
   !-------USE DATA FROM LAST STRESS-PERIOD OF ITMP LESS THAN 1
   !       ISSUE ERROR MESSAGE IF ITMP LESS THAN 1 AND FIRST STRESS PERIOD
   IF (ITMP.LT.1) THEN
      IF (Kkper.GT.1) THEN
         WRITE (IOUT,2000)
         RETURN
   ...
   END IF
   !
   !-------READ REST OF STRESS PERIOD DATA FLAGS - INPUT ITEM 5
   ...
   END IF
   !-------CHECK FOR ERRORS ON FIRST STRESS PERIOD
   IF (Kkper.EQ.1) THEN
   ...
   END IF
   !
   !-------INITIALIZE AND ALLOCATE TEMPORARY STORAGE
   REACH(:)%IGEONUM = -1
   isa = 0
   IF (IRDSTR.GT.0) THEN
   ...
   !           READ REACH NUMBER
         ...
         IF ( irch.LT.1 .OR. irch.GT.NREACHES ) THEN
            ...
            &i, 'REACH', irch, NREACHES
            CALL USTOP(' ')
         END IF
         REACH(irch)%ISWRBND = INT(rlist(2,i))
   !           START: RESET CURRENT VALUES TO ZERO FOR INACTIVE SWR REACHES
         IF ( REACH(irch)%ISWRBND.EQ.0 ) THEN
            ...
            END DO
         END IF
   !           END:   RESET CURRENT VALUES TO ZERO FOR INACTIVE SWR REACHES
      END DO
   !         CLEAN UP TEMPORARY LIST STORAGE
      DEALLOCATE (rlist)
   !
   !---------CHECK THAT BOUNDARY DATA IS CORRECTLY SPECIFIED FOR
   !           REACH GROUPS WITH MORE THAN ONE REACH
   ...
   !           ACTIVE REACHES
         ...
         END IF
      END DO
   !---------WRITE SUMMARY OF BOUNDARIES FOR EACH REACH GROUP
   !         AND COUNT THE NUMBER OF ACTIVE REACH GROUPS
      ibndfail = 0
      NACTIVE  = 0
      WRITE (IOUT,'(//2X,A,/1X,4(A10,1X),/1X,4(10("-"),1X))')&
      &'SUMMARY OF SWR REACH BOUNDARY CONDITIONS',&
      &'       IRG','  INACTIVE','  CONSTANT','    ACTIVE'
      DO n = 1, NRCHGRP
         WRITE (IOUT,'(1X,4(I10,1X))')&
         &n, iinactive(n), iconstant(n), iactive(n)
   !           INCREMENT THE NUMBER OF ACTIVE REACH GROUPS
         IF ( iactive(n).GT.0 ) NACTIVE = NACTIVE + 1
   !           INACTIVE AND ACTIVE REACHES IN REACH GROUP - ERROR CONDITION
         IF ( iinactive(n).GT.IZERO .AND. iactive(n).GT.IZERO ) THEN
            ibndfail = 1
         END IF
   !           CONSTANT AND ACTIVE REACHES IN REACH GROUP - ERROR CONDITION
         IF ( iconstant(n).GT.IZERO .AND. iactive(n).GT.IZERO ) THEN
            ibndfail = 1
         END IF
   !           INACTIVE AND CONSTANT REACHES IN REACH GROUP - ERROR CONDITION
         IF ( iconstant(n)*iinactive(n).GT.IZERO ) THEN
            ibndfail = 1
         END IF
      END DO
   !---------CHECK FOR CORRECT SPECIFICATION OF BOUNDARIES FOR REACH GROUPS
         ...
         DO i = 1, NREACHES
            ...
            IF ( REACH(i)%ISWRBND.GT.0 ) THEN
               IF ( RCHGRP(irg)%CONSTANT ) THEN
                  ...
                  &'ALL REACHES IN RCHGRP',irg,&
                  &'SHOULD BE SPECIFIED AS CONSTANT STAGE REACHES'
               END IF
               IF ( RCHGRP(irg)%INACTIVE ) THEN
                  WRITE (IOUT,'(/1X,A,1X,I5,1X,A)')&
                  &'ALL REACHES IN RCHGRP',irg,&
                  &'SHOULD BE SPECIFIED AS INACTIVE REACHES'
               END IF
            END IF
         END DO
      ...
      END IF
   !
   !---------WRITE SUMMARY OF CONSTANT STAGE REACH GROUPS
      k = 0
      WRITE (IOUT,2141)
      DO n = 1, NRCHGRP
   ...
   !
   !-------CLEAN UP TEMPORARY SWR BOUNDARY STORAGE
   ...
   END IF
   !
   !-------PRINT REACH BOUNDARY AND STRESS DATA
   IF (IPTFLG.GT.0) THEN
   !         BOUNDARY DATA
      IF ( IRDBND.GT.0 ) THEN
         WRITE (IOUT,2081)
   ...
   !         READ REACH NUMBER
         ...
         IF ( irch.LT.1 .OR. irch.GT.NREACHES ) THEN
            WRITE (IOUT,2019)&
            &'INPUT ITEM 10: SWR REACH GEOMETRY DEFINITION',&
            &i, 'REACH', irch, NREACHES
            CALL USTOP(' ')
         END IF
         ival = INT(rlist(2,i))
   ...
   !         CLEAN UP TEMPORARY LIST STORAGE
      DEALLOCATE (rlist)
   !         GET UNIQUE GEOMETRY NUMBERS
      CALL SSWR_UNIQVALUES(ug)
   !       DATA SET 11A-11C - GEOMETRY DATA
      CALL SSWRDSOUT('11')
      CALL SSWR_RD_COMM(In)
      iclose = 0
      CALL SSWREXTRD(In,IOUT,iut,iclose)
   ...
   !           LEAKANCE DATA
         ...
         END IF
   !
   !-----------READ IRREGULAR GEOMETRY AND SPECIFIED RATING CURVE
         IF (igeotype.EQ.3 .OR. igeotype.EQ.4) THEN
            CALL SSWR_RD_COMM(iut)
         ...
         END IF
   !
   !-----------FILL EACH REACH WITH SPECIFIED GEOMETRY DATA
         FILLRCH: DO irch = 1, NREACHES
            ...
            &gmanning,gwidth,gbelev,gsslope,&
            &gcnd,glk,gcndln,getextd,geostor)
   !-------------SET STARTING AND ENDING LAYER
            CALL SSWR_SET_RCHLAY(irch)
         END DO FILLRCH
   !
   !-----------DETERMINE IF ANY QAQ CALCULATIONS ARE PERFORMED
         ICALCQAQ = 0
   ...
   !             SPECIFIED STAGE-AREA-VOLUME RELATION
            ELSE IF (igeotype.EQ.4) THEN
   !             POTENTIAL SURFACE STORAGE COVERING THE ENTIRE CELL (E.G. LAKE)
   ...
   !         CLEAN UP TEMPORARY STORAGE
      DEALLOCATE(ug)
   !         DETERMINE IF GEOMETRY DATA FOR 11B IS READ FROM AN
   !         EXTERNAL FILE SPECIFIED USING OPEN/CLOSE.  IF THIS IS
   !         TRUE, USE SSWREXTRD TO CLOSE FILE
      IF ( iclose.NE.0 ) CALL SSWREXTRD(In,IOUT,iut,iclose)
   END IF
   !
   !       RESET TWO-DIMENSIONAL GEOMETRY DATA
   ...
   END IF
   !
   !       OUTPUT IRREGULAR CROSS-SECTION DATA
   ...
   !         RAW DATA
      DO irch = 1, NREACHES
         ...
         IF (igeotype.EQ.3) THEN
            ...
            DO j = 1, ngeopts
               WRITE (IOUT,2062) j,&
               &REACH(irch)%IRRGEO%XB(j),REACH(irch)%IRRGEO%ELEVB(j)
            END DO
         ELSE IF (igeotype.EQ.4) THEN
            WRITE (IOUT,2063) irch
            DO j = 1, ngeopts
               WRITE (IOUT,2064) j, REACH(irch)%GEO%ELEV(j),&
               &REACH(irch)%GEO%XAREA(j), REACH(irch)%GEO%VOL(j),&
               &REACH(irch)%GEO%WETPER(j),&
               &REACH(irch)%GEO%TOPWID(j), REACH(irch)%GEO%SAREA(j)
            END DO
         END IF
      END DO
   !         PROCESSED DATA
   ...
   !         DATA SET 12
      DO i = 1, IRDSTR
         ...
         IF ( irch.LT.1 .OR. irch.GT.NREACHES ) THEN
            WRITE (IOUT,2019)&
            &'INPUT ITEM 12: SWR REACH STRUCTURE DEFINITION',&
            &i, 'REACH', irch, NREACHES
            CALL USTOP(' ')
         END IF
         ival = INT(rlist(2,i))
   ...
   !
   !         DATA SET 13A
   !         READ REACH STRUCTURE ASSIGNMENT DATA
      CALL SSWRDSOUT('13')
      CALL SSWR_RD_COMM(In)
      iclose = 0
      CALL SSWREXTRD(In,IOUT,iut,iclose)
   ...
   !           ERROR WITH  DATA
         ...
         IF ( istrnum.LT.1 .OR.&
         &istrnum.GT.REACH(istrrch)%NSTRUCT ) THEN
         ...
         END IF
   !           CHECK THAT SPECIFIED STRUCTURE CONNECTION IS VALID FOR THE REACH
   ...
   !             NO STRUCTURE
          CASE (0)
   !             EXCESS VOLUME CONTROLLED BY DEPTH OR STAGE
          CASE (1)
   !             FREE CONNECTION
          CASE (2)
            IF ( istrconn.EQ.IZERO ) THEN
   !                 ZERO-DEPTH GRADIENT BOUNDARY CONDITION
   ...
   !                 CRITICAL-DEPTH BOUNDARY CONDITION
            ...
            ELSE
               ...
               &REACH(istrrch)%GEO%ELEV(1) + SMALL
            END IF
   !               TEST USE OF STRLEN AND STRLEN2 FOR UNCONTROLLED FLOW STRUCTURES - JDH 11/28/2012
            CALL URWORD(line,lloc,istart,istop,3,ival,r,-IOUT,iut)
            IF ( r.LE.RZERO ) THEN
   ...
   !             SPECIFIED H-Q RELATION
          CASE (4)
            ...
            IF(ALLOCATED(REACH(istrrch)%STRUCT(istrnum)%STRELEV))THEN
               ...
               &REACH(istrrch)%STRUCT(istrnum)%STRQ)
            END IF
            ALLOCATE(REACH(istrrch)%STRUCT(istrnum)%STRELEV(ival),&
            &REACH(istrrch)%STRUCT(istrnum)%STRQ(ival))
            CALL SSWR_RD_COMM(iut)
   ...
   !               UPDATE GATE BOTTOM
            ...
            CALL URWORD(line,lloc,istart,istop,2,ival,r,IOUT,iut)
            REACH(istrrch)%STRUCT(istrnum)%ISTRDIR = ival
   !               FIXED AND OPERABLE GATED SPILLWAY
          CASE (7,9,10)
            CALL URWORD(line,lloc,istart,istop,3,ival,r,IOUT,iut)
            REACH(istrrch)%STRUCT(istrnum)%STRCD = SSWR_R2D(r)
            CALL URWORD(line,lloc,istart,istop,3,ival,r,IOUT,iut)
   ...
   !               UPDATE GATE TOP
            ...
            CALL URWORD(line,lloc,istart,istop,2,ival,r,IOUT,iut)
            REACH(istrrch)%STRUCT(istrnum)%ISTRDIR = ival
   !             INFLOW FROM SFR PACKAGE DIVERSION
          CASE (11)
            IF ( istrconn.EQ.IZERO ) THEN
               istrbc = 5
               REACH(istrrch)%STRUCT(istrnum)%ISTRBC = istrbc
            ...
            IF ( ISFRUNIT.LT.1 ) THEN
               ...
               &'IF ISTRTYPE=11')
            END IF
   !             STAGE-DEPENDENT FLUX STRUCTURE
          CASE (12)
            ...
            IF ( ival.GT.0 ) THEN
               IF ( NTABS.LT.1 ) THEN
                  ...
                  &'TO USE EXTERNAL STRUCTURE DATA')
               END IF
               IF ( TABDATA(ival)%ITABTYPE.NE.5 ) THEN
                  CALL USTOP('ERROR: TABULAR STRUCTURE DATA MUST BE '//&
                  &'SPECIFIED AS ITABTYPE=5')
               END IF
            END IF
   !               OVERBANK STRUCTURE BASED ON FIXED CREST WEIR
          CASE (13)
   !               MAKE SURE istrconn IS NOT ZERO
            IF (istrconn.EQ.0) THEN
               WRITE (IOUT,'(A,1X,I10,1X,A,1X,I10,1X,A,1X,I10)')&
               &'REACH', istrrch, 'ISTRNUM', istrnum,&
               &'ISTRCONN', istrconn
               WRITE(errmsg,'(A)')&
               &'ISTRTYPE=13 CANNOT BE '//&
               &'SPECIFIED FOR CONNECTIONS '//&
               &'THAT DO NOT INCLUDE ONE '//&
               &'MODEL-BASED REACH'
               WRITE(IOUT,'(1X,A)') errmsg
               CALL USTOP(errmsg)
            END IF
   !               DATA THAT IS DETERMINED FROM REACH DATA
            ...
            ELSE
               ...
               &'FOR CONNECTIONS THAT DO NOT'//&
               &'INCLUDE ONE MODEL-BASED REACH'
               WRITE(IOUT,'(1X,A)') errmsg
               CALL USTOP(errmsg)
            END IF
            REACH(istrrch)%STRUCT(istrnum)%STRINV = rbot
            REACH(istrrch)%STRUCT(istrnum)%STRWID = dlen
   !               DATA THAT SPECIFIED IN DATASET 13 FOR OVERBANK STRUCTURE
            CALL URWORD(line,lloc,istart,istop,3,ival,r,IOUT,iut)
            REACH(istrrch)%STRUCT(istrnum)%STRCD = SSWR_R2D(r)
            CALL URWORD(line,lloc,istart,istop,3,ival,r,IOUT,iut)
            REACH(istrrch)%STRUCT(istrnum)%STRCD3 = SSWR_R2D(r)
   ...
   !             UNDEFINED STRUCTURE TYPE
         ...
         END SELECT
   !
   !           ADDITIONAL BOUNDARY STRUCTURE DATA FOR COUPLING TO SFR
         IF ( istrconn.EQ.0 .AND. ISFRUNIT.GT.0 ) THEN
            ...
            END IF
         END IF
   !           ALLOCATE SPACE FOR RECALCULATING STRUCTURE FLOWS
         IF ( ABS(istrtype).EQ.3 .OR. ABS(istrtype).EQ.8 .OR.&
         &ABS(istrtype).EQ.9 .OR. ABS(istrtype).EQ.10 .OR.&
         &ABS(istrtype).EQ.12 ) THEN
            ALLOCATE(REACH(istrrch)%STRUCT(istrnum)%OPRVAL(NTMAX))
         END IF
   !           ALLOCATE SPACE FOR SAVING STRUCTURE DATA IF NECESSARY
         IF ( ISWRPSTR.NE.0 ) THEN
            ALLOCATE(REACH(istrrch)%STRUCT(istrnum)%RSLTS(NPMAX))
         END IF
   !           DATASET 13B - STRUCTURE OPERATION DATA
         OPRDATA: IF ( ABS(istrtype).EQ.1  .OR.&
         &ABS(istrtype).EQ.3  .OR.&
         &ABS(istrtype).EQ.8  .OR.&
         &ABS(istrtype).EQ.9  .OR.&
         &ABS(istrtype).EQ.10 ) THEN
            REACH(istrrch)%NOPR = REACH(istrrch)%NOPR + 1
            IF ( istrtype.LT.0 ) THEN
         ...
         END IF OPRDATA
      END DO
   !         CHECK IF ANY STRUCTURE CONNECTION ERRORS DETECTED
      ierr = SUM( istrerr )
      IF ( ierr.GT.0 ) THEN
         CALL USTOP('ERROR: INCONSISTENT STRUCTURE CONNECTIONS '//&
         &'SEE LISTING FILE')
      END IF
   !         RECALCULATE NUMBER OF STRUCTURES IN EACH REACH GROUP
      DO n = 1, NRCHGRP
   ...
   !                 UNCONTROLLED FLOW
                CASE (2)
                  ...
                  ELSE
                     cstruct(11) = 'UNKN. CON.'
                  END IF
   !                 TEST USE OF STRLEN AND STRLEN2 FOR UNCONTROLLED FLOW STRUCTURES - JDH 11/28/2012 - JDH 11/28/2012
                  WRITE (cstruct( 8),2110) REACH(i)%STRUCT(j)%STRLEN +&
                  &REACH(i)%STRUCT(j)%STRLEN2
   !                 SPECIFIED DISCHARGE
                CASE (3)
   ...
   !                 CULVERTS
                CASE (5)
                  ...
                  END IF
   !                 FIXED AND OPERABLE CREST WEIRS
   !                   AND OVERBANK STRUCTURE BASED ON FIXED CREST WEIR
                CASE (6,8,13)
                  WRITE (cstruct( 1),2110) REACH(i)%STRUCT(j)%STRCD
   ...
   !                 STAGE-DEPENDENT FLUX STRUCTURE
               ...
               &REACH(i)%STRUCT(j)%ISTRCONN,&
               &(ADJUSTR(cstruct(k)),k=1,11),&
               &cstrbc(REACH(i)%STRUCT(j)%ISTRBC)
            END DO
         END DO WSDATA
   !
   !-----------WRITE SUMMARY OF RATING CURVE STRUCTURES
         indx = 0
            ...
            DO j = 1, REACH(i)%NSTRUCT
               ...
               IF ( istrtype.EQ.4 ) THEN
                  ...
                  DO istrpts = 1, REACH(i)%STRUCT(j)%NSTRPTS
                     ...
                     &REACH(i)%STRUCT(j)%STRELEV(istrpts)
                     WRITE (cstruct( 2),2110)&
                     &REACH(i)%STRUCT(j)%STRQ(istrpts)
                     WRITE (IOUT,2120) i, j, REACH(i)%STRUCT(j)%ISTRTYPE,&
                     &REACH(i)%STRUCT(j)%ISTRCONN,&
                     &(ADJUSTR(cstruct(k)),k=1,2)
                  END DO
                  WRITE (IOUT,4290)
               END IF
            END DO
   ...
   !
   !-----------WRITE SUMMARY OF OPERATIONAL DATA FOR STRUCTURES
         indx = 0
         WOPRDATA: DO i = 1, NREACHES
            ...
            DO j = 1, REACH(i)%NSTRUCT
               ...
               &istrtype.EQ.10 ) THEN
                  ...
                  WRITE (IOUT,2120) i, j, REACH(i)%STRUCT(j)%ISTRTYPE,&
                  &REACH(i)%STRUCT(j)%ISTRCONN,&
                  &(ADJUSTR(cstruct(k)),k=1,8)
               END IF
            END DO
         END DO WOPRDATA
   !
   !-----------WRITE SUMMARY OF SFR DATA FOR STRUCTURES
         indx     = 0
         ierr11   = 0
         ierrstrm = 0
   ...
   !               ONLY WRITE FOR STRUCTURES CONNECTED TO SFR
               IF ( isfrrch.LT.1 ) CYCLE
               istrtype =  REACH(i)%STRUCT(j)%ISTRTYPE
               isfrseg  =  REACH(i)%STRUCT(j)%ISFRSEG
               isfrstrm =  REACH(i)%STRUCT(j)%ISFRSTRM
   !               ERROR CHECKING
   !                 EVALUATE IF MORE THAN ONE ISTRTYPE=11 SPECIFIED FOR
   !                 A REACH
               IF ( istrtype.EQ.11 ) THEN
                  istr11 = istr11 + 1
                  IF ( istr11.GT.1 ) ierr11 = ierr11 + 1
               END IF
   !                 EVALUATE IF MORE THAN ONE SFR REACH IS SPECIFIED AS THE
   !                 DISCHARGE LOCATION FOR SWR STRUCTURES IN A SINGLE SWR REACH
   ...
   !               WRITE SFR DATA
               ...
               &REACH(i)%STRUCT(j)%ISTRCONN,&
               &(ADJUSTR(cstruct(k)),k=1,3)
            END DO
   !             IF SFR ERRORS ARE ENCOUNTERED WRITE ERROR MESSAGES AND TERMINATE
            IF ( ierr11.GT.0 ) THEN
               WRITE (IOUT,4265) ierr11
               CALL USTOP('SWR1 ERROR: MORE THAN ONE ISTRTYPE=11 '//&
               &'SPECIFIED AT LEAST ONE REACH')
            END IF
            IF ( ierrstrm.GT.0 ) THEN
               WRITE (IOUT,4270) ierrstrm
               CALL USTOP('SWR1 ERROR: MORE THAN ONE SFR COUPLING '//&
               &'LOCATION SPECIFIED AT LEAST ONE REACH')
            END IF
         END DO WSFRDATA
4200     FORMAT(/1X,'SURFACE-WATER ROUTING STRUCTURE OPERATION DATA ',/1X,&
         &'     REACH',1X,' STRUCTURE',1X,'  ISTRTYPE',1X,&
         &'  ISTRCONN',1X,'O VARIABLE',1X,'  ISTRORCH',1X,&
         &' ISTRORCHC',1X,' OPR LOGIC',1X,'   STRCRIT',1X,&
         &'  STRCRITC',1X,'     STRRT',1X,'    STRMAX',&
         &/1X,132('-'))
4250     FORMAT(//1X,'SWR1 ERROR: ISTRCONN MUST BE 0 IF ISTRTYPE=11',&
         &/1X,'            REACH:',1X,I10,1X,'STRUCTURE:',1X,I10)
4255     FORMAT(//1X,'SWR1 ERROR: THE SFR PACKAGE MUST BE ACTIVE'&
         &1X,'IF ISTRTYPE=11',&
         &/1X,'            REACH:',1X,I10,1X,'STRUCTURE:',1X,I10)
4260     FORMAT(/1X,'SURFACE-WATER SFR STRUCTURE CONNECTION DATA ',/1X,&
         &'     REACH',1X,' STRUCTURE',1X,'  ISTRTYPE',1X,&
         &'  ISTRCONN',1X,'   ISFRSEG',1X,'   ISFRRCH',1X,&
         &'  ISFRSTRM',&
         &/1X,76('-'))
4265     FORMAT(//1X,'SWR1 ERROR: MORE THAN ONE SFR INFLOW STRUCTURE'&
         &1X,'(ISTRTYPE=11) SPECIFIED FOR AT LEAST ONE REACH',&
         &/3X,'NUMBER OF ISTRTYPE=11 ERRORS:',1X,I10)
4270     FORMAT(//1X,'SWR1 ERROR: DIFFERENT SFR COUPLING LOCATIONS'&
         &1X,' SPECIFIED FOR SWR STRUCTURES IN AT LEAST ONE REACH',&
         &/3X,'NUMBER OF SFR COUPLING LOCATION ERRORS:',1X,I10)
4280     FORMAT(/1X,'SURFACE-WATER STAGE-DISCHARGE STRUCTURE DATA ',/1X,&
         &'     REACH',1X,' STRUCTURE',1X,'  ISTRTYPE',1X,&
         &'  ISTRCONN',1X,'   STRELEV',1X,'      STRQ',&
         &/1X,66('-'))
4290     FORMAT(/1X)

   !
   !-----------WRITE SUMMARY OF STRUCTURES THAT USE TIMESERIES DATA
   !           TO SPECIFY STRCRIT OR STRVAL
         indx = 0
   ...
   !
   !-------READ STAGE DATA - APPLIED ON STRESS PERIOD 1 OR FOR CONSTANT STAGE REACHES
   ird2d = 0
   ...
   !             READ REACH NUMBER
            ...
            IF (irchstg.LT.0 ) THEN
               IF ( Kkper.EQ.1 ) THEN
                  WRITE (IOUT,*)&
                  &'STAGE CANNOT BE SPECIFIED USING ANOTHER '//&
                  &'REACH FOR THE FIRST STRESS PERIOD'
               ...
               IF ( iotherrch.LT.1 .AND. irchstg.LE.iotherrch ) THEN
                  WRITE (IOUT,*)&
                  &'INVALID SOURCE REACH DEFINING STAGE FOR '//&
                  &'ANOTHER REACH SPECIFIED'
   ...
   !           PROCESS 2D DATA
         DO irch = 1, NREACHES
            ...
            RCHGRP(irg)%IRGUPDATE = 1
         END DO
   !           DEALLOCATE TEMPORARY STORAGE FOR 2D STAGE DATA
         DEALLOCATE( stage2d )
      END IF
   !         UPDATE OFFSETS FOR RCHGRP
      CALL SSWR_SET_RCHOFF(istage,dstage)
      IF (IPTFLG.GT.0) WRITE (IOUT,2021) Kkper
   !         UPDATE dstage IF LESS THAN REACH BOTTOM
   !         AND WRITE RESULTS TO IOUT
      DO i = 1, IRDSTG
         irchstg = istage(i)
   ...
   !           REACH OFFSET
      ...
      END IF
   !         CLEAN UP TEMPORARY STAGE STORAGE
      DEALLOCATE(istage,dstage,cstage)
   END IF
   !
   !-------SET REACH STAGE TO BOTTOM OF EACH REACH ON THE FIRST STRESS PERIOD IF
   !       IRDSTG.EQ.0
   ...
   !       END OF REACH DATA
   !
   !-------SET REACH OFFSET RELATIVE TO GROUP ELEVATION FOR EACH REACH AND
   !         DOWNSTREAM REACH FOR EACH REACH GROUP (RCHGRP)
   IF ( IRDGEO.GT.0 .OR. IRDSTG.GT.0 ) THEN
   !
   !---------CALCULATE STAGE-VOLUME RELATION FOR EACH REACH GROUP
      CALL SSWR_CALC_RATCURVE()
      IF (IPTFLG.GT.0) THEN
   !           RATING CURVE FOR REACH GROUP
         writece = .FALSE.
         DO i = 1, NRCHGRP
   ...
   !             WRITE DATA
            ...
            DO j = 1, SIZE(RCHGRP(k)%RGELEV)
               ...
               WRITE (IOUT,2200) j,&
               &RCHGRP(k)%RGELEV(j), depth, RCHGRP(k)%RGVOL(j)
            END DO
         END DO
      END IF
   END IF
   !
   !-------READ AUXILIARY VARIABLES DATA - IF REQUIRED
   ...
   END IF
   !
   !-------READ DIRECT RUNOFF DATA
   IF ( ISWRDRO.NE.0 ) THEN
      CALL SSWR_RDDRO()
   END IF
   !
   !-------CALCULATE CONNECTIVITY, ALLOCATE/FILL IA AND JA FOR
   !         COMPRESSED STORAGE OF CONNECTIVITY, AND REACH
   !         CONNECTIVITY FOR EACH RCHGRP
   IF (IRDGEO.GT.IZERO .OR. IRDSTR.GT.IZERO .OR.&
   &IRDBND.GT.IZERO) THEN
      CALL SSWR_SET_CONN()
   !
   !---------ALLOCATE NECESSARY MEMORY FOR QCONN ON FIRST STRESS PERIOD
      IF ( Kkper.EQ.1 ) THEN
         DO irg = 1, NRCHGRP
            nc = RCHGRP(irg)%NCONN
            IF ( nc.GT.0 ) THEN
   ...
   !         WRITE CONNECTIVITY DATA TO OUTPUT FILE
      IF (IPTFLG.GT.0) THEN
   !           WRITE SUMMARY OF REACH GROUPS
         WRITE (IOUT,2360) NRCHGRP
   !           WRITE PROBLEM DIMENSION DATA FOR NEWTON SOLVER
         IF ( IRDBND.GT.0 ) THEN
   ...
   !           IRCHN AND IRCHC FOR RG
            ...
            DO n = 1, NRCHGRP
               ...
               DO i = 1, RCHGRP(n)%NCONN
                  ...
                  &RCHGRP(n)%IRCHN(i),'TO',RCHGRP(n)%IRCHC(i)
               END DO
               WRITE (IOUT,2350) 'IRCHC',&
               &(ccerch(i),i=1,MAX(RCHGRP(n)%NCONN,1))
               WRITE (IOUT,2230)
            END DO
   ...
   END IF
2210 FORMAT(/1X,59('-'),&
   &/1X,'MODIFIED COMPRESSED ROW STORAGE DATA STRUCTURE',&
   &/3X,'DIAGONAL IS FIRST ENTRY IN ENTRIES FOR INDIVIDUAL',&
   &/3X,'SOLUTION REACH GROUPS')
2220 FORMAT(A4,1X,9(I5,1X))
2230 FORMAT(1X,59('-'))
2240 FORMAT(A4,1X,9(A5,1X))
2250 FORMAT(/,'cont.')
2320 FORMAT(/1X,59('-'),&
   &/1X,'FULL REACH CONNECTIVITY FOR EACH REACH GROUP',&
   &/3X,'A NEGATIVE IRCHC REACH NUMBER INDICATES THE CONNECTION',&
   &/3X,'IS STRUCTURE CONTROLLED')
2330 FORMAT(/1X,59('-'),/1X,'RCHGRP =',1X,I5,/1X,'IRGNUM =',1X,I5,&
   &/1X,14X,17('-'),'CONNECTIONS',17('-'),&
   &/1X,14X,3(I14,1X),:100(/1X,14X,3(I14,1X)))
2350 FORMAT(1X,A5,1X,'REACHES',1X,3(A14,1X),:100(/1X,14X,3(A14,1X)))
2360 FORMAT(/1X,59('-'),&
   &/1X,'NUMBER OF REACH GROUPS                  = ',I5)
2370 FORMAT( 1X,'LU DECOMPOSITION STORAGE APPROACH:',/,3X,A50)
2380 FORMAT( 1X,'OFF DIAGONAL DIMENSION (JAC%NHALFB)     =',I5,&
   &/1X,'MAXIMUM BANDWIDTH (JAC%NBW)             =',I5)
   !
   !-------SET CONNECTIVITY FOR STRUCTURES USING FLOW CRITERIA
   IF ( IRDSTR.GT.0 ) THEN
      STRFLOWCONN: DO istrrch = 1, NREACHES
         IF ( inewstr(istrrch).LT.1 ) CYCLE STRFLOWCONN
         DO istrnum = 1, REACH(istrrch)%NSTRUCT
            IF ( REACH(istrrch)%STRUCT(istrnum)%ISTROVAL.EQ.2 ) THEN
   !               REACH CONNECTION AND REACH GROUP
   ...
   !
   !
   !-------INITIALIZE IRG AND REACH VOLUMES ON FIRST STRESS PERIOD
   !       BASED ON UPDATED STAGES AND VOLUMES
   IF ( Kkper.EQ.1 ) THEN
      CALL SSWR_SET_INITVOL()
   END IF
   !
   !-------UPDATE STAGES USING CURRENT STAGES TO CALCULATE VOLUME AND OFFSETS
   IF ( IRDGEO.NE.IZERO .OR. IRDSTG.NE.IZERO ) THEN
      CALL SSWR_VOL2STG_UPDATE()
   END IF
   !
   !-------CALCULATE THE TOTAL NUMBER OF QAQ CONNECTIONS
   NQAQCONN = 0
   DO irch = 1, NREACHES
   ...
   END IF
   !
   !-------FINISH SWR TIMER FOR GWF2SWR7RP
   CALL SSWRTIMER(1,tim1,tim2,SWREXETOT)
   !
   !-------RETURN
   RETURN
...
SUBROUTINE GWF2SWR7AD(Kper,Kstp,Igrid,Iunitsub)
   !     ******************************************************************
   !     UPDATE PREVIOUS GROUP CONDITIONS WITH RESULTS FROM
   !     THE LAST TIMESTEP
   !
   !     COMPUTE SURFACE-WATER ROUTING STRUCTURE FLOWS FOR TIME STEP AT
   !     SURFACE-WATER REACHES WITH TIME-VARYING DATA.
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   CALL SGWF2SWR7PNT(Igrid)
   !
   !-------START SWR TIMER FOR GWF2SWR7AD
   CALL SSWRTIMER(0,tim1,tim2,SWREXETOT)
   !
   !-------IF NSWRS<=0 THEN THERE ARE NO SURFACE-WATER ROUTING REACHES.
   !-------RETURN.
   IF(NREACHES.LE.0) RETURN
   !
   !-------VERTICALLY ADJUST DATA USING SUBSIDENCE RESULTS IF NECESSARY.
   !       DONE PRIOR TO SETTING PREVIOUS DATA TO CURRENT DATA.
   ...
   !      END IF
   !
   !-------SET PREVIOUS REACH CONDITIONS (REACH(:)%PREVIOUS TO CONDITIONS FROM
   !         END OF LAST SWR TIMESTEP (REACH(:)%CURRENT)
   DO irch = 1, NREACHES
      RSTAGE(irch,0) = REACH(irch)%STAGE
   END DO
   !
   !-------SET PREVIOUS RCHGRP CONDITIONS (RCHGRP(:)%PREVIOUS) TO CONDITIONS
   !         FROM LAST TIME STEP (RCHGRP(:)%CURRENT)
   DO irg = 1, NRCHGRP
      GSTAGE(irg,0) = RCHGRP(irg)%STAGE
   END DO
   !
   !-------UPDATE INITIAL GATE OPENING DATA FOR CURRENT MODFLOW TIMESTEP TO
   !       GATE OPENING AT THE END OF THE LAST SWR1 TIMESTEP
   IF ( Kper.GT.1 ) THEN
      DO irch = 1, NREACHES
         IF ( REACH(irch)%NSTRUCT.LT.1 ) CYCLE
         DO n = 1, REACH(irch)%NSTRUCT
            REACH(irch)%STRUCT(n)%STRTOPINI =&
            &REACH(irch)%STRUCT(n)%STRTOP
            REACH(irch)%STRUCT(n)%STRBOTINI =&
            &REACH(irch)%STRUCT(n)%STRBOT
         END DO
      END DO
   END IF
   !
   !-------SET NUMTIME0 = NUMTIME TO PRESERVE TIMESTEP FROM PREVIOUS
   !       TIMESTEP
   NUMTIME0 = NUMTIME
   RTIME0   = RTIME
   RSWRPRN0 = RSWRPRN
   NADPCNT0 = NADPCNT
   !
   !-------COMPUTE PROPORTION OF STRESS PERIOD TO CENTER OF THIS TIME STEP
   ...
   ENDIF
   !
   !-------FINISH SWR TIMER FOR GWF2SWR7AD
   CALL SSWRTIMER(1,tim1,tim2,SWREXETOT)
   !
   !-------RETURN
   RETURN
...
SUBROUTINE GWF2SWR7EX_A(Igrid,Iflag,Nlen,Imap,Rval)
   !     *****************************************************************
   !     FILL THE EXTERNAL FLOW ACCUMULATORS QUZFLOW AND QEXTFLOW FOR
   !       THE SWR PROCESS USING AN ARRAY
   !
   !     THIS SUBROUTINE IS CALLED EXTERNAL TO THE SWR PROCESS.
   !
   !       Nlen, Imap, AND Rval MUST BE ALLOCATED AND SPECIFIED EXTERNALLY
   !         TO THE SWR PROCESS.
   !
   !       Iflag IS AN INTEGER FLAG THAT IDENTIFIES THE APPROPRIATE EXTERNAL
   !         REACH FLOW TERM TO ADD THE VALUE (Rval) TO.  CURRENTLY Iflag=1
   !         IDENTIFIES Rval IS ADDED TO QUZFLOW AND ANY OTHER VALUE RESULTS
   !         IN Rval BEING ADDED TO QEXTFLOW.
   !
   !       Nlen IS THE LENGTH OF THE MAPPING (Imap) AND VALUE (Rval)
   !         VECTORS.
   !
   !       Imap IS A MAPPING VECTOR TO IDENTIFIES THE REACH NUMBER
   !         ASSOCIATED WITH THE DATA IN Rval.  IF Iflag=1, IUZFOFFS
   !         IS SUBTRACTED FROM Imap TO DETERMINE THE SWR REACH NUMBER.
   !
   !       Rval IS A SINGLE PRECISION VECTOR THAT CONTAINS THE EXTERNALLY
   !         CALCULATED (EXTERNAL TO THE SWR PROCESS) FLOW DATA.  Rval IS
   !         ADDED TO QUZFLOW OR QEXTFLOW WHICH IS ADDED TO THE CONTINUITY
   !         EQUATION SOLVED FOR EACH REACH GROUP.
   !
   !       IEXTFLOW IS SET TO ZERO AT THE END OF THE GWF2SWR7FM SUBROUTINE
   !         WHICH INDICATES THE QUZFLOW AND QEXTFLOW ACCUMULATORS ARE TO
   !         BE RESET TO ZERO AT THE BEGINNING OF THE GWF2SWR7EX SUBROUTINE.
   !         AFTER SETTING QUZFLOW AND QEXTFLOW TO ZERO IEXTFLOW IS SET
   !         TO ONE.
   !
   !       POSITIVE Rval VALUES INDICATE A SOURCE OF WATER TO A REACH.
   !       NEGATIVE Rval VALUES INDICATE A SINK   OF WATER TO A REACH.
   !
   !     VERSION  1.0: JUNE 28, 2011
   !     *****************************************************************
   ...
   CALL SGWF2SWR7PNT(Igrid)
   !
   !---------START SWR TIMER FOR GWF2SWR7EX
   CALL SSWRTIMER(0,tim1,tim2,SWREXETOT)
   !
   !-------REINITIALIZE QUZFLOW AND QEXTFLOW IF IEXTFLOW = 0
   IF ( IEXTFLOW.LT.1 ) THEN
      CALL SSWR_EXINIT()
      IEXTFLOW = 1
   END IF
   !
   !---------DETERMINE OFFSET FOR THE Imap VECTOR TO ASSIGN
   !         Rval TO THE APPROPRIATE SWR REACH
   SELECT CASE (Iflag)
    CASE (1)
      ioffset = IUZFOFFS
    CASE DEFAULT
   ...
   END SELECT
   !
   !---------ADD Rval TO APPROPRIATE EXTERNAL REACH FLOW TERM (QUZFLOW OR QEXTFLOW)
   DO n = 1, Nlen
      irch = Imap(n) - ioffset
...
SUBROUTINE GWF2SWR7EX_V(Igrid,Iflag,Imap,Rval)
   !     *****************************************************************
   !     FILL THE EXTERNAL FLOW ACCUMULATORS QUZFLOW AND QEXTFLOW FOR
   !       THE SWR PROCESS USING A SINGLE VALUE
   !
   !     THIS SUBROUTINE IS CALLED EXTERNAL TO THE SWR PROCESS.
   !
   !       Iflag IS AN INTEGER FLAG THAT IDENTIFIES THE APPROPRIATE EXTERNAL
...
SUBROUTINE GWF2SWR7ST_STAGE(Igrid,Irch,Dval,Rtim)
   ...
   !     THIS SUBROUTINE IS CALLED EXTERNAL TO THE SWR PROCESS.
   !
   !       Igrid IS AN INTEGER THAT IS USED TO SET THE POINTERS TO THE
   !        CORRECT GRID NUMBER
   !
   !       Irch IS AN INTEGER THAT IDENTIFIES THE REACH NUMBER
   !
   !       Dval IS A DOUBLE PRECISION VARIABLE THAT IS RETURNED. Dval IS
   !         THE STAGE IN REACH Irch AT TIME Rtim (IF Rtim IS DEFINED).
   !
   !       Rtim IS AN OPTIONAL REAL VARIABLE THAT IDENTIFIES THE TIME OF
   !         INTEREST. IF NOT INCLUDE THE STAGE AT THE END OF THE
   !         PREVIOUS MODFLOW TIME STEP (OR INITIAL VALUE) IS RETURNED.
   !
   ...
   CALL SGWF2SWR7PNT(Igrid)
   !
   !---------START SWR TIMER FOR GWF2SWR7ST_STAGE
   CALL SSWRTIMER(0,tim1,tim2,SWREXETOT)
   !
   !---------FIND TIME INDEX
   ...
   END IF
   !
   !---------SET Dval TO STAGE AT Rtime
   Dval = RSTAGE(Irch,n)
   !
   !---------END SWR TIMER FOR GWF2SWR7ST_STAGE
   CALL SSWRTIMER(1,tim1,tim2,SWREXETOT)
   !
   !---------RETURN
   RETURN
...
SUBROUTINE GWF2SWR7FM(Kkiter, Kkper, Kkstp, Igrid)
   !     *****************************************************************
   !     ADD SURFACE-WATER TERMS TO RHS AND HCOF IF FLOW OCCURS IN MODEL CELL
   !     VERSION  7.1: JUNE 29, 2006
   !     *****************************************************************
   USE GWFSWRMODULE
...
2000 FORMAT(/1X,'WARNING:',1X,A,1X,'SOLVER',/,10X,&
   &'DID NOT CONVERGE FOR EACH',1X,&
   &'SURFACE WATER ROUTING TIMESTEP',/,10X,&
   &'TOLR EXCEEDED',1X,I5,1X,&
   &'TIMES IN STRESS PERIOD',1X,I5,1X,&
   &'AND TIME STEP',1X,I5,/,10X,&
   &'MAXIMUM ABSOLUTE ERROR (',G15.7,') IN RCHGRP',1X,I5,/,10X,&
   &'DURING SURFACE WATER ROUTING TIMESTEP',1X,I5,/)
2010 FORMAT(1X,/,1X,'MAXIMUM RESIDUAL FOR EACH SWR TIMESTEP',&
   &1X,'(NEG. ISWRDT VALUES INDICATE REL. TOLR EXCEEDED)',/,&
   &1X,3('ISWRDT,ITER.,MAX.RESID., LOC.',1X),/,&
   &1X,89('-'))
...
2050 FORMAT(1X,89('-'))
2060 FORMAT(/1X,&
   &/1X,27X,'ITERATIONS FOR EACH SWR TIMESTEP',&
   &/1X,14(I5,1X),100000(:/1X,14(I5,1X)))
2070 FORMAT(1X,84('-'))
2080 FORMAT(1X,14(I5,1X))
   ...
   CALL SGWF2SWR7PNT(Igrid)
   !
   !-------START SWR TIMER FOR GWF2SWR7FM
   CALL SSWRTIMER(0,tim1,tim2,SWREXETOT)
   !
   KMFITER = Kkiter
   !
   !-------SET RTMAX AND RTIME TO DELT IF RTMIN EQUALS RZERO
   IF ( ITSPMIN.EQ.1 ) THEN
      RTMIN = DELT
   ...
   END IF
   !
   !-------SCALE TIME BASED ON RAINFALL
   !         THIS IS A PREEMPTIVE STRIKE TO ADJUST THE TIMESTEP BASED ON
   !         RAINFALL INTENSITY
   ...
   END IF
   !
   !-------SCALE TIME BASED ON VERTICAL DISPLACEMENT CALCULATED BY THE
   !       SUBSIDENCE PACKAGE.
   !         THIS IS A PREEMPTIVE STRIKE TO ADJUST THE TIMESTEP BASED ON
   !         THE MAXIMUM ABS(DVZ) FIR ANY SWR REACH
   IF ( IADTIME.GT.IZERO .AND. Kkiter.EQ.1 ) THEN
   ...
   !
   !-------SET INITIAL AND RESTART POSITION
   DO i = 1, NREACHES
      IF ( Kkiter.NE.1 ) THEN
         rs = RSTAGE(i,1)
      ELSE
   ...
   stpend = RZERO
   !
   !-------TIME AT END OF LAST STRESS PERIOD
   DO n = 1, Kkper-1
   ...
   itss   = IZERO
   !
   !-------INITIALIZE FAILTIME ON FIRST ITERATION OF THE FIRST TIME STEP IN
   !       THE FIRST STRESS PERIOD
   IF ( Kkper.EQ.1 .AND. Kkstp.EQ.1 .AND. Kkiter.EQ.1 ) THEN
      FAILTIME = .TRUE.
   END IF
   !
   !-------SET RTIME TO RTIME0 ON THE FIRST SWR TIME STEP OF THE CURRENT
   !       MODFLOW TIME STEP
   RTIME   = RTIME0
   NADPCNT = NADPCNT0
   !       RESET RTIME TO RTMIN IF LAST STRESS PERIOD WAS STEADY-STATE
   IF ( Kkper.GT.1 ) THEN
   ...
   END IF
   !
   !-------UPDATE GROUNDWATER HEAD (HGW) USED IN QAQ CALCULATIONS
   CALL SSWR_UPDATE_QAQHGW()
   !
   !-------UPDATE DIRECT RUNOFF DATA IF NECESSARY
   IF ( ISWRDRO.NE.0 ) THEN
   ...
   SWRSUBTIME: DO
      ...
      ird    = 0
   !
   !---------SET LOWER BOUND FOR NEWTON ITERATIONS
      INWTCNT = 2
      IF ( INWTCORR.GT.0 ) INWTCNT = 1
   !
   !-------SET INITIAL SURFACE WATER ROUTING TIMESTEP BASED ON MODFLOW TIMESTEP LENGTH
      ...
      END IF
   !
   !---------DETERMINE RTIME USING TABULAR DATA
      IF( IADTIME.LT.IZERO ) THEN
         CALL SSWR_GETDT(swrtot,RTIME)
      END IF
   !
   !---------SET RTIME TO DELT IF STEADY-STATE STRESS PERIOD
      IF ( ISWRSS.GT.0 ) THEN
         NUMTIME = 1
         RTIME   = DELT
      END IF
   !
   !---------MAKE SURE RTIME NO SMALLER THAN RTMIN
      IF ( RTIME.LT.RTMIN ) THEN
         RTIME = RTMIN
      END IF
   !
   !---------MAKE SURE THE END OF THE CURRENT MODFLOW TIME STEP
   !         IS NOT EXCEEDED
      ttot = swrtot + RTIME
      ...
      END IF
   !
   !---------MAKE SURE THE CURRENT PRINT TIME IS NOT EXCEEDED
      ttot = swrtot + RTIME
      IF ( (RSWRPRN-ttot).LT.RTMIN .OR.&
      &ttot.GT.RSWRPRN ) THEN
      ...
      END IF
   !
   !---------SET FLAG INDICATING LAST GOOD SWRDT SHOULD BE USED
   !         ON NEXT TIME STEP
      IF ( iprn.NE.0 ) irstt  = 1
   !
   !---------SET FLAG FOR LAST SWR1 TIME STEP OF CURRENT
   !         MODFLOW TIME STEP
      ttot = swrtot + RTIME
      IF ( ABS(ttot-stpend).LT.RTMIN ) ilast = 1
   !
   !---------UPDATE STRCRIT AND STRVAL FOR STRUCTURES
   !         OPERATED USING TIMESERIES DATA
      ttot = swrtot + RTIME
      CALL SSWR_GET_STRGATE(swrtot,ttot)
   !
   !---------UPDATE REACH STRESS DATA
      IF ( NTABS.GT.0 ) THEN
         ttot = swrtot + RTIME
         CALL SSWR_GET_TABS(swrtot,ttot)
      END IF
   !
   !---------SET RESTART POSITION FOR EACH SWR TIME STEP
      DO i = 1, NREACHES
         JAC%S0(i) = REACH(i)%STAGE
      END DO
   !
   !---------GROUNDWATER FLOW NWT OUTER LOOP
      GWFNWT: DO
   !
   !---------ADAPTIVE TIME STEP ALGORITHM
         ADAPTIVE: DO
   !
   !-----------SET TIMESTEP FOR SURFACE-WATER ROUTING PACKAGE
            SWRDT = REAL( RTIME, 8 )
   !
   !-----------SET HEAD WEIGHTING FACTOR
            ...
            END IF
   !
   !-----------INITIALIZATION OF GROUP STAGE (ps) AND TEMPORARY REACH STAGES (s)
            DO i = 1, NREACHES
   ...
   !             STEADY STATE
            ...
            END DO
   !
   !-----------UPDATE STRUCTURE DISCHARGE (PUMP) RATES AND GATE LEVELS
            CALL SWR_CALC_STRVALS(JAC%PS)
   !
   !-----------SOLUTION USING APPROXIMATE NEWTON METHOD AND SPECIFIED SOLVER
            isoln = 0
            fmax0 = DZERO
            ...
            IF ( SWRTIME(n)%FMAX.GT.fmax ) THEN
               ...
               ipos = n
            END IF
   !           RESET checkresult IF TOLR EXCEEDED AFTER RETURN FROM
   !           LINEAR SOLVER FOR TRANSIENT CASES USING ADAPTIVE TIME-STEPPING
   ...
   !-----------TEMPORARY OUTPUT OF RESULTS
            IF ( ISWRSCRN.GT.0 .AND. INWTCNT.EQ.2 ) THEN
               ...
               WRITE (*,10)&
               &Kkper, Kkstp, NUMTIME,&
               &SSWR_SCCHAR(swrtot), SSWR_SCCHAR(swrtot+REAL(SWRDT,IKND)),&
               &SSWR_SCCHAR(REAL(SWRDT,IKND)),&
               &SWRTIME(n)%ITER, SWRTIME(n)%ILOCFMAX,&
               &SSWR_SCCHAR(REAL(SWRTIME(n)%FMAX,IKND)), checkresult
            END IF
5           FORMAT(1X,' ',' Per: Stp: SWRS:     Time0:     Time1:     SWRDT:',&
            &' Iters:  MaxL:  Max Resid: C')
10          FORMAT(1X,'+',2(I4,':'),I5,':',A10,':',A10,&
            &':',A10,':',&
            &1X,I5,':',1X,I5,&
            &':',1X,A10,':',1X,L1)

   !
   !-----------SOLVER FAILURE - RESTART ADAPTIVE AT BEGINNING OF SWR TIMESTEP
            FAILTIME = .FALSE.
            IF ( checkresult ) THEN
               FAILTIME = .TRUE.
               NADPCNT = 1
               IF ( IADTIME.GT.IZERO .AND. ISWRSS.LT.1 ) THEN
                  IF ( RTIME.GT.RTMIN ) THEN
                     ...
                     END DO
                     dtlast = RTIME
   !                 RESET REACH STAGES TO STAGES FROM THE END OF PREVIOUS TIMESTEP
                     DO i = 1, NREACHES
                  ...
                  ELSE
                     ...
                     EXIT ADAPTIVE
                  END IF
   !-------------NOT USING ADAPTIVE TIMESTEPPING
               ELSE
                  IF ( ISWRCONT.LT.1 ) THEN
                     CALL USTOP('SWR SOLUTION DID NOT CONVERGE')
                  END IF
            ...
            ird = 0
   !
   !-----------RESTART SWRSUBTIME AT BEGINNING OF SWR TIMESTEP IF
   !           STAGE CHANGES EXCEED DMAXSTG OR INFLOW EXCEEDS DMAXINF
            IF ( IADTIME.GT.IZERO .AND. ISWRSS.LT.1 ) THEN
               smax    = DZERO
               pinfmax = DZERO
               amax    = DZERO
   ...
   !                 CHECK IF NEW RTIME IS ESSENTIAL EQUAL TO PREVIOUS RTIME
   ...
   !               RESET REACH STAGES TO STAGES FROM THE END OF PREVIOUS TIMESTEP
            ...
            END IF
   !
   !-----------EXIT ADAPTIVE TIMESTEP
            EXIT ADAPTIVE
   !---------ADAPTIVE END
         END DO ADAPTIVE
   !---------INCREMENT GROUNDWATER FLOW NEWTON COUNTER
         IF ( INWTCNT.EQ.2 ) EXIT GWFNWT
         INWTCNT = INWTCNT + 1
   !---------GROUNDWATER FLOW NEWTON STEP END
      END DO GWFNWT
   !
   !---------SAVE RESULTS AT THE END OF THE ADAPTIVE SWR TIME STEP
   !
   !---------UPDATE REACH STAGE AND CURRENT GROUP USING ESTIMATED STAGE
      ...
      END DO
   !
   !---------ADD RCHGRP TERMS FOR TIMESTEP TO INCREMENTAL TERMS
      DO i = 1, NRCHGRP
   ...
   !---------SAVE CURRENT TIME STEP LENGTH
      ...
      END IF
   !
   !---------SET RTIME TO PREVIOUS RTIME IF IT WAS
   !         REDUCED THIS SWR1 TIME STEP TO HIT END OF MODFLOW
   !         TIME STEP OR PRINT TIME
      IF ( irstt.GT.0 ) THEN
         RTIME = dtlast
      END IF
   !
   !---------DETERMINE IF END OF MODFLOW TIMESTEP
      IF ( ilast.NE.0 ) THEN
         EXIT SWRSUBTIME
      END IF
   ...
   !
   !---------EXTEND THE SIZE OF SOLUTION ARRAYS IF NUMTINE IS GREATER THAN NTMAX
   ...
   END DO SWRSUBTIME
   !
   !-------SKIP GROUNDWATER PROCESS INTERACTION CALCULATIONS IF
   !       SURFACE-WATER ONLY SIMULATION ( ISWRONLY > 0 )
   IF ( ISWRONLY.NE.0 ) GOTO 200
   !
   !-------ADD INCREMENTAL AQUIFER-REACH TERMS FOR TIMESTEP TO HCOF AND RHS
   SWRADD2GWL: DO i = 1, NREACHES
   !         DO NOT EVALUATE INACTIVE REACHES
      IF ( REACH(i)%ISWRBND.EQ.0 )    CYCLE SWRADD2GWL
   !         DO NOT EVALUATE REACHES WITH ZERO CONDUCTANCES
      IF ( REACH(i)%ICALCQAQ.LT.1 ) CYCLE SWRADD2GWL
   !         DO NOT EVALUATE REACHES WITH NO UNDERLYING ACTIVE CELLS
   ...
   !-------------CURRENT ESTIMATE OF GROUNDWATER HEAD
            h0   = REAL( HOLD(jc,ir,kact), 8 )
            h1   = HNEW(jc,ir,kact)
            h    = ( DONE - fdelt ) * h0 + fdelt * h1
   !-------------DEFAULT CONDITION IS ADDING TERMS TO HCOF+RHS
            rhsonly = .FALSE.
   !-------------TEST FOR CASE WHERE HEAD IN LAYER IS BELOW REACH BOTTOM
            IF (    h.LT.rbot ) rhsonly = .TRUE.
            IF ( kact.GT.kl   ) rhsonly = .TRUE.
            IF ( rhsonly ) THEN
   !             SIGN CONVENTION FOR REACH QAQFLOW FOLLOWS MODFLOW CONVENTION
   !             QAQFLOW IS CALCULATED IN SSWR_RG_QAQ FUNCTION CALL
         ...
         END DO LAYERS
   !
   !-------------CALCULATE EXPLICIT SWR1 NEWTON TERMS
   ...
   !-------------CURRENT ESTIMATE OF GROUNDWATER HEAD
            ...
            END DO
   !             NEWTON CORRECTION OF HCOF MATRIX FOR MODFLOW-2005
            IF ( INWTUNIT.EQ.0 ) THEN
               HCOF(jc,ir,kact) = HCOF(jc,ir,kact) + hc
   !             NEWTON CORRECTION OF A MATRIX FOR MODFLOW-NWT
            ELSE
               ij   = 0
               !NEWTON
               ij   = ICELL(jc,ir,kact)
   ...
   END IF
   !
   !-------CALCULATE GROUNDWATER EVAPOTRANSPIRATION
   DO i = 1, NREACHES
      IF ( REACH(i)%IGEOTYPE.NE.5 ) CYCLE
      REACH(i)%GWETRATE = DZERO
   !         ONLY PROCESS FOR REACHES THAT HAD ANY POTENTIAL FOR EVAPORATION
      IF ( REACH(i)%EVAP.EQ.DZERO ) CYCLE
   !         ONLY CONTINUE PROCESS FOR REACHES WITH REMAINING EVAPORATION
      ir=REACH(i)%IRCH
      jc=REACH(i)%JRCH
      gwet = DZERO
      DO n = 1, NUMTIME
   ...
   !         FIND UPPER MOST ACTIVE LAYER
      k = 0
      ETLAYER: DO kl = 1, NLAY
         h = HNEW(jc,ir,kl)
   !-----------DETERMINE IF CELL IS DRY
   ...
   !         GROUNDWATER LEVEL ABOVE TOP OF MODEL
      IF ( h.GE.REACH(i)%GBELEV ) THEN
         RHS(jc,ir,k) = RHS(jc,ir,k) + gwet
   !         GROUNDWATER LEVEL BELOW TOP OF MODEL BUT ABOVE EXTINCTION DEPTH
   !         LINEAR RELATION BETWEEN GROUNDWATER HEAD AND GROUNDWATER EVAPOTRANSPIRATION
      ELSE IF ( h.GE.( REACH(i)%GBELEV-getextd ) ) THEN
         RHS(jc,ir,k)  = RHS(jc,ir,k) + gwet -&
         &gwet * REACH(i)%GBELEV / getextd
         HCOF(jc,ir,k) = HCOF(jc,ir,k) - gwet / getextd
      END IF
   !         DO NOT SIMULATE GROUNDWATER EVAPOTRANSPIRATION IF
   !         GROUNDWATER LEVEL IS BELOW EXTINCTION DEPTH
   END DO
   !
   !-------SET ISWRCNVG
200   ISWRCNVG = 1
   ...
   !
   !-------RESET FLAG TO RESET QEXTFLOW TO 0.0
   IEXTFLOW = 0
   !
   !-------END SWR TIMER FOR GWF2SWR7FM
   CALL SSWRTIMER(1,tim1,tim2,SWREXETOT)
   !
   !-------RETURN
   RETURN
...
SUBROUTINE GWF2SWR7CV(Kkiter, Igrid, Icnvg, Mxiter)
   !     *****************************************************************
   !     CHECK FOR CONVERGENCE OF SWR QAQFLOW RATES BETWEEN SUBSEQUENT
   !     INNER ITERATIONS - VERSION  7.0.10: JUNE 19, 2009
   !     *****************************************************************
...
2020 FORMAT(1X,'*** MODFLOW CONVERGENCE FAILURE ***',/,&
   &1X,'***      CONTINUING ANYWAY      ***',//)
   !     + + + CODE + + +
   !-------SET POINTERS FOR GRID
   CALL SGWF2SWR7PNT(Igrid)
   !
   !-------START SWR TIMER FOR GWF2SWR7CV
   CALL SSWRTIMER(0,tim1,tim2,SWREXETOT)
   !
   !-------ONLY A SINGLE CALL TO SWR SOLUTION REQUIRED IS
   !       SURFACE WATER ONLY SIMULATION (ISWRONLY > 0 )
   IF ( ISWRONLY.GT.0 ) THEN
      ICNVG = 1
      RETURN
   END IF
   !
   !-------RETURN IF SIMULTANEOUS SWR AND MODFLOW SOLUTION BUT SWR IS
   !       NOT COUPLED TO MODFLOW WITH A QAQ TERM
   IF ( ICALCQAQ.EQ.IZERO ) RETURN
   !
   !-------RETURN IF NOT A STEADY STATE SIMULATION AND TOLA LESS THAN ZERO
   IF ( ISWRSS.LT.1 .AND. TOLA.LT.DZERO .AND. ISWRCONT.LT.1 ) RETURN
   !
   !-------STEADY STATE EVALUATION OF VOLUME CHANGE
   IF ( ISWRSS.GT.0 ) THEN
      ssdv  = DZERO
      DO n = 1, NRCHGRP
         gs0 = GSTAGE(n,0)
   ...
   END IF
   !
   !-------CHECK FOR BASEFLOW CONVERGENCE, IF NECESSARY
   IF ( TOLA.LT.DZERO .AND. ISWRCONT.LT.1 ) GOTO 9999
   !
   !-------TOLA BASEFLOW CONVERGENCE TEST
   tt = TOLA
   IF ( TOLA.GT.DZERO ) THEN
      CALL SSWR_CALC_BFCNV()
      bfcrit = DZERO
   !---------SET CURRENT QAQFLOW SUMMATION
      incfrac   = DZERO
      pswrtot    = DZERO
      pmftot     = DZERO
      nswrtot    = DZERO
   ...
   !           DO NOT EVALUATE REACHES WITH ZERO CONDUCTANCES
      ...
      IF ( ITOLAOPT.EQ.1 ) THEN
         incfrac = ABS( totdiff )
      END IF
   !---------EVALUATE CHANGES IN QUZFLOW AND QEXTFLOW
   !---------SET CURRENT duzflow AND dexflow SUMMATION
      duzflow = DZERO
      dexflow = DZERO
      duztot0 = DZERO
      dextot0 = DZERO
   ...
   !             EXT
            ...
            END IF
         END IF
   !           RESET QUZFLOW0 AND QEXTFLOW0 FOR EACH REACH
   ...
   !           EXFLOW
      ...
      END IF
   !
   !---------MAKE SURE AT LEAST TWO OUTER ITERATIONS ARE PERFORMED
      IF ( Kkiter.EQ.1 ) THEN
         Icnvg = 0
         GOTO 9999
      END IF
   ...
   !
   !---------IF CONVERGENCE ACHIEVED IN ITERATIVE MODFLOW SOLVER
   !         MAKE SURE DIFFERENCES IN QAQFLOW SUMMATION FOR EACH
   !         REACH IS LESS THAN TOLA
      IF ( Icnvg.EQ.1 ) THEN
         ...
         END IF
      END IF
   !-------END OF TOLA BASEFLOW CONVERGENCE TEST
   END IF
   !
   !-------CONTINUE MODFLOW SIMULATION IF SWR CONTINUATION FLAG SET
   ...
   END IF
   !
   !-------END SWR TIMER FOR GWF2SWR7CV
9999  CALL SSWRTIMER(1,tim1,tim2,SWREXETOT)
   !
   !-------RETURN
   RETURN
...
SUBROUTINE GWF2SWR7BD(Kstp,Kper,Igrid)
   !     ******************************************************************
   !     CALCULATE VOLUMETRIC BUDGET FOR GROUP ROUTING REACHES
   !     ******************************************************************
   !
   !     SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
2000 FORMAT(1X,/1X,79('-'),&
   &/1X,28X,'SWR1 OUTPUT PRINT TIMES',&
   &/1X,23X,'STRESS PERIOD ',I4,'   TIME STEP ',I5,&
   &/1X,5(15('-'),1X))
2010 FORMAT(G15.5)
...
2050 FORMAT(1X,'REACH ',I6,'   LAYER ',I3,'   ROW ',I5,'   COL ',I5,&
   ...
   CALL SGWF2SWR7PNT(Igrid)
   !
   !-------START SWR TIMER FOR GWF2SWR7BD
   CALL SSWRTIMER(0,tim1,tim2,SWREXETOT)
   !
   !-------WRITE SOLVER PRINTOUT
   ...
   IF( ibd.EQ.2 ) THEN
      ...
      &NCOL,NROW,NLAY,&
      &NQAQCONN,IOUT,DELT,PERTIM,TOTIM,IBOUND)
   END IF
   !
   !-------CLEAR THE BUFFER.
   DO k = 1, NLAY
   ...
   END DO
   !
   !-------IF NO REACHES, SKIP FLOW CALCULATIONS.
   IF (NREACHES.EQ.0) GO TO 200
   !
   !-------IF SURFACE WATER ONLY - SKIP FLOW CALCULATIONS
   IF ( ISWRONLY.NE.0 ) GO TO 200
   !
   !-------LOOP THROUGH EACH RIVER REACH CALCULATING FLOW.
   REACHES: DO irch = 1, NREACHES
      ...
      END DO
   !---------GET ROW & COLUMN OF CELL CONTAINING REACH.
      ir      = REACH(irch)%IRCH
      jc      = REACH(irch)%JRCH
   !---------DO NOT EVALUATE REACHES WITH ZERO CONDUCTANCES
   ...
   !-----------SET UPPER MOST ACTIVE LAYER FOR REACH
         kact = REACH(irch)%LAYACT
   !-----------IF NO ACTIVE CELLS UNDERLYING REACH MOVE ON TO NEXT TIME FOR REACH.
         IF( kact.GT.NLAY ) CYCLE TIMES
         dtscale = SWRTIME(n)%SWRDT / REAL( DELT, 8 )
   !-----------CALCULATE BASEFLOW FOR THIS TIME
         ISWRDT = n
         irg    = REACH(irch)%IRG
         rs     = RSTAGE(irch,n)
         q      = SSWR_CALC_MFQAQ(REACH(irch),rs)
   !-----------EVALUATE QAQ FOR EACH LAYER
         LAYERS: DO kl = REACH(irch)%LAYSTR, REACH(irch)%LAYEND
   !-------------SET RATE
            rrate = DZERO
            rate  = RZERO
   !-------------SET GROUNDWATER LAYER TO USE
            kk    = MAX( kl, kact )
   !-------------IF CELL IS CONSTANT-HEAD MOVE ON TO NEXT LAYER FOR REACH.
            IF( IBOUND(jc,ir,kk).LT.0 ) CYCLE LAYERS
            h0    = REAL( HOLD(jc,ir,kk), 8 )
            h1    = HNEW(jc,ir,kk)
            fdelt = SWRTIME(n)%FDELT
            h  = ( DONE - fdelt ) * h0 + fdelt * h1
   !-------------DETERMINE SWR1 AQUIFER-REACH EXCHANGE APPROACH FOR REACH
   !             DEFAULT CONDITION IS AQUIFER-REACH EXCHANGE ADDED TO HCOF+RHS
            rhsonly = .FALSE.
            IF ( h.LT.rbot ) THEN
               h = rbot
               rhsonly = .TRUE.
            END IF
   !-------------CALCULATE AQUIFER-REACH EXCHANGE USING APPROPRIATE METHOD
            IF ( rhsonly ) THEN
         ...
         END DO LAYERS
      END DO TIMES
   !---------PROCESS ACCUMULATED QAQFLOW DATA FOR CBC AND LIST OUTPUT
   !-----------SET UPPER MOST ACTIVE LAYER FOR REACH
      kact = REACH(irch)%LAYACT
      kk   = kact
   ...
   !-------------ADD AVERAGE RATE FOR EACH SWR TIMESTEP TO BUFFER.
            BUFF(jc,ir,kl) = BUFF(jc,ir,kl) + rate
   !-------------PRINT THE INDIVIDUAL RATES IF REQUESTED(IRIVCB<0).
            IF ( IBD.LT.0 ) THEN
         ...
         END DO LAYEROUTPUT
         rate = tsrate
   !-----------SEE IF FLOW IS INTO AQUIFER OR INTO REACH.
         IF(rate.LT.RZERO) THEN
   !-----------AQUIFER IS DISCHARGING TO REACH SUBTRACT RATE FROM RATOUT.
            ratout = ratout - tsrate
         ELSE
   !-----------AQUIFER IS RECHARGED FROM RIVER; ADD RATE TO RATIN.
            ratin = ratin + tsrate
         END IF
      END IF LAYCBC
   !
   !---------IF SAVING CELL-BY-CELL FLOWS IN A LIST, WRITE FLOW.
199   IF( IBD.EQ.2 ) THEN
         IF ( NQAQCONN.GT.0 ) THEN
            ...
            DO kl = REACH(irch)%LAYSTR, REACH(irch)%LAYEND
               ...
               &AUXROW,ival,iaux,1,IBOUND,NLAY)
            END DO
         END IF
      END IF
   END DO REACHES
   !
   !-------IF CELL-BY-CELL FLOW WILL BE SAVED AS A 3-D ARRAY,
...
200 rin  = ratin
   ...
   VBNM(MSUM)   = text(1)
   !
   !-------MODFLOW BUDGET TERMS FOR TIMESTEP - CONVERTED TO SWR1 BASED VALUES
   !       INFLOW AND OUTFLOW ARE SWITCHED
   !       SWR1 INFLOW  = MODFLOW OUTFLOW
   !       SWR1 OUTFLOW = MODFLOW INFLOW
   qaqincout    = rin
   ...
   MSUM = MSUM + 1
   !
   !-------CALCULATE BUDGET FOR SWR GROUNDWATER EVAPOTRANSPIRATION
   !
   !-------INITIALIZE CELL-BY-CELL FLOW TERM FLAG (IBD) AND
   !-------ACCUMULATORS (RATIN AND RATOUT).
   ratin  = DZERO
   ...
   !
   !-------IF CELL-BY-CELL FLOWS WILL BE SAVED AS A LIST, WRITE HEADER.
   !       AUXILIARY VARIABLES NOT WRITTEN FOR SWR1 CALCULATED ET
   IF( ibd.EQ.2 ) THEN
      iaux = 0
      IF(IAUXSV.EQ.0) iaux = 0
      CALL UBDSV4(Kstp,Kper,text(2),iaux,SWRAUX,ISWRCB,&
      &NCOL,NROW,NLAY,&
      &NGWET,IOUT,DELT,PERTIM,TOTIM,IBOUND)
   END IF
   !
   !-------CLEAR THE BUFFER.
   DO k = 1, NLAY
   ...
   DO irch = 1, NREACHES
      k = 0
      rate = RZERO
   !         INITIALIZE EVT DATA FOR MT3D LINK FILE
      REACH(irch)%IEVT = 1
      REACH(irch)%QEVT = 0.0
      IF ( REACH(irch)%IGEOTYPE.NE.5 ) GO TO 299
   !         ONLY PROCESS FOR REACHES WITH REMAINING EVAPORATION
      ir = REACH(irch)%IRCH
      jc = REACH(irch)%JRCH
      k = 1
   ...
   !         GROUNDWATER LEVEL ABOVE BOTTOM OF REACH
      IF ( h.GE.REACH(irch)%GBELEV ) THEN
         qq = -gwet
   !         GROUNDWATER LEVEL BELOW REACH BUT ABOVE EXTINCTION DEPTH
   !         LINEAR RELATION BETWEEN GROUNDWATER HEAD AND GROUNDWATER EVAPOTRANSPIRATION
   ...
   !         GROUNDWATER LEVEL IS BELOW EXTINCTION DEPTH
      ...
      rate   = qq
      ratout = ratout - qq
   !         SAVE EVT DATA FOR MT3D LINK FILE
      REACH(irch)%IEVT = k
...
300 rin  = ratin
   ...
   MSUM = MSUM + 1
   !
   !-------CALCULATE AND WRITE INCREMENTAL AND INCREMENTAL BUDGETS
   CALL SSWRBUDGET(Kper,Kstp,&
   &qaqcumin,qaqincin,qaqcumout,qaqincout)
   !
   !-------WRITE GROUP FLUXES TO OUTPUT FILE
   IF ( ISWRPRGF.NE.0 ) THEN
      CALL SSWR_P_RGFLOW(Kper,Kstp)
   END IF
   !
   !-------WRITE REACH STAGES TO OUTPUT FILE
   IF ( ISWRPSTG.NE.0 .OR.&
   &ISWRPQAQ.NE.0 .OR. ISWRSRIV.NE.0 .OR.&
   &ISWRPSTR.NE.0 .OR.&
   &IOBSSTG.GT.0  .OR.&
   &IOBSDEP.GT.0  .OR.&
   &IOBSRBOT.GT.0 ) THEN
   ...
   END IF
   !
   !-------WRITE QAQFLOW FLUXES TO OUTPUT FILE
   ...
   END IF
   !
   !-------WRITE STRUCTURE FLOWS TO OUTPUT FILE
   IF ( ISWRPSTR.NE.0 .OR. IOBSSTR.GT.0 ) THEN
      CALL SSWR_P_STRFLOW(Kper,Kstp)
   END IF
   !
   !-------WRITE SWR OBSERVATIONS TO OUTPUT FILE
   IF ( NOBS.GT.0 ) THEN
      CALL SSWR_P_OBS(Kper,Kstp)
   END IF
   !
   !-------WRITE SUMMARY OF THE LOCATION AND MAGNITUDE OF THE MAXIMUM
   !       VELOCITY AND FROUDE NUMBER FOR DIFFUSIVE WAVE REACH GROUPS
   IF ( ISWRPFRN.NE.0 ) THEN
      CALL SSWR_P_FROUDE(Kper,Kstp)
   END IF
   !
   !-------WRITE SWRDT TO EXTERNAL FILE
   iu = ABS(ISWRSDT)
   IF ( iu.GT.0 ) THEN
      swrtot = REAL( TOTIM - DELT, 8 )
      DO n = 1, NUMTIME
   ...
   END IF
   !
   !-------WRITE SWR1 CONVERGENCE HISTORY
   iu = ISWRCNV
   IF ( iu.GT.0 ) THEN
      ...
      DO n = 1, NUMTIME
         ...
         WRITE ( iu,2105 )&
         &Kper, Kstp, n,&
         &SSWR_SCCHAR(REAL(t0,IKND)), SSWR_SCCHAR(REAL(swrtot,IKND)),&
         &SSWR_SCCHAR(REAL(dt,IKND)),&
         &SWRTIME(n)%ITER, SWRTIME(n)%ILOCFMAX,&
         &SSWR_SCCHAR(REAL(SWRTIME(n)%FMAX,IKND)),&
         &(SWRTIME(n)%ICNVG>0)
      END DO
   END IF
2100 FORMAT( 1X,'    STRESS',1X,'   MODFLOW',1X,'      SWR1',1X,&
   &'     START',1X,'       END',1X,'      SWR1',1X,&
   &'      SWR1',1X,'   MAXIMUM',1X,'   MAXIMUM',1X,&
   &'      SWR1',&
   &/1X,'    PERIOD',1X,' TIME STEP',1X,' TIME STEP',1X,&
   &'      TIME',1X,'      TIME',1X,'     SWRDT',1X,&
   &'ITERATIONS',1X,'RCHGRP LOC',1X,'  RESIDUAL',1X,&
   &' CONVERGED',&
   &/1X,109('-'))
2105 FORMAT(1X,3(I10,1X),3(A10,1X),2(I10,1X),A10,1X,L10)
   !
   !---------RESET QUZFLOW AND QEXTFLOW
   CALL SSWR_EXINIT()
   !
   !-------SET FLAG FOR HEADER OUTPUT
   SWRHEADER = 1
   !
   !-------END SWR TIMER FOR GWF2SWR7BD
   CALL SSWRTIMER(1,tim1,tim2,SWREXETOT)
   !
   !-------RETURN
   RETURN
...
SUBROUTINE GWF2SWR7OT(IGRID)
   !     ******************************************************************
   !     OUTPUT SWR TIMER RESULTS - FLOATING POINT OPERATIONS
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
...
SUBROUTINE GWF2SWR7DA(Igrid)
   !  Deallocate SWR data for a grid
   USE GWFSWRMODULE
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   INTEGER, INTENT(IN) :: Igrid
...
SUBROUTINE SGWF2SWR7PNT(Igrid)
   !  Set pointers to SWR data for a grid
   USE GWFSWRMODULE
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   INTEGER, INTENT(IN) :: Igrid
...
END SUBROUTINE SGWF2SWR7PNT
   !
   !-------SAVE POINTERS TO SWR DATA FOR A GRID
...
SUBROUTINE SSWR_RD_COMM(Iu)
   ...
   DO
      ...
      IF (ios /= 0) THEN
         ...
         &'COULD NOT READ FROM UNIT Iu', Iu,&
         &'IOSTAT=', ios
         inquire(unit=Iu, opened=openedq)
         IF (.NOT. openedq) then
         ELSE
            WRITE(error_line,'(a,1x,a)')&
            &TRIM(ADJUSTL(error_line)), 'FILE IS NOT OPENED'
         END IF
...
END SUBROUTINE SSWR_SWAP
   !
   !-------CONDITIONAL SWAP - SWAP ONLY IF PASSED MASK IS TRUE
...
END SUBROUTINE SSWR_MSWAP
   ...
   !
   !-------SIMPLE FUNCTION TO CONVERT REAL DATA READ FROM ASCII FILE
   !       TO EQUIVALENT DOUBLEPRECISION REPRESENTATION
...
END FUNCTION SSWR_R2D
   ...
   !-------SIMPLE FUNCTION FOR LINEAR INTERPOLATION OF TWO VECTORS
...
DOUBLEPRECISION FUNCTION SSWR_LININT(X,Y,Z) RESULT(v)
   ...
   IF (Z.LT.X(1)) THEN
      v = Y(1)
   !----------ABOVE HIGHEST VALUE
   !          SLOPE CALCULATED FROM INTERVAL BETWEEN SIZE(X) AND SIZE(X)-1
...
END FUNCTION SSWR_RLININT
   ...
   !
   !-------CHECK IF VALID STRUCTURE CONNECTION SPECIFIED
...
&RESULT(ierr)
   ...
   !     + + + INPUT FORMATS + + +
   !     + + + OUTPUT FORMATS + + +
2000  FORMAT(//1X,'SPECIFIED STRUCTURE CONNECTION ERRORS:',&
   &/1X,88('-'))
2010  FORMAT(1X,'INVALID STRUCTURE CONNECTION FOR REACH',1X,I5,1X,&
   &'STRUCTURE',1X,I5,1X,'SPECIFIED CONNECTION',1X,I5)
   !     + + + CODE + + +
   ierr = 0
   !---------RETURN IF BOUNDARY STRUCTURE
   IF ( Istrconn.LT.1 ) RETURN
   !---------IDENTIFY IF Istrconn IS A VALID CONNECTION
...
END FUNCTION SSWR_CHKSTR
   !
   !  SURFACE-WATER ROUTING CALCULATION SUBROUTINES
   !
   !-------CHECK USER SPECIFIED REACH CONNECTIVITY DATA
...
2000 FORMAT(//1X,'ERRORS IN SPECIFIED REACH CONNECTIONS',&
   &/1X,'   NON-SYMMETRIC REACH CONNECTIONS   ',&
   &/4X,'     REACH CONN. NO.      CONN.',&
   &/4X,30('-'))
2010 FORMAT(  4X,3(I10))
2020 FORMAT(  4X,30('-'),&
   &//1X,I6,1X,'REACH CONNECTIVITY ERRORS')
2030 FORMAT(/1X,59('-'),&
   &/1X,'FULL REACH CONNECTIVITY FOR EACH REACH')
...
2060 FORMAT(1X,'REACHES',7X,3(A14,1X),:100(/1X,14X,3(A14,1X)))
   ...
   !     + + + CODE + + +
   !
   !---------FIND UNIQUE CONNECTIONS FOR EACH REACH AND ELIMINATE ANY DUPLICATE CONNECTIONS
   DO irch1 = 1, NREACHES
   ...
   END DO
   !
   !---------CHECK REACH CONNECTIONS FOR SYMMETRY
   icount = 0
   DO irch1 = 1, NREACHES
      nconn1 = REACH(irch1)%NCONN
      DO n = 1, nconn1
   ...
   END IF
   !
   !---------WRITE REACH CONNECTIVITY TO OUTPUT FILE
...
END SUBROUTINE SSWR_CNCK_RC
   !
   !-------ALLOCATE MEMORY FOR RCHGRP DATA
SUBROUTINE SSWR_ALLO_RG()
   ...
   !     + + + OUTPUT FORMATS + + +
2000 FORMAT(//1X,'ERRORS IN SPECIFIED REACH CONNECTIONS:  ',&
   &/1X,' MULTIPLE REACH CONNECTIONS TO THE SAME ',&
   &/1X,' DIFFUSIVE-WAVE APPROXIMATION REACH.    ',&
   &/1X,40('-'),&
   &/1X,'     ERROR     REACH  C.RCHGRP NO. CONN.',&
   &/1X,' --------- --------- --------- ---------')
2010 FORMAT(  1X,4(I10))
2020 FORMAT(  1X,40('-'))
2030 FORMAT(//1X,'ERRORS IN SPECIFIED REACH ROUTING       ',&
   &/1X,' APPROACH: ALL REACHES IN A REACH GROUP ',&
   &/1X,' MUST USE THE SAME ROUTING APPROACH.    ',&
   &/1X,50('-'),&
   &/1X,'     ERROR     REACH  RCH IROUTETYPE ',&
   &' 1ST IROUTETYPE',&
   &/1X,' --------- --------- --------------- ',&
   &'---------------')
...
2050 FORMAT(  1X,50('-'))
   ...
   END DO
   !
   !---------ALLOCATE SPACE FOR RCHGRP(n)%REACH FOR SIMULATION
   DO n = 1, NRCHGRP
   !           COUNT OF REACHES
      icount = 0
      DO i = 1, NREACHES
   ...
   END IF
   !
   !---------CHECK THAT THE SAME ROUTING APPROACH IS USED FOR ALL
   !         REACHES IN A REACH GROUP
   icount = 0
   DO n = 1, NRCHGRP
   ...
   END IF
   !
   !---------CALCULATE DLEN FOR REACH GROUP
   DO n = 1, NRCHGRP
      DO i = 1, RCHGRP(n)%NRGREACH
         irch = RCHGRP(n)%REACH(i)
...
SUBROUTINE SSWR_SET_CONN()
   ...
   &IPC, NLEVELS, IPTFLG,&
   &NREACHES, REACH,&
   &IRDBND, ISOLVER, NINNER,&
   &NRCHGRP, NSOLRG, NACTIVE, RCHGRP, JAC
   USE GWFSWRINTERFACE, ONLY: SSWR_SORT
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   !     + + + LOCAL DEFINITIONS + + +
   ...
   !     + + + DATA + + +
   !
   !---------DEVELOP FULL CONNECTIVITY ON FIRST STRESS PERIOD
   ...
   !-----------DETERMINE THE NUMBER OF CONNECTIONS
      ...
      INITC: DO n = 1, NRCHGRP
         ...
         irchn   = IZERO
         iimpc   = IZERO
   !             EVALUATE FULL REACH CONNECTIVITY FOR RG IN OTHER REACHES
   !             - IMPLICIT STARTING 1D CONNECTIONS FOR RG
         FRC: DO i = 1, RCHGRP(n)%NRGREACH
            ...
            END IF ICONNECTED
         END DO FRC
   !             REACH GROUP CONNECTIONS
         ilen = SUM(iimpc)
         RCHGRP(n)%NCONN = ilen
   !             ADD DIAGONAL + CONNECTIONS TO NNZ
         innz = innz + 1 + ilen
   !             ALLOCATE AND FILL FULL STARTING CONNECTIONS
   ...
   END IF INITIALIZE
   !
   !---------DEVELOP BANDED OR FULL STORAGE AND MODIFIED CRS STORAGE FOR JACOBIAN
   !         AND MAPPING VECTOR (JAC%ISMAP) FOR RESIDUAL AND STAGE VECTORS
   MODBND: IF ( IRDBND.GT.IZERO ) THEN
      ilen = IZERO
      DO n = 1, NRCHGRP
         IF ( RCHGRP(n)%IRGBND.GT.0 ) ilen = ilen + 1
      END DO
   !           DEALLOCATE MEMORY FOR MATRIX STORAGE
      IF ( JAC%IALLO.GT.IZERO ) THEN
         ...
         END IF
   !             DEALLOCATE TEMPORARY SOLVER STORAGE
   ...
   !           CALCULATE NNZ
      ...
      DO n = 1, NSOLRG
         irg = JAC%ISMAP(n)
   !             FILL IA WITH STARTING POSITION OF SOLUTION ELEMENT
   !             IN JA ROW POINTERS - REPRESENTS THE DIAGONAL ELEMENT
         innz = innz + 1
         JAC%IA(n) = innz
   ...
   !
   !-----------ALLOCATE, INITIALIZE, AND FILL JA
   ...
   !             ADD DIAGONAL
         ...
         DO i = 1, ilen
            ...
            END IF
         END DO
   !             TEST FOR LOWER RIGHT OF MATRIX WHERE NO UPPER PRESENT
   !             SET JAC%IU TO THE LAST ENTRY
         IF (JAC%IU(n).EQ.IZERO) JAC%IU(n) = JAC%IA(n+1) - 1
         DEALLOCATE(dcolptr)
      END DO
   !
   !-----------CALCULATE INITIAL BANDWIDTH FOR DIRECT SOLVER AND SET JAC%NHALFB
      DO n = 1, NSOLRG
         irg = JAC%ISMAP(n)
         DO i = 1, RCHGRP(irg)%NCONN
            irchconn = ABS( RCHGRP(irg)%IRCHC(i) )
   ...
   !             CALCULATE BANDWIDTH OF THE RCM REORDERED NODES
         nbw = GSOL_ADJ_PERM_BANDWIDTH( NSOLRG, JAC%NNZ-NSOLRG,&
         &iadj, jadj, ircm, ircm_inv )
         WRITE (IOUT,2000) JAC%NBW, nbw

2000     FORMAT(//,1X,'REVERSE CUTHILL MCKEE REORDERING INFORMATION',&
         &/,1X,'RAW NODE NUMBERING BANDWIDTH:',1X,I10,&
         &/,1X,'RCM REORDERING BANDWIDTH    :',1X,I10,/)
2010     FORMAT(1X,A15,1X,6(I6,1X))
2020     FORMAT(1X,15('-'),1X,6(6('-'),1X))
2030     FORMAT(1X,57('-'),/)
   !
   !-------------TEST IF RCM REORDERING SHOULD BE USED
         iusercm = 0
         IF (   IRCMRORD.GT.0 .OR.&
         &( IRCMRORD.LT.0 .AND. nbw.LT.JAC%NBW ) ) iusercm = 1
   !
   !-------------FILL TEMPORARY ARRAYS WITH CURRENT VALUES AND
   !           RESET IA, JA, ISMAP, AND ICMAP
         RESETIAJA: IF ( iusercm.NE.0 ) THEN
   !
   !---------------WRITE SUMMARY OF REVERSE CUTHILL MCKEE REORDERING RESULTS TO LST FILE
            IF ( IPTFLG.GT.0 ) THEN
               DO i = 1, NSOLRG, 6
                  WRITE (IOUT,2010) 'REACH GROUP   :',&
                  &(JAC%ISMAP(j),j=i,MIN(i+5,NSOLRG))
                  WRITE (IOUT,2020)
                  WRITE (IOUT,2010) 'ORIGINAL NODE :',&
                  &(inode(j),j=i,MIN(i+5,NSOLRG))
                  WRITE (IOUT,2010) 'RCM MAPPING   :',&
                  &(ircm(j),j=i,MIN(i+5,NSOLRG))
                  WRITE (IOUT,2010) 'RCM NODE      :',&
                  &(ircm_inv(j),j=i,MIN(i+5,NSOLRG))
                  WRITE (IOUT,2030)
               END DO
            END IF
   !
   !---------------RESET BANDWIDTH AND HALF BANDWIDTH
            JAC%NBW = nbw
   ...
   !                 SET JAC%IU TO THE LAST ENTRY
            ...
            END DO
   !
   !-------------DEALLOCATE TEMPORARY STORAGE FOR RESETING IA, JA, ISMAP, AND ICMAP
   !             BASED ON RESULTS OF REVERSE CUTHILL MCKEE REORDERING
            DEALLOCATE ( iat, jat )
            DEALLOCATE ( ismapt, icmapt )
         END IF RESETIAJA
   !
   !-------------DEALLOCATE TEMPORARY STORAGE FOR REVERSE CUTHILL MCKEE REORDERING
         DEALLOCATE ( inode, ircm, ircm_inv )
         DEALLOCATE ( iadj, jadj )
      END IF RCMORDL
   !
   !           ALLOCATE MEMORY AS NEEDED FOR SOLUTION VECTORS/MATRICES
   ...
   !             INITIALIZE
      ...
      END IF
   !
   !-----------DETERMINE STORAGE NEEDS FOR ITERATIVE SOLVE PRECONDITIONERS
      IF ( ISOLVER.GT.1 ) THEN
         ijlu      = 1
         ijw       = 1
         iwlu      = 1
      ...
      END IF
   !
   !-----------ALLOCATE TEMPORARY SOLVER STORAGE
   ...
   END IF MODBND
   !
   !---------PROCESS STRUCTURES FOR EACH STRESS PERIOD
   !         SET REACHES WITH STRUCTURES TO NEGATIVE REACH NUMBERS
   DO irch = 1, NREACHES
      DO ii = 1, REACH(irch)%NCONN
   ...
   !             FIND CONNECTIONS THAT ARE STRUCTURE CONTROLLED
   ...
   !                     SET SYMMETRIC CONNECTION FOR STRUCTURE
   ...
   END DO
   !
   !         SET UP SIMPLE CONNECTION INFORMATION FOR
   !         UNCONTROLLED CONNECTIONS
   DO n = 1, NRCHGRP
      iconn = RCHGRP(n)%NCONN
      IF ( iconn.LT.1 ) CYCLE
   !-----------ALLOCATE AND INITIALIZE ESTIMATED UNMANAGED FLOWS
   ...
   END DO
   !
   !---------SET FLAG INDICATING JAC HAS BEEN ALLOCATED
   JAC%IALLO = 1
   !
   !---------RETURN
   RETURN
END SUBROUTINE SSWR_SET_CONN
   !
   !-------CALCULATE REACH OFFSETS FROM STAGE DATA READ FOR
   !       STRESS PERIOD (INPUT ITEM 14)
SUBROUTINE SSWR_SET_RCHOFF(Istage,Dstage)
   ...
   !---------INITIALIZE OFFSET
   REACH(:)%OFFSET = DZERO
   !---------CALCULATE TEMPORARY STAGE FOR CALCULATIONS
   !         CURRENT STAGES
   DO irch = 1, NREACHES
      s(irch) = REACH(irch)%STAGE
   END DO
   !         FILL WITH UPDATED STAGES
   DO i = 1, IRDSTG
      s0 = Dstage(i)
   ...
   DO n = 1, NRCHGRP
      ...
      ismin = 0
      smin = MAXVAL(s) + DONE
   !           FIND LOCATION AND VALUE OF MINIMUM STAGE IN REACH GROUP
      DO i = 1, RCHGRP(n)%NRGREACH
         irch = RCHGRP(n)%REACH(i)
         IF (REACH(irch)%CROUTETYPE.EQ.'TP' .OR.&
         &REACH(irch)%CROUTETYPE.EQ.'LP') THEN
         ...
         END IF
      END DO
   !           SET LOCATION OF MINIMUM STAGE IN REACH GROUP AND
   !             REACH OFFSET
      RCHGRP(n)%ICALCRCH = ismin
      DO i = 1, NREACHES
         IF (REACH(i)%CROUTETYPE.EQ.'TP' .AND.&
         &REACH(i)%IRG.EQ.irg) THEN
            REACH(i)%OFFSET = s(i) - smin
         END IF
      END DO
   END DO
   !
   !---------SET OFFSET FOR CONSTANT STAGE REACHES
   !         BASED ON STAGE
   DO n = 1, NRCHGRP
      IF ( RCHGRP(n)%CONSTANT ) THEN
         irg = RCHGRP(n)%IRG
...
END SUBROUTINE SSWR_SET_RCHOFF
   !
   !-------CALCULATE STAGE-VOLUME-WETTED PERIMETER-SURFACE AREA-CROSS-SECTIONAL AREA
   !       RELATION FOR REACH
...
&Gcnd,Glk,Gcndln,Getextd,Geostor)
   ...
   !     + + + CODE + + +
   !
   !         FILL REACH WITH GENERIC IRREGULAR CROSS-SECTION DATA
   ...
   !
   !       SET GETEXTD FOR REACHES COVERING ENTIRE CELLS
   ...
   END SELECT
   !
   !       SET FLAG FOR CALCULATION OF QAQ TERMS
   SELECT CASE (Igcndop)
    CASE (0)
   ...
   END IF
   !
   !       REALLOCATE AND FILL REACH GEOMETRY DATA
   nlen    = 0
   ir      = REACH(irch)%IRCH
   jc      = REACH(irch)%JRCH
   ...
   ac      = DELR(jc) * DELC(ir)
   !
   !       DETERMINE GWIDTH AND DLEN FOR REACH
   ...
   ELSE IF (Igeotype.EQ.5) THEN
      ...
      bottom = zg
      zg = zg + 5.0D0
   !         RESET REACH DLEN TO DELC FOR REACHES COVERING ENTIRE CELLS
   ...
   SELECT CASE (Igeotype)
    CASE (1,2,5)
      ...
      nlen = SIZE(uz)
    CASE (3)
   !           DETERMINE THE NUMBER OF UNIQUE POINTS IN CROSS-SECTION ELEVATIONS
   !             ADD TOPOGRAPHY AND MAXIMUM ELEVATION + 10
      nlen = ngeopts + 4 + 9
      ALLOCATE (uz(nlen))
      uz(1:Ngeopts) = REACH(irch)%IRRGEO%ELEVB
      uz(Ngeopts+1) =&
      &MINVAL(uz(1:Ngeopts)) + 1.0D-6
      uz(Ngeopts+2) = zg
      uz(Ngeopts+3) = zg + 1.0D-6
      uz(Ngeopts+4) =&
      &MAXVAL(uz(1:Ngeopts+3)) + 10.0D0
      dz = (MAXVAL(uz(1:Ngeopts)) -&
      &MINVAL(uz(1:Ngeopts)) + 1.0D-6) / 10.0D0
      uz(Ngeopts+5) =&
      &uz(Ngeopts+1) + dz
      DO ipos = (Ngeopts+6),&
      &(Ngeopts + 4 + 9)
   ...
   &REACH(irch)%GEO%XAREA(nlen),&
   &REACH(irch)%GEO%VOL(nlen),&
   &REACH(irch)%GEO%WETPER(nlen),&
   &REACH(irch)%GEO%TOPWID(nlen),&
   &REACH(irch)%GEO%SAREA(nlen))
   !
   !         FILL ELEV AND CALCULATE XAREA, VOLUME, WETTED PERIMETER, AND TOP WIDTH
   SELECT CASE (Igeotype)
    CASE (1,2)
   !             ELEVATIONS
      REACH(irch)%GEO%ELEV      = uz
   !             DATA FOR BOTTOM ELEVATION
      ...
      DO ipos = 2, nlen
         ...
         wpc = DONE
         IF (Igeotype.EQ.5.OR.&
         &REACH(irch)%GEO%ELEV(ipos).GT.zg) THEN
            wpc = DZERO
         END IF
   ...
   !           IRREGULAR CROSS-SECTION
    CASE (3)
      ...
      DO j = 2, Ngeopts
         ...
         &REACH(irch)%IRRGEO%ELEVB(j-1))
         dx   = (REACH(irch)%IRRGEO%XB(j) -&
         &REACH(irch)%IRRGEO%XB(j-1))
         dydx = DZERO
         IF (ABS(dx).GT.DZERO) dydx = dy / dx
   !               INITIALIZE XAREA, PERIMETER, AND TOP_WIDTH
         a  = DZERO
         wp = DZERO
         tw = DZERO
   !             EVALUATE XAREA BELOW EACH UNIQUE DEPTH
         DO i = 1, nlen
            wpc = DONE
            z2 = uz(i)
            z1 = z2
            ...
            IF (i.GT.1) THEN
               ...
               sa0 = sa(i-1)
            END IF
   !                 DEPTHS BELOW BOTH POINTS
            IF (uz(i).LE.REACH(irch)%IRRGEO%ELEVB(j) .AND.&
            &uz(i).LE.REACH(irch)%IRRGEO%ELEVB(j-1)) THEN
               tw(i) = DZERO
               wp(i) = DZERO
               a(i)  = DZERO
               sa(i) = DZERO
   !                 DEPTHS ABOVE ONE POINT BUT BELOW THE OTHER POINT
   !                   COMBINATION OF RECTANGULAR AND TRIANGULAR AREAS
            ELSE IF (uz(i).LE.REACH(irch)%IRRGEO%ELEVB(j) .OR.&
            &uz(i).LE.REACH(irch)%IRRGEO%ELEVB(j-1)) THEN
               dx = REACH(irch)%IRRGEO%XB(j) -&
               &REACH(irch)%IRRGEO%XB(j-1)
               IF (ABS(dydx).GT.DZERO) dx = dh / dydx
   !                   LAST AREA +     RECT. AREA      + TRI. AREA
               tw(i) = tw0 + dx
               sa(i) = tw(i) * length
               wp(i) = wp0 + wpc * SQRT(dx*dx + dh*dh)
               a(i)  =  a0 + tw0 * dh + dx * dh / DTWO
   !                 DEPTHS ABOVE BOTH POINTS - ALL RECTANGULAR AREAS
            ELSE
               dx = REACH(irch)%IRRGEO%XB(j) -&
               &REACH(irch)%IRRGEO%XB(j-1)
   !                   LAST AREA + RECT. AREA
            ...
            END IF
         END DO
   !               ACCUMULATE GEOMETRY TERMS IN CROSS-SECTION GEOMETRY STRUCTURE
   !                 WITHIN CHANNEL
         REACH(irch)%GEO%XAREA(:)  =&
         &REACH(irch)%GEO%XAREA(:)  + a(:)
         REACH(irch)%GEO%VOL(:)    =&
         &REACH(irch)%GEO%VOL(:)    + a(:) * length
         REACH(irch)%GEO%WETPER(:) =&
         &REACH(irch)%GEO%WETPER(:) + wp(:)
         REACH(irch)%GEO%TOPWID(:) =&
         &REACH(irch)%GEO%TOPWID(:) + tw(:)
         REACH(irch)%GEO%SAREA(:)  =&
         &REACH(irch)%GEO%SAREA(:)  + sa(:)
      END DO
      uz = uz
    CASE (4)
   ...
   END SELECT
   !
   !-----------SET CONSTANT RAINFALL AREA FOR REACH
   am = DZERO
   DO ipos = 2, nlen
...
SUBROUTINE SSWR_SET_RCHLAY(irch)
   ...
   !     + + + INPUT FORMATS + + +
   !     + + + OUTPUT FORMATS + + +
2000  FORMAT(//1X,'KRCH MUST BE SET TO 1 FOR REACH',1X,I5,&
...
SUBROUTINE SSWR_CALC_RATCURVE()
   ...
   !     + + + FUNCTIONS + + +
   !     + + + CODE + + +
   !         CLEAN UP REACH GROUP RATING CURVE DATA
   DO n = 1, NRCHGRP
   ...
   END DO
   !
   !         REALLOCATE LEVEL GROUP RATING CURVE DATA
   DO n = 1, NRCHGRP
   !           SKIP IF RCHGRP NOT UPDATED
      IF (RCHGRP(n)%IRGUPDATE.LT.1) CYCLE
   !           CALCULATE NUMBER OF DATA POINTS PER REACH
      nlen = 0
      DO i = 1, RCHGRP(n)%NRGREACH
         irch = RCHGRP(n)%REACH(i)
...
2000 FORMAT(/1X,'WARNING -- ',&
   &'STAGE ADJUSTMENT AT BEGINNING OF STRESS-PERIOD RESULTING FROM',&
   &/1X,11X,'CALCULATED REACH OFFSET AND INITIAL/PREVIOUS STAGE',&
   &1X,'HAS RESULTED IN',&
   &/1X,11X,'A VOLUME ERROR EXCEEDING 0.1%.',&
   &/1X,11X,'THE STAGE FOR THIS TIME STEP SHOULD BE ADJUSTED!!!',&
   &//1X,'   RCHGRP',1X,'   INIT. VOLUME',&
   &1X,'     NEW VOLUME',1X,'   VOLUME DIFF.',1X,'  PERCENT ERROR',&
   &/1X,74('-'))
2010 FORMAT(1X,I10,4(1X,E15.7))
   ...
   !---------CALCULATE VOLUME IN EACH REACH AND
   !         IN EACH REACH GROUP
   vr = DZERO
   vp = DZERO
   DO i = 1, NRCHGRP
   ...
   END DO
   !
   !---------CALCULATE GROUP STAGE BASED ON CALCULATED VOLUME
   !         IN EACH GROUP (I.E., REACH GROUP)
   !         ALSO UPDATE STAGE FOR EACH REACH IN GROUP
   DO i = 1, NRCHGRP
      IF (RCHGRP(i)%CONSTANT) CYCLE
      IF (RCHGRP(i)%IRGUPDATE.LT.1) CYCLE
   ...
   END DO
   !
   !---------PERFORM CHECK OF ORIGINAL AND REVISED GROUP VOLUME
   !         CALCULATED FROM UPDATED REACH STAGES TO DETERMINE IF
   !         WATER IS CREATED OR DESTROYED BECAUSE OF SPECIFIED WSSLOPE.
   !         ISSUE WARNING IF CONDITION OCCURS
   vr  = DZERO
   vpc = DZERO
   e   = EPSILON(DZERO)
   DO i = 1, NRCHGRP
...
END SUBROUTINE SSWR_VOL2STG_UPDATE
   !
   !-------CALCULATE DISTANCE WEIGHTED PARAMETERS FOR REACH GROUP
...
END SUBROUTINE SSWR_CALCA_UFLOW
   !
   !-------SUBROUTINE FOR READING DIRECT RUNOFF DATA
...
END SUBROUTINE SSWR_RDDRO
   !
   !-------SUBROUTINE FOR APPLYING DIRECT RUNOFF DATA
...
END SUBROUTINE SSWR_APPLYDRO
   ...
   !
   !
   !-------WRAPPER FOR CONSOLIDATED OUTER ITERATIONS (NON-LINEAR NEWTON STEP)
   !       AND ALL LINEAR SOLVERS
SUBROUTINE SSWR_GSOLWRP(Kmf,Iouter,Isoln,X,Check)
   ...
   DO n = 1, NSOLRG
      JAC%DX(n) = DONE
   END DO
   !---------SET APPROPRIATE DAMPENING FACTOR
   damp = DAMPTR
   IF ( ISWRSS.GT.0 ) THEN
      damp = ABS( DAMPSS )
   END IF
   !---------CALCULATE EXPLICIT INVARIANT QM
   IF ( ICIQM.GT.0 ) THEN
      CALL SSWR_EST_EXPLICIT_SFM(X)
   END IF
   !---------CALCULATE RESIDUAL FOR CURRENT VALUE OF X
   CALL SSWR_CALC_RGRESI(X,JAC%R)
   CALL SSWRC2S(JAC%R,JAC%F)
   CALL SSWRC2S(X,JAC%XS)
   !---------TEST FOR INITIAL GUESS BEING A SOLUTION
   ...
   OUTER: DO ots = 1, NOUTER
      Iouter = ots
      its    = 0
   !-----------CALCULATE EXPLICIT INVARIANT QM AND UPDATE THE RESIDUAL
      IF ( ICIQM.GT.0 ) THEN
         IF ( ots.GT.1 ) THEN
            CALL SSWR_EST_EXPLICIT_SFM(X)
            CALL SSWR_CALC_RGRESI(X,JAC%R)
         END IF
      END IF
   !-----------CALCULATE COMPRESSED JACOBIAN
      CALL SSWR_FDJACC(X,JAC%R,JAC%XS,JAC%FJACC)
   !-----------CALCULATE PSEUDO-TRANSIENT CONTINUATION TERM
      IF (ISWRSS.GT.0) THEN
         diagmin = 1.0D20
         DO n = 1, NSOLRG
            ia = JAC%IA(n)
   ...
   !     &         ptcfn0, fn, ptcfn0/fn
         ...
         END DO
      END IF
   !-----------CHECK FOR ZERO ELEMENTS ALONG DIAGONAL
      DO n = 1, NSOLRG
         ia = JAC%IA(n)
         ...
         END IF
      END DO
   !-----------CALCULATE THE MATRIX VECTOR PRODUCT J s(k-1)
      IF ( ISOLSTG.NE.0 ) THEN
         CALL GSOL_CMATVEC(NCORESM,NSOLRG,JAC%NNZ,JAC%IA,JAC%JA,&
         &JAC%FJACC,JAC%XS,JAC%JSKM1)
      END IF
   !-----------FILL RHS AND X EQUIVALENT
      DO n = 1, NSOLRG
         IF ( ISOLSTG.NE.0 ) THEN
         ...
         END IF
      END DO
   !-----------SCALE COMPRESSED JACOBIAN, AND RESIDUAL
      IF ( IJACSCL.GT.0 ) THEN
         CALL GSOL_SCALE(0,IJACSCL,NSOLRG,JAC%NNZ,&
         &JAC%IA,JAC%JA,JAC%FJACC,JAC%DX,JAC%F,&
         &JAC%FSCALE,JAC%FSCALE2)
      END IF
   !-----------PRECONDITIONER FOR JACOBIAN
      IF ( ISOLVER.GT.1 ) THEN
         CALL GSOL_PCU(IOUT,IPC,NSOLRG,JAC%NNZ,&
         &JAC%IA,JAC%JA,JAC%FJACC,&
         &JAC%NRLU,JAC%NNZLU,&
         &JAC%NJLU,JAC%NJW,JAC%NWLU,&
         &JAC%JLU,JAC%IU,&
         &JAC%JW,JAC%WLU,JAC%MJACC,&
         &NLEVELS,DROPTOL)
      END IF
   ...
   !             LU DECOMPOSITION
       CASE (1)
         IF (JAC%IBAND.LT.1) THEN
            ...
            &NSOLRG,JAC%NNZ,damp,&
            &JAC%IA,JAC%JA,&
            &JAC%FJACC,JAC%FJACU,JAC%DX,JAC%F,JAC%XSI,&
            &JAC%PINDEXLU,JAC%SLU)
         ELSE
            CALL GSOL_BAND_LUDCAP(icnvg,its,NOUTER,&
            &NSOLRG,JAC%NNZ,JAC%NBW,JAC%NHALFB,damp,&
            &JAC%IA,JAC%JA,&
            &JAC%FJACC,JAC%FJACU,JAC%FJACL,JAC%DX,JAC%F,JAC%XSI,&
            &JAC%PINDEXLU,JAC%SLU)
         END IF
   !             BICONJUGATE GRADIENT STABILIZED
       CASE (2)
         CALL GSOL_BCGSAP(NCORESM,NCORESV,&
         &IPC,icnvg,its,NOUTER,NINNER,&
         &TOLS,TOLR,eta,NSOLRG,JAC%NNZ,damp,&
         &JAC%IA,JAC%JA,JAC%FJACC,&
         &JAC%NRLU,JAC%NNZLU,&
         &JAC%NJLU,JAC%NJW,JAC%NWLU,&
         &JAC%JLU,JAC%IU,&
         &JAC%JW,JAC%WLU,JAC%MJACC,&
         &JAC%DX,JAC%F,JAC%XSI,&
         &JAC%FSCALE,JAC%FSCALE2,&
         &JAC%DBCGS,JAC%DHATBCGS,&
         &JAC%PBCGS,JAC%PHATBCGS,&
         &JAC%SBCGS,JAC%SHATBCGS,&
         &JAC%VBCGS,JAC%TBCGS)
   !             RESTARTED GENERALIZED MINIMUM RESIDUAL
       CASE (3)
         CALL GSOL_GMRMAP(NCORESM,NCORESV,&
         &IPC,icnvg,its,NOUTER,NINNER,&
         &TOLR,eta,NSOLRG,JAC%NNZ,damp,&
         &JAC%IA,JAC%JA,JAC%FJACC,&
         &JAC%NRLU,JAC%NNZLU,&
         &JAC%NJLU,JAC%NJW,JAC%NWLU,&
         &JAC%JLU,JAC%IU,&
         ...
         &JAC%DX,JAC%F,JAC%XSI,&
         &JAC%FSCALE,JAC%FSCALE2,&
         &JAC%RGMR,JAC%ZGMR,JAC%DGMR,JAC%TGMR,&
         &JAC%CSGMR,JAC%SNGMR,JAC%SGMR,JAC%YGMR,&
         &JAC%HGMR,JAC%VGMR)
      END SELECT
   !-----------INCREMENT COUNTER FOR TOTAL NUMBER OF INNER ITERATIONS
      Isoln = Isoln + its
   !-----------UNSCALE DX
      IF ( IJACSCL.GT.0 ) THEN
         ...
         &JAC%FSCALE,JAC%FSCALE2)
      END IF
   !-----------TERMINATE IF NUMERICAL ERROR
      IF ( icnvg.LT.0 ) EXIT OUTER
   !-----------UPDATE f WITH CURRENT X ITERATE
      DO n = 1, NSOLRG
         IF ( ISOLSTG.NE.0 ) THEN
            JAC%XS(n) = JAC%DX(n)
         ELSE
         ...
         END IF
      END DO
   !-----------MAP SOLUTION TO ALL REACH GROUPS
      CALL SSWRS2C(JAC%XS,X)
      CALL SSWR_CALC_RGRESI(X,JAC%R)
      CALL SSWRC2S(JAC%R,JAC%F)
      fn = GSOL_L2NORM(NSOLRG, JAC%F)
   !-----------BACKTRACKING -
   !           ONLY APPLIED WHEN THERE HAS BEEN NO IMPROVEMENT IN THE L2NORM
   ...
   !-----------NEWTON HEAD DAMPENING
      ...
      DO i = 1, NSOLRG
         ...
         END IF
      END DO
   !-----------MAP SOLUTION MODIFIED BY NEWTON HEAD DAMPENING
   !           TO ALL REACH GROUPS
      IF (icheck > 0) THEN
         CALL SSWRS2C(JAC%XS,X)
         CALL SSWR_CALC_RGRESI(X,JAC%R)
         ...
         fn = GSOL_L2NORM(NSOLRG, JAC%F)
      END IF
   !-----------END OF NEWTON HEAD DAMPENING
   !
   !-----------CHECK FOR ABSOLUTE MINIMIZATION OF f
      deltaf  = DZERO
      deltafi = DZERO
      deltax  = DZERO
      DO n = 1, NSOLRG
      ...
      END DO
   !-----------CHECK THAT THE PSEUDO-TRANSIENT CONTINUATION TERM HAS DECAYED
   !
      icheck = 1
   ...
   !-----------SAVE CURRENT OUTER ITERATES
      ...
      fni = fn
   END DO OUTER
   !---------SET Check TO .TRUE. TO INDICATE CONVERGENCE NOT ACHIEVED
   Check = .TRUE.
   !---------RETURN
   RETURN
END SUBROUTINE SSWR_GSOLWRP
   !
   !     FORWARD DIFFERENCE JACOBIAN - MODIFIED CRS FORMAT
SUBROUTINE SSWR_FDJACC(P,Fvec,Vs,Df)
   USE GWFSWRMODULE, ONLY: DZERO, DONE, DTWO, NEARZERO, TIMECONV,&
   &NREACHES, NRCHGRP, NSOLRG, JAC
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   ...
   DO n = 1, JAC%NNZ
      Df(n) = DZERO
   END DO
   !         PERTURBATION BASED ON KELLY (2003)
   ...
   !            dc = (f - Fvec(ii))/ e           !JONES AND WOODWARD (2001)
...
DOUBLEPRECISION FUNCTION SSWR_CALC_RGTERMS(Irg,P) RESULT(value)
   ...
   IF (ISWRSS.LT.1) THEN
      ...
      IF ( RCHGRP(Irg)%CONSTANT ) THEN
         ...
         v0 = v0 / SWRDT
      END IF
   !         FOR STEADY STATE - EVALUATED USING PSEUDO-TRANSIENT CONTINUATION APPROACH
   ELSE
   !           NO STORAGE OPTION
      IF ( ISWRSS.EQ.1 ) THEN
         v0 = DZERO
   !           PSEUDO-TRANSIENT CONTINUATION APPROACH
      ELSE IF ( ISWRSS.EQ.2 ) THEN
         v0 = SSWR_RG_VOL(RCHGRP(Irg),GSTAGE(Irg,0))
         v0 = v0 / SWRDT
      END IF
   !           UPDATE INITIAL VOLUME BASED ON CHANGES IN CONSTANT STAGES
      IF ( RCHGRP(Irg)%CONSTANT ) THEN
         v0 = SSWR_RG_VOL(RCHGRP(Irg),GSTAGE(Irg,0))
         v0 = v0 / SWRDT
      END IF
   END IF
   !
   !---------INITIALIZE REACH GROUP INFLOW AND OUTFLOW TERMS
   RCHGRP(Irg)%INFLOW  = DZERO
   RCHGRP(Irg)%OUTFLOW = DZERO
   !
   !---------GET ESTIMATE OF CURRENT TERMS
   ps = P(Irg)
   ...
   !           PSEUDO-TRANSIENT CONTINUATION APPROACH
      ...
      END IF
   END IF
   !           ELIMINATE VOLUME CHANGE FOR CONSTANT STAGE REACH GROUPS
   !           FROM RESIDUAL CALCULATION
   IF ( RCHGRP(Irg)%CONSTANT ) v = v0
   !---------BACKWARD DIFFERENCE - ALL TERMS ARE RATES
   value = in + r + e + b + qm + bc + c + v0 - v
   !---------SCALE RESIDUAL FROM CONSISTENT UNITS TO CUBIC METERS PER SECOND
   value = value * QSCALE
   !---------MAKE FINAL CALCULATION OF REACH GROUP INFLOW AND OUTFLOW
...
&(Rg,V,Rainfall,Evap,Qaqflow,Qbcflow,Crflow)
   ...
   !     + + + CODE + + +
   !
   !---------FILL RCHGRP WITH CURRENT ESTIMATES
   Rg%CURRENT%VOLUME   = V
   !---------DVOLUME SAVED AS A RATE - [L3/T]
   !         TRANSIENT
   IF (ISWRSS.LT.1) THEN
      irg = Rg%IRG
      v0 = SSWR_RG_VOL(Rg,GSTAGE(irg,ISWRDT-1))
      Rg%CURRENT%DVOLUME  = (v0 - V) / SWRDT
   !         STEADY STATE - ISWRSS > 0
   ELSE
      Rg%CURRENT%DVOLUME = DZERO
   END IF
   !---------CALCULATE REACH TERMS
   linf = DZERO
   uinf = DZERO
   einf = DZERO
   qpos = DZERO
   ...
   DO nr = 1, Rg%NRGREACH
      irch  = Rg%REACH(nr)
   !-----------ADD LATERAL INFLOW: [RATE]
      linf = linf + REACH(irch)%QLATFLOW
   !-----------ADD LATERAL INFLOW: [RATE]
      uinf = uinf + REACH(irch)%QUZFLOW
   !-----------ADD EXTERNAL INFLOW: [RATE]
      einf = einf + REACH(irch)%QEXTFLOW
   !-----------UNCONTROLLED AND CONTROLLED INFLOW
      qpos = qpos + REACH(irch)%RPOSFLOW
   !-----------UNCONTROLLED AND CONTROLLED OUTFLOW
   ...
   !---------BOUNDARY FLOW RESULTING FROM CHANGE IN CONSTANT STAGES
   cv = DZERO
   IF ( Rg%CONSTANT ) THEN
   ...
   Rg%STAGE = ps
   !
   !---------UPDATE REACH DATA FOR CURRENT SWR TIMESTEP
   !         BASED ON NEW VOLUME
   DO nr = 1, Rg%NRGREACH
      irch = Rg%REACH(nr)
   !           UPDATE STAGE
   ...
   !-------------BOUNDARY FLOW RESULTING FROM CHANGE IN CONSTANT STAGES
         IF ( Rg%CONSTANT ) THEN
            ...
            &REACH(irch)%CURRENT%QCRFLOW + (rv - rv0) / SWRDT
         END IF
      ELSE
...
END SUBROUTINE SSWR_RG_UPDATE
   ...
   !
   !-------SUBROUTINE TO CALCULATE INCREMENTAL AND CUMULATIVE
   !       SURFACE WATER ROUTING BUDGET FOR MODEL
...
2000 FORMAT(//20X,'VOLUMETRIC SURFACE WATER BUDGET FOR ENTIRE MODEL',&
   &/20X,'AT END OF TIME STEP',1X,I5,' IN STRESS PERIOD',1X,I4,&
   &/2X,83('-'))
2010 FORMAT(1X,/8X,'CUMULATIVE VOLUMES',6X,'L**3',7X,&
   &'RATES FOR THIS TIME STEP',6X,'L**3/T',/,&
   &8X,18('-'),17X,24('-'),//,&
   &14X,'IN:',38X,'IN:'/14X,'---',38X,'---')
...
2080 FORMAT(/19X,&
   &'DISCREPANCY BETWEEN MODFLOW AND SWR1 AQUIFER-REACH',&
   &/18X,&
   &'             TERMS FOR THE ENTIRE MODEL               ',&
   &/02X,83('-'),&
   &/07X,'CUMULATIVE VOLUMES',7X,'L**3',8X,&
   &'RATES FOR THIS TIME STEP',7X,'L**3/T',/,&
   &07X,18('-'),19X,24('-'))
...
2110 FORMAT(2X,'  PERCENT DISCREPANCY =',F15.2,&
   &5X,'  PERCENT DISCREPANCY =',F15.2,&
   &/2X,'SWR1 CONSTANT MODFLOW =',A17,&
   &3X,'SWR1 CONSTANT MODFLOW =',A17,&
   &/2X,'SWR1 INACTIVE MODFLOW =',A17,&
   &3X,'SWR1 INACTIVE MODFLOW =',A17,&
   &/2X,83('-'),/4X,&
   &'PERCENT DISCREPANCY = 100 x ((SWRIN-MFIN) - ',&
   &'(SWROUT-MFOUT)) / (MFIN+MFOUT) / 2',&
   &///)
   !     + + + CODE + + +
   !---------INITIALIZE INACTIVE CELL ACCUMULATOR
   ia         = IZERO
   !---------ZERO OUT INCREMENTAL TERMS
   INCBD(:,:) = RZERO
   !---------ZERO OUT SUMMATION TERMS
   incrate        = DZERO
   tsrate         = DZERO
   rratin         = DZERO
   rratout        = DZERO
   ...
   REACHES: DO irch = 1, NREACHES
      ...
      mfchqaqrate = DZERO
      irg = REACH(irch)%IRG
   !           SKIP CONNECTION IF CONNECTED REACH IS INACTIVE
   ...
   !-------------NO STORAGE CHANGE FOR STEADY STATE SIMULATIONS
   !             v1.04 JDH 1/29/2016
   !             PRIOR VERSIONS INCLUDED STORAGE CHANGES IN BUDGETS
         IF ( ISWRSS.GT.0 ) THEN
            dv = DZERO
         ELSE
            dv  = ( v0 - v ) / SWRTIME(n)%SWRDT
         END IF
   !             CALCULATE CONSTANT STAGE REACH CONSTANT FLOW CHANGE
   !             DUE TO CHANGES IN CONSTANT STAGES
         cv  = DZERO
         ...
         incrate(9) = dv
   !
   !------------ ACCUMULATE BUDGET ITEMS IN ACCUMULATOR FOR THE CURRENT
   !             MODFLOW TIME STEP
         dtscale = SWRTIME(n)%SWRDT / REAL( DELT, 8 )
         DO iacc = 1, NBDITEMS
         ...
         END DO
   !
   !-------------PROCESS ACCUMULATED QAQFLOW DATA FOR ACTIVE MODFLOW CELLS
   !             FOR USE IN COMPARISON OF SWR1 AND MODFLOW QAQ TERMS
   !
   !-------------SKIP QAQ CALCULATIONS IF QAQ IS NOT CALCULATED FOR THE CURRENT REACH
         IF (REACH(irch)%ICALCQAQ.EQ.IZERO) CYCLE TIMES
   !-------------DETERMINE kact AND SKIP IF FLOW FROM THE REACH DOES NOT INTERACT
   !             WITH AN UNDERLYING ACTIVE CELL
         kact = REACH(irch)%LAYACT
         IF ( kact.GT.NLAY ) THEN
            ia = ia + 1
            CYCLE TIMES
         ...
         END DO LAYERS
      END DO TIMES
   !-----------FILL POSITIVE AND NEGATIVE BUDGET ITEMS
      DO iacc = 1, NBDITEMS
      ...
      END IF
   END DO REACHES
   !---------ADD INDIVIDUAL BUDGET TERMS TO BUDGET ACCUMULATORS
   DO iacc = 1, NBDITEMS
      ratin  = rratin(iacc)
      ratout = rratout(iacc)
      INCBD(1,iacc) = ratin
   ...
   !
   !-----------WRITE DISCREPANCY BETWEEN MODFLOW AND SWR1 QAQFLOW
   ...
   !             QAQFLOW PERCENT DIFFERENCE
   !               RELATIVE TO CUMULATIVE AND INCREMENTAL VALUES FOR SWR1 BUDGET
...
END SUBROUTINE SSWRBUDGET
   !
   !-------SUBROUTINE TO OUTPUT SUMMARY GROUP DATA TO ISWRPRGF
SUBROUTINE SSWR_P_RGFLOW(Kper,Kstp)
   ...
   &ISWRSAVG, RTPRN, NPMAX,&
   &REACH, NRCHGRP, RCHGRP, SWRDT, ISWRDT,&
   &NUMTIME, ISWRPRGF, SWRHEADER, SWRDT,&
   &ISWRDT, SWRTIME, RSTAGE, GSTAGE, GSTAGEP
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   INTEGER, INTENT(IN) :: Kper
   INTEGER, INTENT(IN) :: Kstp
...
2040 FORMAT(2(E10.3,','),4(I10,','),E15.8,',',13(E15.8,','))
   ...
   iu = ABS(ISWRPRGF)
   !
   !---------WRITE FILE HEADER IF FIRST CALL TO SWR BUDGET SUBROUTINE
   IF ( SWRHEADER.EQ.0 ) THEN
      IF ( ISWRPRGF.GT.0 ) THEN
         line = 'TOTTIME,SWRDT,KPER,KSTP,' //&
         &'KSWR,RCHGRP,' //&
         &'STAGE,QPFLOW,QLATFLOW,QUZFLOW,RAIN,EVAP,QAQFLOW,' //&
         &'QNFLOW,QEXTFLOW,QBCFLOW,QCRFLOW,DV,INF-OUT,VOLUME'
   ...
   PTIME: DO n = 1, NUMTIME
      ISWRDT = n
      dt = SWRTIME(n)%SWRDT
   !           UPDATE STRUCTURE VALUES FOR CURRENT TIME
      DO irg = 1, NRCHGRP
   ...
   !             SET TEMPORARY REACH GROUP VALUES
         s      = GSTAGE(i,n)
         ...
         IF ( ISWRSAVG.NE.0 ) THEN
            GSTAGEP(i,ip)                  =&
            &GSTAGEP(i,ip)                 + s      * dt * pfact
            RCHGRP(i)%RSLTS(ip)%QMPOSFLOW =&
            &RCHGRP(i)%RSLTS(ip)%QMPOSFLOW + qpos   * dt * pfact
            RCHGRP(i)%RSLTS(ip)%QLATFLOW  =&
            &RCHGRP(i)%RSLTS(ip)%QLATFLOW  + latinf * dt * pfact
            RCHGRP(i)%RSLTS(ip)%QUZFLOW   =&
            &RCHGRP(i)%RSLTS(ip)%QUZFLOW   + uzfinf * dt * pfact
            RCHGRP(i)%RSLTS(ip)%RAIN      =&
            &RCHGRP(i)%RSLTS(ip)%RAIN      + r      * dt * pfact
            RCHGRP(i)%RSLTS(ip)%EVAP      =&
            &RCHGRP(i)%RSLTS(ip)%EVAP      + e      * dt * pfact
            RCHGRP(i)%RSLTS(ip)%QAQFLOW   =&
            &RCHGRP(i)%RSLTS(ip)%QAQFLOW   + b      * dt * pfact
            RCHGRP(i)%RSLTS(ip)%QMNEGFLOW =&
            &RCHGRP(i)%RSLTS(ip)%QMNEGFLOW + qneg   * dt * pfact
            RCHGRP(i)%RSLTS(ip)%QEXTFLOW  =&
            &RCHGRP(i)%RSLTS(ip)%QEXTFLOW  + ext    * dt * pfact
            RCHGRP(i)%RSLTS(ip)%QBCFLOW   =&
            &RCHGRP(i)%RSLTS(ip)%QBCFLOW   + bc     * dt * pfact
            RCHGRP(i)%RSLTS(ip)%QCRFLOW   =&
            &RCHGRP(i)%RSLTS(ip)%QCRFLOW   + c      * dt * pfact
            RCHGRP(i)%RSLTS(ip)%DVOLUME   =&
            &RCHGRP(i)%RSLTS(ip)%DVOLUME   + dv     * dt * pfact
            RCHGRP(i)%RSLTS(ip)%VOLUME    =&
            &RCHGRP(i)%RSLTS(ip)%VOLUME    + vol    * dt * pfact
   ...
   END DO PTIME
   !---------WRITE GROUP DATA FOR EACH SWR TIMESTEP
   ip = 1
   ...
   !            WRITE (line,2020) swrtot, dt, Kper, Kstp, n
      ...
      DO i = 1, NRCHGRP
         ...
         IF ( ISWRPRGF.GT.0 ) THEN
            ...
            &qpos, latinf, uzfinf,&
            &r, e, b, qneg, ext, bc, c,&
            &dv, f, RCHGRP(irg)%RSLTS(ip)%VOLUME
         ELSE
            ...
            &dv, f, RCHGRP(irg)%RSLTS(ip)%VOLUME
         END IF
   !             RESET PRINT DATA FOR REACH GROUP AFTER PRINTING IF
...
END SUBROUTINE SSWR_P_RGFLOW
   !
   !-------SUBROUTINE TO OUTPUT SUMMARY REACH STAGE DATA TO ISWRPSTG
SUBROUTINE SSWR_P_RCHSTG(Kper,Kstp)
   USE GWFBASMODULE, ONLY: DELT, TOTIM
   USE GLOBAL,       ONLY: ISSFLG
   USE GWFSWRMODULE, ONLY: IZERO, RZERO, DZERO, DONE,&
   &ISWRSAVG, RTPRN, NPMAX, NREACHES, REACH,&
   &ISWRPSTG, ISWRPQAQ, ISWRSRIV, ISWRPSTR,&
   &NUMTIME, SWRHEADER, SWRDT,&
   &ISWRDT, SWRTIME, RSTAGE, RSTAGEP,&
   &NOBS, IOBSSTG, IOBSDEP, IOBSRBOT,&
   &OBSDATA, DOBS
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   INTEGER, INTENT(IN) :: Kper
...
2010 FORMAT(2(E15.7,','),3(I10,','),100(10(E15.7,',')))
   ...
   PTIME: DO n = 1, NUMTIME
      ...
      IF ( ip.GT.NPMAX ) EXIT PTIME !HACK
   END DO PTIME
   !---------WRITE REACH STAGE DATA FOR EACH SWR PRINT TIME
   ...
   !             INCREMENT PRINT COUNTER
      ...
      END DO OTIME
   END IF
   !         RESET PRINT DATA FOR REACH AFTER PRINTING IF
   !         DATA IS BEING AVERAGED AND NOT NEEDED LATER
   IF ( ISWRSAVG.NE.0 ) THEN
      ...
      END IF
   END IF
   !---------SAVE STAGE, DEPTH, AND GBELV (RBOT) OBSERVATION DATA
...
END SUBROUTINE SSWR_P_RCHSTG
   ...
   !
   !-------SUBROUTINE TO OUTPUT SUMMARY QAQFLOW DATA TO ISWRPQAQ
SUBROUTINE SSWR_P_QAQFLOW(Kper,Kstp)
   USE GWFBASMODULE, ONLY: DELT, TOTIM
   USE GLOBAL,       ONLY: NLAY, IFREFM, ISSFLG, HNEW, HOLD
   USE GWFSWRMODULE, ONLY: IZERO, RZERO, DZERO, DONE,&
   &ISWRSRIV, ISWRALLRIV, ISWRSAVG,&
   &RTPRN, NPMAX, NREACHES, REACH, RCHGRP,&
   &ISWRPSTG, ISWRPQAQ, ISWRPSTR,&
   &NUMTIME, SWRHEADER, ISWRDT, SWRDT,&
   &SWRTIME, RSTAGE, RSTAGEP,&
   &NOBS, IOBSQAQ, OBSDATA, DOBS
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   INTEGER, INTENT(IN) :: Kper
   INTEGER, INTENT(IN) :: Kstp
   ...
   !     + + + OUTPUT FORMATS + + +
2000 FORMAT('TOTIME    ,SWRDT     ,KPER      ,KSTP      ,',&
   &'KSWR      ,REACH     ,LAYER     ,GBELEV    ,',&
   &'STAGE     ,DEPTH     ,HEAD      ,WETPERM   ,',&
   &'CONDUCT   ,HEADDIFF  ,QAQFLOW')
...
2030 FORMAT(2(I10),10X,&
   &'STRESS PERIOD',1X,I5,1X,&
   &'TIME STEP',1X,I5,1X,&
   &'SWR1 TIME STEP',1X,I5,1X,&
   &'TOTIM',1X,G10.3)
   !     + + + CODE + + +
   iu   = ABS( ISWRPQAQ )
   iriv = ABS( ISWRSRIV )
   ...
   IF ( SWRHEADER.EQ.0 ) THEN
      ...
      ELSE IF ( ISWRPQAQ.LT.IZERO ) THEN
         WRITE (iu) NREACHES
      END IF
   !-----------CALCULATE THE MAXIMUM NUMBER OF RIV CELLS
      IF ( iriv.GT.0 ) THEN
         mxactr = 0
         DO irch = 1, NREACHES
         ...
         END DO
      END IF
   !-----------WRITE DATASET 0 AND 2 TO THE MODFLOW RIV PACKAGE
   !           GENERATED BY SWR1
      ...
      END IF
   END IF
   !---------DETERMINE THE NUMBER OF MODFLOW RIV CELLS FOR THIS STRESS PERIOD
   IF ( iriv.GT.0 ) THEN
      itmp = 0
      DO irch = 1, NREACHES
   ...
   !-------------LOOP THROUGH EACH LAYER
         PLAYERS: DO kl = REACH(irch)%LAYSTR, REACH(irch)%LAYEND
            IF ( ISWRSAVG.NE.0 ) THEN
               ...
               &REACH(irch)%QAQRSLTS(ip,kl)%WETTEDPERIMETER +&
               &REACH(irch)%CURRENTQAQ(kl)%WETTEDPERIMETER * dt*pfact
               REACH(irch)%QAQRSLTS(ip,kl)%CONDUCTANCE     =&
               &REACH(irch)%QAQRSLTS(ip,kl)%CONDUCTANCE     +&
               &REACH(irch)%CURRENTQAQ(kl)%CONDUCTANCE     * dt*pfact
               REACH(irch)%QAQRSLTS(ip,kl)%HEADDIFFERENCE  =&
               &REACH(irch)%QAQRSLTS(ip,kl)%HEADDIFFERENCE  +&
               &REACH(irch)%CURRENTQAQ(kl)%HEADDIFFERENCE  * dt*pfact
               REACH(irch)%QAQRSLTS(ip,kl)%QAQFLOW         =&
               &REACH(irch)%QAQRSLTS(ip,kl)%QAQFLOW         +&
               &REACH(irch)%CURRENTQAQ(kl)%QAQFLOW         * dt*pfact
            ELSE IF ( SWRTIME(n)%ITPRN.GT.0 ) THEN
               REACH(irch)%QAQRSLTS(ip,kl)%WETTEDPERIMETER =&
               &REACH(irch)%CURRENTQAQ(kl)%WETTEDPERIMETER
               REACH(irch)%QAQRSLTS(ip,kl)%CONDUCTANCE     =&
               &REACH(irch)%CURRENTQAQ(kl)%CONDUCTANCE
               REACH(irch)%QAQRSLTS(ip,kl)%HEADDIFFERENCE  =&
               &REACH(irch)%CURRENTQAQ(kl)%HEADDIFFERENCE
               REACH(irch)%QAQRSLTS(ip,kl)%QAQFLOW         =&
               &REACH(irch)%CURRENTQAQ(kl)%QAQFLOW
            END IF
         END DO PLAYERS
      END DO
      IF ( SWRTIME(n)%ITPRN.LT.1 ) CYCLE PTIME
      ...
      IF ( ip.GT.NPMAX ) EXIT PTIME !HACK
   END DO PTIME
   !---------WRITE REACH QAQFLOW DATA FOR EACH SWR TIMESTEP
   ip = 1
   swrtot = DBLE( TOTIM - DELT )
   OTIME: DO n = 1, NUMTIME
      dt = SWRTIME(n)%SWRDT
   ...
   !             WRITE HEADER THAT DEFINES THE NUMBER OF LAYERS
   !             QAQ DATA IS WRITTEN FOR EACH REACH
         DO irch = 1, NREACHES
         ...
         WRITE (iu) swrtot, dt, Kper, Kstp, n
      END IF
   !           WRITE DATASET 5 FOR MODFLOW RIV PACKAGE
      IF ( ISWRSRIV.GT.0 ) THEN
      ...
      DO irch = 1, NREACHES
         ...
         LAYERS: DO kl = REACH(irch)%LAYSTR, REACH(irch)%LAYEND
            ...
            IF ( ISWRPQAQ.GT.IZERO ) THEN
               ...
               &n, irch, kl, REACH(irch)%GBELEV,&
               &RSTAGEP(irch,ip),&
               &RSTAGEP(irch,ip) - REACH(irch)%GBELEV, h,&
               &REACH(irch)%QAQRSLTS(ip,kl)%WETTEDPERIMETER,&
               &REACH(irch)%QAQRSLTS(ip,kl)%CONDUCTANCE,&
               &REACH(irch)%QAQRSLTS(ip,kl)%HEADDIFFERENCE,&
               &REACH(irch)%QAQRSLTS(ip,kl)%QAQFLOW
   ...
   !               WRITE DATASET 6 FOR MODFLOW RIV PACKAGE
            ...
            END IF
   !               RESET PRINT DATA FOR REACH AFTER PRINTING IF
   !               DATA IS BEING AVERAGED AND IS NOT NEEDED LATER
            IF ( ISWRSAVG.NE.0 ) THEN
...
END SUBROUTINE SSWR_P_QAQFLOW
   !
   !-------SUBROUTINE TO OUTPUT LATERAL FLOW DATA TO ISWRPQM
SUBROUTINE SSWR_P_QMFLOW(Kper,Kstp)
   USE GWFBASMODULE, ONLY: DELT, TOTIM
   USE GLOBAL,       ONLY: ISSFLG
   USE GWFSWRMODULE, ONLY: IZERO, RZERO, DZERO, DONE,&
   &ISWRSAVG, RTPRN, NPMAX, NRCHGRP, RCHGRP,&
   &NUMTIME, ISWRPQM, SWRHEADER, SWRDT,&
   &ISWRDT, SWRTIME, GSTAGE,&
   &REACH, NOBS, IOBSQM, OBSDATA, DOBS
...
2010 FORMAT(2(E15.7,','),6(I10,','),E15.7,',',E15.7)
      ...
      ELSE IF ( ISWRPQM.LT.IZERO ) THEN
         ...
         DO irg = 1, NRCHGRP
            ...
            IF ( nconn.GT.0 ) THEN
               ...
               DO ic = 1, nconn
                  WRITE (iu)&
                  &ABS( RCHGRP(irg)%IRCHN(ic) ),&
                  &ABS( RCHGRP(irg)%IRCHC(ic) )
               END DO
               nrgout = nrgout + 1
               ntconn = ntconn + nconn
            END IF
   ...
   !             UPDATE STRUCTURE VALUES
         ...
         END DO
         DO irg = 1, NRCHGRP
   !               UPDATE CURRENT QM VALUES
            qm  = SSWR_RG_QM(RCHGRP(irg),GSTAGE(:,n))
            DO ic = 1, RCHGRP(irg)%NCONN
               IF ( ISWRSAVG.NE.0 ) THEN
                  RCHGRP(irg)%QCONNP(ic,ip)%FLOW =&
                  &RCHGRP(irg)%QCONNP(ic,ip)%FLOW +&
                  &RCHGRP(irg)%QCONN(ic)%FLOW * dt * pfact
                  RCHGRP(irg)%QCONNP(ic,ip)%VELOCITY =&
                  &RCHGRP(irg)%QCONNP(ic,ip)%VELOCITY +&
                  &RCHGRP(irg)%QCONN(ic)%VELOCITY * dt * pfact
               ELSE IF ( SWRTIME(n)%ITPRN.GT.0 ) THEN
                  RCHGRP(irg)%QCONNP(ic,ip)%FLOW =&
                  &RCHGRP(irg)%QCONN(ic)%FLOW
                  RCHGRP(irg)%QCONNP(ic,ip)%VELOCITY =&
                  &RCHGRP(irg)%QCONN(ic)%VELOCITY
               END IF
            END DO
         END DO
         IF ( SWRTIME(n)%ITPRN.LT.1 ) CYCLE PTIME
      ...
      END DO PTIME
   END IF
   !---------WRITE LATERAL FLOW DATA FOR EACH SWR TIMESTEP
   ip = 1
   swrtot = DBLE( TOTIM - DELT )
   ...
   END DO VTIME
   !---------SAVE LATERAL FLOW (QM) OBSERVATION DATA
   IF ( IOBSQM.GT.0 ) THEN
      DO iobs = 1, NOBS
         iobstype = OBSDATA(iobs)%IOBSTYPE
...
END SUBROUTINE SSWR_P_QMFLOW
   !
   !-------SUBROUTINE TO OUTPUT SUMMARY STRUCTURE FLOW DATA TO ISWRPSTR
SUBROUTINE SSWR_P_STRFLOW(Kper,Kstp)
   ...
   &NUMTIME, SWRHEADER, SWRDT,&
   &ISWRDT, SWRTIME,&
   &RSTAGE, GSTAGE, RSTAGEP,&
   &NOBS, IOBSSTR, OBSDATA, DOBS
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   INTEGER, INTENT(IN) :: Kper
   INTEGER, INTENT(IN) :: Kstp
...
2000 FORMAT('TOTIME         ,SWRDT          ,',&
   &'KPER      ,KSTP      ,',&
   &'KSWR      ,REACH     ,STRUCTURE ,',&
   &'USSTAGE        ,DSSTAGE        ,GATEELEV       ,',&
   &'OPENING        ,STRFLOW    ')
2010 FORMAT(g15.5,',',g15.5,5(',',I10),5(',',G15.5))
   ...
   IF ( ISWRPSTR.NE.IZERO ) THEN
         ...
         DO irch = 1, NREACHES
            ...
            DO istr = 1, REACH(irch)%NSTRUCT
               ...
               SELECT CASE ( ABS(istrtype) )
                ...
                CASE ( 2 )
                  IF ( REACH(irch)%STRUCT(istr)%ISTRBC.EQ.2 ) THEN
                     ...
                     &(RSTAGE(irch,n) - REACH(irch)%GBELEV)
                     b = t
                  END IF
                CASE ( 3 )
                CASE ( 4 )
               ...
               IF ( ISWRSAVG.NE.0 ) THEN
                  ...
                  &REACH(irch)%STRUCT(istr)%RSLTS(ip)%FLOW    +&
                  &REACH(irch)%STRUCT(istr)%CURRENT%FLOW * dt * pfact
               ELSE IF ( SWRTIME(n)%ITPRN.GT.0 ) THEN
                  REACH(irch)%STRUCT(istr)%RSLTS(ip)%TOP     = t
                  REACH(irch)%STRUCT(istr)%RSLTS(ip)%BOTTOM  =&
                  &REACH(irch)%STRUCT(istr)%RSLTS(ip)%BOTTOM  + b
                  REACH(irch)%STRUCT(istr)%RSLTS(ip)%OPENING = o
                  REACH(irch)%STRUCT(istr)%RSLTS(ip)%FLOW    =&
                  &REACH(irch)%STRUCT(istr)%CURRENT%FLOW
               END IF
            END DO
         END DO
         IF ( SWRTIME(n)%ITPRN.LT.1 ) CYCLE PTIME
      ...
      END DO PTIME
   END IF
   !---------WRITE STRUCTURE FLOW FOR EACH SWR TIMESTEP
   IF ( ISWRPSTR.NE.IZERO ) THEN
      ip = 1
      swrtot = DBLE( TOTIM - DELT )
      OTIME: DO n = 1, NUMTIME
   ...
   !               WRITE HEADER THAT DEFINES THE NUMBER OF STRUCTURES
   !               DATA THAT ARE WRITTEN FOR EACH REACH
            DO irch = 1, NREACHES
   ...
   !             INCREMENT PRINT COUNTER
      ...
      END DO OTIME
   END IF
   !---------SAVE STRUCTURE FLOW OBSERVATION DATA
            ...
            DO n = 1, NUMTIME
               IF (istrobs.NE.0) THEN
                  ...
                  SELECT CASE ( iobstype )
                   ...
                   CASE (31)
                     DOBS(iobs,n) =&
                     &REACH(irch)%STRUCT(istr)%CURRENT%TOP
                   CASE (32)
                     DOBS(iobs,n) =&
                     &REACH(irch)%STRUCT(istr)%CURRENT%BOTTOM
                   CASE (33)
                     DOBS(iobs,n) =&
                     &REACH(irch)%STRUCT(istr)%CURRENT%OPENING
                  END SELECT
               ELSE
...
END SUBROUTINE SSWR_P_STRFLOW
   !
   !-------SUBROUTINE TO OUTPUT OBSERVATION DATA TO ISWRPOBS
SUBROUTINE SSWR_P_OBS(Kper,Kstp)
   USE GWFBASMODULE, ONLY: DELT, TOTIM
   USE GWFSWRMODULE, ONLY: IZERO, RZERO, DZERO, DONE, DMISSING,&
   &ISWRSAVG, RTPRN, ISWRPOBS, IPOBSALL,&
   &NUMTIME, SWRHEADER, SWRDT,&
   &ISWRDT, SWRTIME,&
   &NOBS, OBSDATA, DOBS, DOBSP
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   INTEGER, INTENT(IN) :: Kper
   INTEGER, INTENT(IN) :: Kstp
...
2010 FORMAT(5X,G15.5)
   ...
   IF ( SWRHEADER.EQ.0 ) THEN
      ...
      END IF
   END IF
   !---------WRITE OBSERVATION DATA FOR EACH SWR TIMESTEP
   pfact = DONE
   IF ( RTPRN.NE.RZERO ) THEN
...
END SUBROUTINE SSWR_P_OBS
   !
   !-------SUBROUTINE TO OUTPUT SUMMARY FROUDE NUMBER DATA TO IOUT
SUBROUTINE SSWR_P_FROUDE(Kper,Kstp)
   USE GWFBASMODULE, ONLY: DELT, TOTIM
   USE GLOBAL,       ONLY: IOUT,NPER,NSTP
   USE GWFSWRMODULE, ONLY: IZERO, DZERO, DONE, GRAVITY,&
   &DLENCONV, TIMECONV, NUMTIME,&
   &REACH, NRCHGRP, RCHGRP,&
   &ISWRDT, SWRTIME, FROUDE, GSTAGE
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   INTEGER, INTENT(IN) :: Kper
   INTEGER, INTENT(IN) :: Kstp
...
2000 FORMAT(//,29X,'FROUDE NUMBER SUMMARY',/&
   &15X,'FOR STRESS PERIOD',I10,1X,'TIME STEP',I10,/,&
   &1X,'  SWR STEP',&
   &1X,'    MAX FR',1X,'   FR RGRP',&
   &1X,' MAX DFRDX',1X,'DFRDX RGRP',&
   &1X,' MAX DFRDT',1X,'DFRDT RGRP',/,&
   &1X,7(10('-'),1X))
2010 FORMAT(1X,I10,&
   &1X,G10.3,1X,I10,&
   &1X,G10.3,1X,I10,&
...
2030 FORMAT(1X,10X,&
   ...
   !             ALLOCATE TEMPORARY STORAGE FOR REACH GROUP
         ALLOCATE ( fr0(RCHGRP(irg)%NCONN) )
         ALLOCATE (  fr(RCHGRP(irg)%NCONN) )
         fr0 = DZERO
         fr  = DZERO
   !             UPDATE CURRENT QM VALUES TO PREVIOUS VALUES
   ...
   !             UPDATE CURRENT QM VALUES
         ...
         END DO
         DO j = 1, RCHGRP(irg)%NCONN
   !               DETERMINE IF MAX FROUDE NUMBER
            ...
            IF ( fr(j).GT.FROUDE%VAL ) THEN
               ...
               FROUDE%LOCVAL = irg
            END IF
   !               DETERMINE IF MAX CHANGE IN FROUDE NUMBER WITH TIME
            ...
            END IF
         END DO
   !             FIND MAXIMUM CHANGE IN FROUDE NUMBER BETWEEN CONNECTIONS
         frpos = DZERO
         frneg = DZERO
         DO j = 1, RCHGRP(irg)%NCONN
         ...
         END IF
   !             CLEAN UP TEMPORARY STORAGE FOR REACH GROUP
         DEALLOCATE(  fr )
         DEALLOCATE( fr0 )
      END DO RG
   !-----------WRITE SUMMARY FOR TIMESTEP
      WRITE (IOUT,2010) n,&
      &frmax,locdfr,&
      &dfrdlmax,locdfrdl,&
      &dfrdtmax,locdfrdt
   END DO TME
   !
   !---------WRITE SUMMARY FOR ENTIRE SIMULATION
   IF ( Kper.EQ.NPER ) THEN
      IF ( Kstp.EQ.NSTP(NPER) ) THEN
         n = 0
         WRITE (IOUT,2020)
         WRITE (IOUT,2030)&
         &FROUDE%VAL,FROUDE%LOCVAL,&
         &FROUDE%DVALDL,FROUDE%LOCDVALDL,&
         &FROUDE%DVALDT,FROUDE%LOCDVALDT
      END IF
   END IF
   !---------RETURN
   RETURN
END SUBROUTINE SSWR_P_FROUDE
   !
   !  SURFACE-WATER ROUTING CALCULATION FUNCTIONS
   !
   !-------PASSED VOLUME WILL ALWAYS RETURN STAGE AT DEFINED "DOWNSTREAM"
   !       REACH OF REACH GROUP
DOUBLEPRECISION FUNCTION SSWR_RG_STG(Rg,V) RESULT(s)
   ...
   maxoff = -1.0D9
   ts0    =  1.0D9
   !---------FIND MINIMUM STAGE, MAXIMUM STAGE, AND MAXIMUM OFFSET
   DO nr = 1, Rg%NRGREACH
      irch = Rg%REACH(nr)
   ...
   IF (ABS(rl).LT.e) THEN
      ...
      RETURN
   END IF
   !---------USE BI-SECTION TO FIND GROUP STAGE
   ibs = IZERO
   DO
      ibs = ibs + 1
      ts = gsu + (gsl - gsu) / DTWO
      ...
      IF (ABS(r).LT.e) THEN
         ...
         RETURN
      END IF
   !           CHECK THAT PROGRESS IS BEING MADE ON STAGE
...
END FUNCTION SSWR_RG_STG
   !
   !-------PASSED STAGE WILL ALWAYS BE STAGE AT DOWNSTREAM END OF GROUP
...
END FUNCTION SSWR_CALC_VOL
   !
   !-------INFLOW IS SPECIFIED LATERAL INFLOW (QLATFLOW), INFLOW FROM THE UZF
   !       PACKAGE (QUZFLOW), AND INFLOW FROM EXTERNAL SOURCES (QEXTFLOW).
DOUBLEPRECISION FUNCTION SSWR_RG_QLI(Rg) RESULT(value)
   ...
   END DO
   !         UPDATE REACH GROUP INFLOW AND OUTFLOW
...
DOUBLEPRECISION FUNCTION SSWR_RG_QPR(Rg) result(value)
   ...
   !     + + + CODE + + +
   value = DZERO
   !         DO NOT EVALUATE INACTIVE REACH GROUPS
...
DOUBLEPRECISION FUNCTION SSWR_CALC_QPR(Irch) result(value)
   ...
   r     = REACH(Irch)%RAIN
   REACH(Irch)%CURRENT%RAIN = DZERO
   !         DO NOT EVALUATE REACHES WITH ZERO RAINFALL RATES
...
DOUBLEPRECISION FUNCTION SSWR_CALC_QEV(Irch,Rs) RESULT(value)
   ...
   e      = swe
   REACH(Irch)%CURRENT%EVAP = DZERO
   !         DO NOT EVALUATE REACHES WITH ZERO EVAPORATION RATES
   IF ( e.LT.NEARZERO ) GOTO 9999
   rbot   = REACH(Irch)%GEO%ELEV(1)
   d      = Rs - rbot
   e      = MIN( d, e )
   !---------EVALUATE IF DEPTH (d) IS LESS THAN DUPDPTH
...
END FUNCTION SSWR_CALC_QEV
   ...
   !-------REACH GROUP AQUIFER-REACH FLOW GROUNDWATER HEAD
...
DOUBLEPRECISION FUNCTION SSWR_RG_QAQ(Rg,Gs) RESULT(value)
   ...
   !-----------RATE
   !           RELATIVE TO GROUP
   !           POSITIVE IS A SOURCE FOR GROUP
   !           NEGATIVE IS A SINK   FOR GROUP
      value = value + q
   END DO RCHGRPL
   !         UPDATE REACH GROUP INFLOW AND OUTFLOW
   IF ( value.GT.DZERO ) THEN
...
END FUNCTION SSWR_RG_QAQ
   ...
   !-------REACH AQUIFER-REACH FLOW ESTIMATE - SWR1
DOUBLEPRECISION FUNCTION SSWR_CALC_QAQ(Rch,Rs) RESULT(value)
   ...
   USE GWFSWRMODULE, ONLY: DZERO, DONE, NEARZERO,&
   &DMINDPTH, DUPDPTH,&
   &TREACH, ISWRDT, SWRDT, HK,&
   &ISWRSS,&
   &INWTUNIT, INWTCORR, INWTCNT,&
   &SWRTIME, KMFITER, SWRHEPS
   USE GWFSWRINTERFACE, ONLY: SSWR_LININT, SSWR_CALC_DPTHFACT
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   ...
   jcol  = Rch%JRCH
   kact  = NLAY + 1
   !---------FIND UPPERMOST ACTIVE GROUNDWATER CELL
   ...
   END DO TOPLAYER
   Rch%LAYACT = kact
   !         SET dh FOR GROUNDWATER FLOW NEWTON ITERATIONS
   dh   = DZERO
   ...
   !           CALCULATE QAQ FOR LAYER
      ...
      h     = hmin + dh
   !---------SKIP IF STAGE (trs) AND GROUNDWATER HEAD (h) IS BELOW REACH BOTTOM (rbot)
      IF ( trs.EQ.rbot  .AND. h-rbot.LT.DZERO ) CYCLE LAYERS
   !-----------CALCULATE MAXIMUM INCREMENTAL WETTED PERIMETER FOR CURRENT HEAD
      wps = MAX(h,trs)
   ...
   !-----------JAN JEPPESEN FIX
      ...
      &Rch%GEO%WETPER,zgtop)

      wpbot = SSWR_LININT(Rch%GEO%ELEV,&
      &Rch%GEO%WETPER,zgbot)
      twp = wptop
         ...
         SELECT CASE (Rch%IGCNDOP)
          ...
          CASE (3)
            IF ( twp.GT.DZERO ) THEN
               ...
               &(DONE / (twp * length * Rch%GLK))
               cond = DONE / cond
            ELSE
               cond = DZERO
            END IF
         END SELECT
   !-----------USER SPECIFIED CONDUCTANCE
      ELSE
         zfact = DONE
         ...
         cond = Rch%GCND * zfact
      END IF
   !-----------EVALUATE IF SEEPAGE LOSSES WILL OCCUR AND IF
   !           DEPTH IS IN RANGE OF DUPDPTH AND DMINDPTH
   !           IF TRUE SCALE CONDUCTANCE
      hd = trs - h
      d  = trs - rbot
      IF ( Rch%ISWRBND.GT.0 .AND.&
      &hd.GT.DZERO .AND. d.LT.DUPDPTH ) THEN
         fact = SSWR_CALC_DPTHFACT( ABS(d), DUPDPTH, DMINDPTH )
         cond = cond * fact
      END IF
   !-----------DETERMINE IF GROUNDWATER HEAD IS BELOW STREAM BOTTOM
      IF (h.GT.rbot) THEN
         hd = trs - h
      ELSE
   ...
   !           NEGATIVE IS A SINK   FOR GROUP
      value = value - cond * hd
   !-----------RATE - WITH SIGN CHANGE
   !           USING MODFLOW CONVENTION
   !           POSITIVE IS A SOURCE FOR MODFLOW
   !           NEGATIVE IS A SINK   FOR MODFLOW
   !-----------REACH DATA FOR CURRENT SWR TIMESTEP
      Rch%CURRENTQAQ(k)%QAQFLOW        = cond * hd
      Rch%CURRENTQAQ(k)%CONDUCTANCE    = cond
      Rch%CURRENTQAQ(k)%HEADDIFFERENCE = hd
   !-----------REACH DATA FOR SWR TIMESTEP
      Rch%CURRENT%QAQFLOW =&
      &Rch%CURRENT%QAQFLOW - cond * hd
   !---------FILL NEWTON CORRECTION TERMS
      IF ( INWTCORR.NE.0 ) THEN
         IF ( rhsonly ) THEN
            rh = cond * hd
            hc = DZERO
...
END FUNCTION SSWR_CALC_QAQ
   !
   !-------REACH AQUIFER-REACH FLOW ESTIMATE - MODFLOW
...
END FUNCTION SSWR_CALC_MFQAQ
   ...
   !-------REACH STRUCTURE FLOW CALCULATION
...
&RESULT(value)
   ...
   qt    = DZERO
   qsfr  = DZERO
   !---------RETURN IF NO STRUCTURES IN CURRENT REACH
   IF ( Rch%NSTRUCT.LT.1 ) RETURN
   !---------EVALUATE EVERY REACH IN REACH GROUP
   DO ns = 1, Rch%NSTRUCT
      istrconn = Rch%STRUCT(ns)%ISTRCONN
      istrtype = Rch%STRUCT(ns)%ISTRTYPE
   !           CHECK THAT THIS IS THE CORRECT STRUCTURE
...
DOUBLEPRECISION FUNCTION SWR_STR_DIRCORR(Istrdir,Q) RESULT(value)
   ...
   value       = DONE
   SELECT CASE (Istrdir)
   !           U/S FLOW - ONLY NEGATIVE VALUES ALLOWED
    CASE (:-1)
      IF ( Q.GT.DZERO ) VALUE = DZERO
    CASE (0)
      value = DONE
   !           D/S FLOW - ONLY POSITIVE VALUES ALLOWED
    CASE (1:)
...
&RESULT(value)
   ...
   &DTHREEHALVES, DONE, DTWO, DTWOPI,&
   &NEARZERO,&
   &TSTRUCT,&
   &GRAVITY, DLENCONV, TIMECONV
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   ...
   value       = DZERO
   !
   !---------CALCULATE HEADWATER AND TAILWATER ELEVATIONS
   ...
   END IF
   !
   !---------CHECK IF MAXIMUM DEPTH ABOVE CULVERT INVERT IS LESS THAN NEARZERO
   IF ( dmax.LT.NEARZERO ) RETURN
   !
   !---------CALCULATE CULVERT AREA
   !         RECTANGULAR CULVERT
   ...
   co = Str%STRCD2
   !
   !---------CALCULATE RATIO OF MAXIMUM DEPTH TO CULVERT DIAMETER (hcrit)
   hcrit = dmax / d
   !
   !---------UNSUBMERGED CULVERT FLOW
   IF ( hcrit.LT.1.2D0 ) THEN
      q = cw * ac * SQRT( g * dmax )
   !---------SUBMERGED CULVERT FLOW
   ELSE
   !           INLET CONTROLLED
      IF ( dmin.LT.d ) THEN
...
&Cw,Co,Se) RESULT(value)
   ...
   !
   !---------CHECK IF MAXIMUM DEPTH ABOVE THE INVERT ELEVATION IS LESS THAN NEARZERO
   IF ( dus.LT.NEARZERO ) RETURN
   !
   !---------CALCULATE FLOW DIRECTION
   !         FLOW FROM DOWNSTREAM TO UPSTREAM = NEGATIVE
   IF ( Ustage.LT.Dstage ) mult = -mult
   !
   !---------CALCULATE RATIO OF DOWNSTREAM TO UPSTREAM DEPTH (dcrit)
   dcrit = dds / dus
   !
   !---------SET CONTROLLING HEAD DIFFERENCE BASED ON SUBMERGENCE
   ...
   END IF
   !
   !---------CHECK FOR CONTROLLED CONDITIONS
   dgo = Gelev - Strinv
   ...
   IF ( gcrit.LT.0.8D0 ) THEN
      ...
      c = Co
   ELSE
   !           LINEAR SCALING BETWEEN Cw AND Co FROM 0.8 >= gcrit < 1.0
...
&C1,C2,C3) RESULT(value)
   ...
   dds = hds - Strinv
   !
   !---------CALCULATE GATE OPENING (dgo) AND MINIMUM OF GATE OPENING OR
   !         UPSTREAM DEPTH ABOVE THE SPILLWAY GATE INVERT ELEVATION
   dgo = MAX( Gelev - Strinv, DZERO )
   d   = MIN( dgo, dus )
   !
   !---------CHECK IF CONTROLLING DEPTH (d) IS LESS THAN NEARZERO
   ...
   IF ( Ustage.LT.Dstage ) mult = -mult
   !
   !---------CALCULATE RATIO OF DOWNSTREAM TO UPSTREAM DEPTH (dcrit) AND
   !         RATIO OF d AND UPSTREAM DEPTH (gcrit)
   dcrit = dds / dus
   gcrit = d   / dus
   !
   ...
   &( ( 1 - dcrit ) ** C3 )
   !
   !---------CONVERT GRAVITY TO CORRECT UNITS
   g = GRAVITY * DLENCONV * TIMECONV * TIMECONV
   !
...
SUBROUTINE SSWR_GETDT(Rtime0,Rdt)
   ...
   !     + + + CODE + + +
   !
   !---------UPDATE RTIME WHICH IS USED TO DEFINE SWRDT
   n      = ITABTIME
   ...
   !             EVALUATE IF DATA NEEDS TO BE RETRIEVED FROM MEMORY
   ...
   !           INTERPOLATE
    CASE (3)
      ...
      &SSWR_RLININT(TABDATA(n)%TSDATA%T(:),&
      &TABDATA(n)%TSDATA%V(:),Rtime0)
   END SELECT
   Rdt = TABDATA(n)%TSDATA%DVAL
   !
   !---------RETURN
...
SUBROUTINE SSWR_GET_TABS(Rtime0,Rtime1)
   ...
   !     + + + CODE + + +
   !
   !---------UPDATE TABULAR DATA FROM EXTERNAL FILES
   LTAB: DO n = 1, NTABS
   !           SKIP TABULAR DATA SPECIFYING SWRDT
      IF ( TABDATA(n)%ITABTYPE.LT.1 ) CYCLE LTAB
   !           SKIP TABULAR DATA USING SWR STAGE RESULTS
   ...
   !             INTERPOLATE
       CASE (3)
         TABDATA(n)%TSDATA%DVAL =&
         &SSWR_RLININT(TABDATA(n)%TSDATA%T(:),&
         &TABDATA(n)%TSDATA%V(:),Rtime1)
      END SELECT
   !-----------SET TEMPORARY VALUE v
      v = TABDATA(n)%TSDATA%DVAL
   !-----------EVALUATE EACH REACH USING CURRENT TABULAR DATA ITEM
   ...
   !               STAGE - ONLY ADJUST CONSTANT STAGE REACHES
          CASE (4)
   ...
   END DO LTAB
   !
   !---------UPDATE TABULAR DATA FROM SWR STAGE RESULTS FILES
   LSTGRES: DO n = 1, NTABS
   !           SKIP TABULAR DATA NOT USING SWR STAGE RESULTS
   ...
   !               INTERPOLATE
          CASE (3)
            ...
            &SSWR_RLININT(TABDATA(n)%STGRES(nr)%T(:),&
            &TABDATA(n)%STGRES(nr)%V(:),Rtime1)
         END SELECT
   !-------------SET TEMPORARY VALUE v
...
SUBROUTINE SSWR_GET_STRGATE(Rtime0,Rtime1)
   USE GLOBAL,       ONLY: IOUT
   USE GWFSWRMODULE, ONLY: IZERO, DZERO, DONE,&
   &NREACHES, NRCHGRP, REACH, RCHGRP,&
   &ISWRDT, SWRDT, IADTIME,&
   &NTABS, TABDATA
   USE GWFSWRINTERFACE, ONLY: SSWR_RLININT
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   ...
   !
   !
   !---------UPDATE STRUCTURE DATA FROM TIMESERIES FILES
   LRG: DO n = 1, NRCHGRP
   !-----------DO NOT EVALUATE INACTIVE REACH GROUPS
      IF ( RCHGRP(n)%INACTIVE ) CYCLE LRG
   !-----------DO NOT EVALUATE IF NO STRUCTURES IN CURRENT REACH GROUP
      IF ( RCHGRP(n)%NSTRUCT.LT.1 ) CYCLE LRG
   !-----------EVALUATE EACH REACH IN THE REACH GROUP
   ...
   !               DO NOT EVALUATE STRUCTURES THAT ARE NOT OPERATED USING
   !               TIMESERIES DATA
   ...
   !                 INTERPOLATE
             CASE (3)
               ...
               &SSWR_RLININT(TABDATA(itab)%TSDATA%T(:),&
               &TABDATA(itab)%TSDATA%V(:),Rtime1)
            END SELECT
   ...
   !                 STRCRIT
             CASE (1)
               ...
               &REACH(irch)%STRUCT(nn)%STRVAL
   !                 STRVAL
             CASE (2)
               !NOTHING TO DO
             CASE DEFAULT
               CALL USTOP('PROGRAMMING ERROR: UNKNOWN '//&
               &'ISTRTSTYPE IN SSWR_GET_STRGATE')
            END SELECT
         END DO LSTRUCTURE
      END DO LREACHES
   END DO LRG
...
END SUBROUTINE SSWR_GET_STRGATE
   ...
   !       UPDATE OPERABLE STRUCTURE DISCHARGE (PUMP) RATES, GATE
   !       LEVELS FOR ALL OPERABLE STRUCTURES, AND STRVAL FOR ALL
   !       STAGE-DEPENDENT FLUX STRUCTURES
SUBROUTINE SWR_CALC_STRVALS(P)
   USE GWFSWRMODULE, ONLY: IZERO, DZERO, DONE,&
   &ISWRPSTR, ILAGSTROPR,&
   &NREACHES, NRCHGRP, REACH, RCHGRP,&
   &ISWRDT, SWRDT, RSTAGE, GSTAGE
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   DOUBLEPRECISION, DIMENSION(NRCHGRP), INTENT(IN)   :: P
   !     + + + LOCAL DEFINITIONS + + +
   ...
   !-----------DO NOT EVALUATE INACTIVE REACH GROUPS
      IF ( RCHGRP(n)%INACTIVE ) CYCLE LRG
   !-----------RETURN IF NO STRUCTURES IN CURRENT REACH GROUP
      IF ( RCHGRP(n)%NSTRUCT.LT.1 ) CYCLE LRG
   !-----------SET ps TO RCHGRP STAGE
      ps = P(RCHGRP(n)%IRG)
      LREACHES: DO nr = 1, RCHGRP(n)%NRGREACH
         irch   = RCHGRP(n)%REACH(nr)
   !             DO NOT EVALUATE INACTIVE REACHES
         IF ( ABS( REACH(irch)%ISWRBND ).LT.1 ) CYCLE LREACHES
   !             DO NOT EVALUATE REACHES WITHOUT OPERABLE STRUCTURES
         IF ( REACH(irch)%NOPR.LT.1 ) CYCLE LREACHES
         stage  = ps + REACH(irch)%OFFSET
         LSTRUCTURE: DO nn = 1, REACH(irch)%NSTRUCT
            istrtype = REACH(irch)%STRUCT(nn)%ISTRTYPE
   !               CHECK IF CURRENT STRUCTURE IS AN OPERABLE STRUCTURE
            IF ( istrtype.NE.3 .AND. istrtype.NE.8 .AND.&
            &istrtype.NE.9 .AND. istrtype.NE.10 .AND.&
            &istrtype.NE.12 ) CYCLE LSTRUCTURE
   ...
   !                 FLOW
             CASE (2)
               ...
               IF ( ILAGSTROPR.EQ.0 ) THEN
   !                     ESTIMATE CURRENT CONNECTION FLOWS
   !                     REQUIRED FOR SURFACE WATER ONLY SIMULATIONS THAT
   !                     ONLY MAKE ONE CALL TO CALL TO GWF2SWR7FM
                  cval   = SSWR_RG_QM(RCHGRP(irg),P)
   !                     SET cval TO CURRENT ESTIMATE OF QCONN FOR
   !                     SPECIFIED REACH GROUP CONNECTION
                  cval   = RCHGRP(irg)%QCONN(iconn)%FLOW
               ELSE
   !                    CALCULATE FLOW FOR LAST SWR TIME STEP
                  cval   = SSWR_RG_QM(RCHGRP(irg),GSTAGE(:,ISWRDT-1))
                  cval   = RCHGRP(irg)%QCONN(iconn)%FLOW
   !                     RESET QM TO VALUE FOR CURRENT REACH GROUP STAGE
                  qm     = SSWR_RG_QM(RCHGRP(irg),P)
               END IF
            END SELECT
   !               INITIAL GATE OPENING
            ...
            END IF
   !               CALCULATE NEW GATE OPENING
            QISOPRSTR: IF ( istrtype.EQ.3 .OR. istrtype.EQ.8 .OR.&
            &istrtype.EQ.9 .OR. istrtype.EQ.10 .OR.&
            &istrtype.EQ.12 ) THEN
   !                 USE TIMESERIES DATA TO SET GATE-LEVEL
               QUSETS: IF ( istrtstype.EQ.2 ) THEN
                  SELECT CASE ( istrtype )
   !                     SPECIFIED DISCHARGE (PUMP)
                   CASE (3)
   ...
   !                     OPERABLE GATED SPILLWAY
                   CASE (9,10)
                     gt = REACH(irch)%STRUCT(nn)%STRVAL + strinv
                     gb = strinv
   !                     STAGE-DEPENDENT FLUX
                   CASE (12)
                     gt = REACH(irch)%STRUCT(nn)%STRVAL
                     gb = gt
                   CASE DEFAULT
                     ...
                     &'SWR_CALC_STRVALS')
                  END SELECT
   !                 OPERATE GATE USING STRCRIT AND PHYSICAL STRUCTURE PARAMETERS
               ELSE
   !                   MAXIMUM GATE CHANGE OVER SWR TIMESTEP
                  dgo = strrt * SWRDT
   !                   SET VALUES FOR THE LEFT AND RIGHT SIDES OF THE EVALUATION
                  SELECT CASE ( istrlo )
                   CASE (1)
                     cl1 = cval
                     cr1 = strcrit
   ...
   !                       CLOSE GATE
                  ...
                  END IF
               END IF QUSETS
   !                 SET PREVIOUS TOP AND BOTTOM TO TOP AND BOTTOM FROM THE LAST SWR TIMESTEP
               REACH(irch)%STRUCT(nn)%STRTOP0 =&
               &REACH(irch)%STRUCT(nn)%STRTOP
               REACH(irch)%STRUCT(nn)%STRBOT0 =&
               &REACH(irch)%STRUCT(nn)%STRBOT
   !                 SET CURRENT TOP AND BOTTOM TO CALCULATED TOP AND BOTTOM
...
END SUBROUTINE SWR_CALC_STRVALS
   ...
   !       SET OPERABLE STRUCTURE DISCHARGE (PUMP) RATES AND GATE
   !       LEVELS FOR ALL OPERABLE STRUCTURES TO VALUE SAVED FOR SWR1
   !       TIMESTEP Nt
SUBROUTINE SWR_SET_STRVALS(Irch,Nt)
   ...
   !         SET CURRENT VALUE FOR STRUCTURE TO VALUE SAVED FOR TIME Nt
   LSTRUCTURE: DO nn = 1, REACH(Irch)%NSTRUCT
      istrtype = REACH(irch)%STRUCT(nn)%ISTRTYPE
   !           CHECK IF CURRENT STRUCTURE IS AN OPERABLE STRUCTURE
      IF ( istrtype.NE.3 .AND. istrtype.NE.8 .AND.&
   ...
   !
      ...
      SELECT CASE( istrtype )
       CASE (3)
         REACH(Irch)%STRUCT(nn)%STRTOP =&
         &REACH(Irch)%STRUCT(nn)%OPRVAL(Nt)
         REACH(Irch)%STRUCT(nn)%STRBOT =&
         &REACH(Irch)%STRUCT(nn)%OPRVAL(Nt)
       CASE (8)
         REACH(Irch)%STRUCT(nn)%STRTOP =&
...
DOUBLEPRECISION FUNCTION SSWR_CALC_SFRFLOW(Istrm) RESULT(value)
   ...
   !       + + + CODE + + +
   !
   !---------FILL DISCHARGE WITH OUTFLOW FOR THE SELECTED SFR STREAM REACH
...
END FUNCTION SSWR_CALC_SFRFLOW
   !
   !-------CALCULATE REACH TO REACH LATERAL FLOWS
   !       COMBINED UNCONTROLLED AND CONTROLLED FLOWS
DOUBLEPRECISION FUNCTION SSWR_RG_QM(Rg,Gs) RESULT(value)
   ...
   !---------DO NOT EVALUATE INACTIVE REACH GROUPS
   IF ( Rg%INACTIVE ) RETURN
   !---------MAKE SURE REACH GROUP IS CONNECTED TO ADJACENT REACH GROUPS
   IF ( Rg%NCONN.LT.1 ) RETURN
   !
   !---------EVALUATE QM FLOW FOR EACH REACH GROUP AS A RATE
   qpos = DZERO
   qneg = DZERO
   ...
   !
   !---------ADD ESTIMATE OF UNMANAGED AND MANAGED FLOW TO RCHGRP VALUE
   Rg%CURRENT%QMPOSFLOW = qpos
   Rg%CURRENT%QMNEGFLOW = qneg
   value = qpos + qneg
   !
...
&RESULT(value)
   ...
   value = DZERO
   !
   !---------EVALUATE QM FLOW FOR EACH REACH CONNECTION AS A RATE
   ...
   !---------LOOP THROUGH EACH CONNECTION IN REACH
   ...
   !           CALCULATE PARAMETERS NEEDED FOR BOTH METHODS
      irgi    = REACH(ia)%IRG
      irgj    = REACH(ja)%IRG
      irowi   = REACH(ia)%IRCH
      jcoli   = REACH(ia)%JRCH
   ...
   !             SEND REACH TO STRUCTURE CALL TO RETURN TOTAL FLOW FOR CONNECTION
         jstrrch = ja
         mult    = -DONE
   !             CHECK IF EVALUATING STRUCTURE FLOW IN REACH WITH STRUCTURE (ia)
   !             OR REACH ON THE DOWNSTREAM END OF STRUCTURE (ja)
         IF ( istrrch.NE.ia ) THEN
            jstrrch = ia
            mult    = DONE
         END IF
   !             CALCULATE FLOW FOR STRUCTURE
         q  = SSWR_CALC_SFLOW(REACH(istrrch),istrrch,jstrrch,Gs)
         q  = mult * q
   !           DIFFUSIVE OR KINEMATIC-WAVE APPROXIMATION
      ELSE
   !             NOT DIFFUSIVE- OR KINEMATIC-WAVE REACH - SKIP
         IF ( REACH(i)%CROUTETYPE.NE.'DW' .AND.&
         &REACH(i)%CROUTETYPE.NE.'KW' ) CYCLE EACHRCHCONN
   !             UNCONTROLLED FLOW ONLY FOR DIFFUSIVE-WAVE APPROXIMATION CONNECTION
         q = SSWR_CALC_UFLOW(i,j,irowi,jcoli,irowj,jcolj,&
         &irgi,irgj,si,sj,DZERO,DZERO,&
         &Gs)
      END IF
   !           POSITIVE FLOW - INFLOW TO REACH
      IF ( q.GT.DZERO ) THEN
         REACH(ia)%RPOSFLOW          = REACH(ia)%RPOSFLOW + q
         REACH(ia)%CURRENT%QMPOSFLOW =&
         &REACH(ia)%CURRENT%QMPOSFLOW + q
   !           NEGATIVE FLOW
      ELSE
         REACH(ia)%RNEGFLOW          = REACH(ia)%RNEGFLOW + q
         REACH(ia)%CURRENT%QMNEGFLOW =&
         &REACH(ia)%CURRENT%QMNEGFLOW + q
      END IF
   !           CALCULATE DEPTH AND CROSS-SECTIONAL AREA FOR CONNECTION
      CALL SSWR_CALC_DEPTHAREA(ia,ja,irowi,jcoli,irowj,jcolj,&
      &irgi,irgj,si,sj,depth,area)
   !           CALCULATE AVERAGE VELOCITY FOR CONNECTION
      velocity = DZERO
...
&RESULT(value)
   ...
   !     + + + CODE + + +
   value = DZERO
   !---------DO NOT EVALUATE CONSTANT OR INACTIVE REACHES
   ...
   EACHRCHCONN: DO ic = 1, REACH(Irch)%NCONN
      ...
      i     = imult * ia
      j     = imult * ja
   !           SKIP CONNECTION IF CONNECTED REACH IS NOT CONSTANT
...
&RESULT(value)
   ...
   value = DZERO
   qt    = DZERO
   !---------DO NOT EVALUATE QBC FOR INACTIVE REACH GROUPS
   IF ( Rg%INACTIVE ) RETURN
   !---------DO NOT EVALUATE QBC FOR REACH GROUPS WITHOUT STRUCTURES
   IF ( Rg%BCSTRUCT.LT.1 ) RETURN
   !---------CALCULATE QBC FOR REACH GROUP
   DO nr = 1, Rg%NRGREACH
      irch  = Rg%REACH(nr)
...
&RESULT(value)
   ...
   q      = -q
   !---------REACH DATA FOR SWR TIMESTEP
9999  REACH(irch)%CURRENT%QBCFLOW = q
   value  = q
   !---------RETURN
   RETURN
...
&RESULT(value)
   ...
   !---------DO NOT EVALUATE INACTIVE REACH GROUPS
   IF ( Rg%INACTIVE ) RETURN
   !---------CONSTANT HEAD FLOW IS THE SUM OF RAINFALL, EVAPORATION, QAQFLOW, AND OUTFLOW
   !           OPPOSITE SIGN OF TERMS BECAUSE CONSTANT HEAD PROVIDE FLOW TO COMPENSATE FOR
   !           DEFICIT IN FLOW TERMS.
   IF (Rg%CONSTANT) THEN
...
DOUBLEPRECISION FUNCTION SSWR_CALC_QCS(Irch) RESULT(value)
   ...
   !     + + + CODE + + +
   value = DZERO
   !---------DO NOT EVALUATE INACTIVE OR ACTIVE REACHES
   IF ( REACH(Irch)%ISWRBND.LT.0 ) THEN
   !-----------REACH DATA FOR SWR TIMESTEP
      ri    = REACH(Irch)%CURRENT%QLATFLOW +&
      &REACH(Irch)%CURRENT%QUZFLOW  +&
      &REACH(Irch)%CURRENT%QEXTFLOW
   ...
   END IF
   !---------SET CONSTANT REACH FLOW FOR REACH
...
END FUNCTION SSWR_BUDGET_QCS
   !
   !-------INFLOW IS SPECIFIED INFLOW, UPSTREAM INFLOW, DOWNSTREAM INFLOW,
   !       AND LATERAL INFLOW
DOUBLEPRECISION FUNCTION SSWR_RG_QINF(Rg) RESULT(value)
   ...
   DO nr = 1, Rg%NRGREACH
      irch   = Rg%REACH(nr)
   !           ONLY CALCULATE FOR ACTIVE REACHES
      ...
      value  = value +&
      &qpos + latinf + uzfinf + extinf + r + qaq + qneg
   END DO
   !---------RETURN
   RETURN
...
&Gs)  RESULT(value)
   ...
   dl    = DZERO
   gsm   = DZERO
   !---------DETERMINE IF EITHER CONNECTED REACH IS A
   !         KINEMATIC-WAVE REACH
   ikwconn   = IZERO
   IF ( I.GT.IZERO .AND. J.GT.IZERO ) THEN
      IF (REACH(I)%CROUTETYPE.EQ.'KW' .OR.&
      &REACH(J)%CROUTETYPE.EQ.'KW') THEN
         ikwconn = 1
      END IF
   END IF
   !---------
   ...
   !---------ADJUST psi AND psj IF EITHER CONNECTED REACH
   !         IS A KINEMATIC-WAVE REACH
   IF ( ikwconn.NE.IZERO ) THEN
      psi    = REACH(I)%GEO%ELEV(1)
      psj    = REACH(J)%GEO%ELEV(1)
   END IF
   !---------CALCULATE DRIVING GRADIENT
   h     = psj - psi
   dl    = dli + dlj
   sf    = ABS(h) / dl
   !
   ...
   !               IMPLICIT QM MAGNITUDE
      ...
      END IF
   END IF
   !         DO NOT PROCEED WITH QM FLOW CALCULATION IF WATER SURFACE GRADIENT
   !         IS LESS THAN USER SPECIFIED MINIMUM WATER SURFACE GRADIENT

   IF ( sfm.LT.DMINGRAD .OR. (sfm-NEARZERO).LT.DZERO ) RETURN
   !         UPSTREAM WEIGHTING OF UNCONTROLLED FLOW BETWEEN CONNECTED REACHES
   IF ( ISWRUPWT.NE.0 .AND. J.NE.IZERO ) THEN
      ...
      END IF
   END IF
   !         DO NOT PROCEED WITH QM FLOW CALCULATION IF WETTED PERIMETER
   !         IS LESS THAN OR EQUAL TO DZERO
   IF ( p.LE.DZERO ) RETURN
   !
   sfmi = DONE / SQRT( sfm )
   !         GAUCKLER-MANNING-STRICKLER COEFFICIENT
   gsm   = ( DONE / r )
   !         SCALE GAUCKLER-MANNING-STRICKLER COEFFICIENT
   bmin = b
...
CHARACTER (LEN=17) FUNCTION SSWR_BDCHAR(R) RESULT(value)
   ...
   t = ABS(R)
   IF(t.NE.0.0 .AND.&
   &(t.GE.9.99999E11 .OR. t.LT.0.1) ) THEN
...
END FUNCTION SSWR_CALC_DPTHFACT
   !
   ! + + + SOLVER SUBROUTINES AND FUNCTIONS + + +
   !
   !-------BACKTRACKING BASED ON THE L2NORM OF THE RESIDUAL
   !       MINIMIZATION USING BRENT'S METHOD
SUBROUTINE SSWRBTBM(I,Rn0,Rn,R,X0,X)
   USE GLOBAL,       ONLY: IOUT
   USE GWFSWRMODULE, ONLY: SMALL, IZERO,&
   &DZERO, DONE, DTWO, DONEPERCENT,&
   &NRCHGRP, NSOLRG,&
   &TOLR, IBT, IBTPRT
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
...
2000 FORMAT(1X,'OUTER ITER.: ',I5,1X,'BACKTRACK ITERS.: ',I5,&
   &1X,'INIT. L2NORM: ',G10.3,&
   &1X,'FIN. L2NORM: ',G10.3,&
   &1X,'FIN. ALPHA: ',G10.3,1X,'UPDATE PERFORMED')
   !     + + + CODE + + +
   ibtc     = IZERO
   ...
   px = pv
   pe = DZERO
   !----------INITIALIZE RESIDUALS CORRESPONDING TO POLYNOMIAL POINTS
   lx    = Rn0
   lv    = lx
   lw    = lx
   !---------MINIMIZE RESIDUAL
   ...
   !           CONSTRUCT A TRIAL PARABOLIC FIT
      IF ( ABS( pe ).GT.tol1 ) THEN
         ...
         ELSE
            ...
            pu = px + pd
            IF ( ( pu - pa ).LT.tol2 .OR.&
            &( pb - pu ).LT.tol2 ) pd = SIGN( tol1, ( pm - px ) )
         END IF
      ELSE
         pe = MERGE( ( pa - px ), ( pb - px ), px.GE.pm )
         pd = dgold * pe
      END IF
      pu = MERGE( ( px + pd ),&
      &( px + SIGN( tol1, pd ) ), ABS( pd ).GE.tol1 )
   !           EVALUATE RESIDUAL AT pd. PARABOLIC FIT OR GOLDEN SECTION USED
   !           TO CALCULATE pd.
      xt    = X0 + ( pu * dx )
...
END SUBROUTINE SHIFT4
   !
   !-------MAP GENERIC SOLUTION (NSOLRG) VECTOR TO GENERIC
   !       FULL COMPUTATION ELEMENT (NRCHGRP) VECTOR
...
END SUBROUTINE SSWRS2C
   !
   !-------MAP GENERIC FULL COMPUTATION ELEMENT (NRCHGRP) VECTOR
   !       TO GENERIC SOLUTION (NSOLRG) VECTOR
...
SUBROUTINE SSWREXTRD(Iswr,Iout,Iut,Iclose)
   !     ******************************************************************
   !     Determine if data should be processed from an internal, external,
   !     or temporary file.
   !     ******************************************************************
...
2005 FORMAT(1X,/1X,'OPENING FILE ON UNIT ',I4,':',/1X,A)
   ...
   !     ------------------------------------------------------------------
   !
   !---------Check for and decode EXTERNAL and OPEN/CLOSE records.
   IF ( Iclose.EQ.0 ) THEN
      Iut = Iswr
      Iclose = 0
      CALL URDCOM(Iut,IOUT,line)
...
2025 FORMAT(1X,'(THE SCALE FACTOR WAS APPLIED TO FIELDS',I2,&
   ...
   END IF
   !
   !3--------Setup indices for reading the list
   nread = Ldim - Naux
   !3A-------Advance to first
   CALL SSWR_RD_COMM(in)
   !
   ...
   DO ii = 1, Mxlist
   !
   !4A---------Read a line into the buffer.  (The first line has already been
   !4A---------read to scan for EXTERNAL and gfac records.)
      READ(in,'(A)') line
   !
   !4B---------Get the non-optional values from the line.
      lloc = 1
      DO jj = 1, nread
         CALL URWORD(line,lloc,istart,istop,3,i,Rlist(jj,ii),Iout,in)
      END DO
   !
   !4E-------Get the optional values from the line
      IF(NAUX.GT.0) THEN
         DO JJ = nread+1, Ldim
            CALL URWORD(line,lloc,istart,istop,3,i,Rlist(jj,ii),Iout,in)
         END DO
   ...
   END DO
   !
   !5--------Done reading the list.  If file is open/close, close it.
...
SUBROUTINE SSWR_ALLO_SOLN(L,Ira)
   ...
   &TSWRTIME,&
   &DZERO,&
   &INWTCORR, NREACHES, REACH,&
   &NRCHGRP, RCHGRP,&
   &NTMAX, NPMAX, RTIME, SWRTIME,&
   &RSTAGE, GSTAGE
   IMPLICIT NONE
   !     + + + DUMMY ARGUMENTS + + +
   INTEGER, INTENT(IN) :: L
   ...
   IF (Ira.NE.0) THEN
      ...
      &'REALLOCATING SOLUTION SPACE:', NTMAX, 'TO', L
      id = 1
      ALLOCATE( r(NREACHES) )
      ALLOCATE( rg(NRCHGRP) )
      ALLOCATE( s(L) )
   ...
   !
   !---------FILL WITH DATA IN TEMPORARY ARRAYS, IF NECESSARY
   IF ( id.NE.0 ) THEN
      SWRTIME(:) = s(:)
      IF ( INWTCORR.NE.0 ) THEN
         DO n = 1, NTMAX
...
SUBROUTINE SSWR_SET_SOLSUM(Is,Ds)
   !     ******************************************************************
   !     Allocate space for solution data sized using solution summary
   !       Callable subroutine to allow for expansion of data by
   !       adaptive time step algorithm
   !     ******************************************************************
   ...
   Is = IZERO
   Ds = DZERO
   !-------FILL SOLUTION STORAGE WITH NON-ZERO VALUES, IF REQUIRED
   IF ( id.GT.0 ) THEN
   !---------FILL SOLUTION STORAGE
      Is = it
      Ds = dt
   !---------DEALLOCATE TEMPORARY STORAGE
      DEALLOCATE(it,dt)
...
2000 FORMAT(//9X,'             SUBSIDENCE PACKAGE             ',&
   &/9X,'          VERTICAL REACH ADJUSTMENT         ',&
   &/1X,'     REACH',2X,'     INCR. SUB.',&
   &2X,'CUMULATIVE SUB.',2X,'   BOTTOM ELEV.',&
   &/1X,10('-'),2X,15('-'),2X,15('-'),2X,15('-'))
...
2020 FORMAT(1X,61('-'))
   ...
   !           CALCULATE CUMULATIVE SUBSIDENCE FOR THE REACH
      REACH(irch)%GCUMSUB = REACH(irch)%GCUMSUB - z
      IF ( ABS(z).GT.DZERO ) THEN
   !             ADJUST CURRENT STAGE FOR THE REACH
         REACH(irch)%STAGE = REACH(irch)%STAGE - z
   !             ADJUST GEOMETRY FOR THE REACH
         RCHGRP(irg)%IRGUPDATE = 1
   ...
   !             RESET STARTING AND ENDING LAYER
         CALL SSWR_SET_RCHLAY(irch)
   !             ADJUST STRUCTURE DATA FOR REACH
         DO ns = 1, REACH(irch)%NSTRUCT
            istrtype =  REACH(irch)%STRUCT(ns)%ISTRTYPE
   !               ADJUST STRUCTURE INVERT
            REACH(irch)%STRUCT(ns)%STRINV =&
            &REACH(irch)%STRUCT(ns)%STRINV - z
   !               ADJUST OTHER VERTICALLY TRANSLATABLE ELEVATIONS
            SELECT CASE ( istrtype )
             CASE (4)
               DO nt = 1, REACH(irch)%STRUCT(ns)%NSTRPTS
                  REACH(irch)%STRUCT(ns)%STRELEV(nt) =&
                  &REACH(irch)%STRUCT(ns)%STRELEV(nt) - z
               END DO
             CASE (5)
               REACH(irch)%STRUCT(ns)%STRINV2 =&
               &REACH(irch)%STRUCT(ns)%STRINV2 - z
             CASE (6,7,8,9,10,13)
               REACH(irch)%STRUCT(ns)%STRTOP =&
               &REACH(irch)%STRUCT(ns)%STRTOP - z
               REACH(irch)%STRUCT(ns)%STRBOT =&
               &REACH(irch)%STRUCT(ns)%STRBOT - z
            END SELECT
         END DO
      END IF
   !           PRINT VERTICAL ADJUSTMENT FOR THE REACH
      IF ( IPTFLG.GT.0 ) THEN
         IF ( ifirst.GT.0 ) THEN
            WRITE(IOUT,2000)
            ifirst = 0
         ...
         &REACH(irch)%GCUMSUB, REACH(irch)%GBELEV
      END IF
   END DO
   IF ( ifirst.EQ.0 ) WRITE(IOUT,2020)
   !
   !---------RESET CURRENT REACH GROUP STAGES
   DO irg = 1, NRCHGRP
      irch = RCHGRP(irg)%ICALCRCH
      RCHGRP(irg)%STAGE = REACH(irch)%STAGE
   END DO
   !
   !---------RECALCULATE STAGE-VOLUME RELATION FOR EACH REACH GROUP
   CALL SSWR_CALC_RATCURVE()
   !
   !---------RETURN
   RETURN
...
&RESULT(value)
   ...
   !         RESET dstage FOR CULVERTS TO DOWNSTREAM CULVERT
   !         INVERT TO ENSURE INLET CONTROL IF CULVERT SPECIFIED
   !         IN A REACH WITHOUT A DOWNSTREAM CONNECTION
   IF ( Str%ISTRTYPE.EQ.5 ) THEN
      dstage = Str%STRINV2
   END IF
   !         SET dstage USING DOWNSTREAM STAGE WHERE APPROPRIATE
   IF ( Idsrch.GT.0 ) THEN
      dstage = P(idsrchrg) + REACH(Idsrch)%OFFSET
   END IF
   !         CALCULATE STRUCTURE FLOW FOR EACH STRUCTURE TYPE
   SELECT CASE ( istrtype )
   !           NO STRUCTURE
    CASE (0)
      RETURN
   !           SPECIFIED STAGE - LIMITED TO EXCESS VOLUME
    CASE (1)
   ...
   !                   STAGE-DEPENDENT FLUX BOUNDARY
         ...
         END IF
      END IF
   !               RETURNED VALUE IS RELATIVE TO REACH
      q = SSWR_CALC_UFLOW(Irch,idsrch,&
      &irowi,jcoli,irowj,jcolj,&
      &-irchrg,0,stage,dstage,strlen,strlen2,&
      &P)
   !               NOTE: SIGN CONVENTION OF q FROM SSWR_CALC_UFLOW IS OPPOSITE
   !                     FROM OTHER STRUCTURE FLOWS - +VE = OUTFLOW, -VE = INFLOW
      q  = -q
   !             SPECIFIED DISCHARGE - LIMITED TO AVAILABLE VOLUME
    CASE (3)
      q = strtop
   !               SPECIFIED H-Q RELATION
    CASE (4)
   ...
   !               APPLY DIRECTIONAL CORRECTION
      ...
      q = q * fact
   !               FIXED CREST WEIR FORMULA AND
   !                 OVERBANK STRUCTURE BASED ON FIXED CREST WEIR
    CASE (6,13)
      strtop = MAX( stage, dstage ) + 100.0D0
      strinv = strinv + strval
   !             CALCULATE WIDTH SMOOTHING
   ...
   !                 LINEAR
      ...
      &strtop,strinv,strwid,&
      &strcd,strcd2,strcd3)
   !                 APPLY DIRECTIONAL CORRECTION
      fact = SWR_STR_DIRCORR(istrdir,q)
      q = q * fact
   !             SPECIFIED GATE LEVEL GATED SPILLWAY
    CASE (7)
      strtop = strinv + strval
      q = SSWR_CALC_OSFLOW(stage,dstage,&
      &strtop,strinv,&
      &strwid,strcd,strcd2,strcd3)
   !                 APPLY DIRECTIONAL CORRECTION
      fact = SWR_STR_DIRCORR(istrdir,q)
      q = q * fact
   !               MOVABLE CREST WEIR FORMULA
    CASE (8)
      q = SSWR_CALC_OSFLOW(stage,dstage,&
      &strtop,strbot,&
      &strwid,strcd,strcd2,strcd3)
   ...
   !             GENERALIZED SPILLWAY EQUATION
    CASE (10)
      ...
      &strtop,strbot,strwid,&
      &strcd,strcd2,strcd3)
   !                 APPLY DIRECTIONAL CORRECTION
      fact = SWR_STR_DIRCORR(istrdir,q)
      q = q * fact
   !             SFR INFLOW STRUCTURE
    CASE (11)
...
END SUBROUTINE SSWR_CALC_DEPTHAREA
   ...
   !-------READ SWR STAGE DATA INTO MEMORY
SUBROUTINE SSWR_RDSTGRES(Iu,Totim,TABDATA)
   ...
   !     + + + FUNCTIONS + + +
   !     + + + OUTPUT FORMATS + + +
2000  FORMAT(//1X,'NUMBER OF REACHES IN SWR1 STAGE FILE (',I10,')',&
   &1X,'INCONSISTENT WITH NUMBER OF SWR1 REACHES (',I10,')',&
   &/1X,'SPECIFIED IN SWR1 DATASET 1')
2010  FORMAT(//1X,'NO DATA READ FROM SPECIFIED SWR1 STAGE FILE',&
   &1X,'(UNIT',I10,')')
   !     + + + CODE + + +
   i  = 0
   iut = ABS( Iu )
   ...
   END DO
   !
   !---------MAKE SURE SOME DATA HAS BEEN READ
   IF ( i.LT.1 ) THEN
      WRITE (IOUT,2010) iut
      CALL USTOP('NO DATA READ FROM SPECIFIED SWR1 STAGE FILE')
   END IF
   !
   !---------REWIND THE FILE, ALLOCATE MEMORY, AND READ DATA
   REWIND (iut)
   IF ( Iu.GT.0 ) THEN
      READ (iut,'(A200)') line
   ELSE
...
SUBROUTINE SSWR_RDTABDATA(Iu,Totim,TSDATA)
   ...
   iut     = ABS( Iu )
   !---------REWIND THE FILE
   !         STRUCTURE CRITERIA (STRCRIT) OR STRUCTURE
   !         VALUE (STRVAL) MAY BE READ AGAIN IF NEW STRUCTURE
   !         SPECIFIED AND USING A TIME SERIES FILE USED PREVIOUSLY
   REWIND(iut)
   !---------LOOK FOR KEYWORDS IN ASCII FILES
   IF ( Iu.GT.0 ) THEN
      CALL SSWR_RD_COMM(iut)
      lloc = 1
      CALL URDCOM(iut,IOUT,line)
      ...
      END DO
   END IF
   !---------BACKSPACE FILE IF KEYWORD ARGUMENTS ARE NOT PRESENT
...
SUBROUTINE SSWR_SETSFR_STRM(Str)
   ...
   FINDSTRM: DO n = 1, NSTRM
      ...
      END IF
   END DO FINDSTRM
   !---------RESET ISFRRCH IF isfrseg AND isfrrch ARE NOT FOUND
...
2120 FORMAT(/1X,2X,'PERCENT DISCREPANCY =',1X,1PG15.6)
   ...
   !-----------PRINT SWRDT SUMMARY
      ...
      DO n = 1, NUMTIME, 5
         ...
         &SWRTIME(i)%SWRDT,i=n,MIN((n+4),NUMTIME))
      END DO
      WRITE (IOUT,2050)
   !-----------PRINT SUMMARY OF BASEFLOW DIFFERENCES
      IF ( TOLA.GT.DZERO ) THEN
         CALL SSWR_CALC_BFCNV()
         swrin     = DZERO
         swrinsum  = DZERO
   ...
   ELSE IF ( MUTSWR.EQ.1 .AND. iprncnvg.EQ.0 ) THEN
      ...
      WRITE (IOUT,2080)
   ELSE
   !-----------PRINT SUMMARY OF MAXIMUM ERROR IF TOLR EXCEEDED DURING THIS
   !           MODFLOW TIMESTEP
      IF ( ISWRCNVG.LT.1 ) THEN
         ...
         &nfail,Kkper,Kkstp,&
         &SWRTIME(ipos)%FMAX,SWRTIME(ipos)%ILOCFMAX,&
         &ABS(SWRTIME(ipos)%ICNVG)
      END IF
   END IF
   !
   !---------RETURN
...
END SUBROUTINE SSWRTIMER
   !
   !-------SUBROUTINE TO OUTPUT INFORMATION ABOUT THE CURRENT
   !       SWR1 PROCESS DATASET BEING READ
SUBROUTINE SSWRDSOUT(C)
   ...
   !     + + + FUNCTIONS + + +
   !     + + + OUTPUT FORMAT + + +
2000  FORMAT(/1X,'...READING SWR1 PROCESS DATASET',1X,A)
   !     + + + CODE + + +
   WRITE (IOUT,2000) C
   !---------RETURN
   RETURN
...
SUBROUTINE SSWRNWT(n,kl,irch,h,hc)
   ...
   !     + + + CODE + + +
   !
   !---------CALCULATE NEWTON CORRECTIONS TO HCOF AND RHS
   hepsinv = DONE / SWRHEPS
   dtscale = SWRTIME(n)%SWRDT / REAL( DELT, 8 )
   !---------CALCULATE DERIVATIVE AND INSERT IN THE NEWTON A MATRIX
   ...
   !---------ACCUMULATE THE NEWTON CORRECTION TERMS THAT WILL BE APPLIED TO
   !         HCOF IF A MODFLOW-2005 MODEL OR A MATRIX IF A MODFLOW-NWT MODEL.
   hc = hc + dcond * h - drhs
   !
   !---------RETURN
9999 RETURN
...
DOUBLEPRECISION FUNCTION SSWR_KELLEY_PERTB(V) RESULT(value)
   ...
   !        sgn   = DONE
   !        IF ( ABS(V).GT.DZERO ) sgn = V / ABS( V )
   !        value = MAX( e * ABS( V ) , e ) * sgn
   value = MAX( e * av , e )
   !---------RETURN
9999 RETURN
...
&RESULT(value)
   ...
   !       + + + OUTPUT FORMATS + + +
   !       + + + CODE + + +
   !         CALCULATE STAGE DIFFERENCE FOR FACE
   ds      = (Gsj - Gsi)
   sf      = ds / Dl
   sfmag   = ABS( sf )
   value   = sfmag
   !         RETURN IF NO STAGE DIFFERENCE
   IF ( ABS(ds).LT.NEARZERO ) GOTO 9999
   !         DETERMINE IF CONNECTION IS ALONG A ROW OR A COLUMN
   irow    = REACH(I)%IRCH
   icol    = REACH(I)%JRCH
   jrow    = REACH(J)%IRCH
   jcol    = REACH(J)%JRCH
   !         I-J CONNECTION IN Y-DIRECTION - ALONG A COLUMN
   IF ( irow.NE.jrow ) THEN
      ixcon = 0
   !         I-J CONNECTION IN X-DIRECTION - ALONG A ROW
   ELSE
      ixcon = 1
   END IF
   !         DETERMINE REACH TO USE TO DETERMINE THE GRADIENT
   !         FOR FACE(IJ)
   IF ( sf.LT.DZERO ) THEN
      ieval  = I
      peval  = Gsi
   ...
   !         DETERMINE WHICH TWO CELLS TO LOOK IN TO DETERMINE THE
   !         GRADIENT FOR FACE(IJ)
   ...
   jcolm1  = irchcol
   jcolp1  = irchcol
   !         I-J CONNECTION IN X-DIRECTION - EVALUATE ADJACENT ROWS
   IF ( ixcon.NE.0 ) THEN
      jrowm1 = jrowm1 - 1
      jrowp1 = jrowp1 + 1
   !         I-J CONNECTION IN Y-DIRECTION - EVALUATE ADJACENT COLUMNS
   ELSE
      jcolm1 = jcolm1 - 1
      jcolp1 = jcolp1 + 1
   END IF
   jrchm1  = 0
   jrchp1  = 0
   !         DETERMINE THE REACH NUMBERS FOR THE TWO CELLS USED TO
   !         DETERMINE THE GRADIENT FOR FACE(IJ)
   ...
   END IF
   !         DETERMINE IF THE TWO CELLS USED TO DETERMINE THE
   !         GRADIENT FOR FACE(IJ) ARE STRUCTURE CONTROLLED.
   !         ONE-DIMENSIONAL (NOT MODEL-BASED) REACHES HAVE
   !         jrchm1 AND/OR jrchp1 VALUES EQUAL TO IZERO
   DO jc = 1, REACH(ieval)%NCONN
      ...
      END IF
   END DO
   !         CALCULATE THE GRADIENT ACROSS THE TWO CELLS USED TO DETERMINE THE
   !         GRADIENT FOR FACE(IJ)
   d       = DZERO
   sfmagm1 = DZERO
   sfmagp1 = DZERO
...
&RESULT(value)
   ...
   !         RETURN IF NO STAGE DIFFERENCE
   IF ( ABS(ds).LT.NEARZERO ) GOTO 9999
   !         RETURN IF ONE-DIMENSIONAL PROBLEM (NROW=1 OR NCOL=1)
   IF (NROW.EQ.1) RETURN
   IF (NCOL.EQ.1) RETURN
   !         DETERMINE IF CONNECTION IS ALONG A ROW OR A COLUMN
   irow    = REACH(I)%IRCH
   ...
   !         I-J CONNECTION IN Y-DIRECTION - ALONG A COLUMN
   IF ( irow.NE.jrow ) THEN
   !           LOWER FACE CONNECTION
      IF ( irow.LT.jrow ) THEN
         irch43 = I
         irch12 = J
         iirow1 = irow
         iirow2 = jrow
   !           UPPER FACE CONNECTION
      ELSE
         irch43 = J
         irch12 = I
         iirow1 = jrow
...
DOUBLEPRECISION FUNCTION SSWR_GET_MAGQM(I,J,Sf) RESULT(value)
   ...
   !       + + + CODE + + +
   !
   !---------GET CALCULATED STAGE GRADIENT MAGNITUDE
   value = Sf
...
DOUBLEPRECISION FUNCTION SSWR_CALC_FTOLR(R) RESULT(value)
   ...
   !       + + + CODE + + +
   !
   !---------CALCULATE THE MAXIMUM FRACTIONAL RESIDUAL
   value = DZERO
   DO irg = 1, NRCHGRP
...
SUBROUTINE SSWR_CALC_BFCNV()
   !     *****************************************************************
   !     CALCULATE SWR AND MODFLOW QAQFLOW RATES FOR EACH REACH
   !     *****************************************************************
   USE GWFBASMODULE, ONLY: DELT
   USE GWFSWRMODULE
   IMPLICIT NONE
      ...
      DO n = 1, NUMTIME
         ...
         DO k = REACH(irch)%LAYSTR, REACH(irch)%LAYEND
            IF ( REACH(irch)%CURRENTQAQ(k)%QAQFLOW.LT.DZERO ) THEN
               nswrbf = nswrbf -&
               &REACH(irch)%CURRENTQAQ(k)%QAQFLOW * dtscale
            ELSE
               pswrbf = pswrbf +&
               &REACH(irch)%CURRENTQAQ(k)%QAQFLOW * dtscale
            END IF
         END DO
      END DO
   !         DETERMINE MODFLOW REACH-AQUIFER EXCHANGE
...
SUBROUTINE SSWR_GET_STAGE(Irch,T0,T1,Dstage)
   !     *****************************************************************
   !     CALCULATE TIME-WEIGHTED SWR STAGE IN A SPECIFIED REACH
   !     *****************************************************************
   USE GWFBASMODULE, ONLY: DELT
   ...
   dsum = DZERO
   denom = DZERO
   !---------DETERMINE INDICES OF T0 AND T1
   rt = REAL( T0 )
   i0 = SSWR_TIMEINDEX(rt)
   rt = REAL( T1 )
   i1 = SSWR_TIMEINDEX(rt)
   !---------CALCULATE TIME WEIGHTED SWR STAGE BETWEEN T0 AND T1
   TIMES: DO n = i0, i1
...
INTEGER FUNCTION SSWR_TIMEINDEX(Rtim) RESULT(ivalue)
   !     *****************************************************************
   !     DETERMINE THE INDEX FOR TIME Rtim (SWRTOT) BY BISECTION
   !     *****************************************************************
