...
SUBROUTINE PCG7AR(IN,MXITER,IGRID)
   !     ******************************************************************
   !     ALLOCATE STORAGE FOR PCG ARRAYS AND READ PCG DATA
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   &HCLOSEPCG,RCLOSEPCG,RELAXPCG,DAMPPCG,VPCG,SS,P,&
   &HPCG,CD,HCSV,LHCH,HCHG,LRCHPCG,RCHG,IT1,&
   &DAMPPCGT,&
   &IHCOFADD  !JDH 20110814
   !
   CHARACTER*200 LINE
   INTEGER IN,MXITER
   !     ------------------------------------------------------------------
   ...
   ALLOCATE(IHCOFADD)
   !
   !-------PRINT A MESSAGE IDENTIFYING PCG PACKAGE
   WRITE (IOUT,500)
500 FORMAT (1X,/1X,'PCG -- CONJUGATE-GRADIENT SOLUTION PACKAGE',&
   &', VERSION 7, 5/2/2005')
   !
   !-------READ AND PRINT COMMENTS, MXITER,ITER1 AND NPCOND
...
510 FORMAT(' IHCOFADD option -- When an active cell is surrounded ',&
   ...
   &'flow are 0.')
   !
   !-------ALLOCATE SPACE FOR THE PCG ARRAYS
   ...
   ALLOCATE (IT1(ITMEM))
   !
   !-------READ HCLOSEPCG,RCLOSEPCG,RELAXPCG,NBPOL,IPRPCG,MUTPCG
   !rgn changed read to check for different value for transient dampening
   READ (IN,*) HCLOSEPCG,RCLOSEPCG,RELAXPCG,&
   &NBPOL,IPRPCG,MUTPCG,DAMPPCG
   IF ( DAMPPCG.LT.0.0 ) THEN
      BACKSPACE IN
      READ (IN,*) HCLOSEPCG,RCLOSEPCG,RELAXPCG,&
      &NBPOL,IPRPCG,MUTPCG,DAMPPCG,DAMPPCGT
      DAMPPCG = -DAMPPCG
   ...
   END IF
   !
   !-------PRINT MXITER,ITER1,NPCOND,HCLOSEPCG,RCLOSEPCG,RELAXPCG,NBPOL,IPRPCG,
   !-------MUTPCG,DAMPPCG
   WRITE (IOUT,511)
...
&ISS,HDRY,IHCOFADD)
   !
   !     01JULY1990 COMMENT STATEMENTS ADDED AND MODIFIED
   !     01SEPT1990 IPCGCD OMITTED; STATEMENT 590 ADDED
   !     27SEPT1990 STATEMENT IN DO 155 LOOP CHANGED
   !     01SEPT1991 ADDED STATEMENTS RELATED TO SENSITIVITY CALCULATIONS
   !                AT THE END OF THE 115 LOOP.  CHANGED THE 510 FORMAT
   !                STATEMENT AND THE PRECEDING IF STATEMENT
   !     20MAR1992  CHANGED 510 FORMAT STATEMENT; OMITTED 2 LINES IN DO 160
   !                LOOP
   !     01MAY1993  ADDED DEL TO CALCULATION OF THE CHOLESKY DIAGONAL
   !     15JUNE1993 MADE CELLS SURROUNDED BY DRY CELLS INACTIVE
   !     01JUNE1995 ADDED DAMP
   !     29DEC1998  ADDED RES AND HCSV TO AVOID DESTROYING RHS AND HCOF.
   !                REMOVED STATEMENTS THAT CALCULATE ACCURACY OF SOLUTION
   !                FOR SENSITIVITY CALCULATIONS.  BUFF CAN BE USED AS THE
   !                ACTUAL ARGUMENT FOR RES WHEN CALLING PCGAP.
   !     4APRIL2008 ADDED SEPARATE DAMPING FOR TRANSIENT AND STEADY STATE
   !     14AUG2011  ADDED IHCOFADD TO ALLOW ADDITION OF HCOF TO SUM OF
   !                CONDUCTANCE TERM EVALUATED PRIOR TO EVALUATION FOR
   !                SETTING DRY CELLS.  ALSO ADDED CHECK FOR INITIAL
   !                HNEW AND CURRENT INNER ITERATION BEING AN EXACT
   !                SOLUTION (BIGR = ZERO).
   !
   !     ******************************************************************
   ...
   DDAMP=DAMP
   !-------ASSIGN VARIABLE EQUAL TO THE NUMBER OF CELLS IN ONE LAYER
   NRC = NROW*NCOL
   !-------INITIALIZE VARIABLES USED TO CALCULATE ITERATION PARAMETERS
   SRNEW = DZERO
   BPOLY = 0.
...
8     CONTINUE
   END IF
   !-------INITIALIZE VARIABLE USED TO TEST FOR NEGATIVE CHOLESKY DIAGONAL
   CD1 = 0.
   !------CLEAR PCG WORK ARRAYS.
   DO 10 N = 1, NODES
      SS(N) = 0.
      P(N) = 0.
      V(N) = 0.
10 CONTINUE
   !------STORE PREVIOUS HEAD IF MXITER>1
   IF(MXITER.GT.1) THEN
      DO 15 N=1,NODES
         HPCG(N)=HNEW(N)
15    CONTINUE
   END IF
   !------FOR NPCOND=1, INITIALIZE CHOLESKY DIAGONAL
   IF (NPCOND.EQ.1) THEN
      DO 20 N = 1, NODES
         CD(N) = 0.
20    CONTINUE
   ENDIF
   !
   !------CALCULATE THE RESIDUAL. IF NORM=1, CALCULATE THE DIAGONALS OF
   !------THE A MATRIX,AND STORE THEM IN HCOF.
   DO 50 K = 1, NLAY
      DO 40 I = 1, NROW
         DO 30 J = 1, NCOL
   !
   ...
   !-------SURROUNDING CELLS
            ...
            NLL = N - NRC
   !
   !-------CALCULATE 1 DIMENSIONAL SUBSCRIPTS FOR CONDUCTANCE TO THE 6
   !-------SURROUNDING CELLS.
            NCF = N
            NCD = N - 1
            NRB = N - NCOL
   ...
   !
            ...
            IF (J.EQ.NCOL) CR(N) = 0.
   !
   !-------JDH 20110814 - CONSIDER STORAGE CHANGES AND HEAD-DEPENDENT BOUNDARIES (HCOF)
   !       WHEN TESTING FOR DRY CELLS IF IHCOFADD IS GREATER THAN ZERO
            DIA = DZERO
            IF ( IHCOFADD.NE.0 ) DIA = HCOF(N)
   !-------15JUN1993 SKIP CALCULATIONS AND MAKE CELL INACTIVE IF ALL
...
27             FORMAT (/,&
            ...
            ENDIF
   !
   !-------CALCULATE THE RESIDUAL AND STORE IT IN RES.  TO SCALE A,
   !-------CALCULATE THE DIAGONAL OF THE A MATRIX, AND STORE IT IN HCOF.
            E = -Z - B - D - F - H - S
            RRHS = RHS(N)
            HHCOF = HNEW(N)*HCOF(N)
...
50 CONTINUE
   !
   !-------JDH 20110814 - CHECK FOR CASE WHERE INITIAL HNEW IS AN EXACT SOLUTION - JDH 20110814
   ...
   END IF
   !
   !-------SCALE CC,CR,CV,RES AND HNEW IF NORM=1.
   IF (NORM.EQ.1) THEN
      DO 80 K = 1, NLAY
         DO 70 I = 1, NROW
            DO 60 J = 1, NCOL
...
80    CONTINUE
   ENDIF
   !
   !-------CALCULATE PARAMETER B OF THE POLYNOMIAL PRECONDITIONING METHOD
   IF (NPCOND.EQ.2) THEN
      IF (NBPOL.EQ.2) THEN
         BPOLY = 2
         GOTO 120
   ...
   !
   !-------NOTE : ABS. VAL. OF THE DIAG. OF THE SCALED A MATRIX IS 1.
               HHCOF = HCOF(N)
...
110   CONTINUE
   !
   !-------CALCULATE ITERATION PARAMETERS FOR POLYNOMIAL PRECONDITIONING
   !-------METHOD FOR A NEGATIVE DEFINITE MATRIX.
...
130 IITER = IITER + 1
   NITER = NITER + 1
   !
   !-------INITIALIZE VARIABLES THAT TRACK MAXIMUM HEAD CHANGE AND RESIDUAL
   !-------VALUE DURING EACH ITERATIONS
   BIGH = 0.
   BIGR = 0.
   !-------INITIALIZE DEL (ADDED 01MAY1993)
   DEL = 0.
   !
   !
   !-------CHECK NPCOND FOR PRECONDITIONING TYPE AND EXECUTE PROPER CODE
   IF (NPCOND.EQ.2) THEN
   !
...
150   DO 180 K = 1, NLAY
   ...
   !
   !-------CALCULATE CD - FIRST INTERNAL ITERATION ONLY
               IF (IITER.EQ.1) THEN
                  CDCR = DZERO
                  CDCC = DZERO
                  CDCV = DZERO
                  ...
                  IF (NPCOND.EQ.1) THEN
                     IF (IR.GT.0) THEN
                        FV = CV(IR)
   !                 MODIFIED FROM HILL(1990) 9/27/90: 2 REPLACES 1
   ...
   !--------.LT. CHANGED TO .LE. 01SEPT1991
                  IF (CD(N)*CD1.LE.0.) THEN
   !--------CHANGED 500 FORMAT 01SEPT1991 AND 20MAR1992
   !--------CHANGED 500 FORMAT AND WRITE STATEMENT AND ADDED DEL 01MAY1993
                     DEL = 1.5D0*DEL + .001D0
                     IF (DEL.GT..5D0) THEN
                        WRITE (IOUT,500)
                        IERR = 1
...
180   CONTINUE
   ...
   !-------GRADIENT ALGORITHM BY BACK SUBSTITUTION. STORE RESULT IN SS.
      DO 210 KK = NLAY, 1, -1
         DO 200 II = NROW, 1, -1
            DO 190 JJ = NCOL, 1, -1
   !
...
190         CONTINUE
200      CONTINUE
   !-------SKIP OVER OTHER PRECONDITIONING TYPES
210   CONTINUE
   ENDIF
   !
...
240   CONTINUE
   ENDIF
   !
   !-------CALCULATE ALPHA OF THE CONJUGATE GRADIENT ROUTINE.
   ...
   !
   !-------CALCULATE THE PRODUCT OF MATRIX A AND VECTOR P AND STORE
   !-------RESULT IN V.
            PN = HCOF(N)*P(N)
            IF (NORM.EQ.1) PN = -P(N)
...
300 CONTINUE
   !
   !-------UNSCALE LARGEST CHANGE IN HEAD AND LARGEST RESIDUAL, AND
   ...
   !
   !-------CHECK IF CURRENT ITERATION (HNEW) IS AN EXACT SOLUTION - JDH 20110814
   IF ( IHCOFADD.NE.0 .AND. BIGR.EQ.0. ) THEN
      WRITE (IOUT,*)&
   ...
   END IF
   !
   !-------STORE THE LARGEST UNSCALED HEAD CHANGE AND RESIDUAL VALUE
   !-------(THIS ITERATION) AND THEIR LOCATIONS.
   II = NITER
   HCHG(II) = BIGH
   LHCH(1,II) = KH
   LHCH(2,II) = IH
   ...
   !-------GO TO NEXT INTERNAL ITERATION IF CONVERGENCE HAS NOT BEEN
   !-------REACHED AND IITER IS LESS THAN ITER1
   IF (MXITER.EQ.1) THEN
...
310   CONTINUE
   ENDIF
   !
   !-------AT END OF EXTERNAL ITERATION, APPLY DAMP
   IF(MXITER.GT.1) THEN
      DO 320 N=1,NODES
...
320   CONTINUE
   END IF
   !
   !-------IF END OF TIME STEP, PRINT # OF ITERATIONS THIS STEP
...
515      FORMAT (I6,' CALLS TO PCG ROUTINE FOR TIME STEP',I6,& !gsf
         ...
         IF (MUTPCG.LE.0) THEN
   !
   !-------PRINT HEAD CHANGE EACH ITERATION IF PRINTOUT INTERVAL IS REACHED
   ...
   ENDIF
   !
   !-------RESTORE HCOF IF NEEDED AND RETURN
   IF(NORM.EQ.1) THEN
      DO 600 N=1,NODES
         HCOF(N)=HCSV(N)
600   CONTINUE
...
&NPCOND,BPOLY,IT1,MUTPCG,NCOL,NROW)
   !     ******************************************************************
   !     PRINT MAXIMUM HEAD CHANGE AND RESIDUAL VALUE FOR EACH ITERATION
   !                           DURING A TIME STEP
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
...
35    FORMAT (1X,/1X,'MAXIMUM HEAD CHANGE FOR LAST ITER1 ITERATIONS',&
      &/1X,'(1 INDICATES THE FIRST INNER ITERATION):',//,&
      &1X,5('   HEAD CHANGE '),/&
      &1X,5('  LAYER,ROW,COL')/1X,75('-'))
...
&NROW,NLAY,NODES)
   !     ******************************************************************
   !           MATRIX MULTIPLICATIONS FOR POLYNOMIAL PRECONDITIONING
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !
            ...
            Z = DZERO
            ZV = DZERO
   !      IF STATEMENT REARRANGED 01JUN1993
            IF (K.NE.1) THEN
            ...
            ENDIF
   !
   !-------CALCULATE THE PRODUCT OF MATRIX A AND VECTOR VIN AND STORE
   !------ RESULT IN VOUT
            VN = HCOF(N)*VIN(N)
...
SUBROUTINE PCG7PNT(IGRID)
   !  Set pointers to PCG data for a grid
   USE PCGMODULE
   !
