...
SUBROUTINE SIP7AR(IN,MXITER,IGRID)
   !     ******************************************************************
   !     ALLOCATE STORAGE FOR SIP ARRAYS AND READ SIP DATA
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
   ...
   !
   !1------PRINT A MESSAGE IDENTIFYING SIP PACKAGE
   WRITE(IOUT,1)IN
1  FORMAT(1X,&
   &/1X,'SIP -- STRONGLY-IMPLICIT PROCEDURE SOLUTION PACKAGE',&
   &/20X,'VERSION 7, 5/2/2005',' INPUT READ FROM UNIT ',I4)
   !
   !2------READ AND PRINT COMMENTS, MXITER, AND NPARM
...
3  FORMAT(1X,'MAXIMUM OF',I4,' ITERATIONS ALLOWED FOR CLOSURE'/&
   &1X,I2,' ITERATION PARAMETERS')
   !
   !3------ALLOCATE SPACE FOR THE SIP ARRAYS
...
130 FORMAT(1X,5X,'SIP HEAD CHANGE PRINTOUT INTERVAL =',I9)
   !
   !6------CHECK IF SPECIFIED VALUE OF WSEED SHOULD BE USED OR IF
   !6------SEED SHOULD BE CALCULATED
   IF(IPCALC.NE.0) THEN
   !
   !6A-----CALCULATE SEED & ITERATION PARAMETERS PRIOR TO 1ST ITERATION
      WRITE(IOUT,140)
140   FORMAT(1X,/5X,'CALCULATE ITERATION PARAMETERS FROM MODEL',&
      &' CALCULATED WSEED')
   ELSE
   !
   !6B-----USE SPECIFIED VALUE OF WSEED
   !6B-----CALCULATE AND PRINT ITERATION PARAMETERS
150   ONE=1.
      P1=-ONE
      P2=NPARM-1
      DO 160 I=1,NPARM
...
161   FORMAT(1X,/1X,I5,' ITERATION PARAMETERS CALCULATED FROM',&
      &' SPECIFIED WSEED =',F11.8,' :'//(1X,5E13.6))
   END IF
   !
   !7------RETURN
   CALL SIP7PSV(IGRID)
...
&W,HDCG,LRCH,NPARM,KITER,HCLOSE,ACCL,ICNVG,KSTP,KPER,&
&IPCALC,IPRSIP,MXITER,NSTP,NCOL,NROW,NLAY,NODES,IOUT,MUTSIP,&
&IERR)
   !     ******************************************************************
   ...
   !1------CLEAR THE FLAG SO THAT CALCULATION IS DONE ONLY ONCE.
   IF(IPCALC.NE.0)&
   &CALL SSIP7I(CR,CC,CV,IBOUND,NPARM,W,NCOL,NROW,NLAY,IOUT)
   IPCALC=0
   !
   ...
   DITPAR=W(NTH)
   !
   !3------INITIALIZE VARIABLE THAT TRACKS MAXIMUM HEAD CHANGE DURING
   !3------THE ITERATION
   BIGG=ZERO
   BIG=ZERO
   IB=0
...
100 V(I)=ZERO
   !
   !5------SET NORMAL/REVERSE EQUATION ORDERING FLAG (1 OR -1) AND
   !5------CALCULATE INDEXES DEPENDENT ON ORDERING
   IDIR=1
   IF(MOD(KITER,2).EQ.0)IDIR=-1
   IDNRC=IDIR*NRC
   IDNCOL=IDIR*NCOL
   !
   !6------STEP THROUGH CELLS CALCULATING INTERMEDIATE VECTOR V
   !6------USING FORWARD SUBSTITUTION
   DO 150 K=1,NLAY
      DO 150 I=1,NROW
         DO 150 J=1,NCOL
   !
   !6A-----SET UP CURRENT CELL LOCATION INDEXES.  THESE ARE DEPENDENT
   !6A-----ON THE DIRECTION OF EQUATION ORDERING.
...
120         II=NROW-I+1
            ...
            KK=NLAY-K+1
   !
   !6B-----CALCULATE 1 DIMENSIONAL SUBSCRIPT OF CURRENT CELL AND
   !6B-----SKIP CALCULATIONS IF CELL IS NOFLOW OR CONSTANT HEAD
122         N=JJ+(II-1)*NCOL+(KK-1)*NRC
            IF(IBOUND(N).LE.0)GO TO 150
   !
   !6C-----CALCULATE 1 DIMENSIONAL SUBSCRIPTS FOR LOCATING THE 6
   !6C-----SURROUNDING CELLS
            NRN=N+IDNCOL
            NRL=N-IDNCOL
            NCN=N+1
            NCL=N-1
            ...
            NLL=N-IDNRC
   !
   !6D-----CALCULATE 1 DIMENSIONAL SUBSCRIPTS FOR CONDUCTANCE TO THE 6
   !6D-----SURROUNDING CELLS.  THESE DEPEND ON ORDERING OF EQUATIONS.
...
124         NCF=N
            ...
            NLZ=N
   !
   !6E-----ASSIGN VARIABLES IN MATRICES A & U INVOLVING ADJACENT CELLS
   !6E1----NEIGHBOR IS 1 ROW BACK
126         B=DZERO
            ELNRL=DZERO
            FLNRL=DZERO
            GLNRL=DZERO
...
130         D=DZERO
            ...
            VNCL=V(NCL)
   !
   !6E4----NEIGHBOR IS 1 COLUMN AHEAD
132         F=DZERO
            FHNEW=DZERO
            ...
            FHNEW=F*HNEW(NCN)
   !
   !6E5----NEIGHBOR IS 1 LAYER BEHIND
134         Z=DZERO
            ELNLL=DZERO
            FLNLL=DZERO
            GLNLL=DZERO
...
136         S=DZERO
            ...
            SHNEW=S*HNEW(NLN)
   !
   !6E7----CALCULATE THE NEGATIVE SUM OF ALL CONDUCTANCES TO NEIGHBORING
   !6E7----CELLS
138         E=-Z-B-D-F-H-S
   !
   !6F-----CALCULATE COMPONENTS OF THE UPPER AND LOWER MATRICES, WHICH
   !6F-----ARE THE FACTORS OF MATRIX (A+B)
...
139            FORMAT(1X,/1X,'DIVIDE BY 0 IN SIP AT LAYER',I3,',  ROW',I4,&
               &',  COLUMN',I4,/&
               &1X,'THIS CAN OCCUR WHEN A CELL IS CONNECTED TO THE REST OF',/&
               &1X,'THE MODEL THROUGH A SINGLE CONDUCTANCE BRANCH.  CHECK',/&
               &1X,'FOR THIS SITUATION AT THE INDICATED CELL.')
               IERR = 1
               RETURN
            END IF
   ...
   !
   !6G-----CALCULATE THE RESIDUAL
            RRHS=RHS(N)
            RES=RRHS-ZHNEW-BHNEW-DHNEW-E*HNEW(N)-HHCOF*HNEW(N)-FHNEW-HHNEW&
            &-SHNEW
   !
   !6H-----CALCULATE THE INTERMEDIATE VECTOR V
            V(N)=(AC*RES-AL*VNLL-BL*VNRL-CL*VNCL)/DL
   !
150 CONTINUE
   !
   !7------STEP THROUGH EACH CELL AND SOLVE FOR HEAD CHANGE BY BACK
   !7------SUBSTITUTION
   DO 160 K=1,NLAY
      DO 160 I=1,NROW
         DO 160 J=1,NCOL
   !
   !7A-----SET UP CURRENT CELL LOCATION INDEXES.  THESE ARE DEPENDENT
   !7A-----ON THE DIRECTION OF EQUATION ORDERING.
...
152         KK=K
            ...
            JJ=NCOL-J+1
   !
   !7B-----CALCULATE 1 DIMENSIONAL SUBSCRIPT OF CURRENT CELL AND
   !7B-----SKIP CALCULATIONS IF CELL IS NOFLOW OR CONSTANT HEAD
154         N=JJ+(II-1)*NCOL+(KK-1)*NRC
            IF(IBOUND(N).LE.0)GO TO 160
   !
   !7C-----CALCULATE 1 DIMENSIONAL SUBSCRIPTS FOR THE 3 NEIGHBORING CELLS
   !7C-----BEHIND (RELATIVE TO THE DIRECTION OF THE BACK SUBSTITUTION
   !7C-----ORDERING) THE CURRRENT CELL.
            NC=N+1
            NR=N+IDNCOL
            NL=N+IDNRC
   !
   !7D-----BACK SUBSTITUTE, STORING HEAD CHANGE IN ARRAY V IN PLACE OF
   !7D-----INTERMEDIATE FORWARD SUBSTITUTION VALUES.
            ELXI=DZERO
            FLXI=DZERO
            GLXI=DZERO
   ...
   !
   !7E-----GET THE ABSOLUTE HEAD CHANGE. IF IT IS MAX OVER GRID SO FAR.
   !7E-----THEN SAVE IT ALONG WITH CELL INDICES AND HEAD CHANGE.
            TCHK=ABS(V(N))
            ...
            KB=KK
   !
   !7F-----ADD HEAD CHANGE THIS ITERATION TO HEAD FROM THE PREVIOUS
   !7F-----ITERATION TO GET A NEW ESTIMATE OF HEAD.
155         XI=V(N)
            HNEW(N)=HNEW(N)+XI
   !
160 CONTINUE
   !
   !8------STORE THE LARGEST ABSOLUTE HEAD CHANGE (THIS ITERATION) AND
   !8------AND ITS LOCATION.
   HDCG(KITER)=BIG
   LRCH(1,KITER)=KB
   LRCH(2,KITER)=IB
   ...
   !
   !9------IF END OF TIME STEP, PRINT # OF ITERATIONS THIS STEP
...
501   FORMAT(1X,I5,' ITERATIONS FOR TIME STEP',I6,& !gsf
   ...
   END IF
   !
   !10-----PRINT HEAD CHANGE EACH ITERATION IF PRINTOUT INTERVAL IS REACHED
   IF(MUTSIP.EQ.0) THEN
...
&IOUT)
   ...
   NODES=0
   !
   !2------LOOP THROUGH ALL CELLS, CALCULATING A SEED FOR EACH CELL
   !2------THAT IS ACTIVE
   DO 100 K=1,NLAY
      DO 100 I=1,NROW
         DO 100 J=1,NCOL
   ...
   !
   !2A-----CONDUCTANCE FROM THIS CELL
   !2A-----TO EACH OF THE 6 ADJACENT CELLS
            D=ZERO
   ...
   !
   !2B-----FIND THE MAXIMUM AND MINIMUM OF THE 2 CONDUCTANCE COEFFICIENTS
   !2B-----IN EACH PRINCIPAL COORDINATE DIRECTION
            DFMX=MAX(D,F)
            BHMX=MAX(B,H)
            ZSMX=MAX(Z,S)
            DFMN=MIN(D,F)
   ...
   !
   !2C-----CALCULATE A SEED IN EACH PRINCIPAL COORDINATE DIRECTION
            WCOL=ONE
   ...
   !
   !2D-----SELECT THE CELL SEED, WHICH IS THE MINIMUM SEED OF THE 3.
   !2D-----SELECT THE MINIMUM SEED OVER THE WHOLE GRID.
            WMIN=MIN(WCOL,WROW,WLAY)
            WMINMN=MIN(WMINMN,WMIN)
   !
   !2E-----ADD THE CELL SEED TO THE ACCUMULATOR AVGSUM FOR USE
   !2E-----IN GETTING THE AVERAGE SEED.
            DWMIN=WMIN
            AVGSUM=AVGSUM+DWMIN
            NODES=NODES+1
   !
100 CONTINUE
   !
   !3------CALCULATE THE AVERAGE SEED OF THE CELL SEEDS, AND PRINT
   !3------THE AVERAGE AND MINIMUM SEEDS.
   TMP=NODES
   AVGMIN=AVGSUM
   AVGMIN=AVGMIN/TMP
...
101 FORMAT(1X,/1X,'AVERAGE SEED =',F11.8/1X,'MINIMUM SEED =',F11.8)
   !
   !4------CALCULATE AND PRINT ITERATION PARAMETERS FROM THE AVERAGE SEED
   P1=-ONE
   P2=NPARM-1
   DO 50 I=1,NPARM
      P1=P1+ONE
...
SUBROUTINE SSIP7P(HDCG,LRCH,KITER,MXITER,IOUT)
   !     ******************************************************************
   !     PRINT MAXIMUM HEAD CHANGE FOR EACH ITERATION DURING A TIME STEP
   !     ******************************************************************
   !
   !        SPECIFICATIONS:
   !     ------------------------------------------------------------------
